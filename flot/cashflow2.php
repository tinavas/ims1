...
$query = "SELECT SUM(amount) AS amountreceived FROM ac_financialpostings WHERE date <= '$todate' AND venname = '$row11[name]' AND crdr = 'Cr' AND coacode = '$ca'";
$result = mysql_query($query,$conn1) or die(mysql_error());
$rows = mysql_fetch_assoc($result);
$amountrecv = $bal = $rows['amountreceived'];
if($bal == "") $bal = 0;
 $query = "SELECT date,trnum,amount AS finaltotal FROM ac_financialpostings WHERE date <= '$todate' AND venname = '$row11[name]' AND crdr = 'Dr' AND coacode = '$ca'";
$result = mysql_query($query,$conn1) or die(mysql_error());
...
<tr><td  align="center">
	...
	<div><?php /*?><?php include "getemployee.php"; echo "Total Balance is ". changequantity($tot1); ?><?php */?></div>
	<div id="hover3"></div>
	</div>
