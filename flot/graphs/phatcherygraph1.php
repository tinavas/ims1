...
$(function () {
	...
	<?php
	$h=1;
	$q1="select distinct(sector) from tbl_sector where type1='Hatchery'";
	$r1=mysql_query($q1);
	$nrows=mysql_num_rows($r1);
	...
	$query = "SELECT sum( saleablechicks ) AS result FROM `hatchery_hatchrecord` p  WHERE hatchdate between '$date4' and '$date3' AND p.client = '$client' ";
	...
	$m2=$t2[1];
	$y2=$t2[0];
	$query = "SELECT sum( saleablechicks) AS result FROM `hatchery_hatchrecord` p  WHERE hatchdate between '$date4' and '$date3' AND hatchery='$harr[$i]' and p.client = '$client' ";
	$result = mysql_query($query,$conn1) or die(mysql_error());
	while($rows = mysql_fetch_assoc($result))
	{
	 $tproduction += $rows['result'];
	 ?>
	 //alert("<?php echo $date3.$date4;?>");
	 data[++i] = {label: "<?php echo $harr[$i].'-';?>", data:<?php echo round(($rows['result']/$tot)*100,2); ?> , color:"<?php echo $col[$i];?>"}
	 <?php
	}}
	?>
...
function pieHover3(event, pos, obj) 
{
	...
	tot=addCommas(tot1);
	 $("#hover3").html('<span style="font-weight: bold; color: '+obj.series.color+'">'+obj.series.label+' Hatch :'+percent+'%(' + tot +')</span>');
}
...
<tr><td  align="center">
	...
	<center><h4>Chicks Composition</h4></center>
    <div id="production" class="graph"></div><br>
	<div><?php include "getemployee.php"; echo "Total Chicks Hatched is". changequantity($tproduction); ?></div>
	<div id="hover3"></div>
	</div>
