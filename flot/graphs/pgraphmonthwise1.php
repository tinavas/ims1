...
$month2 = date('m', $ts2);
 ...
 $diff = (($year2 - $year1) * 12) + ($month2 - $month1);$m[1]="January";
$m[2]="February";
$m[3]="March";
$m[4]="April";
$m[5]="May";
...
$(function () {
	...
	$query = "SELECT sum( quantity ) AS result FROM `breeder_production` p  WHERE date1 between '$date4' and '$date3' AND p.client = '$client'  and flock = '3-GG-16-3'  ";
	...
	$m2=$t2[1];
	$y2=$t2[0];
	$query = "SELECT sum( quantity ) AS result FROM `breeder_production` p  WHERE date1 between '$date1' and '$date' AND p.client = '$client' and flock = '3-GG-16-3' ";
	$result = mysql_query($query,$conn1) or die(mysql_error());
	while($rows = mysql_fetch_assoc($result))
	{
...
<tr><td  align="center">
	...
	<center><h4>Production Composition for flock 3-GG-16-3</h4></center>
