...
include "config.php";
$qb = "SELECT breed FROM layer_flock WHERE flockcode LIKE '%$c'";
...
{
           include "config.php";
             $q = "select * from ims_stocktransfer where cat = 'Layer Birds' and fromwarehouse LIKE '%$a' and date <= '$b'"; 
...
{
           include "config.php";
             $q = "select * from ims_stocktransfer where cat = 'Layer Birds' and towarehouse LIKE '%$a' and date <= '$b'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
               $toplusfemales = "0";
...
{
           include "config.php";
           $query12 = "SELECT sum(num) as `num` FROM sales where flock LIKE '%$a' and type='Layer Birds' and date <= '$b' ORDER BY date DESC ";
...
var mortality = [[0,],
     <?php 
           include "config.php"; $w = 0; $birds = 0;
           $querya = "SELECT * FROM layer_flock where flockcode LIKE '%$c'";
           ...
           $querya = "SELECT min(age) as age,min(startdate) as startdate FROM layer_flock where flockcode LIKE '%$c'";
           $resulta = mysql_query($querya,$conn1); 
           while($row1a = mysql_fetch_assoc($resulta))
           {
             $age = $row1a['age'];
           ...
           $query12 = "SELECT * FROM layer_consumption where flock LIKE '%$c' GROUP BY date2 ";
           $result12 = mysql_query($query12,$conn1); 
           while($row12 = mysql_fetch_assoc($result12))
           {
              $age = $age + 1; 
              ...
              if ($nrDaysPassed == 6)
              {
                  $query12a = "SELECT sum(fmort) as `fmort`,sum(mmort) as `mmort` FROM layer_consumption where flock LIKE '%$c' and date2 <= '$row12[date2]' ";
...
var feed = [[0,0],
     <?php 
           include "config.php"; $w = 0; $female = 0;
           $querya = "SELECT * FROM layer_flock where flockcode LIKE '%$c'";
           $resulta = mysql_query($querya,$conn1); 
           while($row1a = mysql_fetch_assoc($resulta))
           {
           ...
           while($row12 = mysql_fetch_assoc($result12))
           {
              ...
              if ($nrDaysPassed == 6)
              {
                 ...
                 include "config.php"; 
                 $querya = "SELECT sum(fmort) as `fmort`,sum(fcull) as `fcull` FROM layer_consumption where flock LIKE '%$c' and date2 <= '$odate' ORDER BY date2 DESC ";
                 $resulta = mysql_query($querya,$conn1); 
                 while($row1a = mysql_fetch_assoc($resulta))
                 {
                  ...
                  $query12a = "SELECT sum(quantity) as `fm` FROM layer_consumption where flock LIKE '%$c' and date2 >= '$odate' and date2 <= '$row12[date2]' and itemcode in ($feedtype)";
                  $result12a = mysql_query($query12a,$conn1); 
                  while($row12a = mysql_fetch_assoc($result12a))
                  {
                     $oldw = $w; 
...
var ew = [
     <?php 
           ...
           while($row12 = mysql_fetch_assoc($result12))
           {
              ...
              if ($nrDaysPassed == 6)
              {
                  $query12a = "SELECT max(eggwt) as `ew` FROM layer_consumption where flock LIKE '%$c' and date2 >= '$odate' and date2 <= '$row12[date2]' ";
                  $result12a = mysql_query($query12a,$conn1); 
                  while($row12a = mysql_fetch_assoc($result12a))
                  {
                     $oldw = $w; 
           ...
           while($row12 = mysql_fetch_assoc($result12))
           {
              ...
              if ($nrDaysPassed == 6)
              {
                 ...
                 while($row1a = mysql_fetch_assoc($resulta))
                 {
                  ...
                  $eggtype = "'dummy'";
                  $q = "select * from ims_itemcodes where cat = 'Layer Eggs'"; 
      		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		      while($qr = mysql_fetch_assoc($qrs))
		      {
                  ...
                  $query12a = "SELECT sum(quantity) as 'totaleggs' FROM layer_production where flock LIKE '%$c' and date1 >= '$odate' and date1 <= '$row12[date2]' and itemcode in ($eggtype) ";
                  $result12a = mysql_query($query12a,$conn1); 
                  while($row12a = mysql_fetch_assoc($result12a))
                  {
                     $oldw = $w;
					 //echo $row12a['totaleggs']."/".$getremainingfemale; 
...
var ewstandard = [
     <?php $avgweight = 0;
       include "config.php";
       $query1 = "SELECT * FROM layer_pstandards where breed = '$breedi' ORDER BY age ASC ";
...
var hdstandard = [
     <?php $avgweight = 0;
       include "config.php";
       $query1 = "SELECT age,henday FROM layer_pstandards where  breed = '$breedi' ORDER BY age ASC ";
       $result1 = mysql_query($query1,$conn1);
       while($row1 = mysql_fetch_assoc($result1))
       {
         $age = $row1['age'];
         $oldavg = $avgweight;
         $avgweight = $row1['henday'];
         if($avgweight == 0) { $avgweight = $oldavg; }
     ?>
       [<?php echo $age; ?>,<?php echo $avgweight; ?>],
     <?php  } ?>
...
if(document.getElementById("all").checked)
{
  ...
  var plot =   $.plot($("#placeholder"),
           ...
           { 
             ...
             xaxis: { min: <?php echo $minage; ?>, max: 50,
                      tickSize: 2,
					  label: "test"
                    },
             yaxis: { min: 0, tickSize: 10, max:100 },
             y2axis: { min : 0, tickSize: 10, max:140 },
             legend: { margin: [660,50] } 
    });
}
else
{
