...
$startfemale =0;
$tcount =0;
		 $query1 = "SELECT * FROM breeder_flock WHERE flockcode like '%$flockget' and  client='$client' group by startdate";
$result1 = mysql_query($query1,$conn1); 
$tcount = mysql_num_rows($result1);

if($tcount > 1)
{
  $query1 = "SELECT * FROM breeder_flock WHERE flockcode like '%$flockget' and  client='$client' order by startdate asc limit 1";
  $result1 = mysql_query($query1,$conn1); 
  while($row11 = mysql_fetch_assoc($result1))  
  {
...
{
  $query1 = "SELECT * FROM breeder_flock WHERE flockcode like '%$flockget' and  client='$client'";
  $result1 = mysql_query($query1,$conn1); 
  while($row11 = mysql_fetch_assoc($result1))  
  {
      $startdate = $row11['startdate'];
...
$query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat = 'Female Birds' AND client = '$client' AND towarehouse like '%$flockget' AND date<'$prodate'";
 ...
 // $query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat IN ('Female Birds', 'Male Birds') AND client = '$client' AND fromwarehouse like '$flockget' AND date<'$prodate'";
  $query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat = 'Female Birds' AND client = '$client' AND fromwarehouse like '%$flockget' AND date<'$prodate'";
  $result1t = mysql_query($query1t,$conn1); 
  while($row11t = mysql_fetch_assoc($result1t))  
  {
      /*if($row11t['cat'] == 'Female Birds')
...
$tothe = 0;
 $agewk = $row121['age'];
  ...
  {
    $eggs = explode(":",$cat[$d]);
	if(in_array($eggs[0],$hatcheggtype))
     {
	 $tothe = $tothe + $eggs[1];
	}
  }
