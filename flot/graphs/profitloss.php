...
$(function () {
	...
	<?php
$expense = 0;	
$query1 = "SELECT distinct(schedule) FROM ac_coa WHERE client = '$client' AND type = 'Expense' ORDER BY schedule";
$result1 = mysql_query($query1,$conn1) or die(mysql_error());
while($rows1 = mysql_fetch_assoc($result1))
{ 
 $coacodes = "'',";
 $q = "SELECT code FROM ac_coa WHERE schedule = '$rows1[schedule]'";
 ...
 $query2 = "SELECT SUM(amount) as amount FROM ac_financialpostings WHERE crdr = 'Cr' AND client = '$client' AND coacode IN ($coacodes) AND date BETWEEN '$fromdate' AND '$todate' ";
 $result2 = mysql_query($query2,$conn1) or die(mysql_error());
 $rows2 = mysql_fetch_assoc($result2);
 $cramount = $rows2['amount'];
 
 $query2 = "SELECT SUM(amount) as amount FROM ac_financialpostings WHERE crdr = 'Dr' AND client = '$client' AND coacode IN ($coacodes) AND date BETWEEN '$fromdate' AND '$todate' ";
 $result2 = mysql_query($query2,$conn1) or die(mysql_error());
 $rows2 = mysql_fetch_assoc($result2);
 $dramount = $rows2['amount'];
...
$("#expense").bind("plothover", pieHover);
$("#expense").bind("plotclick", pieClick);
...
function pieClick(event, pos, obj) 
{
	...
	<?php
$revenue = 0;	
$query1 = "SELECT distinct(schedule) FROM ac_coa WHERE client = '$client' AND type = 'Revenue' ORDER BY schedule";
$result1 = mysql_query($query1,$conn1) or die(mysql_error());
while($rows1 = mysql_fetch_assoc($result1))
{ 
 $coacodes = "'',";
...
$("#revenue").bind("plothover", pieHover2);
$("#revenue").bind("plotclick", pieClick2);
...
<tr>
 <td align="center"><strong><font color="#3e3276">Profit &amp; Loss</font></strong></td>
</tr>
<tr height="5px"></tr>
<tr>
...
<tr height="10px"></tr>
	...
	<h3>Expense</h3>
    <div id="expense" class="graph"></div><br>
	<div><?php echo "Total Expense ".changeprice($expense); ?></div><br>
	<div id="hover1"></div>
	</div>
	<div id="graph2" style="float:left">
	<h3>Revenue</h3>
    <div id="revenue" class="graph"></div><br>
	<div><?php echo "Total Revenue ".changeprice($revenue); ?></div><br>
	<div id="hover2">&nbsp;</div>
	</div><br><br><br>
