...
var feed<?php echo $k;?> = [
      ...
      <?php }}
           ...
           $odate = $date;
           $query12 = "SELECT * FROM breeder_consumption where flock = '$c' and date2 between '$fdate' and '$tdate'  GROUP BY date2 ";
...
//alert("<?php echo $age ?>");
		   ...
		   <?php  for ($z=0;$z<$k;$z++)
		   {
		     if($z == $k-1){
		    ?>
             { data: feed<?php echo $z;?>, label: "<?php echo "Flock ".$flkarrn[$z];?>", color:'<?php echo $clorarr[$z];?>'  ,yaxis: 2  }
			 <?php } else { ?>
			 { data: feed<?php echo $z;?>, label: "<?php echo "Flock ".$flkarrn[$z];?>", color:'<?php echo $clorarr[$z];?>'  ,yaxis: 2 },
			 <?php } }?>
           ...
           { 
             ...
             xaxis: { min: 20,
					...
					yaxis: { min : 40, tickSize: 10, max:210 },
             y2axis: { min: 0, tickSize: 50, max:600 },
             
             legend: { margin: [-170,40] } 
    });
