...
<html>
 <head>
    ...
    <table border="0">
     ...
     <tr>
      <td>
       <span style="filter:flipv fliph;writing-mode:tb-rl;"><span style="">Egg / Bird (%)</span></span>
      </td>
      <td width="5px"></td>
      <td>
...
$(function () {
 <?php  
 $q = "select * from layer_flock where  client='$client' and cullflag $tc '$type' and farmcode $uc '$unit'  order by flockcode";
 // $q = "select * from layer_flock where  client='$client'  order by flockcode"; 
 $qrs = mysql_query($q,$conn1) or die(mysql_error());
  while($qr = mysql_fetch_assoc($qrs))
  {
		 $flockget = "";$startfemale =0;$pcount=0;$prodate ="";
		$flockget = $qr['flockcode'];
		 $q22 = "Select * from layer_production where flock='$flockget' and  client='$client'";
		$res22 = mysql_query($q22,$conn1); 
		 $pcount = mysql_num_rows($res22);
		if($flockget!= "" && $pcount>0)
		{
		$tcount =0;
		 $query1 = "SELECT * FROM layer_flock WHERE flockcode like '$flockget' and  client='$client' group by startdate";
$result1 = mysql_query($query1,$conn1); 
$tcount = mysql_num_rows($result1);

if($tcount > 1)
{
  $query1 = "SELECT * FROM layer_flock WHERE flockcode like '$flockget' and  client='$client' order by startdate asc limit 1";
  $result1 = mysql_query($query1,$conn1); 
...
{
  $query1 = "SELECT * FROM layer_flock WHERE flockcode like '$flockget' and  client='$client'";
  $result1 = mysql_query($query1,$conn1); 
  while($row11 = mysql_fetch_assoc($result1))  
  {
      $startdate = $row11['startdate'];
...
$query1 = "SELECT min(date1) as 'date1' FROM layer_production WHERE flock like '$flockget' and  client='$client'";
$result1 = mysql_query($query1,$conn1); 
while($row11 = mysql_fetch_assoc($result1))
{
  $prodate = $row11['date1'];
...
$query1 = "SELECT * FROM layer_consumption WHERE flock like '$flockget' and date2 < '$prodate' and  client='$client' group by date2,flock";
$result1 = mysql_query($query1,$conn1); 
while($row11 = mysql_fetch_assoc($result1))
{
...
$query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat = 'Layer Birds' AND client = '$client' AND towarehouse like '$flockget' AND date<'$prodate'";
 ...
 // $query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat IN ('Female Birds', 'Male Birds') AND client = '$client' AND fromwarehouse like '$flockget' AND date<'$prodate'";
  $query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat = 'Layer Birds' AND client = '$client' AND fromwarehouse like '$flockget' AND date<'$prodate'";
  $result1t = mysql_query($query1t,$conn1); 
  while($row11t = mysql_fetch_assoc($result1t))  
  {
      /*if($row11t['cat'] == 'Female Birds')
...
/* $query1t = "SELECT * FROM `pp_sobi` WHERE code in (select distinct(code) from ims_itemcodes where cat ='Male Birds') AND client = '$client' AND flock like '$flockget' AND date<'$prodate'";
  ...
  $query1t = "SELECT * FROM `pp_sobi` WHERE code in (select distinct(code) from ims_itemcodes where cat ='Layer Birds') AND client = '$client' AND flock like '$flockget' AND date<'$prodate'";
  $result1t = mysql_query($query1t,$conn1); 
  while($row11t = mysql_fetch_assoc($result1t))  
  {
...
/* $query1t = "SELECT * FROM `oc_cobi` WHERE code in (select distinct(code) from ims_itemcodes where cat ='Male Birds') AND client = '$client' AND flock like '$flockget' AND date<'$prodate'";
  ...
  $query1t = "SELECT * FROM `oc_cobi` WHERE code in (select distinct(code) from ims_itemcodes where cat ='Layer Birds') AND client = '$client' AND flock like '$flockget' AND date<'$prodate'";
  $result1t = mysql_query($query1t,$conn1); 
  while($row11t = mysql_fetch_assoc($result1t))  
  {
...
var production<?php echo $k;?> = [
     <?php 
          include "config.php"; $w = 0;$female=0;$age=0;
           ...
           $query12 = "SELECT * FROM layer_consumption where flock = '$flockget' and client='$client' GROUP BY date2 ";
  ...
  if($nrDaysPassed) { $nrDaysPassed = $nrDaysPassed; $nrWeeksPassed = $nrWeeksPassed; } else { $nrDaysPassed = 7; $nrWeeksPassed = $nrWeeksPassed - 1; }
              ...
              if ($nrDaysPassed == 7)
              {
                ...
                $query12a = "SELECT sum(quantity) as 'totaleggs' FROM layer_production where flock = '$flockget'  and date1 <= '$row12[date2]' and client='$client' and itemcode in ($eggtype) ";
                  $result12a = mysql_query($query12a,$conn1); 
                  while($row12a = mysql_fetch_assoc($result12a))
                  {
                     $oldw = $w; 
                     //$totaleggs = $row12a['totaleggs'] / 7; 
					 $totaleggs = $row12a['totaleggs']; 
...
var hdstandard = [
     <?php $avgweight = 0;
       include "config.php";
       $query1 = "SELECT * FROM layer_pstandards where client='$client' ORDER BY age ASC ";
       $result1 = mysql_query($query1,$conn1);
       while($row1 = mysql_fetch_assoc($result1))
       {
         $age = $row1['age'];
         $oldavg = $avgweight;
         $avgweight = $row1['hecum'];
...
<!-- , dashes: { show: true }, hoverable: true  -->
           ...
           [ { data: hdstandard, label: "Egg/Bird  Standard", color:'#FF0000' },
		   <?php  for ($z=0;$z<$k;$z++)
		   {
		     if($z == $k-1){
		    ?>
             { data: production<?php echo $z;?>, label: "<?php echo "Actual Egg/Bird of flock ".$flkarrn[$z];?>",  color:'<?php echo $clorarr[$z];?>'  }
			 <?php } else { ?>
			 { data: production<?php echo $z;?>, label: "<?php echo "Actual Egg/Bird of flock ".$flkarrn[$z];?>", color:'<?php echo $clorarr[$z];?>'  },
			 <?php } }?>
