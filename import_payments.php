...
for($i = 0; $i <count($_POST['vendor']); $i++)
{
 if($_POST['vendor'][$i] <> "" && $_POST['cashbankcode'][$i] <> "" && $_POST['amount'][$i] <> "" && $_POST['amount'][$i] > 0)
 {
  $date = date("Y-m-d",strtotime($_POST['date'][$i]));
  $party = $_POST['vendor'][$i];
  $doc = $_POST['bookinvoice'][$i];
  $paymentmethod = $_POST['pmethod'][$i];
...
$description = $qr['description'];
	...
	$q = "insert into pp_payment (remarks,tid,date,vendor,paymentmethod,tds,tdscode,tdsdescription,tdscr,tdsamount,tdsamount1,paymentmode,code,code1,description,cr,amount,totalamount,cheque,cdate,choice,flag,adate,aempid,aempname,asector,client,actualamount,amountpaid,balance) 
values('$narration','$tid','$date',\"$party\",'$paymentmethod','Without TDS','','','','0','0','$paymentmode','$cbcode','$code1','$description','Cr','$amount','$amount','$chequeno','$cdate','On A/C','1','$date','$empid','$empname','$sector','$client','$amount','$amount','0')";
...
header("Location:dashboardsub.php?page=tally_payments");
?>