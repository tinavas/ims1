...
{ 
 $code = $desc = "";
 $query1 = "SELECT code,description FROM ims_itemcodes WHERE cat = '$rows[type]' ORDER BY code";
 ...
 $catdup = str_replace(" ","_",$rows[type]);
 echo "var $catdup"."_code = [$code];\n";
 echo "var $catdup"."_desc = [$desc];\n";
 $categories .= "'$rows1[type]',";
}
$categories = substr($categories,0,-1);
echo "var categories = [$categories];\n";
...
while($rows = mysql_fetch_assoc($result))
 $farms .= "'$rows[farm]',";
$farms = substr($farms,0,-1);
echo "var farms = [$farms];\n"; 
...
{
theOption1=document.createElement("OPTION");
theText1=document.createTextNode(farms[i]);
theOption1.value = farms[i];
theOption1.title = farms[i];
theOption1.appendChild(theText1);
myselect1.appendChild(theOption1);
}
}
...
<?php }  ?>
	...
	?>
var catdup = cat1.replace(" ","_") + "_code";
var temp1 = new Array();	
var temp1 = eval(catdup);

var catdup = cat1.replace(" ","_") + "_desc";
var temp2 = new Array();	
var temp2 = eval(catdup);

for(var i = 0; i < temp1.length; i++)
{
        theOption1=document.createElement("OPTION");
              theText1=document.createTextNode(temp1[i]);
              theOption1.appendChild(theText1);
	          theOption1.value = temp1[i];
	          theOption1.title = temp1[i];
              code.appendChild(theOption1);

		 	  theOption1=document.createElement("OPTION");
              theText1=document.createTextNode(temp2[i]);
              theOption1.appendChild(theText1);
	          theOption1.value = temp1[i];
	          theOption1.title = temp2[i];
              description.appendChild(theOption1);
}
