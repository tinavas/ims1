...
$query = "select * from pp_purchaseorder where po = '$_GET[po]'";
$result = mysql_query($query,$conn) or die(mysql_error());
while($toprow = mysql_fetch_assoc($result))
{
   $query = "update pp_purchaseindent set approve = 0 where pi = '$toprow[pr]' and icode = '$toprow[code]'";
   $result = mysql_query($query,$conn) or die(mysql_error());
}
$query = "select * from pp_purchaseorder where po = '$_GET[po]' order by id";
$result = mysql_query($query,$conn) or die(mysql_error());
...
<section class="grid_8">
  <div class="block-border">
     <form class="block-content form" id="complex_form" method="post" action="pp_savepurchaseorder_golden.php" onSubmit="return calculate(this);">
		...
		<div class="block-controls"><ul class="controls-tabs js-tabs"></ul></div>
            ...
            <table>
             ...
             <!-- hidden variables start -->
                ...
                <input type="hidden" name="poincr" id="poincr" value="<?php echo $toprow['poincr']; ?>"/>
                <input type="hidden" name="m" id="m" value="<?php echo $toprow['m']; ?>"/>
                <input type="hidden" name="y" id="y" value="<?php echo $toprow['y']; ?>"/>
                
             <!-- hidden variables end -->
               
                <td><strong>P.O</strong></td>
                <td>&nbsp;<input type="text" size="14" id="po" name="po" value="<?php echo $toprow['po']; ?>" readonly style="border:0px;background:none" /></td>
                <td width="20px"></td>
                ...
                <select style="width: 180px" onchange="document.getElementById('codevendor').value = document.getElementById('selcodes').value+'#'+document.getElementById('vendor').value;" name="vendor" id="vendor" >
		                                         ...
		                                         $result = mysql_query($query,$conn);
 		                                         while($row = mysql_fetch_assoc($result))
		                                              {
                                          ?>
                     <option value="<?php echo $row['name'];?>" <?php if($toprow['vendor'] == $row['name']){?> selected="selected" <?php } ?> ><?php echo $row['name']; ?></option>
                <?php } ?>
                </select>
                </td>
                <td width="20px"></td>
							   ...
							   <?php 
		                                         ...
		                                         $result = mysql_query($query,$conn);
 		                                         while($row = mysql_fetch_assoc($result))
		                                              {
                                          ?>
         <option value="<?php echo $row['name'];?>" <?php if($toprow['broker']==$row['name']){?> selected="selected" <?php } ?>><?php echo $row['name']; ?></option>
                <?php } ?>
               </select>                </td>
                <td width="20px"></td>
                ...
                <td>&nbsp;<input type="text" size="15" id="mdate" class="datepicker" name="mdate" value="<?php echo date("d.m.Y",strtotime($toprow['date'])); ?>" onChange="getpo();" ></td>
                <td width="20px"></td>
							   ...
							   <?php 	
		                                         ...
		                                         $result = mysql_query($query,$conn);
 		                                         while($row = mysql_fetch_assoc($result))
		                                              {
                                          ?>
<option value="<?php echo $row['code'].'@'.$row['description'].'@'.$row['value'];?>" <?php if($toprow['credittermcode']==$row['code']){?> selected="selected" <?php } ?> ><?php echo $row['code']; ?></option>
                <?php } ?>
               </select>                </td>
			    </tr>
			   <tr height="10px"></tr>
							   ...
							   <?php 	
		                                         ...
		                                         $result = mysql_query($query,$conn);
 		                                         while($row = mysql_fetch_assoc($result))
		                                              {
                                          ?>
                                                         <option <?php if($row['name']==$toprow[placedby]) echo 'selected="selected"' ?> value="<?php echo $row['name'];?>"><?php echo $row['name'];?></option>
                <?php } ?>
               </select>                </td>
              </tr>
            </table>
		...
		<div class="columns">
			<div class="col200pxL-left">
				...
				<p class="one-line-input grey-bg small-margin">
					<label for="complex-switch-on" class="float-left" id="prtext"><?php if($toprow['pr']){?>With Purchase Request<?php } else { ?>With Out Purchase Request <?php } ?></label><br /><br />
					<span class="float-left"><input type="checkbox" name="complex-switch-on" id="complex-switch-on" value="1" class="switch"   <?php if($toprow['pr']){?> checked="checked" <?php } ?>></span>  <!--onClick="change('withpr');"-->
                               <br /><br />
				</p>
...
<div id="div-pr" style="position:absolute; <?php if($toprow['pr']){ } else{?> visibility:hidden <?php } ?>">
 <table border="0" id="table-pr">
	 ...
	 <?php if($toprow['pr'])
	    {
		 $i=0;
		$query = "select * from pp_purchaseorder where po = '$_GET[po]' order by id";
		$prresult = mysql_query($query,$conn) or die(mysql_error());
		while($prrow = mysql_fetch_assoc($prresult))
		{ 
		  $catarr[$i] = $prrow['category'];
		  $codearr[$i] = $prrow['code'];
		  $descarr[$i] = $prrow['description'];
		  $qtyarr[$i] = $prrow['quantity'];
		  $unitarr[$i] = $prrow['unit'];
		  $taxarr[$i] = $prrow['taxcode'];
		  $frcoa[$i] = $prrow['freightcode'];
		  $framt[$i] = $prrow['freightamount'];
		  $frtype[$i] = $prrow['freightie'];
		  $brokeragearr[$i] = $prrow['brokeragecode'];
		  $discarr[$i] = $prrow['discountcode'];
		  $dlocarr[$i] = $prrow['deliverylocation'];
		  $recarray[$i] = $prrow['receiver'];
		  $flock[$i]=$prrow['flock'];
		?>
     ...
     <tr>
       ...
       <td style="text-align:left;">
         ...
         <select name="prcategory[]" id="prtype/<?php echo $i; ?>" onChange="fun(this.id,'pr');" style="width:90px;">
           ...
           <?php
              ...
              {
           ?>
           <option value="<?php echo $row1['type']; ?>" <?php if($prrow['category']==$row1['type']){?> selected="selected" <?php } ?> ><?php echo $row1['type']; ?></option>
           <?php } ?>
         </select>
       </td>
       <td width="10px"></td>
       ...
       <td style="text-align:leftl;">
         <select name="prcode[]" id="prcode/<?php echo $i; ?>" onChange="seldescription(this.id,'pr');" style="width:70px;">
           <option value="<?php echo $prrow['code']; ?>" selected="selected"><?php echo $prrow['code']; ?></option>
         </select>
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <select name="prdesc[]" id="prdescing/<?php echo $i; ?>" onChange="selcode(this.id,'pr');" style="width:140px;">
           <option value="<?php echo $prrow['description'];?>" selected="selected"><?php echo $prrow['description'];?></option>
         </select>
         <input type="hidden" name="prdescription[]" id="pring/<?php echo $i; ?>" readonly size="15" value="<?php echo $prrow['description'];?>"  /> 
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <select name="prnum[]" id="prnum/<?php echo $i; ?>" onChange="prdetails(this.id);" style="width:90px;">
           <option value="<?php echo $prrow['pr']; ?>"><?php echo $prrow['pr']; ?></option>
         </select>
       </td>
       <td width="10px"></td>
       ...
       <td style="text-align:left;">
         <input type="text" name="prquantity[]" id="pringweight/<?php echo $i; ?>"  size="8" onKeyUp="getvalues(this.id)" onBlur="getvalues(this.id)" value="<?php echo $prrow['quantity']; ?>"  />
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <input type="text" name="prunits[]" id="prmunit/<?php echo $i; ?>"  size="8" value="<?php echo $prrow['unit']; ?>"  />
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <input type="text" name="prrate[]" id="prunit/<?php echo $i; ?>"   size="7" value="<?php echo $prrow['rateperunit']; ?>"  />
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <input type="text" size="11" id="prrdate/<?php echo $i; ?>" name="prrdate[]" class="datepicker" value="<?php echo date("d.m.Y",strtotime($prrow['deliverydate'])); ?>">
       </td>
    </tr>
	<?php $i++; } } ?>
   </table>
   ...
   <input type="hidden"  name="withornot" id="withorwithout" <?php if($toprow['pr']){?> value="with" <?php } else {?> value="without"<?php } ?>  />
  <input type="hidden" id="previouspo" name="previouspo" value="<?php echo $_GET['po']; ?>" />
</div>




<div id="div-po" style="position:absolute;  <?php if($toprow['pr']){?> visibility:hidden <?php } ?>">
...
<?php if($toprow['pr']=="")
     ...
     <tr>
       ...
       <td style="text-align:left;">
         <select name="code[]" id="code/<?php echo $i;?>" onChange="seldescription(this.id,'');" style="width:80px;">
           <option value="<?php echo $prrow['code']; ?>"><?php echo $prrow['code']; ?></option>
         </select>
       </td>
       <td width="10px"></td>
       ...
       <td style="text-align:left;">
         ...
         <select name="desc[]" id="descing/<?php echo $i;?>" onChange="selcode(this.id,'');" style="width:140px;">
           <option value="<?php echo $prrow['description']; ?>"><?php echo $prrow['description']; ?></option>
         </select>
     
         <input type="hidden" name="description[]" id="ing/<?php echo $i;?>" value="<?php echo $prrow['description']; ?>" readonly size="25"  /> 
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <input type="text" name="quantity[]" id="ingweight/<?php echo $i;?>"  size="8"  onfocus = "makeForm('')" onKeyUp="getvalues(this.id)" onBlur="getvalues(this.id)"  value="<?php echo $prrow['quantity']; ?>" />
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <input type="text" name="units[]" id="munit/<?php echo $i;?>" value="<?php echo $prrow['unit']; ?>"  size="8"  />
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <input type="text" name="rate[]" id="unit/<?php echo $i;?>" value="<?php echo $prrow['rateperunit'];?>" size="8"  />
       </td>
       <td width="10px"></td>

       <td style="text-align:left;">
         <input type="text" size="15" id="rdate/<?php echo $i;?>" name="rdate[]" class="datepicker" value="<?php echo date("d.m.Y",strtotime($prrow['deliverydate'])); ?>">
       </td>
    </tr>
	<?php $i++; } }?>
   </table>
...
<th style="text-align:left"><strong>Flock</strong></th>
		  ...
		  <input type="hidden" id="selcodes" name="selcodes" value="<?php echo $selcode ?>" />
          <input type="hidden" id="codevendor" name="codevendor" value="<?php echo $selcode.'#'.$toprow[vendor]; ?>" />
		  
		  <strong>Receiver</strong></th>
        </tr>

        <tr style="height:20px"></tr>
		<?php
        for($i = 0; $i < sizeof($codearr); $i++)
		{ ?>
        <tr>
         <td style="text-align:left;">
		 
           <input type="text" name="" id="delivery-type/<?php echo $i;?>" readonly style="border:0px;background:none;cursor:hand" size="15" value="<?php echo $catarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="delivery-code/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="10" value="<?php echo $codearr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="delivery-ing/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="15" value="<?php echo $descarr[$i]; ?>"  title="<?php echo $descarr[$i]; ?>" />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="delivery-ingweight/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $qtyarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="delivery-munit/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $unitarr[$i]; ?>" />
         </td>
         <td width="10px"></td>
...
<option value="">-Select-</option>
 <?php
           ...
           {
           ?>
<option value="<?php echo $row1['sector']; ?>" title="<?php echo $row1['sector']; ?>" <?php if($dlocarr[$i] == $row1['sector']){?> selected="selected" <?php } ?> ><?php echo $row1['sector']; ?></option>
<?php } ?>
</select>
          </td>
          <td width="10px"></td>
		   <td>
<select name="flock[]" id="flock/<?php echo $i; ?>" onchange= "" style="width:80px;">
<option value="">-Select-</option>
 <?php
           ...
           {
           ?>
<option <?php  if($row1['flkmain']==$flock[$i]) echo 'selected="selected"';?> value="<?php echo $row1['flkmain']; ?>" title="<?php echo $row1['flkmain']; ?>" ><?php echo $row1['flkmain']; ?></option>
<?php } ?>
</select>
          </td>
<td width="10px"></td>
          ...
          <td>
<select name="demp[]" id="demp/<?php echo $i; ?>"  style="width:80px;">
<option value="<?php echo $recarr[$i]; ?>"><?php echo $recarr[$i]; ?></option>
 </select>
          </td>
       </tr>
	   <?php  } ?>
   ...
   <center>
      <table border="0" id="table-taxes">
       ...
       <?php
        ...
        <tr>
         <td style="text-align:left;">
           <input type="text" name="" id="tax-type/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="20" value="<?php echo $catarr[$i];; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="tax-code/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="15" value="<?php echo $codearr[$i]; ?>" />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="tax-ing/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="25" value="<?php echo $descarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="tax-ingweight/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $qtyarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="tax-munit/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $unitarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>
         ...
         <td>
           <select name="tax[]" id="tax/<?php echo $i; ?>"  style="width:80px;">
             ...
             <?php
                ...
                {
             ?>
             <option title="<?php echo $row1['description']; ?>" value="<?php echo $row1['code']."@".$row1['codevalue']."@".$row1['formula']."@".$row1['rule']; ?>" <?php if($taxarr[$i] == $row1['code']){?> selected="selected" <?php } ?> ><?php echo $row1['code']; ?></option>
             <?php } ?>
           </select>
		   <input type="hidden" name="taxamount[]" id="taxamount/<?php echo $i; ?>"  />
         </td>
       </tr>
  ...
  <!-- <p class="one-line-input grey-bg small-margin">
   ...
   <center>
      ...
      <?php
        ...
        <tr>
         <td style="text-align:left;">
           <input type="text" name="" id="charges-type/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="15" value="<?php echo $catarr[$i]; ?>" />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="charges-code/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="10" value="<?php echo $codearr[$i]; ?>" />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="charges-ing/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="10" value="<?php echo $descarr[$i]; ?>" title="<?php echo $descarr[$i]; ?>" />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="charges-ingweight/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $qtyarr[$i]; ?>" />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="charges-munit/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $unitarr[$i]; ?>" />
         </td>
         <td width="10px"></td>
 <td>
            <select name="frtype[]" id="frtype<?php echo $i; ?>"  style="width:60px;">
              <option <?php  if($frtype[$i]=='included')  echo 'selected="selected"'; ?> title="Included" value="included">Included</option>
              <option <?php  if($frtype[$i]=='excluded')  echo 'selected="selected"'; ?> title="Excluded" value="excluded">Excluded</option>   
           </select>
          </td>
          <td width="10px"></td>
		  
		    <td>
            <input type="text" size="10" value="<?php echo $framt[$i]; ?>" name="framt[]" id="framt/<?php echo $i; ?>" > 
          </td>
          <td width="10px"></td>

          <td>
            <select name="frcoa[]" id="frcoa/<?php echo $i; ?>"  style="width:60px;">
              <option value="">-Select-</option>
              <?php
                ...
                $query = "select distinct(code),description from ac_coa where (controltype = '' or controltype is NULL) and type = 'Expense' and schedule = 'INDIRECT EXPENSES' and code not like 'CG%' and code not like  'PV%' and code not like  'PR%' and code not like 'WP%' order by code";
                 ...
                 {
              ?>
              <option <?php if($row1['code']==$frcoa[$i])  echo 'selected="selected"'; ?> title="<?php echo $row1['description']; ?>" value="<?php echo $row1['code']; ?>"><?php echo $row1['code']; ?></option>
              <?php } ?>
           </select>
          </td>
          <td width="10px"></td>
          ...
          <td>
            <select name="brokerage[]" id="brok/<?php echo $i; ?>"  style="width:80px;">
              <option value="0@0@0@0">-Select-</option>
              <?php
                ...
                $query = "SELECT * FROM ims_taxcodes where type='Charges' and ctype='Brokerage' and taxflag = 'P' ORDER BY code DESC ";
                 ...
                 {
              ?>
              <option title="<?php echo $row1['description']; ?>" value="<?php echo $row1['code']."@".$row1['codevalue']."@".$row1['formula']."@".$row1['rule']; ?>" <?php if($brokeragearr[$i] == $row1['code']){?> selected="selected" <?php } ?>><?php echo $row1['code']; ?></option>
              <?php } ?>
            </select>
			
                <input type="hidden" name="brokerageamount[]" id="brokerageamount/<?php echo $i; ?>"  />
  ...
  <!-- <p class="one-line-input grey-bg small-margin">
   ...
   <center>
     ...
     <?php
        ...
        <tr>
         <td style="text-align:left;">
           <input type="text" name="" id="discounts-type/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="20" value="<?php echo $catarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="discounts-code/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="15" value="<?php echo $codearr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="discounts-ing/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand" size="25" value="<?php echo $descarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="discounts-ingweight/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $qtyarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td style="text-align:left;">
           <input type="text" name="" id="discounts-munit/<?php echo $i; ?>" readonly style="border:0px;background:none;cursor:hand"  size="8" value="<?php echo $unitarr[$i]; ?>"  />
         </td>
         <td width="10px"></td>

         <td>
           <select name="discount[]" id="disc/<?php echo $i; ?>"  style="width:80px;">
             <option value="0@0@0@0">-Select-</option>
             <?php
                  include "config.php"; 
                $query = "SELECT * FROM ims_taxcodes where type='Discount' and taxflag = 'P' ORDER BY code DESC ";
                  ...
                  {
            ?>
                  <option title="<?php echo $row1['description']; ?>" value="<?php echo $row1['code']."@".$row1['codevalue']."@".$row1['formula']."@".$row1['rule']; ?>" <?php if($discarr[$i] == $row1['code']) { ?> selected="selected" <?php } ?>><?php echo $row1['code']; ?></option>
            <?php } ?>
          </select> <input type="hidden" name="discountamount[]" id="discountamount/<?php echo $i; ?>"  />
         </td>
       </tr>
	  <?php  } ?> 
     </table>
...
<div id="tab-tandc" class="tabs-content">
           ...
           <select name="disc[]" id="tandccode"  style="width:150px;" multiple onclick="getdescr(this.value)">
             ...
             <?php
                  ...
                  {
            ?>
                  <option value="<?php echo $row1['code'].'@'.$row1['description']; ?>" <?php if($toprow['tandccode'] == $row1['code']){ ?> selected="selected" <?php } ?>><?php echo $row1['code']; ?></option>
            <?php } ?>
          </select><br /><br /><br />
<textarea rows="14" cols="110" id="tandcdesc" type="html" name="tandcdesc" ><?php echo $toprow['tandc']; ?></textarea>

<br /><br />
<strong>Purpose</strong>&nbsp;&nbsp;<textarea rows="3" cols="80" id="purpose" type="html" name="purpose" ><?php echo $toprow[purpose]; ?></textarea>
...
function change(a)
{
 if(a == "withpr")
 {
   if(document.getElementById("complex-switch-on").checked)
   {
      ...
      var index = -1;
	  document.getElementById("withorwithout").value="without";
   ...
   else
   {
      ...
      var index = -1;
	  document.getElementById("withorwithout").value="with";
...
<?php if($toprow['pr']) {?>
if(document.getElementById("pringweight/"+index).value != "" && document.getElementById("pringweight/"+index).value != "0")
<?php } else { ?>
if(document.getElementById("ingweight/"+index).value != "" && document.getElementById("ingweight/"+index).value != "0")
<?php } ?> 
{
...
function description(b,f)
{
 ...
 if(f == "pr")
  ...
  document.getElementById('discounts-munit/'+ a).value = "";
   ...
   if(index == 0) { var a = 0; var j = 0; } else { var a = index;  var j = index;   }
   var i1,j1;
//alert(f);
if(f != "pr")
{
   ...
   for ( var i = 0;i<=index;i++)
   {
    ...
    <?php
	 ...
	 $qrs = mysql_query($q) or die(mysql_error());
 	 while($qr = mysql_fetch_assoc($qrs))
	 { 
		echo "if(document.getElementById('prnum/' + j).value == '$qr[pi]' && document.getElementById('prcode/' + j).value == '$qr[icode]') { ";
    ?>
	
            document.getElementById('pringweight/' + j).value = "<?php echo $qr['quantity']; ?>";
            document.getElementById('prrdate/' + j).value = "<?php echo date("d.m.Y",strtotime($qr['rdate'])); ?>";
...
function calculate(a)
{
  ...
  { 
   ...
   if(document.getElementById('withorwithout').value == "with")
   {
    if(document.getElementById("prtype/" + k).value != "")
   {
     ...
     <!-- Tax Start -->
       ...
       <?php
           include "config.php";  
           $query1 = "SELECT * FROM ims_taxcodes ORDER BY code DESC ";
           $result1 = mysql_query($query1,$conn); 
           while($row1 = mysql_fetch_assoc($result1))
           {
       ?>
	   ...
	   var A = parseFloat(document.getElementById("pringweight/" + k).value * document.getElementById("prunit/" + k).value);
	    
       var withtax = eval(formula1);
 ...
 {
  for(var k = 0;k<=index;k++)
   if((document.getElementById("prtype/"+k).value != "") && (document.getElementById("doffice/"+k).value == ""))
  {
...
if(confirm('Are you sure you want to update the entry'))
{
  return true;
}
else
...
/*
   if(index == -1) { var a = "0"; var j = 0; } else { var a = index;  var j = index;   }
   var i1,j1;
//alert(f);
if(f != "pr")
{
...
//alert('working');
   ...
   <?php
	 ...
	 $qrs = mysql_query($q) or die(mysql_error());
 	 while($qr = mysql_fetch_assoc($qrs))
	 { 
		echo "if(document.getElementById(f + 'code/' + a).value == '$qr[code]') { ";
   ?>
	
   
	 
            document.getElementById(f + 'ing/' + a).value = "<?php echo $qr['description']; ?>";
            document.getElementById(f + 'descing/' + a).value = "<?php echo $qr['code']; ?>";
 	      document.getElementById(f + 'munit/' + a).value = "<?php echo $qr['sunits']; ?>";
...
if(f == "pr")
{
 ...
 if(y == "prcode")
 {
  ...
  myselect1.appendChild(theOption1);
   ...
   <?php
 	 $q = "select pi,icode from pp_purchaseindent where approve = 0 order by icode";
	 $qrs = mysql_query($q) or die(mysql_error());
 	 while($qr = mysql_fetch_assoc($qrs))
	 { 
		echo "if(document.getElementById('prcode/' + a).value == '$qr[icode]') { ";
...
/*selectcode = ""; 
for(var i = 0; i <= index; i++)
 selectcode += document.getElementById('code/'+i).value+"@";
...
else
{
   for(var i = 0;i<=index;i++) 
   {
	for(var j = 0;j<=index;j++)
	{
		...
		if( i != j)
		{
			if(document.getElementById(f + 'descing/' + i1).value == document.getElementById(f + 'descing/' + j1).value)
			{
				document.getElementById(f + 'ing/' + a).value = "";
				document.getElementById(f + 'code/' + a).value = "";
				alert("Please select different combination");
