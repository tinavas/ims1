...
$getop = "SELECT SUM(quantity) as quantity FROM ims_intermediatereceipt WHERE code = '$x_code' AND date < '$fromdate' AND warehouse = '$warehouse' and riflag = 'R'";
 ...
 if($_SESSION['db']=="singhsatrang"){
   ...
   $getop = "SELECT SUM(quantity) as quantity FROM product_itemwise WHERE ingredient = '$x_code' AND date < '$fromdate' AND flag = '0' AND warehouse = '$warehouse' and pid in (select distinct(id) from product_productionunit where date < '$fromdate' and warehouse = '$warehouse' )";
  $resultop = mysql_query($getop,$conn1);
  while($rowop = mysql_fetch_assoc($resultop))
  {
...
// avg cost
 ...
 //echo $closing;
  ...
  $query1 = "select distinct(iac) from ims_itemcodes where code = '$x_code' and client = '$client'   ";
...
?>
	<tr>
		...
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
			<?php echo round($consumed,3); $total_consumed+=$consumed; ?>
		</td>
...
<tr>
	...
	<td align="right" ><?php echo round($total_consumed,3) ;?></td>
