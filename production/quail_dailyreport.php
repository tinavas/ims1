...
<?php include "phprptinc/ewrfn3.php";
            ...
            $agequery = "select max(age) as age from quail_standards";
...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT min(quail_consumption.id) FROM " . $EW_REPORT_TABLE_SQL_FROM;
...
else
{
  ...
  if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
      $q = "select max(date2) as 'date2' from quail_consumption group by flock order by date2 ASC"; 
  else
      $q = "select max(date2) as 'date2' from quail_consumption where flock in ($_GET[flock]) group by flock order by date2 ASC"; 
...
$dummydate = date($datephp,strtotime($comparedate));
...
<br />
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="quail_dailyreport.php?export=html&date=<?php echo $_GET['date']; ?>&flock=<?php echo $_GET['flock']; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="quail_dailyreport.php?export=excel&date=<?php echo $_GET['date']; ?>&flock=<?php echo $_GET['flock']; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="quail_dailyreport.php?export=word&date=<?php echo $_GET['date']; ?>&flock=<?php echo $_GET['flock']; ?>">Export to Word</a>
<?php } else { ?>
<center>
<?php } ?>
<?php if (@$sExport == "") { ?>
...
<form action="quail_dailyreport.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<?php if($_GET['flock'])
{?>
document.location="quail_dailyreport.php?flock=<?php echo $_GET['flock']; ?>&date=" + a; 
<?php } else { ?>
document.location="quail_dailyreport.php?date=" + a; 
<?php } ?>
}
</script>
...
<!--first page button-->
	...
	<td><a href="quail_dailyreport.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="quail_dailyreport.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="quail_dailyreport.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="quail_dailyreport.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
<thead>
	<th class="ewTableHeader">Details</th>
         <?php 
            ...
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
              $q = "select distinct(flock) from quail_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from quail_flock where cullflag = 0) order by flock"; 
            else
              $q = "select distinct(flock) from quail_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) order by flock"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
              $flo[$f] = $qr['flock'];
			  $qbr = "select breed from quail_flock where flockcode = '$qr[flock]'"; 
...
<tr>
	<td<?php echo $sItemRowClass1; ?>>Age</td>
         <?php 
			...
			$f = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q = "select age from quail_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from quail_flock where cullflag = 0)group by flock order by flock "; 
            else
               $q = "select age from quail_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock "; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
              $age = $qr['age'];
...
<tr>
	<td<?php echo $sItemRowClass; ?>>Feed Consumed(Kgs) &nbsp;&nbsp;</td>
         <?php 
			...
			$fquantfc = 0;
			$mquantfc = 0;
            $q = "select * from ims_itemcodes where cat = 'Quail Female Feed'"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
              $feedtype = $feedtype . ",'" . $qr['code'] . "'";
            } 
            $q = "select * from ims_itemcodes where cat = 'Quail Male Feed'"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
            ...
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
              /* $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from quail_consumption where itemcode in ($feedtype) and date2 = '$comparedate'  and flock in (select distinct(flockcode) from quail_flock where cullflag = 0) group by flock order by flock"; 
            else
               $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from quail_consumption where itemcode in ($feedtype) and date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; */
			    $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from quail_consumption where date2 = '$comparedate'  and flock in (select distinct(flockcode) from quail_flock where cullflag = 0) group by flock order by flock"; 
            else
               $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from quail_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
		...
		while($qr = mysql_fetch_assoc($qrs))
		{
		 ...
		 //$femalef = 0;$malef = 0;
		 if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q11 = "select mfeed as 'quantity' from quail_consumption where date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
            else
               $q11 = "select mfeed as 'quantity' from quail_consumption where date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
		...
		if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q11 = "select ffeed as 'quantity' from quail_consumption where date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
            else
               $q11 = "select ffeed as 'quantity' from quail_consumption where date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
  		$qrs11 = mysql_query($q11,$conn1) or die(mysql_error());
		while($qr11 = mysql_fetch_assoc($qrs11))
		...
		//Birds Calculation
		$qj = "select * from quail_flock where flockcode = '$flockkk'"; 
		 ...
		 while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $qj = "select distinct(date2),fmort,fcull,mmort,mcull from quail_consumption where flock = '$flockkk' and date2 < '$comparedate' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		 while($qrj = mysql_fetch_assoc($qrsj))
                $minus = $minus + $qrj['fmort'] + $qrj['fcull'];

            $ftransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Quail Female Birds' and fromwarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		   ...
		   while($qrj = mysql_fetch_assoc($qrsj))
			...
			$ttransfer = 0;
            $qj = "select * from ims_stocktransfer where cat = 'Quail Female Birds' and towarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
			 ...
			 $fquantfc = 0;
			 $q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$flockkk' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Female Birds')";  
		   ...
		   while($qr = mysql_fetch_assoc($r))
			 ...
			 $fquantso = 0;
			 $q = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$flockkk' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Female Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
		...
		$qj = "select * from quail_flock where flockcode = '$flockkk'"; 
		 ...
		 while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $qj = "select distinct(date2),mmort,mcull from quail_consumption where flock = '$flockkk' and date2 < '$comparedate' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		 while($qrj = mysql_fetch_assoc($qrsj))
                $minus = $minus + $qrj['mmort'] + $qrj['mcull'];

             $ftransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Quail Male Birds' and fromwarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $ttransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Quail Male Birds' and towarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
			 ...
			 $mquantfc = 0;
$q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$flockkk' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Male Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
             ...
             else
             {
			  ...
			  $mquantso = 0;
			 $q = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$flockkk' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Male Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
		...
		//echo round($femalef,0)."/";
		 ...
		 <?php 
		 $actualff[$dummf] = round((($femalef * 1000)/($remaining+$mremaining)),0);
		 $actualmf[$dummf] = round((($femalef * 1000)/($remaining+$mremaining)),0);
	...
	<td<?php echo $sItemRowClass; ?>><?php /*echo $finalfeed = $finalfeed / 2; */ echo round(($finalfeed),0) ; ?></td>
</tr>
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Feed Consumed/Bird">F.Cons/Bird(gms.)</td>
         ...
         ?>
	     <td<?php echo $sItemRowClass2; ?>><?php echo $actualff[$f]; ?></td>
         <?php }  ?>   
	<td<?php echo $sItemRowClass2; ?>><?php /*echo $finalfeed = $finalfeed / 2; */ echo round((($finalfeed * 1000)/($ftot+$mtot)),0); ?></td>
</tr>
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Standard Feed Consumed">Std. F.Cons(gms.)</td>
         <?php 
			...
			if($flkbreed[$f] != "")
			{
			$q11 = "select * from quail_standards where age = '$age1[$f]' and breed = '$flkbreed[$f]'";
			}
			else
			{
			$q11 = "select * from quail_standards where age = '$age1[$f]'";
			}
			$qrs11 = mysql_query($q11,$conn1) or die(mysql_error());
			...
			$stdmf[$f] = $mstd;
         ?>
	     <td<?php echo $sItemRowClass2; ?>><?php echo $fstd; ?></td>
         <?php }  ?>   
...
<tr>
	<td<?php echo $sItemRowClass2; ?>  title="Difference">Difference</td>
         ...
         ?>
	     <td<?php echo $sItemRowClass2; if($fediff <= 0) {?> style="color:#66CC66" <?php } else { ?> style="color:#FF0000" <?php } ?>><?php echo round(($actualff[$f] - $stdff[$f]),0); ?></td>
         <?php }  ?>   
	<td<?php echo $sItemRowClass2; ?>>&nbsp;</td>
</tr>
<?php
            $eggtype = "'dummy'";
            $q = "select * from ims_itemcodes where cat = 'Quail Eggs' or cat = 'Quail Hatch Eggs' "; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
            ...
            $q = "select distinct(itemcode) from quail_production where itemcode in ($eggtype) and date1 = '$comparedate'  order by itemcode"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
	...
	<td<?php echo $sItemRowClass2; ?>><?php echo $qr['itemcode']; ?><br />(<?php echo $idesc; ?>)</td>
       <?php for($l = 0;$l < count($flo);$l++) { ?>
         <?php 
            ...
            $q1 = "select sum(quantity) as 'quantity' from quail_production where itemcode = '$qr[itemcode]' and flock = '$flo[$l]' and date1 = '$comparedate' group by flock order by flock"; 
  ...
  <?php for($l = 0;$l < count($flo);$l++) { ?>
         <?php 
           //$l = 0;
            ...
            $q = "select sum(quantity) as 'quantity' from quail_production where itemcode in ($eggtype) and date1 = '$comparedate' and flock = '$flo[$l]' group by flock order by flock"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Opening Quail Female Birds" >Female OB</td>
         <?php 
		 $finalrem = 0;
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             ...
             $q = "select * from quail_flock where flockcode = '$flo[$f]'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $q = "select distinct(date2),fmort,fcull,mmort,mcull from quail_consumption where flock = '$flo[$f]' and date2 < '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
                $minus = $minus + $qr['fmort'] + $qr['fcull'];


             $q = "select * from ims_stocktransfer where cat = 'Quail Female Birds' and fromwarehouse = '$flo[$f]' and date < '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             $q = "select * from ims_stocktransfer where cat = 'Quail Female Birds' and towarehouse = '$flo[$f]' and date < '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
			 ...
			 $q = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$flo[$f]' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Female Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
			...
			$q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$flo[$f]' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Female Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
...
<?php if($sItemRowClass2 == $sItemRowClass1) {$sItemRowClass2 = $sItemRowClass; } else { $sItemRowClass2 = $sItemRowClass1; } ?>
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Opening Quail Male Birds" >Male OB</td>
         <?php 
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $q = "select distinct(date2),mmort,mcull from quail_consumption where flock = '$flo[$f]' and date2 < '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
                $minus = $minus + $qr['mmort'] + $qr['mcull'];


             $q = "select * from ims_stocktransfer where cat = 'Quail Male Birds' and fromwarehouse = '$flo[$f]' and date < '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             $q = "select * from ims_stocktransfer where cat = 'Quail Male Birds' and towarehouse = '$flo[$f]' and date < '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
			 ...
			 $q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$flo[$f]' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Male Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
             ...
             else
             {
			  ...
			  $q = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$flo[$f]' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Quail Male Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Female Mortality" >F.Mortality</td>
         <?php 
			...
			$finalmort = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select fmort from quail_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from quail_flock where cullflag = 0) group by flock order by flock"; 
            else
                $q = "select fmort from quail_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Male Mortality" >M.Mortality</td>
         <?php 
			...
			$finalmort = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select mmort from quail_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from quail_flock where cullflag = 0) group by flock order by flock"; 
            else
                $q = "select mmort from quail_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Female Culls" >F.Culls</td>
         <?php 
			...
			$finalcull = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select fcull from quail_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from quail_flock where cullflag = 0) group by flock order by flock"; 
            else
                $q = "select fcull from quail_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Male Culls" >M.Culls</td>
         <?php 
			...
			$finalcull = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select mcull from quail_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from quail_flock where cullflag = 0) group by flock order by flock"; 
            else
                $q = "select mcull from quail_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr style="display:none">
	...
	<td<?php echo $sItemRowClass2; ?>>Transfered Out Quail Male Birds </td>
         <?php 
...
<tr style="display:none">
	...
	<td<?php echo $sItemRowClass2; ?>>Transfered In Quail Male Birds</td>
         <?php 
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Closing Female Birds" >Female CB</td>
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
		 {
             ...
             $minus = 0; 
             $q = "select distinct(date2),fmort,fcull from quail_consumption where flock = '$flo[$f]' and date2 <= '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $minus = $minus + $qr['fmort'] + $qr['fcull'];
...
<?php if($sItemRowClass2 == $sItemRowClass1) {$sItemRowClass2 = $sItemRowClass; } else { $sItemRowClass2 = $sItemRowClass1; } ?>
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Closing Quail Male Birds" >Male CB</td>
         <?php 
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Female Body Weight" >F.Body.Wt</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select fweight from quail_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $fweight = $qr['fweight'];
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Standard Female Body Weight">Std.F.Body.Wt</td>
         <?php
			...
			{
			if( $age1[$f] > $maxagee )
			 $q = "SELECT * FROM quail_standards WHERE age in (select max(age) from quail_standards where breed = '$flkbreed[$f]')";
			 else	
			$q = "select * from quail_standards where Age = '$age1[$f]'  and breed = '$flkbreed[$f]'";
			}
			else
			{
			if( $age1[$f] > $maxagee )
			 $q = "SELECT * FROM quail_standards WHERE age in (select max(age) from quail_standards)";
			 else	
			$q = "select * from quail_standards where Age = '$age1[$f]'";
			}
			$qrs = mysql_query($q,$conn1) or die(mysql_error());
...
<tr>
	<td title = "Male Body Weight" <?php echo $sItemRowClass2; ?>>M.Body.Wt</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select mweight from quail_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $mweight = $qr['mweight'];
...
<tr style="display:none">
	<td<?php echo $sItemRowClass2; ?>>Water</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select water from quail_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $water = $qr['water'];
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Egg Weight</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select eggwt from quail_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $eggwt = $qr['eggwt'];
...
<tr style="display:none">
	<td<?php echo $sItemRowClass2; ?>>Average Weight</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select avgwt from quail_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $avgwt = $qr['avgwt'];
...
<tr style="display:none">
	<td<?php echo $sItemRowClass2; ?>>Temperature</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select tempmin,tempmax from quail_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Consumption Cost</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select price from quail_consumption where flock = '$flo[$f]' and date2 = '$comparedate' limit 1"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Production Cost</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select price from quail_production where flock = '$flo[$f]' and date1 = '$comparedate' limit 1"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
...
$qury1aq = "select distinct(itemcode) from quail_consumption where date2 = '$comparedate' and flock in (select distinct(flock) from quail_production where date1 = '$comparedate')";
