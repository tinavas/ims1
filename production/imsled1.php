...
if ( $_POST['code'] == "All" )  { header('Location:imsdetailled1.php?date2='.$_POST['date2'].'&date3='.$_POST['date3']); }
?>
<table align="center">
<tr>
<td>Sri Suresh Poultry Complex</td>
</tr>
<tr>
<td>Chamalpur Village</td>
</tr>
<tr>
<td>Irkalgad P.O</td>
</tr>
...
Report 1
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="Report1smry.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="Report1smry.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="Report1smry.php?export=word">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
$q = "select amount,quantity from ims_initialstock where itemcode  = '$code'  ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
...
$q = "select amount,quantity,type,trnum from ac_financialpostings where itemcode  = '$code' and coacode = '$itemcode' and date < '$fdate' group by trnum ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  if ( ($qr['type'] == "PS") or ($qr['type'] == "Consumption") or ($qr['type'] == "COBI") or ($qr['type'] == "Item Consumed") )
	  {
	     $preissueqty = $preissueqty + $qr['quantity'];
		 $preissueval = $preissueval + $qr['amount'];
	  }
	  
	   if ( ($qr['type'] == "GR") or ($qr['type'] == "Production") or ($qr['type'] == "SOBI") or ($qr['type'] == "Feed Produced") )
	  {
	     $prercvqty = $prercvqty + $qr['quantity'];
		 $prercvval = $prercvval + $qr['amount'];
	  }
...
<td align="right"><?php  echo ewrpt_ViewValue( changeprice($obval)) ?></td>
	...
	$isval = 0;
	 include "config.php";
    $query1 = "SELECT type,trnum,date,quantity,amount from ac_financialpostings where itemcode = '$code' and coacode = '$itemcode'  and date >= '$fdate' and date <= '$tdate'  group by trnum order by date ";
...
?>
	<tr>
		...
		<td align="right">
        <?php if ( ($row2['type'] == "GR") or ($row2['type'] == "Production") or ($row2['type'] == "SOBI")  or ($row2['type'] == "Feed Produced")  ) { echo ewrpt_ViewValue(changeprice($row2['quantity'])); $rcvqty = $rcvqty + $row2['quantity']; }
		 else { echo ewrpt_ViewValue(); }?>
        </td>
		<td align="right">
         <?php if ( ($row2['type'] == "GR") or ($row2['type'] == "Production") or ($row2['type'] == "SOBI")  or ($row2['type'] == "Feed Produced")  ) { echo ewrpt_ViewValue(changeprice(round(($row2['amount']/$row2['quantity']),3))); }
		 else { echo ewrpt_ViewValue(); } ?>
        </td>
         <td align="right"> 
		 <?php if ( ($row2['type'] == "GR") or ($row2['type'] == "Production") or ($row2['type'] == "SOBI")  or ($row2['type'] == "Feed Produced")  ) { echo ewrpt_ViewValue(changeprice($row2['amount'])); $rcvval = $rcvval + $row2['amount']; }
		 else { echo ewrpt_ViewValue(); } ?>
         </td>
<td align="right">
<?php if ( ($row2['type'] == "PS") or ($row2['type'] == "Consumption") or ($row2['type'] == "COBI")  or ($row2['type'] == "Item Consumed")  ) { echo ewrpt_ViewValue(changeprice($row2['quantity'])); $isqty = $isqty + $row2['quantity']; }
 else { echo ewrpt_ViewValue(); }?>
</td>
<td align="right">
<?php if ( ($row2['type'] == "PS") or ($row2['type'] == "Consumption") or ($row2['type'] == "COBI") or ($row2['type'] == "Item Consumed")   ) { echo ewrpt_ViewValue(changeprice(round(($row2['amount']/$row2['quantity']),3))); } 
 else { echo ewrpt_ViewValue(); } ?>
</td>
<td align="right">
 <?php if ( ($row2['type'] == "PS") or ($row2['type'] == "Consumption") or ($row2['type'] == "COBI") or ($row2['type'] == "Item Consumed")   ) { echo ewrpt_ViewValue(changeprice($row2['amount'])); $isval = $isval + $row2['amount']; }
 else { echo ewrpt_ViewValue(); } ?>
</td>
