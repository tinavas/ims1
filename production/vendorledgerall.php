...
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Supplier Ledger(All)</font></strong></td>
</tr>
<tr>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="vendorledgerall.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="vendorledgerall.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="vendorledgerall.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php
	...
	$totbalcr = 0;
include "config.php";
    $q111 = "select * from contactdetails  where type = 'vendor' or type = 'vendor and party' order by name ";
	$qrs111 = mysql_query($q111,$conn1) or die(mysql_error());
	while($qr111 = mysql_fetch_assoc($qrs111))
	{
$ven = $qr111['name'];	
$vencode=$qr111['code'];
$vencontrollac = "";
...
include "config.php";
	...
	$q = "select sum(amount) as dramt from ac_financialpostings where (coacode = '$vencontrollac' or coacode = '$venprepayac') and crdr = 'Dr' and date <'$fdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	...
	$q = "select sum(amount) as cramt from ac_financialpostings where (coacode = '$vencontrollac' or coacode = '$venprepayac') and crdr = 'Cr' and date <'$fdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
 ...
 ?>
    ...
    $curdramt = 0;
	$curcramt = 0;
	$q = "select sum(amount) as dramt from ac_financialpostings where (coacode = '$vencontrollac' or coacode = '$venprepayac' ) and crdr = 'Dr' and date >= '$fdate' and date <= '$tdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  $curdramt = $curdramt + $qr['dramt']; 
	}
	
	$q = "select sum(amount) as cramt from ac_financialpostings where (coacode = '$vencontrollac' or coacode = '$venprepayac' ) and crdr = 'Cr' and date >= '$fdate' and date <= '$tdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  $curcramt = $curcramt + $qr['cramt']; 
	}
...
?>
	<tr>
		...
		<?php if($_SESSION[db]=='albustan' || $_SESSION[db]=='albustanlayer') { ?>
		<td class="ewRptGrpField1">
		<?php echo ewrpt_ViewValue($vencode) ?>
		</td>
		 <?php } ?>
		<td class="ewRptGrpField2" align="right">
		<?php  echo ewrpt_ViewValue(changeprice($obdramt)) ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php echo ewrpt_ViewValue(changeprice($obcramt)) ?>
		</td>
		<td align="right">
<?php echo ewrpt_ViewValue(changeprice($curdramt)) ?>
</td>
		<td align="right">
<?php echo ewrpt_ViewValue(changeprice($curcramt)) ?>
</td>
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($obdramt + $curdramt)); $totaldr = $totaldr + $obdramt + $curdramt; ?>
</td>
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($obcramt + $curcramt)); $totalcr = $totalcr + $obcramt + $curcramt;  ?>
</td>
<?php $totcramt = $obcramt + $curcramt;
      $totdramt = $obdramt + $curdramt;
	  $totflag = "Dr";
	  if ( $totcramt > $totdramt  )
	  {
	    $totflag = "Cr";
	  } ?>
<td align="right">
<?php if ( $totflag == "Dr" ) { echo ewrpt_ViewValue(changeprice($totdramt - $totcramt)); $totbaldr = $totbaldr + $totdramt - $totcramt;  } 
      else { echo ewrpt_ViewValue(changeprice("0")); }?>
</td>
<td align="right">
<?php if ( $totflag == "Cr" ) { echo ewrpt_ViewValue(changeprice($totcramt - $totdramt)); $totbalcr = $totbalcr + $totcramt - $totdramt; } 
      else { echo ewrpt_ViewValue(changeprice("0")); }?>
</td>
	</tr>
...
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($totaldr)) ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($totalcr));   ?>
</td>
<?php 
  $lastflag = "Dr";
  if ( $totalcr > $totaldr )
...
?>
<td align="right">
<?php if ( $lastflag == "Dr" ) { echo ewrpt_ViewValue(changeprice($totaldr - $totalcr));   } 
      else { echo ewrpt_ViewValue("&nbsp;"); }?>
</td>
<td align="right">
<?php if ( $lastflag == "Cr" ) { echo ewrpt_ViewValue(changeprice($totalcr - $totaldr));   } 
      else { echo ewrpt_ViewValue("&nbsp;"); }?>
</td>
	</tr>
