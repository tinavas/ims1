...
$EW_REPORT_TABLE_SQL_WHERE = "breeder_consumption.flock IN (SELECT DISTINCT(breeder_flock.flockcode) FROM breeder_flock WHERE $cullflockcondition)";
...
<br/>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="feedeggbird.php?export=html&flocktype=<?php echo $_GET['flocktype'];?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="feedeggbird.php?export=excel&flocktype=<?php echo $_GET['flocktype'];?>">Export to Excel</a>
&nbsp;&nbsp;<a href="feedeggbird.php?export=word&flocktype=<?php echo $_GET['flocktype'];?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="feedeggbird.php?cmd=reset&flocktype=<?php echo $_GET['flocktype'];?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<?php if (@$sExport == "") { ?>
<br /><br />
...
<form action="feedeggbird.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<?php
			  ...
			  $fpur = 0;
			  $mpur = 0;
			  $query14 = "SELECT sum(quantity) as ftin from ims_stocktransfer where towarehouse = '$x_flock' and date <= '$mindate' and cat = 'Female Birds' ";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
                 $ftin = $row14['ftin'];
              }
			  $q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$x_flock' AND date <= '$mindate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Female Birds')";  
		   ...
		   while($qr = mysql_fetch_assoc($r))
             ...
             else
             {
			  ...
			  $query14 = "SELECT sum(quantity) as mtin from ims_stocktransfer where towarehouse = '$x_flock' and date <= '$mindate' and cat = 'Male Birds' ";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
                 $mtin = $row14['mtin'];
		   ...
		   while($qr = mysql_fetch_assoc($r))
             ...
             else
             {
              ...
              $totalfeed = 0;
              $query14 = "SELECT sum(quantity) as 'feed' FROM breeder_consumption where flock = '$x_flock' and itemcode in ($feedtypes)";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
              ...
              $totalhatcheggs = 0;
             $query14 = "SELECT sum(quantity) as 'hatcheggs' FROM breeder_production where flock = '$x_flock' and itemcode in ($hatchegg)";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
...
<?php echo changequantity($totaleggs + $totalhatcheggs); ?>
		...
		<td<?php echo $sItemRowClass; ?> align="right" >
<?php echo changeprice(round(($totalfeed * 1000)/($femaleopening + $maleopening + $ftin + $fpur  + $mtin + $mpur),2)); ?>
</td>
<?php
...
$q2 = "SELECT SUM(quantity) as quantity FROM breeder_consumption WHERE flock = '$x_flock' AND date2 >= '$mindate' AND itemcode in ($feedtypes) AND client = '$client'";
...
function reloadpage()
{
	...
	document.location = "feedeggbird.php?flocktype=" + type ;
}
