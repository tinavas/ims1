...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Birds Sending Report</font></strong></td>
</tr>
<tr>
...
$EW_REPORT_TABLE_SQL_WHERE = "ac_financialpostings.date >= '$fromdate' and ac_financialpostings.date <= '$todate'";
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="birdssending.php?export=html&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="birdssending.php?export=excel&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="birdssending.php?export=word&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Export to Word</a>
...
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" align="center">
		Chicken Shop/ Processing Unit
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Chicken Shop/ Processing Unit</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
include "config.php";
   ...
   ?>
	...
	$query = "select * from broiler_chickentransfer where date >= '$fromdate' and date <= '$todate'";
...
<?php 
$query1 = "select supervisor from broiler_farm where farm = '$rows[farmer]'";
$result1 = mysql_query($query1,$conn1) or die(mysql_error());
while($qr1 = mysql_fetch_assoc($result1))
{
?>
<td<?php echo $sItemRowClass; ?>>
<?php if(($qr1['supervisor'] <> $oldcode1) or ($dumm1 == 0)){ echo ewrpt_ViewValue($qr1['supervisor']);
     $oldcode1 = $qr1['supervisor']; $dumm1 = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
	 } ?>
</td>   
<?php } ?>
<td<?php echo $sItemRowClass; ?>>
<?php if(($rows['farmer'] <> $oldcode2) or ($dumm2 == 0)){ echo ewrpt_ViewValue($rows['farmer']);
     $oldcode2 = $rows['farmer']; $dumm2 = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
	 } ?>
</td>   
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($rows['flock']); ?></td>
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($rows['chickenshop']); ?></td>
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($rows['birds']); $totbirds = $totbirds + $rows['birds'];?></td>
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($rows['weight']); $totweight = $totweight + $rows['weight'];?></td>
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($rows['price']); $totprice = $totprice + $rows['price'];?></td>
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($rows['mortality']);  $totmort = $totmort + $rows['mortality'];?></td>
<?php $amount = ($rows['weight']*$rows['price']) ?>
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($amount); $totamount = $totamount + $amount;?></td>
...
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo $totweight;?></b></td>
...
function reloadpage()
{
	...
	document.location = "birdssending.php?category="+category+"&fromdate=" + fromdate + "&todate=" + todate;
}
</script>-->
<script type="text/javascript">
...
document.location = 'birdssending.php?export=' + a + '&fromdate=' + fdate + '&todate=' + tdate;
}
</script>