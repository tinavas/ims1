...
if($unit == 'All')
{
$cond = "in (select distinct(unitcode) from layerbreeder_unit where client = '$client')";
}
else
{
$cond = "=".""."'".$unit."'";
...
if($_GET['weeks'] <> "")
 ...
 $weeks = "0";
  $url = "&weeks=" . $_GET['weeks']."&unit=" . $_GET['unit']."&cull=" . $_GET['cull'] ;
?>
<?php
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Layer Breeder Future Production Analysis<br/> For Weeks : <?php echo $weeks;?></font></strong></td>
</tr>
</table>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="lbfutureplanning.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="lbfutureplanning.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="lbfutureplanning.php?export=word<?php echo $url; ?>">Export to Word</a>
...
<?php
	...
	$cst = 0;
	$female = 0;
$q1 = "select * from layerbreeder_flock where cullflag = '0' and unitcode $cond order by unitcode";
...
$q2 = "select * from layerbreeder_flock where flockcode = '$flock' and unitcode $cond and client = '$client' and shedcode = '$shed' order by shedcode";
...
$fage = 0;
$q2 = "select max(age) as age from layerbreeder_consumption where flock = '$flock'";
...
$q3 = "select * from layerbreeder_standards where (age >= '$fage' && age <= '$tage') and client = '$client' order by age";
...
$q9 = "select * from layerbreeder_standards where age > $fage and age <= $tage and breed = '$breed' and client = '$client'";
...
//echo $cummhhe[$toage]."/".$cumhhe[$fromage]."/".$hhedif;
 $minus = 0; 
$q4 = "select distinct(date2),fmort,fcull from layerbreeder_consumption where flock = '$flock' and client = '$client'";
...
//Inward
	 $q5 = "select sum(quantity) as trinbirds from ims_stocktransfer where towarehouse = '$flock' and cat = ' LB Female Birds' and client = '$client'";
 ...
 //Outward
		$q6= "select sum(quantity) as troutbirds from ims_stocktransfer where fromwarehouse = '$flock' and cat = 'LB Female Birds' and client = '$client'";
...
//Sale
		 $q7 = "select sum(quantity) as soldbirds from oc_cobi where flock = '$flock' and code in (select code from ims_itemcodes where cat = 'LB Female Birds') and client = '$client'";
		...
		//Purchase
		 $q8 = "select sum(receivedquantity) as purchasebirds from pp_sobi where flock = '$flock' and code in (select code from ims_itemcodes where cat = 'LB Female Birds') and client = '$client'";
...
///////////////For Mortality and Culls
 $minus = 0; 
$q4 = "select distinct(date2),mmort,mcull from layerbreeder_consumption where flock = '$flock' and client = '$client'";
        $r4 = mysql_query($q4,$conn1);
	    while($qr4 = mysql_fetch_assoc($r4))
        {
...
//Inward
$transferin = 0;
	 $q5 = "select sum(quantity) as trinbirds from ims_stocktransfer where towarehouse = '$flock' and cat = 'LB Male Birds' and client = '$client'";
        $r5 = mysql_query($q5,$conn1);
	    while($qr5 = mysql_fetch_assoc($r5))
        {
	      $transferin = $qr5['trinbirds'];
 ...
 //Outward
 $transferout = 0;
		 $q6= "select sum(quantity) as troutbirds from ims_stocktransfer where fromwarehouse = '$flock' and cat = 'LB Male Birds' and client = '$client'";
        $r6 = mysql_query($q6,$conn1);
	    while($qr6 = mysql_fetch_assoc($r6))
        {
	      $transferout = $qr6['troutbirds'];
...
//Sale
$soldbirds = 0;
		 $q7 = "select sum(quantity) as soldbirds from oc_cobi where flock = '$flock' and code in (select code from ims_itemcodes where cat = 'LB Male Birds') and client = '$client'";
        $r7 = mysql_query($q7,$conn1);
	    while($qr7 = mysql_fetch_assoc($r7))
        {
	      $soldbirds = $qr7['soldbirds'];
		...
		//Purchase
		$prchbirds = 0;
		 $q8 = "select sum(receivedquantity) as purchasebirds from pp_sobi where flock = '$flock' and code in (select code from ims_itemcodes where cat = 'LB Male Birds') and client = '$client'";
        $r8 = mysql_query($q8,$conn1);
	    while($qr8 = mysql_fetch_assoc($r8))
        {
	      $prchbirds = $qr8['purchasebirds'];
...
$toage = $qr10['toage'];
$q11 = "select sum(ffeed) as ffeed from layerbreeder_standards where (age between $fromage and $toage) and (age >= $fage and age <= $tage)";
...
$newcull = $newcull;
	...
	<td class="ewRptGrpField2"><?php if(($flock <> "") && ($cull > $age2)) { ?><a href="lbindiflock.php?flock=<?php echo $flock;?>&weeks=<?php echo $newcull; ?>"><?php echo ewrpt_ViewValue($flock); ?></a><?php } else { ?><?php echo ewrpt_ViewValue($flock); ?><?php } ?></td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php  if($birds > 0 ) { echo ewrpt_ViewValue(changequantity($birds)); } else { echo ewrpt_ViewValue("0"); } $totbirds = $totbirds + $birds;?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php  if($male > 0 ) { echo ewrpt_ViewValue(changequantity($male)); } else { echo ewrpt_ViewValue("0"); } $totmbirds = $totmbirds + $male;?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php  if($tp > 0 ) { echo ewrpt_ViewValue(changequantity($tp)); } else { echo ewrpt_ViewValue("0"); } $hatcheggs = $hatcheggs + $tp;?>
</td>

</td>
<td<?php echo $sItemRowClass; ?> align="right">																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																															<?php if($tableeggs > 0 ) { echo ewrpt_ViewValue(changequantity($tableeggs)); } else { echo ewrpt_ViewValue("0"); } $tottable = $tottable + $tableeggs; ?>
</td>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($totaleggs > 0 ) { echo ewrpt_ViewValue(changequantity($totaleggs)); } else { echo ewrpt_ViewValue("0"); } $tottotal = $tottotal + $totaleggs; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($feedcons > 0 ) { echo ewrpt_ViewValue(changeprice($feedcons)); } else { echo ewrpt_ViewValue("0"); } $totfeedcons = $totfeedcons + $feedcons?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($feedconsmale > 0 ) { echo ewrpt_ViewValue(changeprice($feedconsmale)); } else { echo ewrpt_ViewValue("0"); } $totfeedconsmale = $totfeedconsmale + $feedconsmale?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php  if($birds > 0 ) { echo ewrpt_ViewValue(changequantity($birds)); } else { echo ewrpt_ViewValue("0"); } ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php  if($male > 0 ) { echo ewrpt_ViewValue(changequantity($male)); } else { echo ewrpt_ViewValue("0"); } ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php  if($tp > 0 ) { echo ewrpt_ViewValue("0"); } else { echo ewrpt_ViewValue("0"); } ?>
</td>

</td>
<td<?php echo $sItemRowClass; ?> align="right">																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																															<?php if($tableeggs > 0 ) { echo ewrpt_ViewValue("0"); } else { echo ewrpt_ViewValue("0"); }  ?>
</td>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($totaleggs > 0 ) { echo ewrpt_ViewValue("0"); } else { echo ewrpt_ViewValue("0"); }  ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($feedcons > 0 ) { echo ewrpt_ViewValue(changequantity($feedcons)); } else { echo ewrpt_ViewValue("0"); } ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($feedconsmale > 0 ) { echo ewrpt_ViewValue(changequantity($feedconsmale)); } else { echo ewrpt_ViewValue("0"); } ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
...
<td <?php echo $sItemRowClass; ?>><b>&nbsp;</b></td>
<td <?php echo $sItemRowClass; ?>><b>&nbsp;</b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($totbirds); ?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($totmbirds); ?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($hatcheggs); ?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($tottable);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($tottotal);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b> <?php echo changeprice($totfeedcons); ?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changeprice($totfeedconsmale);?></b></td>
...
<tr>
<?php 
$s1 = "select * from layerbreeder_shed where shedcode in('$upshed') GROUP BY shedcode";
...
$q3 = "select * from layerbreeder_standards where age = $newage and client = '$client' order by age";
$r3 = mysql_query($q3,$conn1) or die(mysql_error());
while($qr3 = mysql_fetch_assoc($r3))
{
...
$q9 = "select * from layerbreeder_standards where age <= $newage and breed = '$breed' and client = '$client'";
$r9 = mysql_query($q9,$conn1) or die(mysql_error());
while($qr9 = mysql_fetch_assoc($r9))
{
$cffeed = $cffeed + $qr9['cffeed'];
...
$fromage = $qr10['fromage'];
$toage = $qr10['toage'];
$q11 = "select sum(ffeed) as ffeed from layerbreeder_standards where (age between $fromage and $toage) and (age >= '0' and age <= $newage)";
$r11 = mysql_query($q11,$conn1) or die(mysql_error());
...
<td class="ewRptGrpField2"><?php if($shed <> "") { ?><a href="lbindiflockshed.php?shed=<?php echo $shed;?>&weeks=<?php echo $d[$shed]; ?>&female=<?php echo $fdif; ?>&male=<?php echo $mdif; ?>"><?php echo ewrpt_ViewValue($shed); ?></a><?php } else { ?>&nbsp;<?php } ?></td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php  echo ewrpt_ViewValue(changequantity($fdif)); ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changequantity($mdif)); ?>
</td>
...
function reloadpage(form)
{
	...
	document.location = "lbfutureplanning.php?weeks=" + weeks + "&unit=" +  uni + "&cull=" + cull;
