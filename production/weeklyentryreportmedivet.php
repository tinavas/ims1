...
$minage = 0;
$maxage = 0;
 $query1 = "SELECT min(date1) as mindate,max(date1) as maxdate from breeder_consumption where flock = '$flock' and client = '$client'  ";
...
<?php
  $fopening = 0;
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
		 {
             ...
             $minus = 0; 
             $q = "select distinct(date2),fmort,fcull from breeder_consumption where flock = '$flock' and client = '$client'  and date2 < '$mindatec' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276"> Weekly Report</font></strong></td>
</tr>
<tr>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="weeklyentryreportmedivet.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="weeklyentryreportmedivet.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="weeklyentryreportmedivet.php?export=word<?php echo $url; ?>">Export to Word</a><br /><br />
<?php } ?>
...
<?php
 /* $fopening = 0;
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
		 {
             ...
             $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Birds' and client = '$client'  and fromwarehouse = '$flock' and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
		   ...
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Birds' and client = '$client'  and towarehouse = '$flock' and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
...
while($agefrom <= $maxage )
{
  ...
  $query1 = "SELECT  distinct(date2),fmort,fcull from breeder_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	...
	$query1 = "SELECT  sum(quantity) as quant,max(avgwt) as bdwt from breeder_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' and itemcode in (select code from ims_itemcodes where cat = 'Female Feed' and client = '$client') ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	    $wfeed = $row2['quant'];
	  ...
	  $query1 = "SELECT  itemcode,sum(quantity) as quant,max(date2) as date from breeder_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' and itemcode in (select code from ims_itemcodes where cat = 'Female Feed' and client = '$client') group by itemcode ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
