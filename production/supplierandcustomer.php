...
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Supplier And Customer Ledger</font></strong></td>
</tr>
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Supplier (or) Customer Name&nbsp;<?php echo $ven; ?></font></strong></td>
</tr>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="supplierandcustomer.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="supplierandcustomer.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="supplierandcustomer.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<form action="supplierandcustomer.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
include "config.php";
 ...
 $q = "select vgroup,cgroup from contactdetails  where name = '$ven' and type = 'vendor and party' and client = '$client'";
	...
	$vid = $qr['id'];
		...
		$venc = "";
	$q01 = "select * from ac_vgrmap  where vgroup IN ('$vengroup','$vengroup1') and client = '$client'";
	$qrs01 = mysql_query($q01,$conn1) or die(mysql_error());	
	$qr01 = mysql_fetch_assoc($qrs01);
	$b=$qr01['vca'];
	$qr01 = mysql_fetch_assoc($qrs01);
	$ab=$qr01['vca'];
	
	
 $q = "select sum(amount) as dramt from ac_financialpostings where coacode in ('$b','$ab')  and client = '$client' and crdr = 'Dr' and date <'$fdate' and venname = '$ven'";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	$obdramt = $obdramt + $qr['dramt']; 
	 ...
	 $q = "select sum(amount) as cramt from ac_financialpostings where coacode in ('$b','$ab') and client = '$client' and crdr = 'Cr' and date <'$fdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
...
<?php  if ( $obflag == "Cr" ) { echo ewrpt_ViewValue(changeprice($obbal)); $crtotal = $crtotal + $obbal;  } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
	...
	$q = "select * from ac_financialpostings where date >= '$fdate' and date <= '$tdate'  and venname = '$ven' and coacode in ('$b','$ab') order by date ";
	...
	if ( $qr['type'] == "SOBI" )
	  {
	    ...
	    $q1 = "select invoice,receivedquantity,rateperunit,description from pp_sobi where so = '$qr[trnum]'";
	  ...
	  {
	      $mode = "PMT-Payment";
	     $q1 = "select paymentmode,cheque from pp_payment where tid = '$qr[trnum]'";
	      $qrs1 = mysql_query($q1,$conn1) ;
	      while($qr1 = mysql_fetch_assoc($qrs1))
	      {
	  ...
	  {
	    $mode = "Setting Charges";
	  }

	 if ( $qr['type'] == "COBI" )
	  {
	    $mode = "COBI-Customer Order Based Invoice";
	    $q1 = "select distinct(code),description,price,quantity from oc_cobi where invoice = '$qr[trnum]'";
	    $qrs1 = mysql_query($q1,$conn1) ;
		$cnt = mysql_num_rows($qrs1);
		$cur = 1;
	    while($qr1 = mysql_fetch_assoc($qrs1))
	    {
		$invoice = $qr1['bookinvoice'];
		   if($cur == $cnt)
		  {
		    $narration = $narration.$qr1['description'];
