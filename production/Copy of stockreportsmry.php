...
<?php
		...
		$total_opening=$total_purchase=$total_consumed=$total_sold=$total_closing=$total_amount=0;
	}
...
$getop = "SELECT SUM(quantity) as quantity FROM ims_stockadjustment WHERE code = '$x_code' AND date < '$fromdate' AND unit = '$warehouse' and type = 'Deduct'";
  ...
  //$getop = "SELECT sum(quantity) as quantity FROM breeder_consumption WHERE itemcode = '$x_code' AND date1 < '$fromdate' AND flock = '$warehouse'";
   ...
   if($cat=='Male Feed' || $cat=='Female Feed')
  {
  ...
  else if ($cat=='LB Female Feed' || $cat=='LB Male Feed')
  {
   $getop = "SELECT sum(quantity) as quantity FROM layerbreeder_consumption WHERE itemcode = '$x_code' AND date2 < '$fromdate' AND flock in (select distinct(flockcode) from layerbreeder_flock where unitcode = '$brdunit')";
  $resultop = mysql_query($getop,$conn1) or die(mysql_error());
  while($rowop = mysql_fetch_assoc($resultop))
...
//echo $consumedop."/".$salesop."/".$iiscop."/".$staded."/".$bcop."/".$bdeop."/".$stockfrom; 
...
$getop = "SELECT SUM(quantity) as quantity FROM ims_stockadjustment WHERE code = '$x_code' AND date >= '$fromdate' AND  date <= '$todate' and unit = '$warehouse' and type = 'Deduct'";
  ...
  {
   $getop = "SELECT sum(quantity) as quantity FROM layerbreeder_consumption WHERE itemcode = '$x_code' AND date2 >= '$fromdate' AND  date2 <= '$todate' AND flock in (select distinct(flockcode) from layerbreeder_flock where unitcode = '$brdunit')";
  $resultop = mysql_query($getop,$conn1) or die(mysql_error());
  while($rowop = mysql_fetch_assoc($resultop))
     $consumed = $consumed + $rowop['quantity'];
  }
...
<?php echo round($opening,2); $total_opening+=$opening; ?>
</td>

		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
<?php echo round($purchased,2); $total_purchase+=$purchased; ?>
</td>

		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
<?php echo round($consumed,2); $total_consumed+=$consumed; ?>
</td>

<td<?php echo $sItemRowClass; ?> style="text-align:right;">
<?php echo $sales; $total_sold+=$sales; ?>
</td>

		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
<?php echo round(($closing),2); $total_closing+=$closing; ?>
</td>
...
<td<?php echo $sItemRowClass; ?> style="text-align:right;">
<?php echo round(($closing*$avgcost),2); $total_amount+=($closing*$avgcost); ?>
</td>
<?php } ?>
	</tr>
<?php }
...
?>
<tr><td colspan="3" align="right">Total</td><td align="right" ><?php echo round($total_opening,2) ;?></td><td align="right" ><?php echo round($total_purchase,2) ;?></td><td align="right" ><?php echo round($total_consumed,2) ;?></td><td align="right" ><?php echo round($total_sold,2) ;?></td><td align="right" ><?php echo round($total_closing,2) ;?></td><?php if($_SESSION[db]=='maharashtra') {  ?> <td align="right" ><?php echo '&nbsp;' ?></td><td align="right" ><?php echo round($total_amount,2) ;?></td> <?php } ?></tr>
	</tbody>
	<tfoot>
<?php
