...
$weeks = $_GET['weeks'];
$flock = $_GET['flock'];
$url = "&flock=" . $_GET['flock']."&weeks=" . $_GET['weeks'] ;
?>
<?php
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Layer Breeder Future Weekly Report</font></strong></td>
</tr>
<tr>
<td colspan="2" style="text-align:center" ><strong><font color="#3e3276" size="2">Flock:<?php echo $flock; ?> For Weeks : <?php echo $weeks;?></font></strong></td>
</tr>
</table>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="lbindiflock.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="lbindiflock.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="lbindiflock.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php
$ceggs = 0;
 $q1 = "select * from layerbreeder_flock where flockcode = '$flock' and client = '$client'";
...
$qr8 = mysql_fetch_assoc($r8);
$prchbirds = $qr8['purchasebirds'];
//echo $femalenew."/".$minus."/".$transferin."/".$transferout."/".$soldbirds."/".$prchbirds;
$female = $femalenew - $minus + $transferin - $transferout - $soldbirds + $prchbirds;
$totfe = $female;
/////////////////////////////////MALE BIRDS CALCULATION///////////////////////////////////////////////////////
///////////////For Mortality and Culls
$minus = 0; 
...
$male = $malenew - $minus + $transferin - $transferout - $soldbirds + $prchbirds;
$totma = $male;

$qyu = "select * from layerbreeder_consumption where flock = '$flock' and client = '$client'";
$ryu = mysql_query($qyu,$conn1);
$nofr = mysql_num_rows($ryu);
while($qryu = mysql_fetch_assoc($ryu))
{
 $fage = ceil($qryu['age']/7);
  $tage = ceil($fage + $weeks);
}  
...
$bweight = 0;
$eweight = 0;
$query1 = "SELECT * FROM layerbreeder_standards where client = '$client' and (age >  '$fage' and age <= '$tage') and breed = '$breed' ORDER BY age ASC ";
$r = mysql_query($query1,$conn1);
while($qr = mysql_fetch_assoc($r))
{
if($qr['age'] == " ")
{
$q = "select * from layerbreeder_flock where flock = '$flock' and client = '$client'";
...
$hepercen = $qr['heggper'];
//echo $qr['eggbird']."/".$female; echo "<br/>";
$hatcheggs = round(($qr['eggbird'] * $female),0);
$totaleggsprod = round(($qr['alleggbird'] * $female),0);
$heggper = round((($hepercen * $prodstd)/100),0);
$tableeggs = $totaleggsprod - $hatcheggs;
...
$query3 = "select * from feed_cost where code in (select distinct(code) from ims_itemcodes where cat = 'Male Feed') and client = '$client'";
$result3 = mysql_query($query3,$conn1) or die(mysql_error());
while($qr3 = mysql_fetch_assoc($result3))
{
$mcostperkg = $qr3['costperkg'];
}

$pc2 = round(( $mfeed * $mcostperkg ),2);
$pc = $pc1 + $pc2;
$cumpc = $cumpc + $pc;
?>
<tr>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($female > 0){ echo ewrpt_ViewValue(changequantity($female)); } else { echo ewrpt_ViewValue("0"); } ?>
</td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($hatcheggs > 0) {  echo ewrpt_ViewValue(changequantity($hatcheggs)); } else { echo ewrpt_ViewValue("0"); } $tothegg = $tothegg + $hatcheggs; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($tableeggs > 0) {  echo ewrpt_ViewValue(changequantity($tableeggs)); } else { echo ewrpt_ViewValue("0"); } $tottable = $tottable + $tableeggs;?></td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($totaleggsprod > 0) {  echo ewrpt_ViewValue(changequantity($totaleggsprod)); } else { echo ewrpt_ViewValue("0"); } $totprodstd = $totprodstd + $totaleggsprod;?></td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($ffeed > 0) {  echo ewrpt_ViewValue(changeprice($ffeed)); } else { echo ewrpt_ViewValue("0"); } $ffintake = $ffintake + $ffeed; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($mfeed > 0) {  echo ewrpt_ViewValue(changeprice($mfeed)); } else { echo ewrpt_ViewValue("0"); } $mfintake = $mfintake + $mfeed;?>
</td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($fweight > 0) {  echo ewrpt_ViewValue(changeprice($fweight)); } else { echo ewrpt_ViewValue("0"); } ?>
</td>
...
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($totfe);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($totma);?></b></td>
...
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($tothegg);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($tottable);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($totprodstd);?></b></td>
<td <?php echo $sItemRowClass; ?>><b>&nbsp;</b></td>
<td <?php echo $sItemRowClass; ?>><b>&nbsp;</b></td>
<td <?php echo $sItemRowClass; ?>><b>&nbsp;</b></td>
<td <?php echo $sItemRowClass; ?>><b>&nbsp;</b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changeprice($ffintake);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changeprice($mfintake);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php $totfperegg = round((($ffintake/$totprodstd)*1000),2); echo changeprice($totfperegg);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php $totfperbird = round((($ffintake/$totfe)*1000),2); echo changeprice($totfperbird);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b> <?php echo changeprice($bweight); ?></b></td>
