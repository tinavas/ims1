...
$EW_REPORT_TABLE_SQL_WHERE = "broiler_daily_entry.place $pc '$place' and broiler_daily_entry.supervisior $sc '$supervisor' and broiler_daily_entry.farm $fc '$farm' and broiler_daily_entry.flock $flc '$flock' and $cullflockcondition ";
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="broilerdailyentry.php?export=html&place=<?php echo $place; ?>&supervisor=<?php echo $supervisor; ?>&farm=<?php echo $farm; ?>&flock=<?php echo $flock; ?>&flocktype=<?php echo $_GET['flocktype']; ?>&fromdate=<?php echo date($datephp,strtotime($fromdate)); ?>&todate=<?php echo date($datephp,strtotime($todate)); ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="broilerdailyentry.php?export=excel&place=<?php echo $place; ?>&supervisor=<?php echo $supervisor; ?>&farm=<?php echo $farm; ?>&flock=<?php echo $flock; ?>&flocktype=<?php echo $_GET['flocktype']; ?>&fromdate=<?php echo date($datephp,strtotime($fromdate)); ?>&todate=<?php echo date($datephp,strtotime($todate)); ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="broilerdailyentry.php?export=word&place=<?php echo $place; ?>&supervisor=<?php echo $supervisor; ?>&farm=<?php echo $farm; ?>&flock=<?php echo $flock; ?>&flocktype=<?php echo $_GET['flocktype']; ?>&fromdate=<?php echo date($datephp,strtotime($fromdate)); ?>&todate=<?php echo date($datephp,strtotime($todate)); ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="broilerdailyentry.php?cmd=reset&place=<?php echo $place; ?>&supervisor=<?php echo $supervisor; ?>&farm=<?php echo $farm; ?>&flock=<?php echo $flock; ?>&fromdate=<?php echo date($datephp,strtotime($fromdate)); ?>&todate=<?php echo date($datephp,strtotime($todate)); ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<form action="broilerdailyentry.php" name="ewpagerform" id="ewpagerform" class="ewForm">
<table>
 <tr>
 <td>From Date </td>
...
<?php $q1 = "select distinct(supervisior) from broiler_daily_entry where place = '$place' order by supervisior";
...
<?php $q1 = "select distinct(farm) from broiler_daily_entry where supervisior = '$supervisor' order by farm";
	$q1rs = mysql_query($q1,$conn1) or die(mysql_error());
			while($qr = mysql_fetch_assoc($q1rs))
			{
			?>
...
<?php $q1 = "select distinct(flock) from broiler_daily_entry where supervisior = '$supervisor' order by flock";
	$q1rs = mysql_query($q1,$conn1) or die(mysql_error());
			while($qr = mysql_fetch_assoc($q1rs))
			{
			?>
...
<?php } ?>
<?php if(($_SESSION[db])=='cps') { ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Prod.Cost/Kg">
		Prod.Cost/Kg
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Prod.Cost/Kg">Prod.Cost/Kg</td>
			</tr></table>
		</td>
<?php } }?>
...
{ 
  ...
  $soldbirds = 0;
   ...
   $query111a = "SELECT sum(quantity) as chicks,sum(tmort) as mort,sum(shortage) as short FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' AND (cat = 'Broiler Chicks' or cat = 'Broiler Day Old Chicks') and date <= '$x_entrydate'"; $result111a = mysql_query($query111a,$conn1);  $rowsa = mysql_num_rows($result111a);
  if ( $rowsa > 0) 
  { 
  while($row111a = mysql_fetch_assoc($result111a)) 
  { 
  $chicks=$row111a['chicks']; 
 $mort=$row111a['mort']; 
 $short=$row111a['short']; 
 $chk=$chicks-$mort-$short;
 $birds = $birds + $chk;
  //$birds = $birds + $row111a['chicks']; 
   } 
   }

  //if(($birds == "") OR ($birds == 0))
  {
 $query111a = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm' and (description = 'Broiler Chicks' or description = 'Broiler Day Old Chicks') and adate <= '$x_entrydate'"; $result111a = mysql_query($query111a,$conn1);  $rowsa = mysql_num_rows($result111a);
   if ( $rowsa > 0) { while($row111a = mysql_fetch_assoc($result111a)) {  $birds = $birds + $row111a['chicks']; $c=$row111a['chicks'];  } }
  }
  if($_SESSION['db']!="cps")
  {
  ...
  $x = "0"; $mortcumm = $mortcumm + $x_mortality; 
  $cummfeed = $cummfeed + $x_feedconsumed; $oldfarmer = $x_farm; $flock = $x_flock;
  //if ($birds == "") { $birds = $birds - $x_mortality; } else { $birds = $birds - $x_mortality; }
  $oobirds = $birds;
// if($x_flock=="KHR2-1")
// {
// echo $mortcumm ."/".$soldbirds."/".$cull;
// }
...
else
{
 ...
 $x = "1"; include "../config.php"; $b = 0; $birds = 0;
  $query111 = "SELECT sum(quantity) as chick,sum(tmort) as mort,sum(shortage) as short FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' and date <= '$x_entrydate' and (cat = 'Broiler Chicks' or cat = 'Broiler Day Old Chicks')"; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
 if ( $rows > 0)
  { 
 ...
 //if(($birds == "") OR ($birds == 0))
 {
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm' and adate <= '$x_entrydate' and (description = 'Broiler Chicks' or description = 'Broiler Day Old Chicks')";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  { $birds = $birds + $row111['chicks'];  $c=$row111['chicks']; } }
 }
   ...
   if($_SESSION['db'] == 'central')
{
  ...
  $query2 = "select sum(mortality) as cmortality,sum(cull) as ccull from broiler_daily_entry where flock = '$x_flock' and farm = '$x_farm' and (entrydate <  '$x_entrydate' or entrydate = '$x_entrydate')";
...
$res2 = mysql_fetch_assoc($result2);
 ...
 if($_SESSION['db']=="cps") 
 $birds = $birds - $cmortality;
  else if($_SESSION['db']=="apf")
$birds = $birds - $cmortality-$ccull;
 else
 ...
 $mortcumm = "0";
 $cummfeed = "0";
  if($_SESSION['db']=="cps" ||$_SESSION['db']=="apf")
 $mortcumm = $mortcumm + $cmortality;
...
{
$medvac=$labch=0;
 $q1="select sum(feedconsumed) as feed from broiler_daily_entry where entrydate<='$x_entrydate' and flock='$x_flock'";
$q1=mysql_query($q1) or die(mysql_error());
$r1=mysql_fetch_assoc($q1);
  $cummfeed=$r1['feed'];
   $oobirds=$birds;
  $qmed = "SELECT sum(quantity) as qty,avg(price) as price FROM ims_stocktransfer where aflock = '$x_flock'  and date <= '$x_entrydate' and code in (select code from ims_itemcodes where cat like 'Medicines' or cat like 'Vaccines')"; 
   $rmed = mysql_query($qmed,$conn1);  
   $remed = mysql_fetch_array($rmed);
   $medvac=$remed[qty]*$remed[price];
   $qlc = "SELECT sum(cramount+dramount) as lc FROM ac_gl where warehouse= = '$x_flock'  and date <= '$x_entrydate' and description='Labour Charges' and voucher='P'"; 
   $rlc = mysql_query($qlc,$conn1);  
   $rlc = mysql_fetch_array($rlc);
   $labch=$rlc[lc];
   
   
      $query = "SELECT SUM(quantity) as sum,sum(quantity*price) as freturnamount FROM ims_stocktransfer where  aflock='$x_flock' and cat = 'Broiler Feed' and date<='$x_entrydate'";
           $result = mysql_query($query,$conn1); 
           while($row1 = mysql_fetch_assoc($result))
           {
            $feedreturn=$fr = $row1['sum'];
			$freturnamount=$row1['freturnamount'];
           }
		 //  if($x_farm=="AKIL B")
           	  // echo $query;
       $query = "SELECT SUM(quantity * price) as sum,SUM(quantity) as qty  FROM ims_stocktransfer where towarehouse = '$x_farm' and cat = 'Broiler Feed' and date<='$x_entrydate' AND client = '$client'";
		$result = mysql_query($query,$conn1); 
		$row1 = mysql_fetch_assoc($result);	
			...
			$fq1=$row1[qty];
		     //if($x_farm=="HERAGU")
           	  // echo $query;
       $query = "SELECT SUM(quantity * price) as sum,SUM(quantity) as qty FROM ims_stocktransfer where fromwarehouse = '$x_farm' and cat = 'Broiler Feed' and date<='$x_entrydate' AND client='$client'";
		$result = mysql_query($query,$conn1); 
		$row1 = mysql_fetch_assoc($result);
			$feedcost1 = $row1['sum'];
			$fq2=$row1[qty];
		   
		  $query = "SELECT SUM(receivedquantity * rateperunit) as sum, SUM(receivedquantity ) as qty FROM pp_sobi where warehouse = '$x_farm' and code like 'BFD%' and date<='$x_entrydate' AND client = '$client'";
		$result = mysql_query($query,$conn1); 
		$row1 = mysql_fetch_assoc($result);
			$feedpurchase = $row1['sum'] ;
			$fp=$row1[qty];
		   
	$feedq=$fq1-$fq2+$fp;
	$feedq	=$feedq-$fr;  
 $feedc = $feedcost - $feedcost1+$feedpurchase;
$feedc=$feedc-$freturnamount;
   $rt=$feedc/$feedq;
...
$fcr = round(($cummfeed / (($x_average_weight / 1000) * $birds )),2)
//$fcr = round(($feedtotal / (($x_average_weight / 1000) * $birds )),2)
...
<?php if($fcr<0) echo "0"; else echo $fcr;  ?>
	...
	<?php if(($_SESSION[db])=='cps') { ?>
    <td<?php echo $sItemRowClass; ?>>
<?php   $x_avg=(($cummfeed*$rt)+$medvac+$labch)/($oobirds*($x_average_weight/1000)); //echo $cummfeed."*31.5+".$x_medicine_quantity."/".$oobirds."*".(intval($x_average_weight)*1000)."="; 
//echo $oobirds."/".$birds;
echo ewrpt_ViewValue(round($x_avg,2)) ?>
</td>
    	<?php }?>
		<td<?php echo $sItemRowClass; ?>>
...
<?php if ($nTotalGrps > 0) { ?>
	...
	<tr class="ewRptGrandSummary"><td<?php if($_SESSION['db'] == 'central'){?> colspan="25" <?php } else { ?> colspan="23" <?php } ?> >Grand Total (<?php echo ewrpt_FormatNumber($rstotcnt,0,-2,-2,-2); ?> Detail Records)</td></tr>
...
<!--first page button-->
	...
	<td><a href="broilerdailyentry.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="broilerdailyentry.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="broilerdailyentry.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="broilerdailyentry.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function reloadpage()
{
	...
	document.location = "broilerdailyentry.php?place=" + place + "&supervisor=" + supervisor + "&farm=" + farm + "&flock=" + flock+ "&flocktype=" + type + '&fromdate=' + fdate + '&todate=' + tdate;
}
