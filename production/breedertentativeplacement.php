...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="breedertentativeplacement.php?export=html&culldays=<?php echo $culldays; ?>&gapdays=<?php echo $gapdays; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="breedertentativeplacement.php?export=excel&culldays=<?php echo $culldays; ?>&gapdays=<?php echo $gapdays; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="breedertentativeplacement.php?export=word&culldays=<?php echo $culldays; ?>&gapdays=<?php echo $gapdays; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="breedertentativeplacement.php?cmd=reset&culldays=<?php echo $culldays; ?>&gapdays=<?php echo $gapdays; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$query1 = "select distinct(farmcode) from breeder_flock where cullflag = '0' and client = '$client' order by farmcode";
$result1 = mysql_query($query1,$conn1) or die(mysql_error());
while($rows1 = mysql_fetch_assoc($result1))
{
 $farm = $dfarm = $rows1['farmcode'];
 $query2 = "select distinct(unitcode) from breeder_flock where cullflag = '0' and farmcode = '$farm' and client = '$client' order by unitcode";
 $result2 = mysql_query($query2,$conn1) or die(mysql_error());
 while($rows2 = mysql_fetch_assoc($result2))
 {
 $unit = $dunit = $rows2['unitcode'];
 $query3 = "select distinct(shedcode) from breeder_flock where cullflag = '0' and farmcode = '$farm' and unitcode = '$unit' and client = '$client' order by shedcode";
 $result3 = mysql_query($query3,$conn1) or die(mysql_error());
 while($rows3 = mysql_fetch_assoc($result3))
 {
 $shed = $dshed = $rows3['shedcode'];
 $query4 = "select distinct(flockcode),age,startdate from breeder_flock where cullflag = '0' and farmcode = '$farm' and unitcode = '$unit' and shedcode = '$shed' and client = '$client' order by flockcode";
 ...
 while($rows4 = mysql_fetch_assoc($result4))
 {
  ...
  $query5 = "select max(age) as age,max(date2) as date2 from breeder_consumption where flock = '$flock' and client = '$client'";
...
<?php echo ewrpt_ViewValue(date($datephp,strtotime($rows5['date2']))); ?></td>
...
function reloadpage()
{
	...
	document.location = "breedertentativeplacement.php?cullage=" + cullage + "&gapdays=" + gapdays;
}
</script>