...
$flock='';
$flkmain = $_GET['flock'];
 $query = "SELECT flockcode FROM layerbreeder_flock WHERE flockcode = '$flkmain' AND client = '$client'";
...
echo $flock = $_GET['flock'];

 $agequery = "select max(age) as age from layerbreed_standards where breed = '$breedi'";
...
$minage = 0;
$maxage = 0;
  $query1 = "SELECT min(date1) as mindate,max(date1) as maxdate from layerbreeder_production where flock in ($flock) and client = '$client'  ";
...
$mindatec = "";
$maxdatec = "";
  $query1 = "SELECT min(date2) as mindate,max(date2) as maxdate,min(age) as minage,max(age) as maxage from layerbreeder_consumption where  flock in ($flock) and client = '$client'  ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	  ...
	  if($mindatep <> "")
	  {
	  $query1 = "SELECT age from layerbreeder_consumption where date2 = '$mindatep' and flock in ($flock) and client = '$client'  ";
      $result1 = mysql_query($query1,$conn1);
	  while($row2 = mysql_fetch_assoc($result1))
      {
	    $maxage = (floor($row2['age']/7)*7)+1;
	   }
	  }
	 $maxage = $maxage - 1;
	  $startweek = $minage/7;
...
<?php
  $fopening = 0;
              $q = "select * from layerbreeder_flock where flockcode in ($flock) and client = '$client'  "; 
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
		 {
             ...
             $minus = 0; 
            echo $q = "select distinct(date2),fmort,fcull from layerbreeder_consumption where flock in ($flock) and client = '$client'  and date2 < '$mindatec' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
            ...
            echo $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Layer Birds' and client = '$client'  and fromwarehouse in ($flock) and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		   ...
		   while($qr = mysql_fetch_assoc($qrs))
            ...
            echo $q = "select sum(quantity) as 'quantity' from oc_cobi where flock in ($flock) and client = '$client'  and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
           ...
           echo  $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Layer Birds' and client = '$client'  and towarehouse in ($flock) and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
...
$minwkdate = "";
$maxwkdate = "";
echo $query1 = "SELECT  min(date2) as minwkdate,max(date2) as maxwkdate from layer_consumption where client = '$client' and flock in ($flock) and age >= '$agefrom' and age < '$ageto' ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
...
<tr>
<td colspan="2" style="text-align:center" ><strong><font color="#3e3276" size="2">Flock:<?php echo $_GET['flock']; ?></font></strong></td>
</tr>
</table>
...
// Table level SQL
echo $EW_REPORT_TABLE_SQL_FROM = "`ac_financialpostings`";
echo $EW_REPORT_TABLE_SQL_SELECT = "SELECT count(*) FROM " . $EW_REPORT_TABLE_SQL_FROM;
echo $EW_REPORT_TABLE_SQL_WHERE = "client = '$client'";
echo $EW_REPORT_TABLE_SQL_GROUPBY = "";
echo $EW_REPORT_TABLE_SQL_HAVING = "";
echo $EW_REPORT_TABLE_SQL_ORDERBY = "`date` ASC";
$EW_REPORT_TABLE_SQL_USERID_FILTER = "";
$EW_REPORT_TABLE_SQL_CHART_BASE = $EW_REPORT_TABLE_SQL_FROM;

// Table Level Group SQL
define("EW_REPORT_TABLE_FIRST_GROUP_FIELD", "`type`", TRUE);
echo $EW_REPORT_TABLE_SQL_SELECT_GROUP = "SELECT DISTINCT " . EW_REPORT_TABLE_FIRST_GROUP_FIELD . " AS `type` FROM " . $EW_REPORT_TABLE_SQL_FROM;

// Table Level Aggregate SQL
echo $EW_REPORT_TABLE_SQL_SELECT_AGG = "SELECT * FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_AGG_PFX = "";
$EW_REPORT_TABLE_SQL_AGG_SFX = "";
echo $EW_REPORT_TABLE_SQL_SELECT_COUNT = "SELECT COUNT(*) FROM " . $EW_REPORT_TABLE_SQL_FROM;
...
echo $sSql = ewrpt_BuildReportSql($EW_REPORT_TABLE_SQL_SELECT_GROUP, $EW_REPORT_TABLE_SQL_WHERE, $EW_REPORT_TABLE_SQL_GROUPBY, $EW_REPORT_TABLE_SQL_HAVING, $EW_REPORT_TABLE_SQL_ORDERBY, $sFilter, @$sSort);
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="layerbreeder_growerrep.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="layerbreeder_growerrep.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="layerbreeder_growerrep.php?export=word<?php echo $url; ?>">Export to Word</a><br /><br />
<?php } ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="1" class="ewTableHeaderBtn" ><tr>
			<td align="center">Feed/Bird Cum</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
while($agefrom <= $maxage )
{
  ...
  echo $query1 = "SELECT  distinct(date2),fmort,fcull from layerbreeder_consumption where client = '$client' and flock in ($flock) and age >= '$agefrom' and age < '$ageto' ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	...
	echo $query1 = "SELECT  sum(quantity) as quant,max(avgwt) as bdwt from layerbreeder_consumption where client = '$client' and flock in ($flock) and age >= '$agefrom' and age < '$ageto' and itemcode in (select code from ims_itemcodes where cat = 'Layer Feed' and client = '$client') ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	    $wfeed = $row2['quant'];
	  ...
	  echo $query1 = "SELECT  itemcode,sum(quantity) as quant,max(date2) as date from layerbreeder_consumption where client = '$client' and flock in ($flock) and age >= '$agefrom' and age < '$ageto' and itemcode in (select code from ims_itemcodes where cat = 'Layer Feed' and client = '$client') group by itemcode ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	...
	//	$q = "select sum(materialcost)/sum(production) as cost from feed_productionunit where mash = '$row2[itemcode]' and client = '$client'"; 
	 ...
	 echo $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Layer Birds' and client = '$client'  and fromwarehouse in ($flock) and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
                $wtrout = $qr['quantity'];
               }
         echo $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Layer Birds' and client = '$client'  and towarehouse in ($flock) and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
                $wtrin = $qr['quantity'];
			 ...
			 if($agewk > $maxagee)
			echo $q = "select * from layerbreed_standards  where client = '$client' and age = '$maxagee' and  breed = '$breedi' "; 
			 else
			 echo $q = "select * from layerbreed_standards  where client = '$client' and age = '$agewk' and  breed = '$breedi' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
              while($qr = mysql_fetch_assoc($qrs))
    		   {
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php $cum += $wfeedkg; $weekcum = $cum/$remaining; echo ewrpt_ViewValue(round($weekcum,2)); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
...
<?php
	...
	echo $sSql = ewrpt_BuildReportSql($EW_REPORT_TABLE_SQL_SELECT_COUNT, $EW_REPORT_TABLE_SQL_WHERE, $EW_REPORT_TABLE_SQL_GROUPBY, $EW_REPORT_TABLE_SQL_HAVING, $EW_REPORT_TABLE_SQL_ORDERBY, $sFilter, @$sSort);
	...
	echo $sSql = ewrpt_BuildReportSql($EW_REPORT_TABLE_SQL_SELECT_AGG, $EW_REPORT_TABLE_SQL_WHERE, $EW_REPORT_TABLE_SQL_GROUPBY, $EW_REPORT_TABLE_SQL_HAVING, $EW_REPORT_TABLE_SQL_ORDERBY, $sFilter, @$sSort);
	...
	if ($rsagg) {
		...
		echo $sSql = ewrpt_BuildReportSql($EW_REPORT_TABLE_SQL_SELECT, $EW_REPORT_TABLE_SQL_WHERE, $EW_REPORT_TABLE_SQL_GROUPBY, $EW_REPORT_TABLE_SQL_HAVING, $EW_REPORT_TABLE_SQL_ORDERBY, "", "");
