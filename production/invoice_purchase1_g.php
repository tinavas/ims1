<?php
 ...
 include "configinvoice.php";
	 ...
	 /*if($_SESSION['db'] == "central")
	  ...
	  $img = ImageCreatefromjpeg('logo/thumbnails/albustan.jpg');
   ...
   $pdf->line(270,690,580,690);
	...
	// $pdf->addText(275,740,10,"<b>$invoice</b>");  $pdf->addText(430,700,10,"<b>$bookinvoice</b>");
    ...
    $pdf->addText(430,715,10,"Book Invoice");
     ...
     $pdf->addText(120,635,10,"Description");
	 if($_SESSION['db']=="vista")    
     $pdf->addText(290,635,10," Weight");
	 else
...
{ //$camount = $row['camount'];
  ...
  if($_SESSION['db']=="apf")
   {
  if($row['freighttype'] == "Freight Amount in Bill")
   $freight = -$row['freightamount'] ;
  else if($row['freighttype'] == "Included")
   $freight = -$row['freightamount'];
    if($row['freighttype'] == "Exclude")
   $freight = $row['freightamount'] ;
  ...
  {
   if($row['freighttype'] == "Amount in Bill" || $row['freighttype'] == "Freight Amount in Bill" )
   $freight = $row['freightamount'] ;
  else if($row['freighttype'] == "Included")
   $freight = -$row['freightamount'];
...
$start = 195;
    if($discount<>0)
    {
	 ...
	 if($freight <> 0)
    {
	 $freight1=changeprice($freight);
     $pdf->addText(223,$start,10,"Freight");
     $width = $pdf->getTextWidth(10,$freight1);	 
     $pdf->addText((575-$width),$start,10,"<b>$freight1</b>");
	 $start += 15;
	 $pdf->setLineStyle(0.5);
     $pdf->line(20,$start,580,$start);
     $pdf->setStrokeColor(0,0,0);
...
$pdf->addText(450,5, 8, "© Powered By Tulasi Technologies Pvt Ltd.");
//$pdf->line(50,70,562,60);
$pdf->closeObject();
