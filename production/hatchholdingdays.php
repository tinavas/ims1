...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Hatch Eggs Holding Days<br/>  Flock :<?php echo $flock;?> <br/>Date  :<?php echo $date;?> </font></strong></td>
</tr>
...
<a name="top"></a>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="hatchholdingdays.php?export=html&date=<?php echo $date;?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="hatchholdingdays.php?export=excel&date=<?php echo $date;?>">Export to Excel</a>
&nbsp;&nbsp;<a href="hatchholdingdays.php?export=word&date=<?php echo $date;?>">Export to Word</a>
<?php } ?>
<br /><br />
</div></td></tr>
<!-- Top Container (End) -->
...
<form action="hatchholdingdays.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
Flock&nbsp;&nbsp;
	<select id="flock" onchange="reloadpage();">
	<option value="All" <?php if($flock == "All") { ?> selected="selected"<?php } ?>>All</option>
	<?php 
	$q = "SELECT distinct(flkmain) as flock FROM breeder_flock ORDER BY flock ASC";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	?>
	<option value="<?php echo $qr['flock'];?>" <?php if($qr['flock'] == $flock) { ?> selected="selected"<?php } ?>><?php echo $qr['flock'];?></option>
	<?php } ?>
</select>
&nbsp;&nbsp;Date&nbsp;
<input type="text" class="datepicker" id="fromdate" name="fromdate" value="<?php echo date("d.m.Y",strtotime($date)); ?>" onchange="reloadpage();"/>
&nbsp;&nbsp;&nbsp;
</span>
</td>
</tr>	
...
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" align="center" style="width:40px">
		Flock
		</td>
<?php } else { ?>
		<td class="ewTableHeader" style=" width:40px">
...
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td  class="ewTableHeader" style=" width:50px">
		Holding Days
		</td>
<?php } else { ?>
		<td class="ewTableHeader" style=" width:50px" >
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Holding Days</td>
			</tr></table>
		</td>
<?php } ?>
</tr>
...
<?php 
$eggs = 0;
$query = "select date,sum(availableeggs) as eggs,flock from ims_eggreceiving where client = '$client' and date < '$date' and flock $fk '$flock' and availableeggs != '' GROUP BY date,flock order by flock";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($qr = mysql_fetch_assoc($result))
{
$flock = $qr['flock'];
$eggs = $qr['eggs'];
$recdate = strtotime($qr['date']);
$recedate = date("d.m.Y",strtotime($qr['date']));
$repdate = strtotime($date);
if($repdate > $recdate)
{
$diff = $repdate- $recdate;
}
else
{
$diff = $recdate - $repdate;
}
$holddays = $diff/86400;
	   ?>
	    <tr>
	<td class="ewRptGrpField2" align="right">
<?php if(($flock <> $oldcode) or ($dumm == 0)){ echo ewrpt_ViewValue($flock);
     $oldcode = $flock; $dumm = 1; }
	 else{
...
<!--first page button-->
	...
	<td><a href="hatchholdingdays.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="hatchholdingdays.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="hatchholdingdays.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="hatchholdingdays.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
document.location = "hatchholdingdays.php?date="+date + "&flock=" + flock;
}
</script>
