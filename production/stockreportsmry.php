...
include "config.php";
 ...
 if($_GET['cat'] == "")
 {
  $cat = '';
  $cond="cat<>''";
  $cond1="ims_itemcodes.cat<>''";
 }
 else
 {
  $cat = $_GET['cat'];
    $cond="cat='$cat'";
	  $cond1="ims_itemcodes.cat='$cat'";
 }
...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT ims_itemcodes.code,ims_itemcodes.cat, ims_itemcodes.description, ims_itemcodes.sunits FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_WHERE = $cond1;
...
{
 $q1 ="SELECT DISTINCT (sector) as sector FROM tbl_sector WHERE type1='Warehouse'";
 }
 else
 {
  $sectorlist = $_SESSION['sectorlist'];
 $q1 ="SELECT  DISTINCT (sector) as sector FROM tbl_sector WHERE type1 = 'Warehouse'  and sector in ($sectorlist)";
 }

$qrs = mysql_query($q1,$conn1);
$n1 = mysql_num_rows($qrs);
while($qr = mysql_fetch_assoc($qrs))
{
?>
...
<?php 
 ...
 $getop = "SELECT SUM(receivedquantity) as quantity FROM pp_sobi WHERE code = '$x_code' AND date < '$fromdate' AND warehouse = '$warehouse' AND dflag = '0'";
  ...
  $getop = "SELECT SUM(quantity) as quantity FROM oc_cobi WHERE code = '$x_code' AND date < '$fromdate' AND warehouse = '$warehouse' and dflag='0'";
  $resultop = mysql_query($getop,$conn1);
  while($rowop = mysql_fetch_assoc($resultop))
  {
  ...
  $getop="select sum(squantity) as quantity from oc_packslip where itemcode='$x_code' and date < '$fromdate' and warehouse = '$warehouse'";
  
  $getop=mysql_query($getop) or die(mysql_error());
  
  $rowop=mysql_fetch_assoc($getop);
  
  $salesop=$salesop+$rowop['quantity'];
...
$opening = $firstopening + $purchasedop + $irecop + $staadd + $grop + $stockto - ($consumedop + $salesop + $iiscop + $staded  + $stockfrom+$preturn) + $salesreturnop + $prod;
//echo $firstopening ."/". $purchasedop  ."/". $irecop  ."/". $staadd ."/". $grop  ."/". $stockto  ."/". $consumedop  ."/". $salesop  ."/". $iiscop  ."/". $staded   ."/". $stockfrom ."/".$preturn ."/". $salesreturnop ;
...
$consumed = $consumed + $rowop['quantity'];
  ...
  $get = "SELECT SUM(quantity) as quantity FROM oc_cobi WHERE code = '$x_code' AND date >= '$fromdate' AND date <= '$todate' AND warehouse = '$warehouse' and dflag='0' ";
  $result = mysql_query($get,$conn1);
  while($row = mysql_fetch_assoc($result))
  {
  $sales = $row['quantity'];
  ...
  $getop="select sum(squantity) as quantity from oc_packslip where itemcode='$x_code' and date >= '$fromdate' AND date <= '$todate' and warehouse = '$warehouse'";
  
  $getop=mysql_query($getop) or die(mysql_error());
  
  $rowop=mysql_fetch_assoc($getop);
  
  $sales=$sales+$rowop['quantity'];
 ...
 $getop = "SELECT sum(production) as quantity FROM product_productionunit WHERE producttype = '$x_code' AND warehouse = '$warehouse' and date >= '$fromdate' AND date <= '$todate' ";
  ...
  while($row = mysql_fetch_assoc($result))
  {
   ...
   $consumed = $consumed+$row['quantity'];
...
//echo $x_code.'/'.$opening ."+". $purchased ."-"." (".$consumed ."+". $sales.")". "+". $salesreturn;
// avg cost
 ...
 //echo $closing;
  ...
  $query1 = "select distinct(iac),cunits,sunits from ims_itemcodes where code = '$x_code' and client = '$client'   ";
   $result1 = mysql_query($query1,$conn1);
   $row2 = mysql_fetch_assoc($result1);
   $ingacc = $row2['iac'];
   $x_cunits=$row2['sunits'];
...
?>
	<tr>
		...
		<td<?php echo $sItemRowClass; ?>>
			<?php echo ewrpt_ViewValue($x_cunits) ?></td>
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
			<?php echo changeprice($opening); $total_opening+=$opening; ?>
		</td>
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
			<?php echo changeprice($purchased); $total_purchase+=$purchased; ?>			        </td>
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
			<?php echo changeprice($consumed); $total_consumed+=$consumed; ?>
		</td>
		...
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
			<?php echo changeprice($closing); $total_closing+=$closing; ?>
		</td>
		
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
			<?php echo changeprice(($ratepunit));$totalratepunit=$totalratepunit+$ratepunit;?></td>
	
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
			<?php echo changeprice(($inginitbal));$totalvalue=$totalvalue+$inginitbal;?></td>
...
<tr>
	...
	<td align="right" ><?php echo changeprice($total_opening) ;?></td>
	<td align="right" ><?php echo changeprice($total_purchase) ;?></td>
	<td align="right" ><?php echo changeprice($total_consumed) ;?></td>
	<td align="right" ><?php echo changeprice($total_sold) ;?></td>
	<td align="right" ><?php echo changeprice($total_salesreturn) ;?></td>
	
	<td align="right" ><?php echo changeprice($total_closing) ;?></td>
	
	
	
	<td align="right" ><?php echo changeprice($totalratepunit) ;?></td>

	<td align="right" ><?php echo changeprice($totalvalue) ;?></td>
