...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="broilerlatestdateflocks.php?export=html&fromdate=<?php echo $fromdate; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="broilerlatestdateflocks.php?export=excel&fromdate=<?php echo $fromdate; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="broilerlatestdateflocks.php?export=word&fromdate=<?php echo $fromdate; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="broilerlatestdateflocks.php?cmd=reset&fromdate=<?php echo $fromdate; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
// Retrieving Standard Values
	$queryaa = "SELECT feed,cumfeed,fcr,avgweight FROM broiler_allstandards WHERE age = '$res[age]' ";
...
while($rowaa = mysql_fetch_assoc($resultaa))
{
  ...
  $stdwt = $rowaa['avgweight'];
	...
	<td><?php echo date("d.m.Y",strtotime($res['entrydate'])); ?></td>
	...
	<td><?php echo $feedstd; ?></td>
	<td><?php echo round((($res['feedconsumed']/$birds) * 1000),2); ?></td>
	<td><?php echo $cumfeedstd; ?></td>
	<td><?php echo round((($cfeedconsumed/$oobirds) * 1000),2); ?></td>
	
	<td><?php echo $stdwt; ?></td>
	
	<td><?php echo $res['average_weight']; ?></td>
	<td><?php echo $stdfcr; ?></td>
	<td><?php echo round(($cfeedconsumed / (($res['average_weight'] / 1000) * $birds )),2); ?></td>
	
	<td><?php if($res['medicine_name'])echo $res['medicine_name']; else echo "0";  ?></td>
	<td><?php if($res['medicine_quantity'])echo $res['medicine_quantity']; else echo "0"; ?></td>
	<td><?php if($res['vaccine_name'])echo $res['vaccine_name']; else echo "0";?></td>
