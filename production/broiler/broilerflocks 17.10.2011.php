...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Broiler Live Flocks Summary</font></strong></td>
</tr>
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="broilerflocks.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="broilerflocks.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="broilerflocks.php?export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="broilerflocks.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<form action="broilerflocks.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="broilerflocks.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="broilerflocks.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="broilerflocks.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="broilerflocks.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
//	echo "sql: " . $sSql . "<br>";
	...
	$oldage = 0;
	$print = 0;	
$query1 = "SELECT flock,max(age) as maxage FROM `broiler_daily_entry` WHERE flock NOT IN (SELECT flock FROM broiler_transferrate) GROUP BY flock ORDER BY max( age ) DESC";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   if($oldage <> $row1['maxage'])
	   {
	     if($print == 1)
		 {
		 ?>
...
<?php echo ewrpt_ViewValue($chickstotal) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($mortalitytotal) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(round((($mortalitytotal/$chickstotal) * 100),2)); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($saletotal) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($chickstotal - $mortalitytotal - $saletotal) ?>
</td>
	</tr>
<?php		 
		    $chickstotal = 0;
		 ...
		 $oldage = $row1['maxage'];
		 //Chicks Imducted
		 $query11 = "SELECT sum(quantity) as chicks from ims_stocktransfer where cat = 'Broiler Chicks' and aflock = '$row1[flock]'";
		 ...
		 //Chicks Purchase
		  $query11 = "SELECT sum(receivedquantity) as chicks from pp_sobi where category = 'Broiler Chicks' and flock = '$row1[flock]'";
	     $result11 = mysql_query($query11,$conn1);
	     while($row11 = mysql_fetch_assoc($result11))
	     {
	       $chickstotal = $chickstotal + $row11['chicks'];
		 ...
		 //Mortality
		  $query11 = "SELECT sum(mortality) as mortality,max(entrydate) as maxentrydate from broiler_daily_entry where flock = '$row1[flock]'";
	     $result11 = mysql_query($query11,$conn1);
	     while($row11 = mysql_fetch_assoc($result11))
	     {
	       $mortalitytotal = $mortalitytotal + $row11['mortality'];
		   $dates = $dates.date("d-m-Y",strtotime($row11['maxentrydate']));
	     }	
		 //Sales
		  $query11 = "SELECT sum(birds) as sbirds from oc_cobi where code = 'BROB101' and flock = '$row1[flock]'";
	     $result11 = mysql_query($query11,$conn1);
	     while($row11 = mysql_fetch_assoc($result11))
	     {
