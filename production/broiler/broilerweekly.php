...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Broiler Weeekly Report</font></strong></td>
</tr>
...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT distinct(broiler_daily_entry.flock),broiler_daily_entry.supervisior, broiler_daily_entry.place, broiler_daily_entry.farm FROM " . $EW_REPORT_TABLE_SQL_FROM;
	   if($_SESSION['db'] == "feedatives")
		{
		   if($_SESSION['sectorr'] == "all")
		   {
		   	$EW_REPORT_TABLE_SQL_WHERE = "broiler_daily_entry.cullflag = $cullflag";
		   }
		   else
		   {
		   $sectorr = $_SESSION['sectorr'];
		   $EW_REPORT_TABLE_SQL_WHERE = "broiler_daily_entry.cullflag = $cullflag AND broiler_daily_entry.place = '$sectorr'";
		   }
...
$EW_REPORT_TABLE_SQL_ORDERBY = "broiler_daily_entry.supervisior ASC, broiler_daily_entry.place ASC, broiler_daily_entry.farm ASC";
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="broilerweekly.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="broilerweekly.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="broilerweekly.php?export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="broilerweekly.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<br /><br />
<?php } ?>
...
<form action="broilerweekly.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
$soldbirdssum = 0;
 ...
 $query1 = "SELECT max(age) as maxage,min(age) as minage FROM broiler_daily_entry WHERE place = '$x_place' AND supervisior = '$x_supervisior' AND farm = '$x_farm' AND flock = '$x_flock' ";
...
$sentbirds = 0;
$sentweight = 0;
 $query1 = "SELECT max(entrydate) as maxdate,max(age) as maxage,sum(mortality) as totmort,sum(feedconsumed) as feedcons,min(entrydate) as mindate FROM broiler_daily_entry WHERE place = '$x_place' AND supervisior = '$x_supervisior' AND farm = '$x_farm' AND flock = '$x_flock' and age >= '$agefrom' and age < '$ageto' ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   $age = $row1['maxage'];
	...
	$query111 = "SELECT sum(birds) as chicks FROM oc_cobi where flock = '$x_flock' and warehouse = '$x_farm' and code = 'BROB101' and date >= '$fromdate' and date <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
...
//Opening Birds
  ...
  $query111 = "SELECT sum(quantity) as chicks FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' and (cat = 'Broiler Chicks' or cat='Broiler Day Old Chicks') and date <= '$fromdate'  "; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
 if ( $rows > 0) { while($row111 = mysql_fetch_assoc($result111)) {  $obirds = $obirds + $row111['chicks']; } }
 
 
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm'  and   code in (select distinct(code) from ims_itemcodes where (cat = 'Broiler Chicks' or cat = 'Broiler Day Old Chicks')) and date <= '$fromdate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  { $obirds = $obirds + $row111['chicks'];  } }
   
  $query111 = "SELECT sum(birds) as chicks FROM oc_cobi where flock = '$x_flock' and warehouse = '$x_farm' and code in (select distinct(code) from ims_itemcodes where cat = 'Broiler Birds' ) and date < '$fromdate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $obirds = $obirds - $row111['chicks'];  } }
  ...
  if($_SESSION['db'] == 'central')
{
  $xx_date = date("Y-m-d",strtotime($enddate));
  $sentquery = "SELECT birds,weight from broiler_chickentransfer where flock = '$x_flock' and date = '$xx_date' ";
  $sentresult = mysql_query($sentquery,$conn1) or die(mysql_error()); 
  while($sentres = mysql_fetch_assoc($sentresult))
  { 
  
    $obirds = $obirds - $sentres['birds']; 
     $sentbirds += $sentres['birds']; 
	 $previousbirdssent += $sentres['birds'];
	  $totsentweight+=$sentres['weight'];
	  $sentweight+=$sentres['weight']; 
  } 
 } 
  
  
   $query111 = "SELECT sum(mortality) as chicks FROM broiler_daily_entry where flock = '$x_flock' and farm = '$x_farm' and supervisior = '$x_supervisior' and place = '$x_place' and entrydate < '$fromdate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $obirds = $obirds - $row111['chicks'];  } }
...
/* $query111 = "SELECT sum(quantity) as chicks FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' and cat = 'Broiler Chicks' and date <= '$todate'  "; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
 if ( $rows > 0) { while($row111 = mysql_fetch_assoc($result111)) { $cbirds = $cbirds + $row111['chicks']; } }
 
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm'  and category = 'Broiler Chicks' and date <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $cbirds = $cbirds + $row111['chicks'];  } }
  
  $query111 = "SELECT sum(birds) as chicks FROM oc_cobi where flock = '$x_flock' and warehouse = '$x_farm' and code = 'BROB101' and date <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $cbirds = $cbirds - $row111['chicks'];  } } */
  
   $query111 = "SELECT max(average_weight) as bodywt FROM broiler_daily_entry where flock = '$x_flock' and farm = '$x_farm' and supervisior = '$x_supervisior' and place = '$x_place' and entrydate <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
...
{?>
	<td<?php echo $sItemRowClass; ?>>
<?php echo changequantity($sentbirds); ?>
</td> 
<td<?php echo $sItemRowClass; ?>>
<?php echo changeprice($sentweight); ?>
</td>
<?php } ?>
...
<!--first page button-->
	...
	<td><a href="broilerweekly.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="broilerweekly.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="broilerweekly.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="broilerweekly.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function reloadpage()
{
	...
	document.location = "broilerweekly.php?cullflag=" + cullflag ;
}
</script>
