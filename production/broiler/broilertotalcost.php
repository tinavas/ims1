...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Broiler Production Cost Report of <?php echo $supervisor; ?></font></strong></td>
</tr>
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="broilertotalcost.php?supervisor=<?php echo $supervisor; ?>&export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="broilertotalcost.php?supervisor=<?php echo $supervisor; ?>&export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="broilertotalcost.php?supervisor=<?php echo $supervisor; ?>&export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="broilertotalcost.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<form action="broilertotalcost.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<?php } }?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Average Body Weight">
		B.Wt(Kgs)
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Average Body Weight"><tr>
			<td>B.Wt(Kgs)</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Feed Consumption">
		F.C(Kgs)
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Feed Consumption"><tr>
			<td>F.C(Kgs)</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Feed Conversion Ration">
		FCR
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Feed Conversion Ratio">FCR</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Total Feed Cost">
		F.Cost
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Total Feed Cost">F.Cost</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Feed Cost/Kg">
        F.C/Kg
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Feed Cost/Kg"><tr>
			<td>F.C/Kg</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Total Medicine Cost">
		Med.Cost
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Total Medicine Cost"><tr>
			<td>Med.Cost</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Medicine Cost/Kg">
		M.C/Kg
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Medicine Cost/Kg"><tr>
			<td>M.C/Kg</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Production Cost/Kg">
		P.Cost/Kg 
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Production Cost/Kg"><tr>
			<td>P.Cost/Kg </td>
			</tr></table>
		</td>
<?php } ?>
...
//	echo "sql: " . $sSql . "<br>";
	...
	if (1) { // Loop detail records
		...
		include "config.php";
		 $q = "select distinct(flock),farm,max(age) as maxage,max(entrydate) as maxdate,max(average_weight) as avgwt,sum(mortality) as totmort,sum(feedconsumed) as feedcons,min(entrydate) as mindate from broiler_daily_entry where supervisior = '$supervisor' and cullflag = '$cullflag'   group by flock order by max(age) DESC  ";
...
<?php echo ewrpt_ViewValue($qr['farm']); ?>
		</td>
		
		<td class="ewRptGrpField3">
		
<?php echo ewrpt_ViewValue($qr['flock']); ?>
		</td>
		<td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue($qr['maxage']); ?>
</td>
<?php
$startdate = "";
$enddate = "";
...
$sentbirds =0;
$sentweight =0;
 $query111 = "SELECT sum(quantity) as chicks FROM ims_stocktransfer where aflock = '$qr[flock]' and towarehouse = '$qr[farm]' and cat = 'Broiler Chicks'  "; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
 if ( $rows > 0) { while($row111 = mysql_fetch_assoc($result111)) { $birds = $birds + $row111['chicks']; } }
 
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$qr[flock]' and warehouse = '$qr[farm]'  and description = 'Broiler Chicks'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $birds = $birds + $row111['chicks'];  } }
  
  $query111 = "SELECT sum(birds) as chicks,sum(quantity) as qty FROM oc_cobi where flock = '$qr[flock]' and warehouse = '$qr[farm]' and code = 'BROB101'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {  $soldbirds = $row111['chicks'];  $soldwt = $row111['qty'];   } }


// broiler send birds calculation
if($_SESSION['db'] == 'central')
{
  $sentquery = "SELECT sum(birds) as birds,sum(weight) as weight from broiler_chickentransfer where flock = '$qr[flock]'";
  ...
  $sentweight = $sentres['weight']; 
	 ...
	 $qtybal = 0;
	 $mintrnsfrdate = 0;
	$query1 = "SELECT sum(quantity * price) as feedin,sum(quantity) as qtyin,min(date) as mindate from ims_stocktransfer where cat = 'Broiler Feed' and towarehouse = '$qr[farm]' and aflock = '$qr[flock]' ";
	 ...
	 $query1 = "SELECT min(date) as mindate from ims_stocktransfer where cat = 'Broiler Feed' and towarehouse = '$qr[farm]' and aflock = '$qr[flock]' ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	  $mintrnsfrdate = $row1['mindate'];
	   
	 }
	 $query1 = "SELECT sum(quantity * price) as feedout,sum(quantity) as qtyout from ims_stocktransfer where cat = 'Broiler Feed' and fromwarehouse = '$qr[farm]' and tflock = '$qr[flock]' ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	 ...
	 $mintrnsfrdate = strtotime($mintrnsfrdate);
	 $mintrnsfrdate = $mintrnsfrdate - (24 * 60 * 60);
	 $mintrnsfrdate = date("Y-m-d",$mintrnsfrdate);
	 $totfeedpr = 0;
	 $totfeedcost = 0;
	 $feedprcost = 0;
	  $query1 = "SELECT sum(production) as production,sum(materialcost) as matcost from feed_productionunit where date >= '$mintrnsfrdate' and date <= '$qr[maxdate]' and mash in (select distinct(code) from ims_itemcodes where cat = 'Broiler Feed') ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   $totfeedpr = $row1['production'];
	  $totfeedcost = $row1['matcost'];
	 }
	 $feedprcost = round(($totfeedcost/$totfeedpr),2);
	  $query1 = "SELECT sum(quantity * price) as feedout,sum(quantity) as qtyout from ims_stocktransfer where cat = 'Broiler Feed' and fromwarehouse = '$qr[farm]' and tflock = '$qr[flock]' ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	 ...
	 $feedcost = $feedin - $feedout;
	 $qtybal = $qtyin - $qtyout;
	 $cbirds = $birds - $soldbirds - $qr['totmort'];
	 $costin = 0;
	 $costout = 0;
	 $prodcost = 0;
	 $mqtyin = 0;
	 ...
	 $medcost = 0;
	 $medqty = 0;
	  $query1 = "SELECT quantity,code,date  from ims_stocktransfer where towarehouse = '$qr[farm]' and aflock = '$qr[flock]' and (cat = 'Medicines' or cat = 'Vaccines') ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   //$costin = $row1['costin'];
	   $mqtyin = $mqtyin + $row1['quantity'];
	   $costindummy = calculate("Weighted Average",$row1['code'],$row1['date']);
	   $costin = $costin + ($mqtyin * $costindummy);
	   
	 }
	  $query1 = "SELECT quantity,code,date from ims_stocktransfer where tflock = '$qr[flock]' and fromwarehouse = '$qr[farm]' and (cat = 'Medicines' or cat = 'Vaccines') ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   //$costout = $row1['costout'];
	   $mqtyout = $mqtyout + $row1['quantity'];
	   $costoutdummy = calculate("Weighted Average",$row1['code'],$row1['date']);
	   $costout = $costout + ($mqtyout * $costoutdummy);
	 }
	 $medcost = $costin - $costout;
	 $medqty = $mqtyin - $mqtyout;
	  $costin = 0;
	 $costout = 0;
	 $prodcost = 0;
	  $query1 = "SELECT sum(quantity * price) as costin from ims_stocktransfer where towarehouse = '$qr[farm]' and aflock = '$qr[flock]' and cat = 'Broiler Chicks' ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   $costin = $row1['costin'];
	 ...
	 $prodcost = $costin + round(($feedprcost * $qr['feedcons']),2) + $medcost;
	 ?>
	 <td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue(date($datephp,strtotime($qr['maxdate']))); ?>
</td>
<td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue($birds) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($qr['totmort']); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(round((($qr['totmort']/$birds) * 100),2)) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($soldbirds > 0){ echo ewrpt_ViewValue($soldbirds); } else { echo ewrpt_ViewValue("0"); } ?>
</td>
...
<?php } ?>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($qr['avgwt'] > 0) { echo ewrpt_ViewValue(round(($qr['avgwt']/1000),2)); } else { echo ewrpt_ViewValue("0.00"); } ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($qr['feedcons']); ?>
</td>
<?php
$fcr = 0;
$totwt = 0;
...
if($cullflag == 0)
{
  //echo $qr['flock']; echo "&nbsp;";
	 ...
	 //$totwt =  round((($qr['avgwt']/1000) * $birds),2); 
	  $totwt = $totwt + $soldwt + $sentweight;
	 $totwt = $totwt + round((($qr['avgwt']/1000) * $cbirds),2); 
   $fcr = round(($qr['feedcons']/$totwt),2); 
}
?>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($fcr) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(round(($feedprcost * $qr['feedcons']),2)); ?>
</td>
<td <?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(round($feedprcost,2)); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($medcost > 0){  echo ewrpt_ViewValue($medcost); } else { echo ewrpt_ViewValue("0.00"); }  ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($medcost > 0){  echo ewrpt_ViewValue(round(($medcost/$medqty),2)); } else { echo ewrpt_ViewValue("0.00"); } ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($prodcost == 0){ echo "0"; } else { echo ewrpt_ViewValue(changeprice(round(($prodcost),2))); } ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($totwt == 0){ echo "0"; } else { echo ewrpt_ViewValue(round(($prodcost/$totwt),2)); } ?>
</td>
...
<!--first page button-->
	...
	<td><a href="broilertotalcost.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="broilertotalcost.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="broilertotalcost.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="broilertotalcost.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function reloadpage()
{
	...
	document.location = "broilertotalcost.php?cullflag=" + cullflag ;
}
</script>
