...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Lifting Details</font></strong></td>
</tr>
<tr>
<td> From : <?php echo date("d.m.Y",strtotime($fromdate)); ?></td><td>&nbsp; To : <?php echo date("d.m.Y",strtotime($todate)); ?></td>
</tr>
</table>
<br/>
...
define("EW_REPORT_TABLE_VAR", "Report2", TRUE);
define("EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE", "Report2_grpperpage", TRUE);
define("EW_REPORT_TABLE_SESSION_START_GROUP", "Report2_start", TRUE);
define("EW_REPORT_TABLE_SESSION_SEARCH", "Report2_search", TRUE);
define("EW_REPORT_TABLE_SESSION_CHILD_USER_ID", "Report2_childuserid", TRUE);
define("EW_REPORT_TABLE_SESSION_ORDER_BY", "Report2_orderby", TRUE);

// Table level SQL
$EW_REPORT_TABLE_SQL_FROM = "oc_cobi, broiler_farm";
$EW_REPORT_TABLE_SQL_SELECT = "SELECT oc_cobi.date, broiler_farm.supervisor, broiler_farm.place, oc_cobi.farm, oc_cobi.flock, oc_cobi.party, oc_cobi.invoice,oc_cobi.bookinvoice, oc_cobi.birds, oc_cobi.quantity, oc_cobi.price, (oc_cobi.quantity * oc_cobi.price) as Amount FROM " . $EW_REPORT_TABLE_SQL_FROM;
	   if($_SESSION['db'] == "feedatives")
		{
		   if($_SESSION['sectorr'] == "all")
		   {
		   	$EW_REPORT_TABLE_SQL_WHERE = "broiler_farm.farm = oc_cobi.farm and oc_cobi.code = 'BROB101' and oc_cobi.date >= '$fromdate' and oc_cobi.date <= '$todate'";
		   }
		   else
		   {
		   $sectorr = $_SESSION['sectorr'];
		   $EW_REPORT_TABLE_SQL_WHERE = "broiler_farm.farm = oc_cobi.farm and oc_cobi.code = 'BROB101' and oc_cobi.date >= '$fromdate' and oc_cobi.date <= '$todate' AND broiler_farm.farm IN (SELECT distinct(broiler_farm.farm) FROM broiler_farm WHERE broiler_farm.place = '$sectorr')";
		   }
...
$EW_REPORT_TABLE_SQL_ORDERBY = "oc_cobi.date ASC, broiler_farm.supervisor ASC, broiler_farm.place ASC, oc_cobi.farm ASC";
$EW_REPORT_TABLE_SQL_USERID_FILTER = "";
$EW_REPORT_TABLE_SQL_CHART_BASE = "";

// Table Level Group SQL
define("EW_REPORT_TABLE_FIRST_GROUP_FIELD", "oc_cobi.date", TRUE);
...
ewrpt_CreatePopup("Report2_date", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_supervisor, $sel_supervisor, $ft_supervisor) ?>
ewrpt_CreatePopup("Report2_supervisor", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_place, $sel_place, $ft_place) ?>
ewrpt_CreatePopup("Report2_place", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_farm, $sel_farm, $ft_farm) ?>
ewrpt_CreatePopup("Report2_farm", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_flock, $sel_flock, $ft_flock) ?>
ewrpt_CreatePopup("Report2_flock", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_party, $sel_party, $ft_party) ?>
ewrpt_CreatePopup("Report2_party", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_invoice, $sel_invoice, $ft_invoice) ?>
ewrpt_CreatePopup("Report2_invoice", [<?php echo $jsdata ?>]);
</script>
<div id="Report2_date_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report2_supervisor_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report2_place_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report2_farm_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report2_flock_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report2_party_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report2_invoice_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<?php } ?>
<?php if (@$sExport == "") { ?>
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="broilerlifting.php?export=html&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="broilerlifting.php?export=excel&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="broilerlifting.php?export=word&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="broilerlifting.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<form action="broilerlifting.php" name="ewpagerform" id="ewpagerform" class="ewForm">
<table>
 <tr>
 <td>From Date </td>
...
<!--first page button-->
	...
	<td><a href="broilerlifting.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="broilerlifting.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="broilerlifting.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="broilerlifting.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			...
			<td>Date</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report2_date', true, '<?php echo $rf_date; ?>', '<?php echo $rt_date; ?>');return false;" name="x_date<?php echo $cnt[0][0]; ?>" id="x_date<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Supervisor</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report2_supervisor', false, '<?php echo $rf_supervisor; ?>', '<?php echo $rt_supervisor; ?>');return false;" name="x_supervisor<?php echo $cnt[0][0]; ?>" id="x_supervisor<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Place</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report2_place', false, '<?php echo $rf_place; ?>', '<?php echo $rt_place; ?>');return false;" name="x_place<?php echo $cnt[0][0]; ?>" id="x_place<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Farm</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report2_farm', false, '<?php echo $rf_farm; ?>', '<?php echo $rt_farm; ?>');return false;" name="x_farm<?php echo $cnt[0][0]; ?>" id="x_farm<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Flock</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report2_flock', false, '<?php echo $rf_flock; ?>', '<?php echo $rt_flock; ?>');return false;" name="x_flock<?php echo $cnt[0][0]; ?>" id="x_flock<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Party</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report2_party', false, '<?php echo $rf_party; ?>', '<?php echo $rt_party; ?>');return false;" name="x_party<?php echo $cnt[0][0]; ?>" id="x_party<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Invoice</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report2_invoice', false, '<?php echo $rf_invoice; ?>', '<?php echo $rt_invoice; ?>');return false;" name="x_invoice<?php echo $cnt[0][0]; ?>" id="x_invoice<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
//	echo "sql: " . $sSql . "<br>";
	...
	while ($rs && !$rs->EOF) { // Loop detail records
		...
		if ((is_null($x_supervisor) && is_null($o_supervisor)) ||
			(($x_supervisor <> "" && $o_supervisor == $dg_supervisor) && !ChkLvlBreak(2))) {
			$dg_supervisor = "&nbsp;";
		...
		if ((is_null($x_place) && is_null($o_place)) ||
			(($x_place <> "" && $o_place == $dg_place) && !ChkLvlBreak(3))) {
			$dg_place = "&nbsp;";
...
?>
	<tr style="display:none">
		...
		<td colspan="10" class="ewRptGrpSummary4">Summary for Farm: <?php $t_farm = $x_farm; $x_farm = $o_farm; ?>
<?php echo ewrpt_ViewValue($x_farm) ?>
<?php $x_farm = $t_farm; ?> (<?php echo ewrpt_FormatNumber($cnt[4][0],0,-2,-2,-2); ?> Detail Records)</td></tr>
	<tr  style="display:none">
		<td class="ewRptGrpField1">&nbsp;</td>
		<td class="ewRptGrpField2">&nbsp;</td>
		<td class="ewRptGrpField3">&nbsp;</td>
...
<?php echo ewrpt_ViewValue($x_quantity) ?>
		...
		<?php $x_Amount = $smry[4][7]; // Load SUM ?>
<?php echo ewrpt_ViewValue($x_Amount) ?>
...
?>
	<tr  style="display:none">
		<td class="ewRptGrpField1">&nbsp;</td>
		<td class="ewRptGrpField2">&nbsp;</td>
		<td colspan="11" class="ewRptGrpSummary3">Summary for Place: <?php $t_place = $x_place; $x_place = $o_place; ?>
<?php echo ewrpt_ViewValue($x_place) ?>
<?php $x_place = $t_place; ?> (<?php echo ewrpt_FormatNumber($cnt[3][0],0,-2,-2,-2); ?> Detail Records)</td></tr>
	<tr  style="display:none">
		<td class="ewRptGrpField1">&nbsp;</td>
		<td class="ewRptGrpField2">&nbsp;</td>
...
<?php echo ewrpt_ViewValue($x_quantity) ?>
		...
		<td class="ewRptGrpSummary3" align="right">
		<?php $t_Amount = $x_Amount; ?>
		<?php $x_Amount = $smry[3][7]; // Load SUM ?>
<?php echo ewrpt_ViewValue($x_Amount) ?>
		<?php $x_Amount = $t_Amount; ?>
		</td>
	</tr>
...
?>
	<tr  style="display:none">
		<td class="ewRptGrpField1">&nbsp;</td>
		<td colspan="12" class="ewRptGrpSummary2">Summary for Supervisor: <?php $t_supervisor = $x_supervisor; $x_supervisor = $o_supervisor; ?>
<?php echo ewrpt_ViewValue($x_supervisor) ?>
<?php $x_supervisor = $t_supervisor; ?> (<?php echo ewrpt_FormatNumber($cnt[2][0],0,-2,-2,-2); ?> Detail Records)</td></tr>
	<tr  style="display:none">
		<td class="ewRptGrpField1">&nbsp;</td>
...
?>
	<tr  style="display:none">
		<td colspan="13" class="ewRptGrpSummary1">Summary for Date: <?php $t_date = $x_date; $x_date = $o_date; ?>
<?php echo ewrpt_ViewValue(ewrpt_FormatDateTime($x_date,7)) ?>
	...
	switch ($lvl) {
		...
		case 2:
			return (is_null($GLOBALS["x_supervisor"]) && !is_null($GLOBALS["o_supervisor"])) ||
				...
				($GLOBALS["x_supervisor"] <> $GLOBALS["o_supervisor"]) || ChkLvlBreak(1); // Recurse upper level
		case 3:
			return (is_null($GLOBALS["x_place"]) && !is_null($GLOBALS["o_place"])) ||
				(!is_null($GLOBALS["x_place"]) && is_null($GLOBALS["o_place"])) ||
				($GLOBALS["x_place"] <> $GLOBALS["o_place"]) || ChkLvlBreak(2); // Recurse upper level
		case 4:
...
function ResetLevelSummary($lvl) {
	...
	if ($lvl <= 2) $GLOBALS["o_supervisor"] = "";
	if ($lvl <= 3) $GLOBALS["o_place"] = "";
...
function GetRow($opt) {
	...
	if (!$rs->EOF) {
		$GLOBALS['x_supervisor'] = $rs->fields('supervisor');
...
function SetupPopup() {
	...
	while ($rswrk && !$rswrk->EOF) {
		...
		if (is_null($x_invoice)) {
			...
			$t_invoice = $x_invoice;
			ewrpt_SetupDistinctValues($GLOBALS["val_invoice"], $x_invoice, $t_invoice, FALSE);
		}
		$rswrk->MoveNext();
	}
	if ($rswrk)
	...
	if (is_array(@$_SESSION["sel_Report2_date"])) {
		LoadSelectionFromSession('date');
	} elseif (@$_SESSION["sel_Report2_date"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_date"] = "";
	}

	// Get Supervisor selected values
	if (is_array(@$_SESSION["sel_Report2_supervisor"])) {
		LoadSelectionFromSession('supervisor');
	} elseif (@$_SESSION["sel_Report2_supervisor"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_supervisor"] = "";
	}

	// Get Place selected values
	if (is_array(@$_SESSION["sel_Report2_place"])) {
		LoadSelectionFromSession('place');
	} elseif (@$_SESSION["sel_Report2_place"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_place"] = "";
	}

	// Get Farm selected values
	if (is_array(@$_SESSION["sel_Report2_farm"])) {
		LoadSelectionFromSession('farm');
	} elseif (@$_SESSION["sel_Report2_farm"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_farm"] = "";
	}

	// Get Flock selected values
	if (is_array(@$_SESSION["sel_Report2_flock"])) {
		LoadSelectionFromSession('flock');
	} elseif (@$_SESSION["sel_Report2_flock"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_flock"] = "";
	}

	// Get Party selected values
	if (is_array(@$_SESSION["sel_Report2_party"])) {
		LoadSelectionFromSession('party');
	} elseif (@$_SESSION["sel_Report2_party"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_party"] = "";
	}

	// Get Invoice selected values
	if (is_array(@$_SESSION["sel_Report2_invoice"])) {
		LoadSelectionFromSession('invoice');
	} elseif (@$_SESSION["sel_Report2_invoice"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_invoice"] = "";
	}
}
...
function LoadSelectionFromSession($parm) {
	$GLOBALS["sel_$parm"] = @$_SESSION["sel_Report2_$parm"];
	$GLOBALS["rf_$parm"] = @$_SESSION["rf_Report2_$parm"];
	$GLOBALS["rt_$parm"] = @$_SESSION["rt_Report2_$parm"];
}
...
function GetPopupFilter() {
	...
	if (is_array($GLOBALS["sel_invoice"])) {
		if ($sWrk <> "") $sWrk .= " AND ";
		$sWrk .= ewrpt_FilterSQL($GLOBALS["sel_invoice"], "oc_cobi.invoice", EW_REPORT_DATATYPE_STRING, $GLOBALS["af_invoice"]);
	}
	return $sWrk;
}
?>
...
function getSort()
{
	...
	if (strlen(@$_GET["cmd"]) > 0) {
		...
		if ($sCmd == "resetsort") {
			...
			$_SESSION["sort_Report2_date"] = "";
			$_SESSION["sort_Report2_supervisor"] = "";
			$_SESSION["sort_Report2_place"] = "";
			$_SESSION["sort_Report2_farm"] = "";
			$_SESSION["sort_Report2_flock"] = "";
			$_SESSION["sort_Report2_party"] = "";
			$_SESSION["sort_Report2_invoice"] = "";
			$_SESSION["sort_Report2_birds"] = "";
			$_SESSION["sort_Report2_quantity"] = "";
			$_SESSION["sort_Report2_price"] = "";
			$_SESSION["sort_Report2_Amount"] = "";
		}
	...
	if (@$_SESSION[EW_REPORT_TABLE_SESSION_ORDER_BY] == "") {
		@$_SESSION[EW_REPORT_TABLE_SESSION_ORDER_BY] = "oc_cobi.flock ASC, oc_cobi.party ASC, oc_cobi.invoice ASC, oc_cobi.birds ASC";
		$_SESSION["sort_Report2_flock"] = "ASC";
		$_SESSION["sort_Report2_party"] = "ASC";
		$_SESSION["sort_Report2_invoice"] = "ASC";
		$_SESSION["sort_Report2_birds"] = "ASC";
	}
...
if($diffm<0) {
$diff--; 
  echo "<script> var sec=$diff;var msec=".substr($diffm,1).' </script>' ;
 }
else {
echo "<script> var sec=$diff;var msec=$diffm;".' </script>' ;
 }
  ?>
<div align="center" id="loadgingpage"></div>
<script type="text/javascript">
...
document.location = 'broilerlifting.php?export=' + a + '&fromdate=' + fdate + '&todate=' + tdate;
}
</script>