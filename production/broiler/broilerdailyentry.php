...
{ 
  $birds = 0;
  $soldbirds = 0;
  $query111a = "SELECT (sum(quantity) - sum(tmort) - sum(shortage)) as chicks FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' AND (cat = 'Broiler Chicks' or cat = 'Broiler Day Old Chicks')"; $result111a = mysql_query($query111a,$conn1);  $rowsa = mysql_num_rows($result111a);
  if ( $rowsa > 0) { while($row111a = mysql_fetch_assoc($result111a)) { $birds = $birds + $row111a['chicks'];  } }

  //if(($birds == "") OR ($birds == 0))
  {
   $query111a = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm' and (description = 'Broiler Chicks' or description = 'Broiler Day Old Chicks')"; $result111a = mysql_query($query111a,$conn1);  $rowsa = mysql_num_rows($result111a);
   if ( $rowsa > 0) { while($row111a = mysql_fetch_assoc($result111a)) {   $birds = $birds + $row111a['chicks'];  } }
  }
...
else
{
 ...
 $query111 = "SELECT (sum(quantity) - sum(tmort) - sum(shortage)) as chicks FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' and date <= '$x_entrydate' and (cat = 'Broiler Chicks' or cat = 'Broiler Day Old Chicks')"; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
