...
//	echo "sql: " . $sSql . "<br>";
	...
	$print = 0;	
	$flkcnt = 0;
$query1 = "SELECT flock,max(age) as maxage FROM `broiler_daily_entry` WHERE cullflag <> 1 and flock NOT IN (SELECT flock FROM broiler_transferrate) GROUP BY flock ORDER BY max( age ) DESC";
	 ...
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	  $flkcnt = $flkcnt + 1;
	   if(($oldage <> $row1['maxage']) or ($flkcnt == $cnt))
	   {
		 ...
		 //Chicks Purchase
		  $query11 = "SELECT sum(receivedquantity) as chicks from pp_sobi where description = 'Broiler Chicks' and flock = '$row1[flock]'";
	     $result11 = mysql_query($query11,$conn1);
	     while($row11 = mysql_fetch_assoc($result11))
	     {
	       $chickstotal = $chickstotal + $row11['chicks'];
...
<?php $rbirds = $chickstotal - $mortalitytotal - $saletotal; ?>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changequantity($chickstotal - $mortalitytotal - $saletotal)); $totrbirds = $totrbirds + $rbirds; ?>
</td>
	</tr>
<?php		 
		    $chickstotal = 0;
