	                ...
	                while($num3 > 0)
	                 {
	                   if ( $num3 == 1)
					   {
					   $queryf = "SELECT SUM(mortality) as mort, MAX(average_weight) as avw,SUM(feedconsumed) as weekfeed,MAX(entrydate) as maxdate,MAX(cullflag) as cull FROM broiler_daily_entry WHERE place = '$place' AND farm = '$farmer' AND supervisior = '$supervisor' AND flock = '$flock' AND  age <= '$j' ORDER BY entrydate ASC ";
					   }
					   else
					   {
					   $queryf = "SELECT SUM(mortality) as mort, MAX(average_weight) as avw,SUM(feedconsumed) as weekfeed,MAX(entrydate) as maxdate,MAX(cullflag) as cull FROM broiler_daily_entry WHERE place = '$place' AND farm = '$farmer' AND supervisior = '$supervisor' AND flock = '$flock'  AND age < '$j' ORDER BY entrydate ASC ";
