...
<?php
 ...
 $query111 = "SELECT sum(quantity) as chicks FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' and (cat = 'Broiler Chicks' or cat='Broiler Day Old Chicks')  "; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
...
/* $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm'  and (category = 'Broiler Chicks' or category='Broiler Day Old Chicks'  ";  */
  $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm'  and description = 'Broiler Chicks'"; 
   $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $birds = $birds + $row111['chicks'];  } }
  
  $query111 = "SELECT sum(birds) as chicks FROM oc_cobi where flock = '$x_flock' and warehouse = '$x_farm' and code in (select distinct(code) from ims_itemcodes where cat = 'Broiler Birds' )  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
...
{
 if($feedbal == 0){ echo "0";  } else { echo ewrpt_ViewValue(changeprice($feedbal)); }   $totfeedremain = $totfeedremain + $feedbal; 
}
 ?>
