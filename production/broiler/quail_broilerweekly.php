...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Quail Weeekly Report</font></strong></td>
</tr>
...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT distinct(quail_broiler_daily_entry.flock),quail_broiler_daily_entry.supervisior, quail_broiler_daily_entry.place, quail_broiler_daily_entry.farm FROM " . $EW_REPORT_TABLE_SQL_FROM;
	   if($_SESSION['db'] == "feedatives")
		{
		   if($_SESSION['sectorr'] == "all")
		   {
		   	$EW_REPORT_TABLE_SQL_WHERE = "quail_broiler_daily_entry.cullflag = $cullflag";
		   }
		   else
		   {
		   $sectorr = $_SESSION['sectorr'];
		   $EW_REPORT_TABLE_SQL_WHERE = "quail_broiler_daily_entry.cullflag = $cullflag AND quail_broiler_daily_entry.place = '$sectorr'";
		   }
...
$EW_REPORT_TABLE_SQL_ORDERBY = "quail_broiler_daily_entry.supervisior ASC, quail_broiler_daily_entry.place ASC, quail_broiler_daily_entry.farm ASC";
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="quail_broilerweekly.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="quail_broilerweekly.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="quail_broilerweekly.php?export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="quail_broilerweekly.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<br /><br />
<?php } ?>
...
<form action="quail_broilerweekly.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
?>
	...
	<tr>
<?php if (@$sExport <> "") { ?>
		<td style="display:none" valign="bottom" class="ewRptGrpHeader1">
		Supervisior
		</td>
<?php } else { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Place</td>
			<td  align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report1_place', false, '<?php echo $rf_place; ?>', '<?php echo $rt_place; ?>');return false;" name="x_place<?php echo $cnt[0][0]; ?>" id="x_place<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Birds Sold">
		B.Sold
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Birds Sold">B.Sold</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php
 ...
 if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Transfere Birds">
		Tr.In
		</td>
<?php } else { ?>
		<td class="ewTableHeader" title="Transfere Birds">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Tr.In</td>
			</tr></table>
		</td>
...
<?php } else { ?>
		<td class="ewTableHeader" title="Transfer Out">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Tr.Out</td>
			</tr></table>
		</td>
...
$soldbirdssum = 0;
 ...
 $query1 = "SELECT max(age) as maxage,min(age) as minage FROM quail_broiler_daily_entry WHERE place = '$x_place' AND supervisior = '$x_supervisior' AND farm = '$x_farm' AND flock = '$x_flock' ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   $numdaysmin = $row1['minage'];
	   $numdaysmax = $row1['maxage'];
	 }
	 $startweek = round(($numdaysmin/7),0);
	 if(($startweek % 7) == 1)
	 {
	   $agefrom = $startweek;
	   $ageto = $agefrom + 7;
	 ...
	 {
	   $agefrom = $startweek;
	   $ageto = $agefrom + (8 - ($agefrom % 7));
	 }	 
	 $initflag = 0;
	 //echo $agefrom;echo $numdaysmax;
	 //while($agefrom <= ($numdaysmax/7))
	  while($agefrom <= $numdaysmax)
	 {
	 ?>
	 <tr>
		<td style="display:none" class="ewRptGrpField1">
		<?php $t_supervisior = $x_supervisior; $x_supervisior = $dg_supervisior; ?>
<?php if($initflag == 0) { echo ewrpt_ViewValue($x_supervisior); } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
		<?php $x_supervisior = $t_supervisior; ?></td>
		<td class="ewRptGrpField2">
		<?php $t_place = $x_place; $x_place = $dg_place; ?>
<?php if($initflag == 0) { echo ewrpt_ViewValue($x_place); } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
		<?php $x_place = $t_place; ?></td>
		<td class="ewRptGrpField3">
		<?php $t_farm = $x_farm; $x_farm = $dg_farm; ?>
<?php if($initflag == 0) { echo ewrpt_ViewValue($x_farm); } else { echo ewrpt_ViewValue("&nbsp;");  } ?>
		<?php $x_farm = $t_farm; ?></td>
		<td<?php echo $sItemRowClass; ?>>
<?php if($initflag == 0) { echo ewrpt_ViewValue($x_flock); } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
</td>
<?php
...
$sentweight = 0;
 $transferedbirds = 0;
 $query1 = "SELECT max(entrydate) as maxdate,max(age) as maxage,sum(mortality) as totmort,sum(feedconsumed) as feedcons,min(entrydate) as mindate FROM quail_broiler_daily_entry WHERE place = '$x_place' AND supervisior = '$x_supervisior' AND farm = '$x_farm' AND flock = '$x_flock' and age >= '$agefrom' and age < '$ageto' ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   $age = $row1['maxage'];
	    $fromdate = $row1['mindate'];
	    $todate = $row1['maxdate']; 
	   $enddate = date("d-m-Y",strtotime($row1['maxdate']));
	...
	$query111 = "SELECT sum(birds) as chicks FROM oc_cobi where flock = '$x_flock' and code = 'FBRD100' and date >= '$fromdate' and date <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
...
//Opening Birds
  ...
  $query111 = "SELECT sum(quantity) as chicks FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' and (cat = 'Quail Chicks' or cat='Broiler Day Old Chicks') and date <= '$fromdate'  "; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
 if ( $rows > 0) { while($row111 = mysql_fetch_assoc($result111)) {  $obirds = $obirds + $row111['chicks']; } }
 
 
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm'  and   code in (select distinct(code) from ims_itemcodes where (cat = 'Quail Chicks' or cat = 'Broiler Day Old Chicks')) and date <= '$fromdate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  { $obirds = $obirds + $row111['chicks'];  } }
   
  $query111 = "SELECT sum(birds) as chicks FROM oc_cobi where flock = '$x_flock' and code in (select distinct(code) from ims_itemcodes where cat = 'Fattener Birds' ) and date < '$fromdate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $obirds = $obirds - $row111['chicks'];  } }
  
  if($_SESSION[db]=='mallikarjunkld')
   { 
   $query111 = "SELECT birds as chicks from broiler_chickentransfer where flock = '$x_flock' and date < '$fromdate'";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $obirds = $obirds - $row111['chicks'];  } }
  }
  
   // fattener birds transfer calculation
  $fatquery = "select sum(mbirds) as mbirds,sum(fbirds) as fbirds from quail_broiler_fattenertransfer where ffarm = '$x_farm' and fflock = '$x_flock' and client = '$client'  and date < '$fromdate'";
  ...
  $obirds -= $transferedbirds;
  
  
  // fattener birds sending details
  $sentquery = "SELECT sum(birds) as birds from quail_broiler_chickentransfer where flock = '$x_flock' and date < '$fromdate' ";
  ...
  $sentres = mysql_fetch_assoc($sentresult);
   ...
   /// transfer in birds 
  $transferin = 0;
    $query111a = "SELECT (sum(quantity) - sum(tmort) - sum(shortage)) as chicks FROM ims_stocktransfer where aflock = '$x_flock'  AND cat = 'Fattener Birds' and date < '$fromdate'"; 
 ...
 $transferout = 0;
    $query111a = "SELECT (sum(quantity) - sum(tmort) - sum(shortage)) as chicks FROM ims_stocktransfer where tflock = '$x_flock'  AND (cat = 'Fattener Birds') and date < '$fromdate'"; 
	$result111a = mysql_query($query111a,$conn1);  
	$rowsa = mysql_num_rows($result111a);
  if( $rowsa > 0) { while($row111a = mysql_fetch_assoc($result111a)) { if($row111a['chicks']) echo $transferout = $row111a['chicks'];  } }
  
  
   $query111 = "SELECT sum(mortality) as chicks FROM quail_broiler_daily_entry where flock = '$x_flock' and farm = '$x_farm' and supervisior = '$x_supervisior' and place = '$x_place' and entrydate < '$fromdate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $obirds = $obirds - $row111['chicks'];  } }
 
 if($agefrom == $startweek)
 {
  $initbirds = $obirds;
 }
//Closing Birds

/* $query111 = "SELECT sum(quantity) as chicks FROM ims_stocktransfer where aflock = '$x_flock' and towarehouse = '$x_farm' and cat = 'Quail Chicks' and date <= '$todate'  "; $result111 = mysql_query($query111,$conn1);  $rows = mysql_num_rows($result111);
 if ( $rows > 0) { while($row111 = mysql_fetch_assoc($result111)) { $cbirds = $cbirds + $row111['chicks']; } }
 
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$x_flock' and warehouse = '$x_farm'  and category = 'Quail Chicks' and date <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $cbirds = $cbirds + $row111['chicks'];  } }
  
  $query111 = "SELECT sum(birds) as chicks FROM oc_cobi where flock = '$x_flock' and warehouse = '$x_farm' and code = 'FBRD100' and date <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $cbirds = $cbirds - $row111['chicks'];  } } */
  
   $query111 = "SELECT max(average_weight) as bodywt FROM quail_broiler_daily_entry where flock = '$x_flock' and farm = '$x_farm' and supervisior = '$x_supervisior' and place = '$x_place' and entrydate <= '$todate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {    $bodyweight = $row111['bodywt']; } }  
 
	
	 $query1 = "SELECT * from broiler_allstandards where age >= '$agefrom' and age <= '$age' order by age ";
	 $result1 = mysql_query($query1,$conn1);
	 while($row1 = mysql_fetch_assoc($result1))
	 {
	   
	  $cumfeedstd = $cumfeedstd + $row1['feed'];
...
<td<?php echo $sItemRowClass; ?>>
<?php if(date("d.m.Y",strtotime($enddate)) <> "01.01.1970") echo ewrpt_ViewValue(date($datephp,strtotime($enddate))); else echo ewrpt_ViewValue(); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changequantity($obirds - $previousbirdssent)) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changequantity($totalmortality)); $mortsum = $mortsum + $totalmortality; ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(round((($totalmortality/$obirds) * 100),2)); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($soldbirds > 0) { echo ewrpt_ViewValue(changequantity($soldbirds)); $soldbirdssum = $soldbirdssum + $soldbirds; } else { echo "0"; } ?>
</td>

	<td<?php echo $sItemRowClass; ?>>
<?php echo changequantity($transferin); $previousbirdssent-=$transferin; ?>
</td> 

<td<?php echo $sItemRowClass; ?>>
<?php echo changequantity($sentbirds+$transferedbirds+$transferout); $previousbirdssent+=$sentbirds+$transferedbirds+$transferout; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changeprice(round((($cumfeedstd/1000) * $obirds),2))); $feedconsstdsum = $feedconsstdsum + round((($cumfeedstd/1000) * $obirds),2); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right" <?php if($feedcons < (round((($cumfeedstd/1000) * $obirds),2))) { ?> style="color:#009900" <?php } else { ?> style="color:#FF0000" <?php } ?>  >
<?php echo ewrpt_ViewValue(changeprice($feedcons)); $feedconssum = $feedconssum + $feedcons; ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changeprice($bodywtstd)) ?>
</td>
<td<?php echo $sItemRowClass; ?> <?php if($bodyweight >= $bodywtstd) { ?> style="color:#009900" <?php } else { ?> style="color:#FF0000" <?php } ?> align="right">
<?php echo ewrpt_ViewValue(changeprice($bodyweight)) ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php  echo ewrpt_ViewValue(changeprice($stdfcr));  ?>
</td>
<?php
$bodywtkg = $bodyweight/1000;$bodywtkg = $bodywtkg * $initbirds; 
?>
<td<?php echo $sItemRowClass; ?> align="right" <?php echo $sItemRowClass; ?> <?php if($stdfcr >= round(($feedconssum/$bodywtkg),2)) { ?> style="color:#009900" <?php } else { ?> style="color:#FF0000" <?php } ?>>
<?php if($bodyweight <= 0) { echo "0";  } else { echo ewrpt_ViewValue(changeprice(round(($feedconssum/$bodywtkg),2))); }  ?>
</td>
	</tr>
	  <?php  $agefrom = $ageto;
		$ageto = $ageto + 7;
...
?>
	...
	<td><?php echo $age;?></td>
	<td><?php echo ewrpt_ViewValue("&nbsp;"); ?></td>
	<td align="right"><?php echo $initbirds;?></td>
	<td align="right"><?php echo $mortsum; ?></td>
	<td align="right"><?php echo round((($mortsum/$initbirds) * 100),2);?></td>
	<td align="right"><?php echo $soldbirdssum; ?>
	<?php 
	if($_SESSION['db'] == 'central')
     {?>
	<td><?php echo $previousbirdssent; $previousbirdssent =0; ?></td>
	<td><?php echo $totsentweight; $totsentweight =0;?></td>
	<?php } ?>
	<td align="right"><?php echo $feedconsstdsum;?></td>
	<td align="right" <?php if($feedconssum < $feedconsstdsum) { ?> style="color:#009900" <?php } else { ?> style="color:#FF0000" <?php } ?> ><?php echo $feedconssum; ?></td>
	<td align="right"><?php echo $bodywtstd; ?></td>
	<td align="right"  <?php if($bodywtstd < $bodyweight) { ?> style="color:#009900" <?php } else { ?> style="color:#FF0000" <?php } ?>><?php echo $bodyweight; ?></td>
	<td align="right"><?php echo ewrpt_ViewValue($stdfcr); ?></td>
	<?php
	$bodywtkg = $bodyweight/1000;$bodywtkg = $bodywtkg * $initbirds; 
	?>
	<td align="right" <?php if($stdfcr >= round(($feedconssum/$bodywtkg),2)) { ?> style="color:#009900" <?php } else { ?> style="color:#FF0000" <?php } ?>><?php if($bodyweight <= 0){ echo "0"; } else { echo ewrpt_ViewValue(round(($feedconssum/$bodywtkg),2)); } ?></td>
	</tr>-->
<?php
...
<!--first page button-->
	...
	<td><a href="quail_broilerweekly.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="quail_broilerweekly.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="quail_broilerweekly.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="quail_broilerweekly.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function reloadpage()
{
	...
	document.location = "quail_broilerweekly.php?cullflag=" + cullflag ;
}
</script>
