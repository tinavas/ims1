...
if($currencyunits == "")
{
$currencyunits = "Rs";if($_SESSION[db] == "alkhumasiyabrd")
     	$currencyunits = "SR";
}
?>
<table align="center" border="0">
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Supplier Statement Of Account</font></strong></td>
</tr>
<tr>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="vendorstatement.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="vendorstatement.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="vendorstatement.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
include "config.php";
	...
	while($qr = mysql_fetch_assoc($qrs))
	{
	  ...
	  {
	   $cgroup = $qr['cgroup'];
	   $cond1 = "OR coacode IN (select vca from ac_vgrmap  where vgroup = '$cgroup' and client = '$client')";
	  }
	}
...
<?php echo ewrpt_ViewValue() ?>
	...
	$q = "select * from ac_financialpostings where date <= '$tdate' and date >= '$fdate' and client = '$client' and venname = '$ven' and ( coacode = '$vencontrollac' $cond1 ) order by date ";
...
$bkinvoice = $qr['trnum'];
	  if ( $qr['type'] == "SOBI" )
	  {
	    ...
	    $q1 = "select invoice,remarks,credittermvalue from pp_sobi where so = '$qr[trnum]' and client = '$client' ";
	    ...
	    while($qr1 = mysql_fetch_assoc($qrs1))
	    {
		  ...
		  {
		  $creditterm=$qr1['credittermvalue'];
...
$bkinvoice = $qr1['invoice'];
	  ...
	  {
	      $mode = "PMT-Payment";
	      $q1 = "select * from pp_payment where tid = '$qr[trnum]' and client = '$client' ";
	      $qrs1 = mysql_query($q1,$conn1) ;
	      while($qr1 = mysql_fetch_assoc($qrs1))
	      {
...
include "config.php";
	...
	while($qr = mysql_fetch_assoc($qrs))
	{
	  ...
	  {
	   $cgroup = $qr['cgroup'];
	   $cond1 = "OR coacode IN (select vppac from ac_vgrmap  where vgroup = '$cgroup' and client = '$client')";
	  }
	}
...
<?php echo ewrpt_ViewValue() ?>
	...
	$q = "select * from ac_financialpostings where date BETWEEN '$fdate' AND '$tdate' and venname = '$ven' and client = '$client' and ( coacode = '$venprepayac' $cond1 )";
	$qrs = mysql_query($q,$conn1) ;
	while($qr = mysql_fetch_assoc($qrs))
	{
	$mode = "";
