...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Egg Receiving </font></strong></td>
</tr>
</table>
<?php 
if($currencyunits == "")
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="eggreceivingsmry.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="eggreceivingsmry.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="eggreceivingsmry.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
?>
	...
	$nxtdate = "";
	 $dumfrmdate = ""; 
	 include "config.php";
    $q = "select * from ims_eggreceiving where flock in (select distinct(flockcode) from ims_eggtransfer where client = '$client') and tid in (select distinct(tid) from ims_eggtransfer where client = '$client') order by date";
...
?>
	<tr>
		...
		<?php echo ewrpt_ViewValue(date($datephp,strtotime($qr['date2']))) ?>
		</td>
		<td class="ewRptGrpField1">
		<?php echo ewrpt_ViewValue(date($datephp,strtotime($qr['date']))) ?>
		</td>
           <?php $q1 = "select * from ims_eggtransfer where flockcode = '$qr[flock]' and tid = '$qr[tid]' and client = '$client'";
		   $r1 = mysql_query($q1,$conn1);
		   $qr1 = mysql_fetch_assoc($r1);
		   ?>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($qr1['fromwarehouse']); ?>
		</td>
		
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($qr1['shedcode']); ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($qr['flock']); ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($qr1['fromdescription']); ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo ewrpt_ViewValue(changequantity($diff)); ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($qr['todescription']); ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo ewrpt_ViewValue(changequantity($qr['toeggs'])); ?>
		</td>
<td class="ewRptGrpField2" align="right">
		<?php echo ewrpt_ViewValue(changequantity($qr1['wastage'])); ?>
		</td>
