...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Day Wise Fattener Feed Remaining</font></strong></td>
</tr>
...
$EW_REPORT_TABLE_SQL_SELECT_AGG = "SELECT SUM(`feedconsumed`) AS SUM_feedconsumed FROM " . $EW_REPORT_TABLE_SQL_FROM;
...
$EW_REPORT_FIELD_AGE_SQL_SELECT = "SELECT DISTINCT `age` FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_FIELD_AGE_SQL_ORDERBY = "`age`";
$EW_REPORT_FIELD_FEEDTYPE_SQL_SELECT = "SELECT DISTINCT `feedtype` FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_FIELD_FEEDTYPE_SQL_ORDERBY = "`feedtype`";
...
$col = array(FALSE, FALSE, FALSE, TRUE, FALSE);
...
ewrpt_CreatePopup("Report1_age", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_feedtype, $sel_feedtype, $ft_feedtype) ?>
ewrpt_CreatePopup("Report1_feedtype", [<?php echo $jsdata ?>]);
...
<div id="Report1_age_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="Report1_feedtype_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="fattenerfeedremain.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="fattenerfeedremain.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="fattenerfeedremain.php?export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="fattenerfeedremain.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<form action="fattenerfeedremain.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="fattenerfeedremain.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="fattenerfeedremain.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="fattenerfeedremain.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="fattenerfeedremain.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Age</td>
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report1_age', true, '<?php echo $rf_age; ?>', '<?php echo $rt_age; ?>');return false;" name="x_age<?php echo $cnt[0][0]; ?>" id="x_age<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			...
			<td style="width: 20px;" align="right"><a href="#" onClick="ewrpt_ShowPopup(this.name, 'Report1_feedtype', false, '<?php echo $rf_feedtype; ?>', '<?php echo $rt_feedtype; ?>');return false;" name="x_feedtype<?php echo $cnt[0][0]; ?>" id="x_feedtype<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
$totalfeed = 0;
 ...
 include "../config.php";
  $q1 = "SELECT shedcode  FROM pig_herd WHERE herdcode = '$x_flock'";
 $result1 = mysql_query($q1,$conn1);
 while($row1 = mysql_fetch_assoc($result1))
 {
  $shed = $row1['shedcode'];
 }
if(($oldfarmer <> $shed)  OR ($oldflock <> $x_flock)) {
 if(($oldfarmer <> $shed) OR ($oldflock <> $x_flock)) {   
    $feedrem = "0";
 }

 include "../config.php";
 $get1 = "SELECT SUM(quantity) as totalfeed1 FROM ims_stocktransfer WHERE towarehouse = '$x_flock'  AND aflock = '$x_flock' AND date <= '$x_entrydate' and cat = 'Fattener Feed' ";
 ...
 $get1a = "SELECT SUM(quantity) as totalfeed1 FROM ims_stocktransfer WHERE fromwarehouse = '$x_flock'  AND aflock = '$x_flock' AND date <= '$x_entrydate' and cat = 'Fattener Feed' ";
...
else
{
 ...
 include "../config.php";
 $get1a = "SELECT quantity as totalfeed1 FROM ims_stocktransfer WHERE fromwarehouse = '$x_flock'  AND tflock = '$x_flock' AND date = '$x_entrydate' and cat = 'Fattener Feed' ";
 $getresult1a = mysql_query($get1a,$conn1);
...
?>
		<td<?php echo $sItemRowClass; ?>>
<?php if( ($totalfeed1a - $totalfeed1b) == 0 ){ echo "0"; } else { echo ewrpt_ViewValue(($totalfeed1a - $totalfeed1b)); }  ?>
</td>
		<td<?php echo $sItemRowClass; ?>>
...
?>
	<tr>
		<td colspan="8" class="ewRptGrpSummary1">Summary for Flock: <?php $t_flock = $x_flock; $x_flock = $o_flock; ?>
<?php echo ewrpt_ViewValue($x_flock) ?>
<?php $x_flock = $t_flock; ?> (<?php echo ewrpt_FormatNumber($cnt[1][0],0,-2,-2,-2); ?> Detail Records)</td></tr>
	<tr>
		...
		<?php $x_feedconsumed = $smry[1][3]; // Load SUM ?>
<?php echo ewrpt_ViewValue($x_feedconsumed) ?>
...
<?php
	...
	if ($rsagg) {
		$grandsmry[3] = $rsagg->fields("SUM_feedconsumed");
	}
	else {
...
<?php if ($nTotalGrps > 0) { ?>
	...
	<tr class="ewRptGrandSummary">
		...
		<td>
		<?php $t_feedconsumed = $x_feedconsumed; ?>
		<?php $x_feedconsumed = $grandsmry[3]; // Load SUM ?>
<?php echo ewrpt_ViewValue($x_feedconsumed) ?>
		<?php $x_feedconsumed = $t_feedconsumed; ?>
		</td>
		<td>&nbsp;</td>
...
function SetupPopup() {
	...
	if (is_array(@$_SESSION["sel_Report1_age"])) {
		LoadSelectionFromSession('age');
	} elseif (@$_SESSION["sel_Report1_age"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_age"] = "";
	}

	// Get Feedtype selected values
	if (is_array(@$_SESSION["sel_Report1_feedtype"])) {
		LoadSelectionFromSession('feedtype');
	} elseif (@$_SESSION["sel_Report1_feedtype"] == EW_REPORT_INIT_VALUE) { // Select all
...
function GetPopupFilter() {
	...
	if (is_array($GLOBALS["sel_age"])) {
		if ($sWrk <> "") $sWrk .= " AND ";
		$sWrk .= ewrpt_FilterSQL($GLOBALS["sel_age"], "`age`", EW_REPORT_DATATYPE_NUMBER, $GLOBALS["af_age"]);
	}
	if (is_array($GLOBALS["sel_feedtype"])) {
		if ($sWrk <> "") $sWrk .= " AND ";
		$sWrk .= ewrpt_FilterSQL($GLOBALS["sel_feedtype"], "`feedtype`", EW_REPORT_DATATYPE_STRING, $GLOBALS["af_feedtype"]);
	}
	if (is_array($GLOBALS["sel_entrydate"])) {
		if ($sWrk <> "") $sWrk .= " AND ";
