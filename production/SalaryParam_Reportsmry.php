...
$genlink = "&sector=".$_GET['sector']."&desig=".$_GET['desig']."&employee=".$_GET['employee']."&month=".$_GET['month']."&year=".$_GET['year'];
...
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Salary Parameters Report for <?php echo $_GET['employee'].", ".$desig. " of ".$_GET['sector']; ?></font></strong></td>
</tr>
...
<?php } ?>
<?php
$genlinkm = "&sector=".$_GET['sector']."&desig=".$_GET['desig']."&employee=".$_GET['employee']."&month=".$_GET['month']."&year=".$_GET['year'];?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="SalaryParam_Reportsmry.php?<?php echo $genlinkm; ?>&export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="SalaryParam_Reportsmry.php?<?php echo $genlinkm; ?>&export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="SalaryParam_Reportsmry.php?<?php echo $genlinkm; ?>&export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="SalaryParam_Reportsmry.php?<?php echo $genlinkm; ?>&cmd=reset">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
?>
    <thead>
      <tr>
	  	<?php if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader"> Employee </td>
        <?php } else { ?>
        ...
        <?php } ?>
		<?php if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader"> Sector </td>
        <?php } else { ?>
        <td class="ewTableHeader"><table cellspacing="0" class="ewTableHeaderBtn">
            <tr>
              <td>Sector</td>
        ...
        <?php } ?>
		<?php if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader"> Designation </td>
        <?php } else { ?>
        <td class="ewTableHeader"><table cellspacing="0" class="ewTableHeaderBtn">
            <tr>
              <td>Designation</td>
        ...
        <?php } ?>
		<?php if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader"> From Date </td>
        <?php } else { ?>
        <td class="ewTableHeader"><table cellspacing="0" class="ewTableHeaderBtn">
            <tr>
              <td>From Date</td>
        ...
        <?php } ?>
		<?php if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader"> To Date </td>
        <?php } else { ?>
        <td class="ewTableHeader"><table cellspacing="0" class="ewTableHeaderBtn">
            <tr>
              <td>To Date</td>
        ...
        <?php } ?>
		<?php if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader"> Salary </td>
        <?php } else { ?>
        <td class="ewTableHeader"><table cellspacing="0" class="ewTableHeaderBtn">
            <tr>
              <td>Salary</td>
		...
		<?php
		$stdarr1 = "";$stdarr2 = "";$stdi =0;
		$stdarr1[0] = "id";$stdarr1[1] = "eid";$stdarr1[2] = "sector";$stdarr1[3] = "designation";$stdarr1[4] = "name";
		$stdarr1[5] = "fromdate";$stdarr1[6] = "todate";$stdarr1[7] = "salary";$stdarr1[8] = "leavesal";$stdarr1[9] = "finalsal";
		$stdarr1[10] = "flag";$stdarr1[11] = "client";$stdarr1[12] = "updated";
		$totfields = 7 ;
		$q1="Describe hr_salary_parameters";
		$q1rs = mysql_query($q1,$conn1) or die(mysql_error());
		while($q1r = mysql_fetch_assoc($q1rs))
		{
		 $currfield = $q1r['Field'];
		if(!in_array($currfield,$stdarr1)) 
		{
		$stdarr2[$stdi]=$currfield;
		$stdi = $stdi+1; $totfields = $totfields + 1;
		}
		}
		sort($stdarr2);
		for($i=0;$i<count($stdarr2);$i++)
		{
		$currtotvar = "tot".$stdarr2[$i];
		$$currtotvar = 0;
		if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader"> <?php echo $stdarr2[$i]; ?> </td>
        <?php } else { ?>
        <td class="ewTableHeader"><table cellspacing="0" class="ewTableHeaderBtn">
            <tr>
              <td><?php echo $stdarr2[$i]; ?></td>
            </tr>
        </table></td>
        <?php }
		}
		...
		<?php if (@$sExport <> "") { ?>
        <td valign="bottom" class="ewTableHeader" title="Final Salary"> Final Sal. </td>
        <?php } else { ?>
        <td class="ewTableHeader"><table cellspacing="0" class="ewTableHeaderBtn" title="Final Salary">
            <tr>
              <td>Final Sal.</td>
            </tr>
        </table></td>
...
?>
      <?php
		...
		$month = $_GET['month'];
		$year = $_GET['year'];
		$totrec =0;$totsalary =0;$totfinalsalary=0;

		include "config.php";
		if($fd!= "" && $td!="")
		{
		 $qc = "Select * from hr_salary_parameters where sector $sc '$sector' and designation $dc '$desig' and name $nc '$empname' and (('$fd' between fromdate and todate) or ('$td' between fromdate and todate))";
	    }
		else if($month == "All" && $year == "All")
		{
		 $qc = "Select * from hr_salary_parameters where sector $sc '$sector' and designation $dc '$desig' and name $nc '$empname' ";
		}
		else if($month == "All" && $year != "All")
		{
		 $qc = "Select * from hr_salary_parameters where sector $sc '$sector' and designation $dc '$desig' and name $nc '$empname' and ((fromdate like '$year-%') or (todate like '$year-%')) ";
		}
		else if($month != "All" && $year == "All")
		{
		 $qc = "Select * from hr_salary_parameters where sector $sc '$sector' and designation $dc '$desig' and name $dc '$empname' and ((fromdate like '%-$month-%') or (todate like '%-$month-%')) ";
		}
		...
		?>
      <tr>
        <td class="ewRptGrpField1"  ><?php echo $qcr['name']; ?></td>
        <td<?php echo $sItemRowClass; ?>><?php echo $qcr['sector']; ?></td>
        <td class="ewRptGrpField3"><?php echo $qcr['designation']; ?></td>
        <td><?php echo date("j-m-Y",strtotime($qcr['fromdate'])); ?></td>
        <td ><?php echo date("j-m-Y",strtotime($qcr['todate'])); ?></td>
		<td class="ewRptGrpField3"><?php echo $qcr['salary']; $totsalary=$totsalary+$qcr['salary'] ?></td>
		<?php for($j=0;$j<count($stdarr2);$j++)
		{ $currtotvar = "tot".$stdarr2[$j];
		?>
		
		<td<?php echo $sItemRowClass; ?> ><?php if($qcr[$stdarr2[$j]] == "") {echo "0";} else {echo $qcr[$stdarr2[$j]]; $$currtotvar = $$currtotvar + $qcr[$stdarr2[$j]]; }?></td>
		<?php } ?>
		<td class="ewRptGrpField3"><?php echo $qcr['finalsal'];  $totfinalsalary=$totfinalsalary+$qcr['finalsal'] ?></td>
      </tr>
      <?php $totrec = $totrec +1;}		
...
?>
      ...
      <tr class="ewRptGrandSummary">
        <td colspan="<?php echo $totfields;?>">Grand Total (<?php echo ewrpt_FormatNumber($totrec,0,-2,-2,-2); ?> Detail Records)</td>
      </tr>
      <tr class="ewRptGrandSummary">
        ...
        <td>
            <b><?php echo ewrpt_ViewValue($totsalary) ?></b>
            </td>
		<?php for($j=0;$j<count($stdarr2);$j++)
		{ $currtotvar = "tot".$stdarr2[$j];
		?>
        <td>
            <b><?php echo ewrpt_ViewValue($$currtotvar) ?></b>
                   </td>
				   <?php }?>
        <td>
            <b><?php echo ewrpt_ViewValue($totfinalsalary) ?></b>
            </td>
