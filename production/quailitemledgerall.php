...
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Quail Item Ledger</font></strong></td>
</tr>
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Sector:&nbsp;<?php echo $_GET['sector']; ?></font></strong></td>
</tr>
<tr>
...
$url = "&fromdate=" . $_GET['fromdate'] . "&todate=" . $_GET['todate'] ."&code=" . $_GET['code']."&sector=" . $_GET['sector'];
// Get page start time
...
define("EW_REPORT_TABLE_VAR", "quailquailitemledgerall", TRUE);
define("EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE", "quailquailitemledgerall_grpperpage", TRUE);
define("EW_REPORT_TABLE_SESSION_START_GROUP", "quailquailitemledgerall_start", TRUE);
define("EW_REPORT_TABLE_SESSION_SEARCH", "quailquailitemledgerall_search", TRUE);
define("EW_REPORT_TABLE_SESSION_CHILD_USER_ID", "quailquailitemledgerall_childuserid", TRUE);
define("EW_REPORT_TABLE_SESSION_ORDER_BY", "quailquailitemledgerall_orderby", TRUE);
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="quailitemledgerall.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="quailitemledgerall.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="quailitemledgerall.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<form action="quailquailitemledgerallsmry.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="quailquailitemledgerallsmry.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="quailquailitemledgerallsmry.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="quailquailitemledgerallsmry.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="quailquailitemledgerallsmry.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewRptGrpHeader2" align="center">
		Unit
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
...
<?php } else { ?>
		<td class="ewTableHeader" colspan="3">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td colspan="3" align="center">Receipt</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader" colspan="3">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td colspan="3" align="center">Issue</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader" colspan="3">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td colspan="3" align="center">Closing Balance</td>
			</tr></table>
		</td>
<?php } ?>
	</tr>
...
<?php
	...
	$finalmaintoal = 0;
      ...
      $totissval = 0;
      $totremval = 0;
	  $warehouse = $_GET['sector'];
	include "config.php";
	$q11 = "select code,description,sunits,iac from ims_itemcodes WHERE (cat in ( select distinct(cat) from quail_category) or cat LIKE '%Birds') and client = '$client' order by code  ";
...
$q = "select amount,quantity,type,trnum from ac_financialpostings where crdr = 'Cr' and itemcode  = '$code' and coacode = '$itemcode' and date < '$fdate' AND warehouse = '$warehouse' order by date ";
    ...
    // For Productions and Consumptions 
$q = "select amount,quantity,type,trnum from ac_financialpostings where itemcode  = '$code' and crdr = 'Dr' and coacode = '$itemcode' and date < '$fdate' AND warehouse = '$warehouse' order by date ";
 ...
 $query1 = "SELECT sum(quantity) as itotqty,sum(amount) as itotamt from ac_financialpostings where itemcode = '$code' and crdr = 'Dr' and coacode = '$itemcode' and date >= '$fdate' and date <= '$tdate'  AND warehouse = '$warehouse' ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	  
	 
       $rcvqty = $rcvqty + $row2['itotqty']; 
         $rcvval = $rcvval + $row2['itotamt']; 
  ...
  $query1 = "SELECT sum(quantity) as itotqty,sum(amount) as itotamt from ac_financialpostings where itemcode = '$code' and crdr = 'Cr' and coacode = '$itemcode' and date >= '$fdate' and date <= '$tdate'  AND warehouse = '$warehouse' ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
        ...
        $isqty = $isqty + $row2['itotqty']; 
         $isval = $isval + $row2['itotamt'];
		
	} // End detail records loop
	
	if ( ($rcvqty > 0) or ($isqty > 0) or ($obqty > 0))
	{
	...
	<tr>
		...
		<?php echo ewrpt_ViewValue($qr11['code']) ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($qr11['description']) ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($qr11['sunits']) ?>
		</td>
		<td align="right">
<?php echo ewrpt_ViewValue(changeprice($rcvqty)) ?>
</td>
<?php $totrcvqty = $totrcvqty + $rcvqty; ?>
		<td align="right">
...
<?php $totrcvval = $totrcvval + $rcvval;  ?>
<?php ?>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($isqty));  ?>
</td>
<?php $totissqty = $totissqty + $isqty; ?>
<td align="right">
...
<?php $totissval = $totissval + $isval; ?>

<td align="right">
<?php echo ewrpt_ViewValue(changeprice($obqty + $rcvqty - $isqty)); ?>
</td>
<?php $totremqty = $totremqty + ($obqty + $rcvqty - $isqty); ?>
...
<?php $totremval = $totremval + ($obval + $rcvval - $isval); ?>

	</tr>
<?php
}
$rem = 0;
$rem = $obqty + $rcvqty - $isqtyl;
$godown = "Godown1";


	} // End Items Loop
?>
<?php if(($totrcvqty > 0) or ($totissqty > 0) or ($totremqty > 0)){ ?>
<tr>
		<td class="ewRptGrpField1">
		...
		<td align="right">
<?php echo ewrpt_ViewValue(changeprice($totrcvqty)) ?>
</td>
		<td align="right">
<?php echo ewrpt_ViewValue() ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($totrcvval));   ?>
</td>
<?php ?>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($totissqty));  ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue();  ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($totissval)); ?>
</td>
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($totremqty)); ?>
</td>
<td align="right">
<?php echo ewrpt_ViewValue(); ?>
</td>
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($totremval));  ?>
</td>
...
function getSort()
{
	...
	if (strlen(@$_GET["cmd"]) > 0) {
		...
		if ($sCmd == "resetsort") {
			...
			$_SESSION["sort_quailquailitemledgerall_type"] = "";
			$_SESSION["sort_quailquailitemledgerall_schedule"] = "";
			$_SESSION["sort_quailquailitemledgerall_controltype"] = "";
			$_SESSION["sort_quailquailitemledgerall_code"] = "";
			$_SESSION["sort_quailquailitemledgerall_description"] = "";
		}
	...
	if (@$_SESSION[EW_REPORT_TABLE_SESSION_ORDER_BY] == "") {
		...
		$_SESSION["sort_quailquailitemledgerall_code"] = "ASC";
		$_SESSION["sort_quailquailitemledgerall_description"] = "ASC";
	}
