...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Candling  Rejection</font></strong></td>
</tr>
</table>
<?php 
if($currencyunits == "")
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="rejections.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="rejections.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="rejections.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="10th Day Rejection">
		10th Day Rej
		</td>
<?php } else { ?>
		<td class="ewTableHeader" title="10th Day Rejection">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="10th Day Rejection">10th Day Rej</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Remaining Eggs ">
		Rem Eggs 
		</td>
<?php } else { ?>
		<td class="ewTableHeader" title="Remaining Eggs ">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Remaining Eggs ">Rem Eggs</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Candling Rejection">
		Candling Rej
		</td>
<?php } else { ?>
		<td class="ewTableHeader" title="Candling Rejection">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Candling Rejection">Candling Rej</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
?>
	...
	$nxtdate = "";
	 $dumfrmdate = ""; 
	 include "config.php";
    $q = "select * from hatchery_traysetting where client = '$client' and transferdate in (select distinct(transferdate) from transferdetails_hatchery where client = '$client')";
      $r = mysql_query($q,$conn1);
	while($qr = mysql_fetch_assoc($r))
      {
	 $tdate = $qr['transferdate'];
	 $flock = $qr['flock'];
	 $eggsset = $qr['eggsset'];
	  $dayrejection = $qr['dayrejection'];
	   $rejectpercen = $qr['rejectionpercen'];
	   $reggs = $eggsset - $dayrejection;
	 
?>
	<tr>
	<td<?php echo $sItemRowClass; ?>>
<?php if(($tdate <> $oldcode) or ($dumm == 0)){ echo ewrpt_ViewValue(date($datephp,strtotime($tdate)));
     $oldcode = $tdate; $dumm = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
		...
		<td class="ewRptGrpField1">
		<?php echo ewrpt_ViewValue($eggsset) ?>
		</td>
		<td class="ewRptGrpField1">
		<?php if($rejectpercen <> "") { echo ewrpt_ViewValue($dayrejection); } else { echo "0"; } ?>
		</td>
		<td class="ewRptGrpField1">
		<?php if($rejectpercen <> "") { echo ewrpt_ViewValue($rejectpercen); } else { echo "0"; } ?>
		</td>
		<td class="ewRptGrpField1">
		<?php if($reggs <> "") { echo ewrpt_ViewValue($reggs); } else { echo "0"; } ?>
		</td>
           <?php 
$q1 = "select * from transferdetails_hatchery where settingno = '$qr[settingno]' and flock = '$flock' and client = '$client'";
		   $r1 = mysql_query($q1,$conn1);
		   $qr1 = mysql_fetch_assoc($r1);
		   ?>
		   <td class="ewRptGrpField1">
		<?php if($reggs <> "") { echo ewrpt_ViewValue($qr1['candlingrejection']); } else { echo "0"; } ?>
		</td>
		 <td class="ewRptGrpField1">
		<?php if($reggs <> "") { echo ewrpt_ViewValue($qr1['candlingpercen']); } else { echo "0"; } ?>
		</td>
