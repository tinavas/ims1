...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Layer Future Weekly Report</font></strong></td>
</tr>
<tr>
...
<?php } else { ?>
		<td class="ewTableHeader" align="center">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align = "center" >Feed Intake (In Kgs)</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php
$ceggs = 0;
 $q1 = "select * from layer_flock where flockcode = '$flock' and client = '$client'";
...
//Purchase
$q8 = "select sum(receivedquantity) as purchasebirds from pp_sobi where flock = '$flock' and code in (select code from ims_itemcodes where cat = 'Layer Birds') and client = '$client'";
...
$totfe = $female;
$qyu = "select * from layer_consumption where flock = '$flock' and client = '$client'";
$ryu = mysql_query($qyu,$conn1);
$nofr = mysql_num_rows($ryu);
while($qryu = mysql_fetch_assoc($ryu))
{
$fage = ceil(round(($qryu['age']/7),1));
...
$bweight = 0;
$eweight = 0;
$query1 = "SELECT * FROM layer_pstandards where client = '$client' and (age >=  '$fage' and age <= '$tage') ORDER BY age ASC ";
$r = mysql_query($query1,$conn1);
while($qr = mysql_fetch_assoc($r))
{
if($qr['age'] == " ")
{
$q = "select * from layer_flock where flock = '$flock' and client = '$client'";
...
//echo $qr['heperweek']."/".$female; echo "<br/>";
$totaleggs = round(($qr['heperweek'] * $female),0);
$ffeed = $qr['fm'];
//echo $ffeed."/".$female;
$feedintake = round(((($ffeed*$female)/1000) * 7 ),2);
$fmortpercen = 0.01;
$fmort = floor(($female*$fmortpercen)/100);
$livstd = round($qr['livability'],2);
$feedperegg = round((($feedintake/$totaleggs)*1000),2);
$feedperbird = round((($feedintake/$female)*1000),2);
$bodyweight = round(($qr['bodyweight']),2);
$eggweight = round(($qr['eggweight']),2);
...
//echo $feedintake."/".$costperkg;
$pc1 = round(( $feedintake * $costperkg ),2);
$pc = $pc1;

}
//echo $feedintake."/".$costperkg; echo "<br/>";

$cumpc = $cumpc + $pc;

//echo $ffeed."/".$costperkg; echo "<br/>";
?>
<tr>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($totaleggs > 0) {  echo ewrpt_ViewValue(changequantity($totaleggs)); } else { echo ewrpt_ViewValue("0"); } $tottotaleggs = $tottotaleggs + $totaleggs;?></td>

</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($feedintake > 0) {  echo ewrpt_ViewValue(changeprice($feedintake)); } else { echo ewrpt_ViewValue("0"); } $totfeedintake = $totfeedintake + $feedintake; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($feedperegg > 0) {  echo ewrpt_ViewValue(changeprice($feedperegg)); } else { echo ewrpt_ViewValue("0"); } $totfeedperegg = $totfeedperegg + $feedperegg; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($feedperbird > 0) {  echo ewrpt_ViewValue(changeprice($feedperbird)); } else { echo ewrpt_ViewValue("0"); } $totfeedperbird = $totfeedperbird + $feedperbird; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($bodyweight > 0) {  echo ewrpt_ViewValue($bodyweight); } else { echo ewrpt_ViewValue("0"); } $totbodyweight = $totbodyweight + $bodyweight; ?>
</td>

<td<?php echo $sItemRowClass; ?> align="right">
<?php if($eggweight  > 0) {  echo ewrpt_ViewValue($eggweight ); } else { echo ewrpt_ViewValue("0"); } $toteggweight = $toteggweight + $eggweight; ?>
</td>
...
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changeprice($totfeedintake) ;?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changeprice($totfeedperegg);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changeprice($totfeedperbird);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b> <?php echo changequantity($totbodyweight); ?></b></td>
<td <?php echo $sItemRowClass; ?> align="right"><b> <?php echo changequantity($toteggweight); ?></b></td>
