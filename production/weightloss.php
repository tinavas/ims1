...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Weight Loss</font></strong></td>
</tr>
<tr>
<td colspan="2"><font size="2">From Date&nbsp;<?php echo date($datephp,strtotime($fdate)); ?>
                To Date &nbsp;<?php echo date($datephp,strtotime($tdate));?></font></td>
</tr>
</table>
<?php 
if($currencyunits == "")
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="weightloss.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="weightloss.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="weightloss.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
&nbsp;&nbsp;From Date&nbsp;
<input type="text" class="datepicker" id="fromdate" name="fromdate" value="<?php echo date($datephp,strtotime($fdate)); ?>" onchange="reloadpage();"/>
&nbsp;&nbsp;&nbsp;

To Date&nbsp;
<input type="text" class="datepicker" id="todate" name="todate" value="<?php echo date($datephp,strtotime($tdate)); ?>" onchange="reloadpage();"/>
&nbsp;&nbsp;&nbsp;
		<td nowrap>
...
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Purchase Quantity">
		Purchase Qty
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn" ><tr>
			<td title="Purchase Quantity">Purchase Qty</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Purchase Weight">
		Purchase Wt.
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Purchase Weight">Purchase Wt.</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Process Quantity">
		Process Qty.
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Process Quantity">Process Qty.</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" title="Process Weight">
		Process Wt.
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td title="Process Weight">Process Wt.</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
$q = "select distinct(date) from chicken_chickentransfer where client = '$client' and date between '$fdate1' and '$tdate1' group by date ";
$r = mysql_query($q,$conn1) or die(mysql_error());
while($qr = mysql_fetch_assoc($r))
{
$q2 = "select distinct(tid),birds as proqty,weight as prowt from chicken_chickentransfer where client = '$client' and date = '$qr[date]' group by date,tid ";
...
$q1 = "select sum(receivedquantity) as purqty,sum(weight) as purwt  from pp_sobi where date = '$qr[date]' and client = '$client' and date between '$fdate1' and '$tdate1'";
$r1 = mysql_query($q1,$conn1) or die(mysql_error());
$qr1 = mysql_fetch_assoc($r1);
$purqty = $qr1['purqty'];
$purwt = $qr1['purwt'];
$ploss = ($purwt/$purqty) * $proqty;
$plossper = round(((($ploss - $prowt)/$ploss)*100),2);

?>
<tr>
		
<td class="ewRptGrpField1"><?php echo ewrpt_ViewValue($date) ?></td>
<td class="ewRptGrpField1"><?php if($purqty <> "") { echo ewrpt_ViewValue($purqty); } else { echo "0";}?> </td>
<td class="ewRptGrpField1"><?php if($purwt <> "") { echo ewrpt_ViewValue($purwt); } else { echo "0"; } ?></td>
<td class="ewRptGrpField1"><?php if($proqty <> "") { echo ewrpt_ViewValue($proqty); } else { echo "0"; } ?></td>
<td class="ewRptGrpField1"><?php if($prowt <> "") { echo ewrpt_ViewValue($prowt); } else { echo "0"; } ?></td>
<td class="ewRptGrpField1"><?php if($plossper <> "") { echo ewrpt_ViewValue($plossper); } else { echo "0"; } ?></td>
</tr>
...
if(Date.parse(date1) < Date.parse(date2))
	...
	document.location = "weightloss.php?fromdate=" + fromdate + "&todate=" + todate;
	}
	else
	{
