...
// Table level SQL
$EW_REPORT_TABLE_SQL_FROM = "`ac_financialpostingssummary`";
...
<?php
	...
	$quer11 = "select distinct(schedule),type from ac_schedule where  schedule in ( select distinct(schedule) from ac_coa where type = 'Capital' or type = 'Liability' ) order by schedule  ";
	   ...
	   {
	     $coacount = 0;
	      $quer = "select * from ac_financialpostingssummary where coacode in (select distinct(code) from ac_coa where schedule = '$row111[schedule]')  and date <= '$tdate' ";
		  ...
		  if ( $coacount > 0)
		  {
	          ...
	          $quers = mysql_query($quer,$conn1) or die(mysql_error());
		       while($row1 = mysql_fetch_assoc($quers))
			   {
	             ...
	             $bal = 0;
	             $mbal = 0;
	             $q = "select sum(amount) as cramount  from ac_financialpostingssummary where crdr = 'Cr' and date <= '$tdate' and coacode = '$code' ";
	             $qrs = mysql_query($q,$conn1) or die(mysql_error());
	             while($qr = mysql_fetch_assoc($qrs))
	             {
	                $cramount = $cramount + $qr['cramount'];
	             }
	            $q = "select sum(amount) as dramount  from ac_financialpostingssummary where crdr = 'Dr' and date <= '$tdate'  and coacode = '$code' ";
	            $qrs = mysql_query($q,$conn1) or die(mysql_error());
	            while($qr = mysql_fetch_assoc($qrs))
	            {
