...
&nbsp;&nbsp;<a href="eggstocksmryhatch1.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="eggstocksmryhatch1.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="eggstocksmryhatch1.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="eggstocksmryhatch1.php?cmd=reset<?php echo $url; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
...
Warehouse&nbsp;
<select id="warehouse" name="warehouse" onChange="">
<option value = "select">-select-</option>	
<?php
$q = "select distinct(unitcode) from layer_flock order by unitcode";
...
<?php } ?>
<?php
$q = "select distinct(unitcode) from layerbreeder_unit order by unitcode";
$qrs = mysql_query($q,$conn1);
$n1 = mysql_num_rows($qrs);
while($qr = mysql_fetch_assoc($qrs))
{
...
<?php } ?>
 ...
 Egg Type&nbsp;
<select id="etype" name="etype" onChange="getetype(this.value);">
<option value = "select">-select-</option>	
<?php
$q = "select distinct(type) from ims_itemtypes where type like '%Eggs' order by type";
$qrs = mysql_query($q,$conn1);
$n1 = mysql_num_rows($qrs);
while($qr = mysql_fetch_assoc($qrs))
{
?>
<option value="<?php echo $qr['type'];?>" <?php if($qr[type] == $etype) { ?> selected="selected"<?php } ?>><?php echo $qr['type'];?></option>
...
if($etype=='Layer Eggs')
{
$q = "SELECT distinct(flockcode) from layer_flock where flockcode in (select distinct(flock) from layer_production) order by flockcode";
$qrs = mysql_query($q,$conn1);
$n1 = mysql_num_rows($qrs);
while($qr = mysql_fetch_assoc($qrs))
{
...
$u = $qr['flockcode'];
?>
<option value="<?php echo $qr['flockcode'];?>" <?php if($qr[flockcode] == $_GET['flock']) { ?> selected="selected"<?php } ?>><?php echo $qr['flockcode'];?></option>

	
<?php } }else {
	
 
	     $q = "SELECT distinct(flockcode) from layerbreeder_flock where flockcode in (select distinct(flock) from layerbreeder_production) order by flockcode";
$qrs = mysql_query($q,$conn1);
$n1 = mysql_num_rows($qrs);
while($qr = mysql_fetch_assoc($qrs))
{
...
if($etype=='Layer Eggs') 
  ...
  $getop = "SELECT * FROM ims_itemcodes WHERE cat ='Layer Eggs' and (code in (select distinct(itemcode) from layer_production) or code in (select distinct(code) from oc_cobi ))  order by code";
else if($etype=="Layer Hatch Eggs")
$getop = "SELECT * FROM ims_itemcodes WHERE cat = 'Layer Hatch Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  order by code";
else {
	$getop = "SELECT * FROM ims_itemcodes WHERE cat = 'Layer Breeder Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  order by code";
...
<?php
if($etype=="Layer Eggs")
$getop1 = "SELECT * FROM ims_itemcodes WHERE cat = 'Layer Eggs' and (code in (select distinct(itemcode) from layer_production) or code in (select distinct(code) from oc_cobi ))  order by code";
 
 
 else if($etype=="Layer Hatch Eggs")
$getop1 = "SELECT * FROM ims_itemcodes WHERE cat = 'Layer Hatch Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  order by code";
else {
	$getop1 = "SELECT * FROM ims_itemcodes WHERE cat = 'Layer Breeder Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  order by code";
...
//	echo "sql: " . $sSql . "<br>";
  ...
  if($etype=="Layer Eggs")
    $q = "SELECT sum( quantity ) as quant,itemcode FROM layer_production WHERE flock IN (SELECT DISTINCT (
flockcode) FROM layer_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode
IN (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Eggs') and date1 < '$fdate' group by itemcode";
  
  else if($etype=="Layer Hatch Eggs")
   $q = "SELECT sum( quantity ) as quant,itemcode FROM layerbreeder_production WHERE flock IN (SELECT DISTINCT (
flockcode) FROM layerbreeder_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode
IN (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Hatch Eggs') and date1 < '$fdate' group by itemcode";
  
  else 
  	$q = "SELECT sum( quantity ) as quant,itemcode FROM layerbreeder_production WHERE flock IN (SELECT DISTINCT (
flockcode) FROM layerbreeder_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode
IN (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Breeder Eggs') and date1 < '$fdate' group by itemcode";
 ...
 $qury1 = "select sum(quantity) as quant,code from oc_cobi where code in (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Eggs') and flock = '$flock' and unit = '$warehouse' and date < '$fdate'";
else if($etype=="Layer Hatch Eggs") 
$qury1 = "select sum(quantity) as quant,code from oc_cobi where code in (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Hatch Eggs') and flock = '$flock' and unit = '$warehouse' and date < '$fdate'";
else {
	$qury1 = "select sum(quantity) as quant,code from oc_cobi where code in (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Breeder Eggs') and flock = '$flock' and unit = '$warehouse' and date < '$fdate'";
 ...
 $query1 = "select sum(noofeggs) as eggs from hatchery_traysetting where flock LIKE '%$flock' and unit = '$warehouse' and settingdate < '$fdate'";
   ...
   while($qr2 = mysql_fetch_assoc($resu1))
	...
	if($etype=="Layer Eggs")
	$getop = "SELECT SUM(fromeggs) as quantity FROM ims_eggtransfer WHERE fromcode in (select distinct(code) from ims_itemcodes where cat = 'Layer Eggs') AND date < '$fdate' AND fromwarehouse in (select distinct(flockcode) from layer_flock where flockcode = '$flock' and unitcode = '$warehouse')";
else if($etype=="Layer Hatch Eggs")
	$getop = "SELECT SUM(fromeggs) as quantity FROM ims_eggtransfer WHERE fromcode in (select distinct(code) from ims_itemcodes where cat = 'Layer Hatch Eggs') AND date < '$fdate' AND fromwarehouse in (select distinct(flockcode) from layerbreeder_flock where flockcode = '$flock' and unitcode = '$warehouse')";
  
	else
	$getop = "SELECT SUM(fromeggs) as quantity FROM ims_eggtransfer WHERE fromcode in (select distinct(code) from ims_itemcodes where cat = 'Layer Breeder Eggs') AND date < '$fdate' AND fromwarehouse in (select distinct(flockcode) from layerbreeder_flock where flockcode = '$flock' and unitcode = '$warehouse')";
...
if($etype=="Layer Eggs")
   $getop = "SELECT SUM(toeggs) as quantity FROM ims_eggtransfer WHERE tocode in (select distinct(code) from ims_itemcodes where cat = 'Layer Eggs') AND date < '$fdate' AND towarehouse in (select distinct(flockcode) from layer_flock where flockcode = '$flock' and unitcode = '$warehouse')";
  else if($etype=="Layer Hatch Eggs")
  $getop = "SELECT SUM(toeggs) as quantity FROM ims_eggtransfer WHERE tocode in (select distinct(code) from ims_itemcodes where cat = 'Layer Hatch Eggs') AND date < '$fdate' AND towarehouse in (select distinct(flockcode) from layerbreeder_flock where flockcode = '$flock' and unitcode = '$warehouse')";
else
$getop = "SELECT SUM(toeggs) as quantity FROM ims_eggtransfer WHERE tocode in (select distinct(code) from ims_itemcodes where cat = 'Layer Breeder Eggs') AND date < '$fdate' AND towarehouse in (select distinct(flockcode) from layerbreeder_flock where flockcode = '$flock' and unitcode = '$warehouse')";
...
if($etype=="Layer Eggs")	
$q = "select distinct(code) from ims_itemcodes where cat = 'Layer Eggs' and (code in (select distinct(itemcode) from layer_production) or code in (select distinct(code) from oc_cobi ))  ORDER by code";
else if($etype=="Layer Hatch Eggs")
$q = "select distinct(code) from ims_itemcodes where cat = 'Layer Hatch Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  ORDER by code";
else {
	$q = "select distinct(code) from ims_itemcodes where cat = 'Layer Breeder Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  ORDER by code";
...
while($rows = mysql_fetch_assoc($r))	
 ...
 $obfirst = ($quant +$otrin - $otrout - $quant1 - $eggst[$rows['code']]);
	...
	if($etype=="Layer Eggs")
	  ...
	  $get = "SELECT distinct(date1) FROM layer_production WHERE flock IN (SELECT DISTINCT (
flockcode) FROM layer_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode
IN (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Eggs') AND date1 >= '$fdate' AND date1 <= '$tdate' ORDER BY date1 ASC ";
	else if($etype=="Layer Hatch Eggs")
$get = "SELECT distinct(date1) FROM layerbreeder_production WHERE flock IN (SELECT DISTINCT (
flockcode) FROM layerbreeder_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode
IN (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Hatch Eggs') AND date1 >= '$fdate' AND date1 <= '$tdate' ORDER BY date1 ASC ";
	else
$get = "SELECT distinct(date1) FROM layerbreeder_production WHERE flock IN (SELECT DISTINCT (
flockcode) FROM layerbreeder_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode
IN (SELECT DISTINCT (code) FROM ims_itemcodes WHERE cat = 'Layer Breeder Eggs') AND date1 >= '$fdate' AND date1 <= '$tdate' ORDER BY date1 ASC ";
 ...
 $dt1 = date($datephp,strtotime($row['date1']));
  ?>
   <tr>
    ...
    if($etype=="Layer Eggs")
	    $query0 = "select distinct(code) from ims_itemcodes where cat = 'Layer Eggs' and (code in (select distinct(itemcode) from layer_production) or code in (select distinct(code) from oc_cobi ))  order by code";
	else if($etype=="Layer Hatch Eggs")
	$query0 = "select distinct(code) from ims_itemcodes where cat = 'Layer Hatch Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  order by code";
else
	$query0 = "select distinct(code) from ims_itemcodes where cat = 'Layer Breeder Eggs' and (code in (select distinct(itemcode) from layerbreeder_production) or code in (select distinct(code) from oc_cobi ))  order by code";
...
if($etype=="Layer Eggs")
	...
	{ 
	 ...
	 $trin = 0;
	 if($etype=="Layer Eggs")
	$getop = "SELECT SUM(toeggs) as quantity FROM ims_eggtransfer WHERE tocode = '$code' AND date = '$dt' AND towarehouse in (select distinct(flockcode) from layer_flock where flockcode = '$flock' and unitcode = '$warehouse')";
  else if($etype=="Layer Hatch Eggs")
  $getop = "SELECT SUM(toeggs) as quantity FROM ims_eggtransfer WHERE tocode = '$code' AND date = '$dt' AND towarehouse in (select distinct(flockcode) from layerbreeder_flock where flockcode = '$flock' and unitcode = '$warehouse')";
else
  $getop = "SELECT SUM(toeggs) as quantity FROM ims_eggtransfer WHERE tocode = '$code' AND date = '$dt' AND towarehouse in (select distinct(flockcode) from layerbreeder_flock where flockcode = '$flock' and unitcode = '$warehouse')";
  ...
  if($etype=="Layer Eggs")
	$query3 = "SELECT sum(quantity) as quantity FROM layer_production WHERE flock IN (SELECT DISTINCT (flockcode) FROM layer_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode = '$code' AND date1 = '$dt' ORDER BY date1 ASC ";
	else if($etype=="Layer Hatch Eggs")
	$query3 = "SELECT sum(quantity) as quantity FROM layerbreeder_production WHERE flock IN (SELECT DISTINCT (flockcode) FROM layerbreeder_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode = '$code' AND date1 = '$dt' ORDER BY date1 ASC ";
else
	$query3 = "SELECT sum(quantity) as quantity FROM layerbreeder_production WHERE flock IN (SELECT DISTINCT (flockcode) FROM layerbreeder_flock WHERE flockcode = '$flock' AND unitcode = '$warehouse') AND itemcode = '$code' AND date1 = '$dt' ORDER BY date1 ASC ";
...
//issue
  ...
  $query1 = "select sum(noofeggs) as eggs from hatchery_traysetting where flock LIKE '%$flock' and unit = '$warehouse' and settingdate = '$dt'";
   ...
   $quant1 = 0;
   if($etype=="Layer Eggs")
 $qury1 = "select sum(quantity) as quant from oc_cobi where flock = '$flock' and unit = '$warehouse' and date = '$dt' and code in (select distinct(code) from ims_itemcodes where cat = 'Layer Eggs')";
    else if($etype=="Layer Hatch Eggs")
   $qury1 = "select sum(quantity) as quant from oc_cobi where flock = '$flock' and unit = '$warehouse' and date = '$dt' and code in (select distinct(code) from ims_itemcodes where cat = ' Layer Hatch Eggs')";
else
  $qury1 = "select sum(quantity) as quant from oc_cobi where flock = '$flock' and unit = '$warehouse' and date = '$dt' and code in (select distinct(code) from ims_itemcodes where cat = 'Layer Breeder Eggs')";
...
<tr>
	...
	{ ?>
	<td class="ewRptGrpField1" align="right">
<?php if($obfirst <> 0){ echo ewrpt_ViewValue(changequantity()); } else { echo ewrpt_ViewValue("0"); } ?></td>
   <?php } ?>
...
<?php if($obr[$rows0[code]] <> 0){ echo ewrpt_ViewValue(changequantity($obr[$rows0[code]])); } else { echo ewrpt_ViewValue("0"); } ?></td>
	...
	{
      ?><td class="ewRptGrpField1" align="right">
<?php if($obi[$rows0[code]] <> 0){  ewrpt_ViewValue(changequantity($obi[$rows0[code]])); } else { echo ewrpt_ViewValue("0"); } ?></td>
 <?php } ?>
	...
	while($rows0 = mysql_fetch_assoc($result0))
	{ ?><td class="ewRptGrpField1" align="right"> 
  <?php	 if($obint[$rows0[code]] <> 0){  ewrpt_ViewValue(changequantity($obint[$rows0[code]])); } else { echo ewrpt_ViewValue("0"); } ?></td> 
   <?php } ?>
	</tr>
<?php
?>
...
function reloadpage()
{
	...
	var etype = document.getElementById('etype').value;
		//document.location = "eggstocksmryhatch1.php?fromdate=" + fromdate + "&todate=" + todate + "&warehouse=" + warehouse + "&flock=" + flock;
...
var date2 = new Date(yr2, mon2, dt2); 
	...
	document.location = "eggstocksmryhatch1.php?fromdate=" + fromdate + "&todate=" + todate + "&warehouse=" + warehouse + "&flock=" + flock+ "&etype=" + etype;
	
}
function getetype(a)
{

var t = document.getElementById('etype').value;
