...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="chickensales.php?export=html&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="chickensales.php?export=excel&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="chickensales.php?export=word&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="chickensales.php?cmd=reset&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<?php } ?>
	 ...
	 <?php $i = -1;
	  $query = "select sector from tbl_sector where type1 IN ('Chicken Center','Processing Unit')";
...
$query = " select distinct(code),cat,description from ims_itemcodes where cat in (select distinct(cat) from chicken_category where type = 'Chicken') order by code";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
{
 $itemcode = $rows['code'];
...
function reloadpage()
{
	...
	document.location = "chickensales.php?fromdate=" + fdate + "&todate=" + tdate;
}
</script>