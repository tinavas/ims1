...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="breeder.php?export=html&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="breeder.php?export=excel&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="breeder.php?export=word&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="breeder.php?cmd=reset&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<div class="ewGridMiddlePanel">
	...
	$teggs=0;
	 ...
	 $feedcost=$avg*$feed;
	 $medvac=0;
	  $query8 = "SELECT (itemcode) as item,sum(quantity) as so from breeder_consumption where   date2 >= '$fromdate' and date2 <= '$todate' and itemcode in(select code from ims_itemcodes where cat in ('Medicines','Vaccines')) group by itemcode";
     ...
     {
	 $cm= $row9['cm']; 
	 $medvac=$medvac+($so*round(calculate($cm,$item,$fromdate),2));
	 }
     }
...
<tr>
			<td  align="bottom"> <b>Feed Consumed Excluding (Chick A Grower)</b></td>
...
function reloadpage()
{
	...
	document.location = "breeder.php?fromdate=" + fdate + "&todate=" + tdate;
}
</script>