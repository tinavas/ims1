...
$discostcenter = str_replace("','",", ",substr($costcenter,1,-1));

$url = "&fromdate=" . $_GET['fromdate'] . "&todate=" . $_GET['todate']. "&costcenter=" . $costcenter. "&s=". $_GET["s"];
...
<tr>
 ...
 <td align="center"><strong><font color="#3e3276">From Date </font></strong><?php echo $fdatedump; ?>&nbsp;&nbsp;<strong><font color="#3e3276">To Date </font></strong><?php echo $tdatedump; ?></td>

</tr>
<tr>
 <td colspan="2" align="center"><strong><font color="#3e3276">Cost Center(s) : </font><?php echo $discostcenter; ?></strong></td>
</tr>
</table>
...
&nbsp;&nbsp;<a href="profitloss.php?export=html<?php echo $url; ?>">Printer Friendly</a>

&nbsp;&nbsp;<a href="profitloss.php?export=excel<?php echo $url; ?>">Export to Excel</a>

&nbsp;&nbsp;<a href="profitloss.php?export=word<?php echo $url; ?>">Export to Word</a>
...
<form action="profitloss.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<?php if ($Pager->RecordCount > 0) { ?>
	...
	<td><a href="profitloss.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	...
	<td><a href="profitloss.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	...
	<td><a href="profitloss.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	...
	<td><a href="profitloss.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
...
$rightcount = 0;
	...
	if($_GET['s']==1)
	{
	$query = "SELECT sum(amount) as cramount,coacode FROM ac_financialpostings WHERE crdr = 'Cr' AND date BETWEEN '$fdate' AND '$tdate'  GROUP BY coacode";

	}
	else
	{
		$query = "SELECT sum(amount) as cramount,coacode FROM ac_financialpostings WHERE crdr = 'Cr' AND date BETWEEN '$fdate' AND '$tdate' AND warehouse IN ($costcenter) GROUP BY coacode";
...
if($_GET['s']==1)
	{
		$query = "SELECT sum(amount) as dramount,coacode FROM ac_financialpostings WHERE crdr = 'Dr' AND date BETWEEN '$fdate' AND 	'$tdate'  GROUP BY coacode";

	}
	else
	{
	$query = "SELECT sum(amount) as dramount,coacode FROM ac_financialpostings WHERE crdr = 'Dr' AND date BETWEEN '$fdate' AND 	'$tdate' AND warehouse IN ($costcenter) GROUP BY coacode";
	}
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
 $drarray[$rows['coacode']] = $rows['dramount'];	
  ...
  $query1 = "select * from ac_coa where schedule in (select schedule from ac_schedule where (type = 'Expense' or type = 'Revenue') and ptype = 'Direct') order by type,code   ";
	  ...
	  /* $q = "select sum(amount) as cramount  from ac_financialpostings where crdr = 'Cr' and date >='$fdate' and date <= '$tdate' and coacode = '$code' ";
...

	   {
	    ...
	    $q = "select sum(amount) as dramount  from ac_financialpostings where crdr = 'Dr' and date >='$fdate' and date <= '$tdate'  and coacode = '$code' ";
...
*/
	...
	$cramount = $crarray[$code];
	  ...
	  if ($row2['type'] == "Expense")  {  $nexpsum = $nexpsum + $mbal; }   

	  if ($row2['type'] == "Revenue")  {  $nrevsum = $nrevsum + $mbal; } 
...

	  {
	    ...
	    $leftarray1[$leftcount] = $row2['code'];

		$leftarray2[$leftcount] = $row2['description'];
...

	  {
	     ...
	     $rightarray1[$rightcount] = $row2['code'];

		$rightarray2[$rightcount] = $row2['description'];
...

	{
		...
		$rightarray3[$rightcount] = $nexpsum - $nrevsum;
		
		$rightarray4[$rightcount] =($rightarray3[$rightcount]/$nexpsum)*100;
		
		$rightarray2[$rightcount] = "Gross Loss(".round($rightarray4[$rightcount],2)."%)";

		$rightcount = $rightcount + 1;

		$expsum = $expsum + ($nexpsum - $nrevsum);

		$leftarray1[$leftcount] = "";


		$leftarray3[$leftcount] = $nexpsum - $nrevsum;
		
		$leftarray4[$leftcount] =($leftarray3[$leftcount]/$nexpsum)*100;
		
		$leftarray2[$leftcount] = "Gross Loss C/D";
...

	{
	   ...
	   $leftarray1[$leftcount] = "";
		...
		$leftarray3[$leftcount] = $nrevsum - $nexpsum;
		
		$leftarray4[$leftcount] =($leftarray3[$leftcount]/$nrevsum)*100;
		
		$leftarray2[$leftcount] = "Gross Profit(".round($leftarray4[$leftcount],2)."%)";

		$leftcount = $leftcount + 1;

		$revsum = $revsum + ($nrevsum - $nexpsum);

		 $rightarray1[$rightcount] = "";

		

		$rightarray3[$rightcount] = $nrevsum - $nexpsum;
		
		$rightarray4[$rightcount] =($rightarray3[$rightcount]/$nrevsum)*100;
		
		$rightarray2[$rightcount] = "Gross Profit C/D";
  ...
  $rcount = 0;
    ...
    $query1 = "select * from ac_coa where (type = 'Expense' or type = 'Revenue') and schedule in (select schedule from ac_schedule where (type = 'Expense' or type = 'Revenue') and ptype <> 'Direct' ) order by description  ";
...
<?php if($leftarray1[$dumm] <> "") { ?><a href="dummy.php?fromdate=<?php echo $fdatedump; ?>&todate=<?php echo $tdatedump; ?>&code=<?php echo $leftarray1[$dumm]; ?>&wh=<?php echo $costcenter; ?>&flag=1" target="_blank" title="View Ledger Details" style="color:0000FF;"><?php echo ewrpt_ViewValue($leftarray1[$dumm]); ?></a><?php } else { ?>&nbsp;<?php } ?>
		</td>
		...
		<?php if( $leftarray3[$dumm] == "" ) { echo ewrpt_ViewValue("0.00");  } else { echo ewrpt_ViewValue(changeprice($leftarray3[$dumm])); $abal = $abal + $leftarray3[$dumm]; } ?>

		</td>

		<td>

<?php if($rightarray1[$dumm] <> "") { ?><a href="dummy.php?fromdate=<?php echo $fdatedump; ?>&todate=<?php echo $tdatedump; ?>&code=<?php echo $rightarray1[$dumm]; ?>&wh=<?php echo $costcenter; ?>&flag=1" target="_blank" title="View Ledger Details" style="color:0000FF;"><?php echo ewrpt_ViewValue($rightarray1[$dumm]); ?></a><?php } else { ?>&nbsp;<?php } ?>
        </td>
		...
		<td>
		 ...
		 <?php if ( $rightarray3[$dumm] == "" ) { echo ewrpt_ViewValue("0.00");  } else { echo ewrpt_ViewValue(changeprice($rightarray3[$dumm])); $lbal = $lbal + $rightarray3[$dumm]; } ?>
...
<?php if($rightarray1[$dumm] <> "") { ?><a href="dummy.php?fromdate=<?php echo $fdatedump; ?>&todate=<?php echo $tdatedump; ?>&code=<?php echo $rightarray1[$dumm]; ?>&wh=<?php echo $costcenter; ?>&flag=1" target="_blank" title="View Ledger Details" style="color:0000FF;"><?php echo ewrpt_ViewValue($rightarray1[$dumm]); ?></a><?php } else { ?>&nbsp;<?php } ?>
	...
	<tr>
		...
		<?php if ( $finflag == "R") { echo ewrpt_ViewValue("Net Profit(".round((($revsum-$expsum)/$revsum )* 100,2)."%)"); } else { echo ewrpt_ViewValue("&nbsp;"); } ?>

		</td>

		<td class="ewRptGrpField3" align="right">

		<?php if ( $finflag == "R") { echo ewrpt_ViewValue(changeprice($revsum-$expsum)); } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
...
<?php if ( $finflag == "E") { echo ewrpt_ViewValue("Net Loss(".round((($expsum-$revsum)/$expsum )* 100,2)."%)"); } else { echo ewrpt_ViewValue("&nbsp;"); } ?>

</td>

<td align="right">

<?php if ( $finflag == "E") { echo ewrpt_ViewValue(changeprice($expsum-$revsum)); } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
