...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="quailcatproduction.php?export=html&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>&cat=<?php echo $cat; ?>&unit=<?php echo $unit; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="quailcatproduction.php?export=excel&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>&cat=<?php echo $cat; ?>&unit=<?php echo $unit; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="quailcatproduction.php?export=word&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>&cat=<?php echo $cat; ?>&unit=<?php echo $unit; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="quailcatproduction.php?cmd=reset&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>&cat=<?php echo $cat; ?>&unit=<?php echo $unit; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
$query = " select distinct(date) from quail_processing where processingunit = '$unit' and code in (select distinct(code) from ims_itemcodes where cat = '$cat') and date between '$fromdate' and '$todate' order by code";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
{
 $date = $rows['date'];
...
<?php
 ...
 {
 $query2 = "select sum(amount) as quantity from quail_processing where processingunit = '$unit' and code = '$array[$i]' and date = '$date'";
...
document.location = "quailcatproduction.php?fromdate=" + fromdate + "&todate=" + todate + "&cat=" + cat + "&unit=" + unit1; 
}
</script>