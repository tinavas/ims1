...
if($unit == 'All')
{
$cond = "in (select distinct(farmcode) from layer_farm where client = '$client')";
}
else
{
$cond = "=".""."'".$unit."'";
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Layer Future Production Analysis<br/> For Weeks : <?php echo $weeks;?></font></strong></td>
</tr>
</table>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="expenseledger.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="expenseledger.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="expenseledger.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php } else { ?>
		<td class="ewTableHeader" >
			<table cellspacing="0" class="ewTableHeaderBtn" ><tr>
			<td align="center" >Feed Consumption</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php
	...
	$cst = 0;
	$female = 0;
$q1 = "select * from layer_flock where cullflag = '0' and farmcode $cond order by shedcode";
	$r1 = mysql_query($q1,$conn1) or die(mysql_error());
	while($qr1 = mysql_fetch_assoc($r1))
	{
$flock = $qr1['flockcode'];
$shed = $qr1['shedcode'];
$q2 = "select * from layer_flock where flockcode = '$flock' and client = '$client' and shedcode = '$shed' order by shedcode";
...
$fage = 0;
$q2 = "select max(age) as age from layer_consumption where flock = '$flock'";
...
$q3 = "select sum(heperweek) as hecum from layer_pstandards where (age >= '$fage' && age <= '$tage') and client = '$client' order by age";
...
$cffeed = 0;
$q9 = "select * from layer_pstandards where age >= $fage and age <= $tage and client = '$client'";
$r9 = mysql_query($q9,$conn1) or die(mysql_error());
while($qr9 = mysql_fetch_assoc($r9))
{
$cffeed = $cffeed + $qr9['fm'];
}
//echo $tage."/".$fage; echo "<br/>";
//$hhpdif =  $cumhhp[$tage] - $cumhhp[$fage];

 $minus = 0; 
 $q4 = "select distinct(date2),fmort,fcull from layer_consumption where flock = '$flock' and client = '$client'";
...
//Inward
 $q5 = "select sum(quantity) as trinbirds from ims_stocktransfer where towarehouse = '$flock' and cat = 'Layer Birds' and client = '$client'";
 ...
 //Outward
		$q6= "select sum(quantity) as troutbirds from ims_stocktransfer where fromwarehouse = '$flock' and cat = 'Layer Birds' and client = '$client'";
...
//Sale
		 $q7 = "select sum(quantity) as soldbirds from oc_cobi where flock = '$flock' and code in (select code from ims_itemcodes where cat = 'Layer Birds') and client = '$client'";
		...
		//Purchase
		 $q8 = "select sum(receivedquantity) as purchasebirds from pp_sobi where flock = '$flock' and client = '$client'";
        $r8 = mysql_query($q8,$conn1);
	    while($qr8 = mysql_fetch_assoc($r8))
        {
	      $prchbirds = $qr8['purchasebirds'];
		}
//echo $female."/".$minus."/".$transferin."/".$transferout."/".$soldbirds."/".$prchbirds; echo "<br/>";
$birds = $female -  $minus + $transferin - $transferout - $soldbirds + $prchbirds;
$feedcons = round(((($cffeed * $birds)/1000)*7),2);
//echo $birds."/".$hhpdif;
$totaleggs = round(($birds * $hhpdif),0);

$q10 = "select * from breeder_feedcodestandards where code in(select code from ims_itemcodes where cat = 'Layer Feed') and ((fromage <= $fage and toage >= $fage) or (fromage <= $tage and toage >= $tage) or (fromage >= $fage and toage <= $tage)) order by fromage";
...
$toage = $qr10['toage'];
$feedconsnew = 0;
$q11 = "select sum(fm) as ffeed from layer_pstandards where (age between $fromage and $toage) and (age >= $fage and age <= $tage)";
$r11 = mysql_query($q11,$conn1) or die(mysql_error());
while($qr11 = mysql_fetch_assoc($r11))
{
$feedconsnew =  round(((($qr11['ffeed']*$birds)/1000) * 7 ),2);;
}
...
$newcull = $newcull;
	...
	<td class="ewRptGrpField2"><?php if(($flock <> "") && ($cull > $age2)) { ?><a href="layerindiflock.php?flock=<?php echo $flock;?>&weeks=<?php echo $newcull; ?>"><?php echo ewrpt_ViewValue($flock); ?></a><?php } else { ?><?php echo ewrpt_ViewValue($flock); ?><?php } ?></td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php  echo ewrpt_ViewValue(changequantity($birds)); $totbirds = $totbirds + $birds;?>
</td>


<td<?php echo $sItemRowClass; ?> align="right">
<?php  echo ewrpt_ViewValue(changequantity($totaleggs)); $tottotaleggs = $tottotaleggs + $totaleggs;?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changeprice($feedcons)); $totfeedcons = $totfeedcons + $feedcons?>
</td>


<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changeprice($productioncost)); $totpc = $totpc + $productioncost;?>
</td> 
...
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($tottotaleggs);?></b></td>
...
<tr>
<?php 
$s1 = "select * from layer_shed where shedcode in('$upshed') GROUP BY shedcode";
...
//$u[$shed]  = (Sustaining Capacity Of Shed)
//fcap = (Shed Capacity)
$fdif = round(($fcap - $u[$shed]),0); 
$cffeed = 0;
$totaleggs = 0;
$q3 = "select * from layer_pstandards where age <= $newage and client = '$client' order by age";
$r3 = mysql_query($q3,$conn1) or die(mysql_error());
while($qr3 = mysql_fetch_assoc($r3))
{
$layerage = $qr3['age'];
$cumhhp[$layerage] = $qr3['heperweek'];
$cffeed = $cffeed + $qr3['fm'];


$hhpdif =  $cumhhp[$layerage]; 
$feedcons = round(((($cffeed * $fdif)/1000)*7),2);
$totaleggs = $totaleggs + round(($fdif * $hhpdif),0);
}
$a = 0;
$b = 0;
$mul = 0;
$productioncost = 0;
$q10 = "select * from breeder_feedcodestandards where ((fromage <= '0' and toage >= $newage) or (fromage <= $newage and toage >= $newage) or (fromage >= '0' and toage <= $newage)) and code in(select code from ims_itemcodes where cat = 'Layer Feed') order by fromage";
$r10 = mysql_query($q10,$conn1) or die(mysql_error());
$n = mysql_num_rows($r10);
while($qr10 = mysql_fetch_assoc($r10))
{
...
$toage = $qr10['toage'];
$feedconsnew = 0;
$q11 = "select sum(fm) as ffeed from layer_pstandards where (age between $fromage and $toage) and (age >= 0 and age <= $newage)";
$r11 = mysql_query($q11,$conn1) or die(mysql_error());
while($qr11 = mysql_fetch_assoc($r11))
{
//echo $qr11['ffeed']."/".$fdif; echo "<br/>";
$feedconsnew =  round(((($qr11['ffeed']*$fdif)/1000) * 7 ),2);;
}
$query3 = "select * from feed_cost where code = '$code' and client = '$client'";
$result3 = mysql_query($query3,$conn1) or die(mysql_error());
while($qr3 = mysql_fetch_assoc($result3))
...
//echo $feedconsnew."/".$cst; echo "<br/>";
$a = $feedconsnew * $cst;
$productioncost = $productioncost + $a;
}
...
<td class="ewRptGrpField2"><?php if($shed <> "") { ?><a href="layerindiflockshed.php?shed=<?php echo $shed;?>&weeks=<?php echo $newage; ?>&female=<?php echo $fdif; ?>"><?php echo ewrpt_ViewValue($shed); ?></a><?php } else { ?>&nbsp;<?php } ?></td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($newage) ?>
</td>
...
function reloadpage(form)
{
	...
	document.location = "layerfutureplan.php?weeks=" + weeks + "&unit=" +  uni + "&cull=" + cull;
