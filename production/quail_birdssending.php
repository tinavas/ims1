...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Quail Birds Sending Report</font></strong></td>
</tr>
<tr>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="quail_birdssending.php?export=html&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="quail_birdssending.php?export=excel&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="quail_birdssending.php?export=word&fromdate=<?php echo date("d.m.Y",strtotime($fromdate)); ?>&todate=<?php echo date("d.m.Y",strtotime($todate)); ?>">Export to Word</a>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Processing Unit</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
include "config.php";
   ...
   ?>
	...
	<?php
	 ...
	 $query = "select * from quail_breeder_sender where date >= '$fromdate' and date <= '$todate'";
	$result = mysql_query($query,$conn1) or die(mysql_error());
	while($rows = mysql_fetch_assoc($result))
	{
	$date = date($datephp,strtotime($rows['date']));
...
<td<?php echo $sItemRowClass; ?>>
<?php if(($rows['farm'] <> $oldcode2) or ($dumm2 == 0)){ echo ewrpt_ViewValue($rows['farm']);
     $oldcode2 = $rows['farm']; $dumm2 = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
	 } ?>
</td>   
...
<td<?php echo $sItemRowClass; ?>>
<?php if(($rows['unit'] <> $oldcode3) or ($dumm3 == 0)){ echo ewrpt_ViewValue($rows['unit']);
     $oldcode3 = $rows['unit']; $dumm3 = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
	 } ?>
</td>   
...
<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($rows['processingunit']); ?></td>
...
function reloadpage()
{
	...
	document.location = "quail_birdssending.php?category="+category+"&fromdate=" + fromdate + "&todate=" + todate;
}
</script>-->
<script type="text/javascript">
...
document.location = 'quail_birdssending.php?export=' + a + '&fromdate=' + fdate + '&todate=' + tdate;
}
</script>