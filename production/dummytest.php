...
<?php if ( $oflag == "Cr" ) { echo ewrpt_ViewValue(changeprice($obal)); $crtotal = $crtotal + $obal; } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
	...
	$nxtdate = "";
	 $dumfrmdate = ""; 
	 include "config.php";
      $query1 = "SELECT * from ac_financialpostings where coacode = '$coa'  and date >= '$fdate' and date <= '$tdate' $cond order by date,trnum ";
		...
		else if ( $row2['type'] == "RV")
		{
		  ...
		  $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'R' and code = '$coa'  ";
	       ...
	       while($row21 = mysql_fetch_assoc($result11))
           {
			...
			if($cash == "YES") {
		    $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'R' and code <> '$coa' "; if($row2[trnum]=='264') echo $query11; }
			else
			{
			$query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'R' and code = '$coa' ";
			if($row2[trnum]=='264') echo $query11; }
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['rremarks'];
			 $desc = $row21['description'];
			if($row2[trnum]=='264')  echo $row21['cramount'];
			 ?>
			 	<tr>
		<td class="ewRptGrpField1">
...
<td align="right">
<?php if ( $row21['crdr'] == "Dr" ) { echo ewrpt_ViewValue(changeprice($row2['amount'])); $drtotal = $drtotal + $row2['amount']; }
      else { echo ewrpt_ViewValue("&nbsp;"); } ?>
</td>
<td align="right">
<?php if ( $row21['crdr'] == "Cr" ) { echo ewrpt_ViewValue(changeprice($row2['amount'])); $crtotal = $crtotal + $row2['amount']; } 
      else { echo ewrpt_ViewValue("&nbsp;"); }?>
</td>
	</tr>
<?php
