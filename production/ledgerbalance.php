...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="ledgerbalance.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="ledgerbalance.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="ledgerbalance.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
$obal = 0;
include "config.php";
  $q = "select sum(cramount) as crtot,sum(dramount) as drtot from ac_gl where code = '$coa' and vstatus = 'A' and date< '$fdate' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  
	   $drtotal1 = $drtotal + $qr['drtot'];
	   $crtotal1 = $crtotal + $qr['crtot'];
	   
	   $dtot = $drtotal1 - $crtotal1;
	   if ( $dtot < 0)
	   {
	   $ctot = $crtotal1 - $drtotal1;
	   }
...
?>
	<tr>
		...
		<td class="ewRptGrpField3">
		<?php echo ewrpt_ViewValue($row2['type']) ?>
		</td>
		<td>
...
<?php
    ...
    $query11 = "SELECT min(date) as mindate from ac_financialpostings where coacode = '$coa'  and date >= '$row2[date]' and date <= '$tdate' ";
      $result11 = mysql_query($query11,$conn1);
	  
      while($row21 = mysql_fetch_assoc($result11))
      {
	     $nxtdate = $row21['mindate'];
	  }
	  if ( $nxtdate <> "" )
	  {
	     $query11 = "select * from ac_gl where code = '$coa' and date>='$row2[date]' and vstatus = 'A' and date< '$nxtdate' order by date,transactioncode  ";
         $result11 = mysql_query($query11,$conn1);
		 ...
		 ?>
			...
			<tr>
		<td class="ewRptGrpField1">
		<?php echo ewrpt_ViewValue(date("j-m-Y",strtotime($row21['date']))) ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($row21['transactioncode']) ?>
		</td>
		<td class="ewRptGrpField3">
		<?php echo ewrpt_ViewValue($type) ?>
		</td>
		<td>
<b><?php echo ewrpt_ViewValue($row21['rremarks']) ?></b>
</td>
		<td>
<?php echo ewrpt_ViewValue($row21['chequeno']) ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($row21['dramount'])); $drtotal = $drtotal + $row21['dramount'];  ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($row21['cramount'])); $crtotal = $crtotal + $row21['cramount']; ?>
</td>
	</tr>
...
?>
<?php 
   $query11 = "select * from ac_gl where code = '$coa' and date>='$dumfrmdate' and vstatus = 'A' and date< '$fdate' order by date,transactioncode  ";
         $result11 = mysql_query($query11,$conn1);
		 ...
		 ?>
			...
			<tr>
		<td class="ewRptGrpField1">
		<?php echo ewrpt_ViewValue(date("d-m-Y",strtotime($row21['date']))) ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($row21['transactioncode']) ?>
		</td>
...
<?php  echo ewrpt_ViewValue(changeprice($row21['cramount'])); $crtotal = $crtotal + $row21['cramount']; ?>
	   ...
	   $query11 = "select * from ac_crdrnote where coa = '$coa' and date >='$dumfrmdate' and flag = 'A' and date < '$fdate' order by date  ";
         $result11 = mysql_query($query11,$conn1);
		...
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($row21['crnum']) ?>
		</td>
		<td class="ewRptGrpField3">
		<?php echo ewrpt_ViewValue($type) ?>
		</td>
		<td>
<b><?php echo ewrpt_ViewValue($row21['remarks']) ?></b>
</td>
		<td>
<?php echo ewrpt_ViewValue() ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($row21['amount'])); $drtotal = $drtotal + $row21['amount'];  ?>
</td>
<td align="right">
<?php  echo ewrpt_ViewValue(changeprice($row21['cramount'])); $crtotal = $crtotal + $row21['cramount']; ?>
</td>
