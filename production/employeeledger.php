...
if ( $_GET['code'] == "All" )  { header('Location:customerledgerall.php?date2='.$_POST['date2'].'&date3='.$_POST['date3']); }
...
$url = "&accounts=" .$accounts."&vendor=" . $ven."&fromdate=" . $fdate . "&todate=" . $tdate;
...
<td colspan="2" align="center"><strong><font color="#3e3276">Employee Ledger</font></strong></td>

</tr>

<tr>

<td colspan="2" align="center"><strong><font color="#3e3276">Employee Name&nbsp;<?php echo $ven; ?></font></strong></td>
...
<td colspan="2" align="center"><strong><font color="#3e3276">Account Name:&nbsp;<?php echo $accounts; ?></font></strong></td>
<?php } ?>
...
<td colspan="2" align="center"><strong><font color="#3e3276">Customer Code&nbsp;<?php echo $vendorcode; ?></font></strong></td>
...
&nbsp;&nbsp;<a href="employeeledger.php?export=html<?php echo $url; ?>">Printer Friendly</a>

&nbsp;&nbsp;<a href="employeeledger.php?export=excel<?php echo $url; ?>">Export to Excel</a>

&nbsp;&nbsp;<a href="employeeledger.php?export=word<?php echo $url; ?>">Export to Word</a>
...
<select name="accounts" id="accounts" style="width:150px"; onchange="reloadpage();" >
<option value="">-Select-</option>
<?php
			 $query = "SELECT salaryac,loanac FROM hr_employee WHERE name = '$ven'";
			...
			{
			 ?>
			 <option value="<?php echo $rows['code']; ?>" title="<?php echo $rows['description']; ?>"<?php if($rows['code'] == $accounts) { ?> selected="selected" <?php } ?>><?php echo $rows['code']; ?></option>
			 <?php
			}
			?>
...
include "config.php";
 ...
 $q = "select sum(amount) as dramt from ac_financialpostings where coacode = '$accounts'  and client = '$client' and crdr = 'Dr' and date <'$fdate' and venname = '$ven' ";
	...
	$q = "select sum(amount) as cramt from ac_financialpostings where coacode = '$accounts' and client = '$client' and crdr = 'Cr' and date <'$fdate' and venname = '$ven' ";
...
<td class="ewRptGrpField2">
	...
	<?php
	 ...
	 $q = "select * from ac_financialpostings where date >= '$fdate' and date <= '$tdate'  and venname = '$ven' and coacode = '$accounts' order by date ";
	...
	$price='';
...

	  {
	    ...
	    $mode = "Employee Credit Note";

	    $q1 = "SELECT * FROM hr_empcrdrnote where tid = '$qr[trnum]' and mode = 'ECN'  ";
...

	  {
	    ...
	    $mode = "Employee Debit Note";

	    $q1 = "SELECT * FROM hr_empcrdrnote where tid = '$qr[trnum]' and mode = 'EDN'  ";
...

	  {
	    ...
	    $mode = "Employee Payment Voucher";

 $q1 = "SELECT * FROM ac_gl where transactioncode = '$qr[trnum]' and code = '$accounts'  ";
...
<?php if ($crtotal>$drtotal){ echo ewrpt_ViewValue(changeprice($crtotal-$drtotal)."(Cr)");  } 

      else { echo ewrpt_ViewValue(changeprice($drtotal-$crtotal)."(Dr)"); }?>
...

{
	...
	var vendor="<?php echo $ven?>";
	
	var fdate = "<?php echo $fdate?>";
	var tdate = "<?php echo $tdate?>";
	document.location = "employeeledger.php?accounts=" + accounts+"&vendor=" + vendor+"&fromdate=" + fdate+"&todate=" + tdate;
