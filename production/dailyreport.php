...
function reload(a)
{
document.location="dailyreport.php?flock=<?php echo $_GET['flock']; ?>&date=" + a; 
}
</script>

<span class="phpreportmaker">
Date&nbsp;
<input type="text" class="datepicker" id="date1" name="date1" value="<?php if($_GET['date']) { echo ''; } else { echo $dummydate; }  ?>" onchange="reload(this.value);" />
&nbsp;&nbsp;&nbsp;
</span>
</td>
...
<thead>
	<th class="ewTableHeader">Details</th>
         <?php 
            ...
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
              $q = "select distinct(flock) from breeder_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from breeder_flock ) order by flock"; 
            else
...
<tr>
	<td<?php echo $sItemRowClass1; ?>>Age</td>
         <?php 
			...
			$f = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q = "select age from breeder_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from breeder_flock )group by flock order by flock "; 
            else
            ...
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
              /* $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from breeder_consumption where itemcode in ($feedtype) and date2 = '$comparedate'  and flock in (select distinct(flockcode) from breeder_flock where cullflag = 0) group by flock order by flock"; 
               ...
               $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from breeder_consumption where itemcode in ($feedtype) and date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; */
			    $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from breeder_consumption where date2 = '$comparedate'  and flock in (select distinct(flockcode) from breeder_flock ) group by flock order by flock"; 
            else
		 ...
		 while($qrj = mysql_fetch_assoc($qrsj))
		   ...
		   while($qr = mysql_fetch_assoc($r))
             ...
             else
             {
			  ...
			  $q = "SELECT sum(shortagefemale) - sum(excessfemale) AS female FROM breeder_birdsadjustment WHERE flock = '$flockkk' AND date < '$comparedate'";
			 $r = mysql_query($q,$conn1) or die(mysql_error());
			 ...
			 $remaining = $fopening - $minus - $ftransfer + $ttransfer + $fquantfc+$fadjust ;
			 $ftot = $ftot + $remaining;
...
$q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$flockkk' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Male Birds')";  
		...
		//echo round($femalef,0)."/";
		 ...
		 $dummf = $dummf + 1;
		 echo round($femalef,2)."/".round($malef,2); 
		?> 
		 </td>
         <?php 
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Female Mortality" >F.Mortality</td>
         <?php 
			...
			$finalmort = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select fmort from breeder_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from breeder_flock ) group by flock order by flock"; 
            else
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Male Mortality" >M.Mortality</td>
         <?php 
			...
			$finalmort = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select mmort from breeder_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from breeder_flock ) group by flock order by flock"; 
            else
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Female Culls" >F.Culls</td>
         <?php 
			...
			$finalcull = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select fcull from breeder_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from breeder_flock ) group by flock order by flock"; 
            else
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title = "Male Culls" >M.Culls</td>
         <?php 
			...
			$finalcull = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select mcull from breeder_consumption where date2 = '$comparedate' and flock in (select distinct(flockcode) from breeder_flock ) group by flock order by flock"; 
            else
...
$lessdate = strtotime($comparedate);
$lessdate = $lessdate-(15*24*60*60);
$lessdate = date('y-m-d',$lessdate);
...
{
$ic = $qr1aq['itemcode'];
  $qry1 = "select sum(production) as production,sum(materialcost) as mc from feed_productionunit where mash = '$ic' and date = '$comparedate'";
$res1 = mysql_query($qry1,$conn1) or die(mysql_error());

$qr12 = mysql_fetch_assoc($res1);
if($qr12['production'])
{
 $p = $p + $qr12['production'];
$mc = $mc + $qr12['mc'];
...
{
  $qry1 = "select sum(production) as production,sum(materialcost) as mc from feed_productionunit where mash = '$ic' and date <= '$comparedate' and date >= '$lessdate'";
$res1 = mysql_query($qry1,$conn1) or die(mysql_error());
$qr12 = mysql_fetch_assoc($res1);
$p = $p + $qr12['production'];
$mc = $mc + $qr12['mc'];
 ...
 $c = ($mc)/($p);
//echo "<br>Feed $lastfinalfeed<br>";
//echo "Egg $fegg";
 $cr = round((($lastfinalfeed) / $fegg) ,3) * 1000;
$cost = $c * (($cr)/1000);
?>
