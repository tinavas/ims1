...
// Table level SQL
$EW_REPORT_TABLE_SQL_FROM = "turkey_consumption";
$EW_REPORT_TABLE_SQL_SELECT = "SELECT turkey_consumption.id FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_WHERE = "turkey_consumption.id = 1";
...
<?php } ?>
<br />
<center><strong>Turkey Daily Consumption & Production Report on <?php echo $_GET['date']; ?></strong></center>
<br />
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="dailyreport.php?export=html&date=<?php echo $_GET['date']; ?>&flock=<?php echo $_GET['flock']; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="dailyreport.php?export=excel&date=<?php echo $_GET['date']; ?>&flock=<?php echo $_GET['flock']; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="dailyreport.php?export=word&date=<?php echo $_GET['date']; ?>&flock=<?php echo $_GET['flock']; ?>">Export to Word</a>
<?php } else { ?>
<center>
<?php } ?>
<?php if (@$sExport == "") { ?>
...
else
{
  ...
  if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
      $q = "select max(date2) as 'date2' from turkey_consumption group by flock order by date2 ASC"; 
  else
      $q = "select max(date2) as 'date2' from turkey_consumption where flock in ($_GET[flock]) group by flock order by date2 ASC"; 
...
function reload(a)
{
document.location="turkeydailyreport.php?flock=<?php echo $_GET['flock']; ?>&date=" + a; 
}
</script>
...
<!--next page button-->
	...
	<td><a href="turkeydailyreport.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="turkeydailyreport.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
<thead>
	<th class="ewTableHeader">Details</th>
         <?php 
            ...
            include "config.php";
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
              $q = "select distinct(flock) from turkey_consumption where date2 = '$comparedate' order by flock"; 
            else
              $q = "select distinct(flock) from turkey_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) order by flock"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
              $flo[$f] = $qr['flock'];
...
<tr>
	<td<?php echo $sItemRowClass1; ?>>Age</td>
         <?php 
			...
			$f = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q = "select age from turkey_consumption where date2 = '$comparedate' group by flock order by flock "; 
            else
               $q = "select age from turkey_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock "; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
              $age = $qr['age'];
...
<tr>
	<td<?php echo $sItemRowClass; ?>>Feed Consumed(Kgs) &nbsp;&nbsp;</td>
         <?php 
			...
			$ftot = 0;
			$dummf = 0;
            $q = "select * from ims_itemcodes where cat = 'Turkey Female Feed'"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
              $feedtype = $feedtype . ",'" . $qr['code'] . "'";
            } 
            $q = "select * from ims_itemcodes where cat = 'Turkey Male Feed'"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
            ...
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from turkey_consumption where itemcode in ($feedtype) and date2 = '$comparedate' group by flock order by flock"; 
            else
               $q = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1',flock from turkey_consumption where itemcode in ($feedtype) and date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
		 $flockkk = $qr['flock'];
		 if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q11 = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1' from turkey_consumption where itemcode in (select distinct(code) from ims_itemcodes where cat = 'Turkey Male Feed') and date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
            else
               $q11 = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1' from turkey_consumption where itemcode in (select distinct(code) from ims_itemcodes where cat = 'Turkey Male Feed') and date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
		...
		if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
               $q11 = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1' from turkey_consumption where itemcode in (select distinct(code) from ims_itemcodes where cat = 'Turkey Female Feed') and date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
            else
               $q11 = "select sum(quantity) as 'quantity',sum(quantity1) as 'quantity1' from turkey_consumption where itemcode in (select distinct(code) from ims_itemcodes where cat = 'Turkey Female Feed') and date2 = '$comparedate' and flock = '$qr[flock]' order by flock"; 
  		$qrs11 = mysql_query($q11,$conn1) or die(mysql_error());
		while($qr11 = mysql_fetch_assoc($qrs11))
		...
		//Birds Calculation
		$qj = "select * from turkey_flock where flockcode = '$flockkk'"; 
		 ...
		 while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $qj = "select distinct(date2),fmort,fcull,mmort,mcull from turkey_consumption where flock = '$flockkk' and date2 < '$comparedate' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		 while($qrj = mysql_fetch_assoc($qrsj))
                $minus = $minus + $qrj['fmort'] + $qrj['fcull'];

            $ftransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Turkey Female Birds' and fromwarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		   ...
		   while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $ttransfer = 0;
            $qj = "select * from ims_stocktransfer where cat = 'Turkey Female Birds' and towarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $remaining = 0;
			 //echo $flockkk."/".$fopening."/".$minus."/".$ftransfer."/".$ttransfer; echo"/B";
             $remaining = $fopening - $minus - $ftransfer + $ttransfer;
			 $ftot = $ftot + $remaining;
		...
		$qj = "select * from turkey_flock where flockcode = '$flockkk'"; 
		 ...
		 while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $qj = "select distinct(date2),mmort,mcull from turkey_consumption where flock = '$flockkk' and date2 < '$comparedate' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		 while($qrj = mysql_fetch_assoc($qrsj))
                $minus = $minus + $qrj['mmort'] + $qrj['mcull'];

             $ftransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Turkey Male Birds' and fromwarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $ttransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Turkey Male Birds' and towarehouse = '$flockkk' and date < '$comparedate'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Feed Consumed/Bird">F.Cons/Bird(gms.)</td>
         ...
         <?php }  ?>   
	<td<?php echo $sItemRowClass2; ?>><?php /*echo $finalfeed = $finalfeed / 2; */ echo round((($finalfeed * 1000)/(2 * $ftot)),2)."/".round((($finalfeed1 * 1000)/(2 * $mtot)),2); ?></td>
</tr>
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Standard Feed Consumed">Std. F.Cons(gms.)</td>
         <?php 
			...
			$fstd = "0";
			$mstd = "0";
			$q11 = "select * from breed_standards where age = '$age1[$f]'";
			$qrs11 = mysql_query($q11,$conn1) or die(mysql_error());
...
<?php
            $eggtype = "'dummy'";
            $q = "select * from ims_itemcodes where cat = 'Turkey Eggs' or cat = 'Turkey Hatch Eggs' "; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
            ...
            $q = "select distinct(itemcode) from turkey_production where itemcode in ($eggtype) and date1 = '$comparedate'  order by itemcode"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
...
<tr>
	<td<?php echo $sItemRowClass2; ?>><?php echo $qr['itemcode']; ?><br />(<?php echo $idesc; ?>)</td>
       <?php for($l = 0;$l < count($flo);$l++) { ?>
         <?php 
            include "config.php";
            $q1 = "select sum(quantity) as 'quantity' from turkey_production where itemcode = '$qr[itemcode]' and flock = '$flo[$l]' and date1 = '$comparedate' group by flock order by flock"; 
...
<tr>
  ...
  <?php for($l = 0;$l < count($flo);$l++) { ?>
         <?php 
           //$l = 0;
            include "config.php";
            $q = "select sum(quantity) as 'quantity' from turkey_production where itemcode in ($eggtype) and date1 = '$comparedate' and flock = '$flo[$l]' group by flock order by flock"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Opening Female Birds</td>
		 ...
		 $finalrem = 0;
            ...
            {
			 $fopening = 0; $ftransfer = 0; $ttransfer = 0; 
             $q = "select * from turkey_flock where flockcode = '$flo[$f]'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $q = "select distinct(date2),fmort,fcull,mmort,mcull from turkey_consumption where flock = '$flo[$f]' and date2 < '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
                $minus = $minus + $qr['fmort'] + $qr['fcull'];


             $q = "select * from ims_stocktransfer where cat = 'Turkey Female Birds' and fromwarehouse = '$flo[$f]' and date < '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             $q = "select * from ims_stocktransfer where cat = 'Turkey Female Birds' and towarehouse = '$flo[$f]' and date < '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Opening Male Birds</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
			 $mopening = 0; $ftransfer = 0; $ttransfer = 0; 
             $q = "select * from turkey_flock where flockcode = '$flo[$f]'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $q = "select distinct(date2),mmort,mcull from turkey_consumption where flock = '$flo[$f]' and date2 < '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
                $minus = $minus + $qr['mmort'] + $qr['mcull'];


             $q = "select * from ims_stocktransfer where cat = ' Turkey Male Birds' and fromwarehouse = '$flo[$f]' and date <= '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             $q = "select * from ims_stocktransfer where cat = 'Turkey Male Birds' and towarehouse = '$flo[$f]' and date <= '$comparedate'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             $remaining = $mopening - $minus - $ftransfer + $ttransfer;
         ?>
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Female Mortality</td>
         <?php 
			...
			$finalmort = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select fmort from turkey_consumption where date2 = '$comparedate' group by flock order by flock"; 
            else
                $q = "select fmort from turkey_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Male Mortality</td>
         <?php 
			...
			$finalmort = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select mmort from turkey_consumption where date2 = '$comparedate' group by flock order by flock"; 
            else
                $q = "select mmort from turkey_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Female Culls</td>
         <?php 
			...
			$finalcull = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select fcull from turkey_consumption where date2 = '$comparedate' group by flock order by flock"; 
            else
                $q = "select fcull from turkey_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Male Culls</td>
         <?php 
			...
			$finalcull = 0;
            if(!isset($_GET['flock']) || $_GET['flock'] == "All" || $_GET['flock'] == "")
                $q = "select mcull from turkey_consumption where date2 = '$comparedate' group by flock order by flock"; 
            else
                $q = "select mcull from turkey_consumption where date2 = '$comparedate' and flock in ($_GET[flock]) group by flock order by flock"; 
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Female Body Weight</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select fweight from turkey_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $fweight = $qr['fweight'];
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Standard Female Body Weight">Std. Female Body Weight</td>
         <?php 
            ...
            {
			$sfbw = "0";
			$q = "select * from breed_standards where Age = '$age1[$f]'";
			$qrs = mysql_query($q,$conn1) or die(mysql_error());
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Male Body Weight</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select mweight from turkey_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $mweight = $qr['mweight'];
...
<tr style="display:none">
	<td<?php echo $sItemRowClass2; ?>>Water</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select water from turkey_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $water = $qr['water'];
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Egg Weight</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select eggwt from turkey_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $eggwt = $qr['eggwt'];
...
<tr style="display:none">
	<td<?php echo $sItemRowClass2; ?>>Average Weight</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select avgwt from turkey_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $avgwt = $qr['avgwt'];
...
<tr style="display:none">
	<td<?php echo $sItemRowClass2; ?>>Temperature</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select tempmin,tempmax from turkey_consumption where flock = '$flo[$f]' and date2 = '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Turkey Hatch Eggs %</td>
         <?php 
            include "config.php";
            for($f = 0;$f<count($flo);$f++)
            {
...
<tr>
	<td<?php echo $sItemRowClass2; ?> title="Standard Turkey Hatch Eggs Percentage">Std. Turkey Hatch Eggs %</td>
         <?php 
            include "config.php";
            for($f = 0;$f<count($age1);$f++)
            {
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Consumption Cost</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select price from turkey_consumption where flock = '$flo[$f]' and date2 = '$comparedate' limit 1"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Production Cost</td>
         <?php 
            ...
            for($f = 0;$f<count($flo);$f++)
            {
             $q = "select price from turkey_production where flock = '$flo[$f]' and date1 = '$comparedate' limit 1"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
