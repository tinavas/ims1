...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Hatch Summary</font></strong></td>
</tr>
...
<a name="top"></a>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="hatchsummary.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="hatchsummary.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="hatchsummary.php?export=word">Export to Word</a>
<?php } ?>
<br /><br />
</div></td></tr>
<!-- Top Container (End) -->
...
<?php } else { ?>
		<td class="ewTableHeader">
			...
			<td align="center">Parent Flock</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader" >
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Saleable Chicks</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td  align="center">Chicks/Parent</td>
			</tr></table>
		</td>
<?php } ?>
</tr>
...
<?php
	...
	?>
<?php 
	$totaleggsset =0; $totalrejections=0; $totaltotalhatch =0; $totalsalablechicks= 0;$totalhatchper=0;$totalchicksper=0;$totalrejectionsper=0;$totalchicksperparent=0;
	$quer11 = "SELECT flock, shed, unit FROM hatchery_hatchrecord GROUP BY flock, shed, unit ORDER BY flock ";
...
$query1 = "SELECT min(date1) as 'date1' FROM breeder_production WHERE flock like '%$flock'";
$result1 = mysql_query($query1,$conn1); 
while($row11 = mysql_fetch_assoc($result1))
{
  $prodate = $row11['date1'];
...
$query1 = "SELECT * FROM breeder_consumption WHERE flock like '%$flock' and date2 < '$untildate' group by date2,flock";
$result1 = mysql_query($query1,$conn1); 
while($row11 = mysql_fetch_assoc($result1))
{
...
$querymin = "SELECT min(date2) as 'date2' FROM breeder_consumption WHERE flock like '%$flock' ";
...
$femalet = 0;$malet=0;
 $query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat IN ('Female Birds', 'Male Birds') AND client = '$client' AND towarehouse like '%$flock' AND date<='$mincmpdate'";
...
/*	  else
 ...
 /*  if($malet > 0)
  ...
  $startmale = $startmale + $malet;
	...
	$eggsset =0; $rejections=0; $totalhatch =0; $salablechicks= 0;$hatchper=0;$chicksper=0;$rejectionspe=0;$chicksperparent=0;
	$q1 = "select sum(noofeggsset) as noofeggsset, sum(totalrejections) as totalrejections, sum(hatch) as hatch, sum(saleablechicks) as saleablechicks FROM hatchery_hatchrecord where flock = '$flock' and shed = '$shed' and unit = '$unit'";
    $q11 = mysql_query($q1,$conn1) or die(mysql_error());
	while($row11 = mysql_fetch_assoc($q11))
	{
	$eggsset = $row11['noofeggsset'];
	$rejections = $row11['totalrejections'];
	$totalhatch = $row11['hatch'];
	$salablechicks = $row11['saleablechicks'];
	$hatchper = ($totalhatch/$eggsset)*100;
	$chicksper = ($salablechicks/$eggsset)*100;
	$rejectionsper = ($rejections/$eggsset)*100;
	 $chicksperparent = $salablechicks/$startfemale;
	}


	
	
	$totaleggsset =$totaleggsset + $eggsset; 
	$totalrejections=$totalrejections+$rejections; 
	$totaltotalhatch =$totaltotalhatch+$totalhatch; 
	$totalsalablechicks= $totalsalablechicks+$salablechicks;
	$totalhatchper=($totaltotalhatch/$totaleggsset)*100;
	$totalchicksper=($totalsalablechicks/$totaleggsset)*100;
	$totalrejectionsper=($totalrejections/$totaleggsset)*100;
	   ?>
	    <tr>
		...
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($hatchper,2); ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php echo  $salablechicks; ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($chicksper,2); ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($cull,2); $totalculls =$totalculls + $cull; ?>
		</td>
		<?php $cullper = ($cull/$eggset)*100; ?>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($cullper,2); $totalcullper =$totalcullper + $cullper;  ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php echo  round($rejectionsper,2); ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($chicksperparent,2); ?>
		</td>
...
<?php } ?>	   
	 <tr>
		...
		<td class="ewRptGrpField2" align="right">
		<?php  echo $totaleggsset; ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php echo  $totaltotalhatch; ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($totalhatchper,2); ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php echo  $totalsalablechicks; ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($totalchicksper,2); ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($totalculls,2); ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo  round($totalcullper,2); ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php echo  round($totalrejectionsper,2); ?>
		</td>
		<td class="ewRptGrpField2" align="right">
		<?php echo "&nbsp;"; ?>
		</td>
...
<?php 	
	...
	$quer11 = "select distinct(schedule),type from ac_schedule where  schedule in ( select distinct(schedule) from ac_coa where type = 'Capital' or type = 'Liability' ) order by schedule  ";
	   ...
	   {
	     $coacount = 0;
	      $quer = "select * from ac_financialpostings where coacode in (select distinct(code) from ac_coa where schedule = '$row111[schedule]')  and date <= '$tdate' ";
...
/*	   $q = "select sum(amount) as cramount  from ac_financialpostings where crdr = 'Cr' and date <= '$tdate' and coacode = '$code' ";
    ...
    $q = "select sum(amount) as dramount  from ac_financialpostings where crdr = 'Dr' and date <= '$tdate'  and coacode = '$code' ";
	  ...
	  if ( $row1['type'] == "Expense" ) {  $expsum = $expsum + $mbal;} 
  if ( $row1['type'] == "Revenue" ) {  $revsum = $revsum + $mbal; } 
  $finflag = "";
   $tot = 0;
   if ( $expsum > $revsum )
   {
 ...
 /*  while ( $dumm <= $commncount )
   {
	...
	<?php $dumm = $dumm + 1; }*/ ?>
<?php /*?>	<?php if ( $highflag == "left" ) {
	while ($dumm <= $leftcount) { ?>

	<?php $dumm = $dumm + 1; } } ?><?php */?>
<?php /*?>	<?php if ( $highflag == "right" ) { 
