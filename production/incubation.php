...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Hatch Eggs Under Incubation</font></strong></td>
</tr>
...
<a name="top"></a>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="incubation.php?export=html&date=<?php echo $date;?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="incubation.php?export=excel&date=<?php echo $date;?>">Export to Excel</a>
&nbsp;&nbsp;<a href="incubation.php?export=word&date=<?php echo $date;?>">Export to Word</a>
<?php } ?>
<br /><br />
</div></td></tr>
<!-- Top Container (End) -->
...
<form action="incubation.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<?php } else { ?>
		<td class="ewTableHeader" style=" width:50px" >
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Setting Date</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php } else { ?>
		<td class="ewTableHeader" style=" width:50px" >
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Hatch Date</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
		<td  class="ewTableHeader" style=" width:50px" title="Machine Number">
		Mc/No
		</td>
<?php } else { ?>
		<td class="ewTableHeader" style=" width:50px" title="Machine Number" >
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Mc/No</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php 
$eggs = 0;
$query = "select * from hatchery_traysetting where settingdate NOT IN (select distinct(settingdate) from hatchery_hatchrecord where client = '$client') and flock NOT IN (select distinct(flock) from hatchery_hatchrecord where client = '$client') and client = '$client'";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($qr = mysql_fetch_assoc($result))
{
$flock = $qr['flock'];
$age = $qr['eggage'];
$transferdate = date("d.m.Y",strtotime($qr['transferdate']));
$settingdate = date("d.m.Y",strtotime($qr['settingdate']));
$hatchdate = date("d.m.Y",strtotime($qr['hatchdate']));
$machinenum = $qr['setter'];
$eggsset = $qr['eggsset'];	   
	   ?>
	    <tr>
...
<?php if(($flock <> $oldcode) or ($dumm == 0)){ echo ewrpt_ViewValue($flock);
	 ...
	 echo ewrpt_ViewValue("&nbsp;");
		...
		<td class="ewRptGrpField3" align="right">
		<?php echo  $eggsset; $totaleggs += $eggsset;?>
		</td>
...
<!--first page button-->
	...
	<td><a href="incubation.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="incubation.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="incubation.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="incubation.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
document.location = "incubation.php?date="+date + "&flock=" + flock;
}
</script>
