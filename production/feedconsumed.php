...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT Distinct (breeder_consumption.flock), breeder_consumption.wo FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_WHERE = "breeder_consumption.flock IN (SELECT DISTINCT(breeder_flock.flockcode) FROM breeder_flock WHERE cullflag = 0)";
$EW_REPORT_TABLE_SQL_GROUPBY = "";
$EW_REPORT_TABLE_SQL_HAVING = "";
$EW_REPORT_TABLE_SQL_ORDERBY = "(breeder_consumption.flock) ASC";
...
<br />
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="feedconsumed.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="feedconsumed.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="feedconsumed.php?export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="feedconsumed.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<?php } ?>
<?php if (@$sExport == "") { ?>
<br /><br />
...
<form action="feedconsumed.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<?php
			  ...
			  $fpur = 0;
			  $mpur = 0;
			  $query14 = "SELECT sum(quantity) as ftin from ims_stocktransfer where towarehouse = '$x_flock' and date <= '$maxdate' and cat = 'Female Birds' ";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
                 $ftin = $row14['ftin'];
              }
			  $q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$x_flock' AND date <= '$maxdate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Female Birds')";  
		   ...
		   while($qr = mysql_fetch_assoc($r))
             ...
             else
             {
			  ...
			  $query14 = "SELECT sum(quantity) as mtin from ims_stocktransfer where towarehouse = '$x_flock' and date <= '$maxdate' and cat = 'Male Birds' ";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
                 $mtin = $row14['mtin'];
              }
			   $q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$x_flock' AND date <= '$maxdate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Male Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
             ...
             else
             {
              ...
              $totalfeed1 = 0;
              $query14 = "SELECT sum(quantity) as 'feed' FROM breeder_consumption where flock = '$x_flock' and itemcode in ($feedtypes1)";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
              ...
              $totalfeed2 = 0;
              $query14 = "SELECT sum(quantity) as 'feed' FROM breeder_consumption where flock = '$x_flock' and itemcode in ($feedtypes2)";
              $result14 = mysql_query($query14,$conn1);  
              while($row14 = mysql_fetch_assoc($result14)) 
              {
...
<?php  echo (changequantity($maleopening + $mtin + $mpur)); ?>
		...
		<td<?php echo $sItemRowClass; ?> align="right">
<?php echo changeprice(round($totalfeed1,2)); ?>
</td>
<?php $fbirds =$femaleopening + $ftin + $fpur;?>
<?php $mbirds = $maleopening + $mtin + $mpur;?>

<td<?php echo $sItemRowClass; ?> align="right">
<?php echo changeprice(round((($totalfeed1/$fbirds)*1000),2)); ?>
</td>
		<td<?php echo $sItemRowClass; ?> align="right">
<?php echo changeprice(round($totalfeed2,2)); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo changeprice(round((($totalfeed2/$mbirds)*1000),2)); ?>
</td>
