...
$url = "&fromdate=" . $_GET['fromdate'] . "&todate=" . $_GET['todate']. "&code=" . $_GET['code']. "&desc=" . $_GET['desc'];
?>
<?php
...
include "config.php";
 ...
 $q = "select distinct(type) as typ from ac_financialpostings where date >= '$fdate' and date<= '$tdate' order by type ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	   //if( ($qr['typ'] == "Feed Produced") or ($qr['typ'] == "Item Consumed"))
	   if( ($qr['typ'] == "Feed Produced") or ($qr['typ'] == "Item Consumed") )
	   {
	   
	           $q11 = "select distinct(trnum) from ac_financialpostings where date >= '$fdate' and date <= '$tdate' and  (type = 'Feed Produced' or type = 'Item Consumed') order by trnum ";
	           $qrs11 = mysql_query($q11,$conn1) or die(mysql_error());
	           while($qr11 = mysql_fetch_assoc($qrs11))
	           {
	           
			   $q1 = "select distinct(date) from ac_financialpostings where date >= '$fdate' and date <= '$tdate' and  (type = 'Feed Produced' or type = 'Item Consumed') and trnum = '$qr11[trnum]' order by date ";
	           $qrs1 = mysql_query($q1,$conn1) or die(mysql_error());
	           while($qr1 = mysql_fetch_assoc($qrs1))
	           {
                    $cramountdumm = 0;
                    $dramountdumm =0;
               $cr1 = "select sum(amount) as cramt from ac_financialpostings where  trnum = '$qr11[trnum]' and date = '$qr1[date]' and crdr = 'Cr' and (type = 'Feed Produced' or type = 'Item Consumed') ";
	           $crs1 = mysql_query($cr1,$conn1) or die(mysql_error());
	           while($crts1 = mysql_fetch_assoc($crs1))
	           {
                  $cramountdumm = $crts1['cramt'];
               }

              $d1 = "select sum(amount) as dramt from ac_financialpostings where trnum = '$qr11[trnum]' and date = '$qr1[date]' and crdr = 'Dr' and (type = 'Feed Produced' or type = 'Item Consumed') ";
	          $ds = mysql_query($d1,$conn1) or die(mysql_error());
	          while($dts1 = mysql_fetch_assoc($ds))
	          {
                 $dramountdumm = $dts1['dramt'];
              }
		  
              if ( $dramountdumm == $cramountdumm )
             {
	   ...
	   elseif ( ($qr['typ'] == "Consumption") or ($qr['typ'] == "Production") )
	   {
	           $q11 = "select distinct(trnum) from ac_financialpostings where date >= '$fdate' and date <= '$tdate' and  type = '$qr[typ]' order by trnum ";
	           $qrs11 = mysql_query($q11,$conn1) or die(mysql_error());
	           while($qr11 = mysql_fetch_assoc($qrs11))
	           {
	           
			   $q1 = "select distinct(date) from ac_financialpostings where date >= '$fdate' and date <= '$tdate' and  type = '$qr[typ]' and trnum = '$qr11[trnum]' order by date ";
	           $qrs1 = mysql_query($q1,$conn1) or die(mysql_error());
	           while($qr1 = mysql_fetch_assoc($qrs1))
	           {
                    $cramountdumm = 0;
                    $dramountdumm =0;
               $cr1 = "select sum(amount) as cramt from ac_financialpostings where trnum = '$qr11[trnum]' and date = '$qr1[date]' and crdr = 'Cr' ";
	           $crs1 = mysql_query($cr1,$conn1) or die(mysql_error());
	           while($crts1 = mysql_fetch_assoc($crs1))
	           {
                  $cramountdumm = $crts1['cramt'];
              ...
              $d1 = "select sum(amount) as dramt from ac_financialpostings where trnum = '$qr11[trnum]' and date = '$qr1[date]' and crdr = 'Dr' ";
	          $ds = mysql_query($d1,$conn1) or die(mysql_error());
	          while($dts1 = mysql_fetch_assoc($ds))
	          {
                 $dramountdumm = $dts1['dramt'];
	   ...
	   { 
	         $q1 = "select distinct(trnum),date from ac_financialpostings where date >= '$fdate' and date <= '$tdate' and  type = '$qr[typ]' order by trnum ";
	$qrs1 = mysql_query($q1,$conn1) or die(mysql_error());
	while($qr1 = mysql_fetch_assoc($qrs1))
	{
           ...
           $cramountdumm = 0;
           $dramountdumm =0;
         $cr1 = "select sum(amount) as cramt from ac_financialpostings where type = '$qr[typ]' and trnum = '$qr1[trnum]' and crdr = 'Cr' ";
	    $crs1 = mysql_query($cr1,$conn1) or die(mysql_error());
	    while($crts1 = mysql_fetch_assoc($crs1))
	    {
                $cramountdumm = $crts1['cramt'];
          ...
          $d1 = "select sum(amount) as dramt from ac_financialpostings where type = '$qr[typ]' and trnum = '$qr1[trnum]' and crdr = 'Dr' ";
	    $ds = mysql_query($d1,$conn1) or die(mysql_error());
	    while($dts1 = mysql_fetch_assoc($ds))
	    {
              $dramountdumm = $dts1['dramt'];
...
echo "Amount ";
  ...
  if ( $type == "Expense" or $type = "Revenue")
  {
	...
	while($qr = mysql_fetch_assoc($qrs))
	{
	  ...
	  $q1 = "select sum(amount) as cramount from ac_financialpostings where coacode = '$coa' and crdr = 'Cr' and date >= '$startdate' and date < '$fdate' ";
...
$q = "select sum(amount) as dramount from ac_financialpostings where coacode = '$coa' and crdr = 'Dr' and date >= '$startdate'  and date < '$fdate' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
  ...
  {
     $q1 = "select sum(amount) as cramount from ac_financialpostings where coacode = '$coa' and crdr = 'Cr' and date < '$fdate' ";
	$qrs = mysql_query($q1,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  $crtotal = $crtotal + $qr['cramount'];
...
$q = "select sum(amount) as dramount from ac_financialpostings where coacode = '$coa' and crdr = 'Dr' and date < '$fdate' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  $drtotal = $drtotal + $qr['dramount'];
...
<td align="2">
<?php if ( $oflag == "Dr" ) { echo ewrpt_ViewValue(changeprice1($obal)); $drtotal = $drtotal + $obal; } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
</td>
<td align="2">
<?php if ( $oflag == "Cr" ) { echo ewrpt_ViewValue(changeprice1($obal)); $crtotal = $crtotal + $obal; } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
</td>
	</tr>
	<?php
	$nxtdate = "";
	 $dumfrmdate = ""; 
	 include "config.php";
      $query1 = "SELECT * from ac_financialpostings where coacode = '$coa'  and date >= '$fdate' and date <= '$tdate' order by date,trnum ";
...
?>
	<tr>
		<td class="ewRptGrpField1">
		<?php echo ewrpt_ViewValue(date("d-m-Y",strtotime($row2['date']))) ?>
		</td>
		...
		else if ( $row2['type'] == "Consumption")
		{
		   $query11 = "select * from breeder_consumption where flock = '$row2[trnum]' and date2 = '$row2[date]' ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = "";
		...
		{
		  $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'R'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['rremarks'];
		...
		{
		  $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'P'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['rremarks'];
		...
		else if ( $row2['type'] == "JV")
		{
		  $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'J'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['rremarks'];
