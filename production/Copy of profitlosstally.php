   ...
   $query1 = "select sum(amount) as dramount from ac_financialpostings where coacode = '$ingacc' and client = '$client'  and crdr = 'Dr' and date < '$fdate'  ";
	...
	// $eggnuminitbal = $eggnuminitdr - $eggnuminitcr;
	 ...
	 $query1 = "select sum(amount) as cramount from ac_financialpostings where coacode = '$medacc' and client = '$client'  and crdr = 'Cr' and date < '$fdate'  ";
	  ...
	  //Eggs Opening Stock
	  $query1 = "select distinct(iac),pvac from ims_itemcodes where cat = 'Eggs' and client = '$client'   ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	   $eggacc = $row2['iac'];
	...
	$query11 = "select * from oc_cobi where client = '$client' and date = '$fdate'  and code = 'EG101'  ";
	 ...
	 $query1 = "select sum(quantity * price)/sum(quantity) as price from oc_cobi where client = '$client' and date = '$maxdate'  and code = 'EG101' ";
	  ...
	  //Feed Opening Stock
	  $query1 = "select distinct(iac),stdcost from ims_itemcodes where cat = 'Feed' and client = '$client'   ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	    $feedacc = $row2['iac'];
	   $feedstdcost = $row2['stdcost'];
	  }
	 ...
	 $feedinitbal = $feedinitbal + $feedinitir;
	  ...
	  if($totprod <= 0)
	  {
	    $feedinitrate = $feedinitbal * $feedstdcost;
	  }
	  else
	  {
	 ...
	 // echo $feedinitbal;
	  ...
	  //Birds Opening Stock
	   $query1 = "select distinct(iac),stdcost from ims_itemcodes where cat = 'Birds' and client = '$client'   ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	   $birdsacc = $row2['iac'];
	   $birdsstdcost = $row2['stdcost'];
	  }
	  $previnitdate = strtotime($fdate);
	  $previnitdate = $previnitdate - (24 * 60 * 60);
	  $previnitdate = date("Y-m-d",$previnitdate);
	  $maxagestd = 0;
	  $query111 = "select max(toweek) as maxage from common_stdprices  where type = 'Birds'  and client = '$client'   ";
...
else
{
  ...
  //$age = $age + 1;
		...
		if($maxagestd < $age)
		{
		 $query11 = "select * from common_stdprices where type = 'Birds' and  toweek = $maxagestd and client = '$client'   ";
        $result11 = mysql_query($query11,$conn1);
	    while($row21 = mysql_fetch_assoc($result11))
        {
		...
		{
		 $query11 = "select * from common_stdprices where type = 'Birds' and toweek >= '$age' and   fromweek < '$age' and client = '$client'   ";
        $result11 = mysql_query($query11,$conn1);
	    while($row21 = mysql_fetch_assoc($result11))
        {
	      $stdprice = $row21['stdprice'];
 ...
 $finnxtdate = strtotime($tdate);
	 ...
	 $query1 = "select sum(quantity) as cramount from ac_financialpostings where coacode = '$feedacc' and client = '$client'  and crdr = 'Cr' and date <= '$tdate'  ";
	   ...
	   while($row2 = mysql_fetch_assoc($result1))
      {
		...
		//Inward
		$query11 = "select sum(quantity) as trinbirds from ims_stocktransfer where towarehouse = '$row2[flock]' and client = '$client' and date <= '$tdate' and cat = 'Birds'  ";
        $result11 = mysql_query($query11,$conn1);
	    while($row21 = mysql_fetch_assoc($result11))
        {
	      $transferincls = $row21['trinbirds'];
		 }
		 //Outward
		 $query11 = "select sum(quantity) as troutbirds from ims_stocktransfer where fromwarehouse = '$row2[flock]' and client = '$client' and date <= '$tdate'  and cat = 'Birds'  ";
        $result11 = mysql_query($query11,$conn1);
	    while($row21 = mysql_fetch_assoc($result11))
        {
	      $transferoutcls = $row21['troutbirds'];
		 }
		 //Sale
		...
		{
		$query11 = "select * from common_stdprices where type = 'Birds' and  fromweek < '$agecls' and toweek >= '$agecls' and client = '$client'   ";
        $result11 = mysql_query($query11,$conn1);
	    while($row21 = mysql_fetch_assoc($result11))
        {
	      $stdpricecls = $row21['stdprice'];
	 ...
	 //echo $totbirdscls;
	   $rightarray1[$rightcount] = "Birds Closing Stock";
