...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Trial Balance</font></strong></td>
</tr>
<tr>
<td colspan="2">Date &nbsp;<?php echo $tdatedump;?></font></td>
</tr>
</table>
<br/>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="trialbalance.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="trialbalance.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="trialbalance.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php
 ...
 $query = "SELECT sum(amount) as cramount,coacode FROM ac_financialpostings WHERE crdr = 'Cr' AND date <= '$tdate' GROUP BY coacode ORDER BY coacode";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
 $crarray[$rows['coacode']] = $rows['cramount'];	
 
$query = "SELECT sum(amount) as dramount,coacode FROM ac_financialpostings WHERE crdr = 'Dr' AND date <= '$tdate' GROUP BY coacode ORDER BY coacode";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
 $drarray[$rows['coacode']] = $rows['dramount'];	

$coacodes = "'dummy',"; 
$query = "select distinct(coacode) from ac_financialpostings where date <= '$tdate' order by coacode";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
  $coacodes .= "'".$rows['coacode']."',";
$coacodes = substr($coacodes,0,-1);
 $query1 = "select distinct(type) from ac_coa where code IN ($coacodes) order by type";
$result1 = mysql_query($query1,$conn1) or die(mysql_error());
while($rows1 = mysql_fetch_assoc($result1))
{
 $crtotal = $drtotal = 0;
 $type = $rows1['type'];
 $query2 = "select code,description from ac_coa where type = '$type' and code IN ($coacodes) order by code";
 $result2 = mysql_query($query2,$conn1) or die(mysql_error());
 while($rows2 = mysql_fetch_assoc($result2))
 {
  $coacode = $rows2['code'];
  /*$query3 = "select sum(amount) as cramount from ac_financialpostings where date between '$fdate' and '$tdate' and crdr = 'Cr' and coacode = '$coacode'";
  $result3 = mysql_query($query3,$conn1) or die(mysql_error());
  $rows3 = mysql_fetch_assoc($result3);
  $cramount = $rows3['cramount'];
  
  $query3 = "select sum(amount) as dramount from ac_financialpostings where date between '$fdate' and '$tdate' and crdr = 'Dr' and coacode = '$coacode'";
  $result3 = mysql_query($query3,$conn1) or die(mysql_error());
  $rows3 = mysql_fetch_assoc($result3);
  $dramount = $rows3['dramount'];
	 $bal = round($cramount - $dramount,2);*/
  ...
  ?>
	<tr>
		...
		<?php  echo ewrpt_ViewValue($coacode) ?>
		</td>
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue($rows2['description']) ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php echo ewrpt_ViewValue($type) ?>
		</td>
		<td align="right">
        <?php  if ( $flag == "Dr" ) {  echo ewrpt_ViewValue(changeprice($mbal)); $drtotal = $drtotal + $mbal; } else { echo "&nbsp;"; } ?>
        </td>
		<td align="right">
         <?php  if ( $flag == "Cr" ) {  echo ewrpt_ViewValue(changeprice($mbal)); $crtotal = $crtotal + $mbal; } else { echo "&nbsp;"; }  ?>
        </td>
 ...
 if($crtotal > 0 or $drtotal > 0)
 {
 	?>
	<tr>
		<td class="ewRptGrpField3" align="right" colspan="3"><b>
...
?>
			...
			<tr>
		<td class="ewRptGrpField1" colspan="3" align="right"><b>
		<?php echo ewrpt_ViewValue("Grand Total") ?></b>
		</td>
		<td align="right"><b>
<?php echo ewrpt_ViewValue(changeprice($gdrtotal)) ?></b>
</td>
		<td align="right"><b>
<?php echo ewrpt_ViewValue(changeprice($gcrtotal));  ?></b>
</td>
