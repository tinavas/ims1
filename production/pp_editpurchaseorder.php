...
while($a=mysql_fetch_array($r))
{
 ?>
     <tr>
       ...
       <td style="text-align:left;">
         <select name="category[]" id="type" onchange="fun(this.id,'');" style="width:80px;">
           <option>-Select-</option>
           <?php
              include "config.php"; 
              $query = "SELECT * FROM ims_itemtypes ORDER BY type ASC ";
       ...
       <td style="text-align:left;">
         <input type="text" name="description[]" id="ing" value="<?php echo $a[description]; ?>" readonly size="8"  /> 
       </td>
       <td width="10px"></td>
       ...
       <td style="text-align:left;">
         <input type="text" size="8" id="rdate" name="rdate[]" class="datepicker" value="<?php echo  date("d.m.Y",strtotime($a[deliverydate])); ?>">
       </td>
       
       <td>
       <?php if($_SESSION['db']=="vista"){?>
       <input type="text" style="background:none;border:0px;" readonly="readonly" size="8" id="bird" name="bird[]" value="<?php echo $bird;?>" />
       <?php } ?>
       </td>
...
mybox=document.createElement("input");
  ...
  var c12ca = document.createElement('td');
  myspace2= document.createTextNode('\u00a0');
  c12ca.appendChild(myspace2);
  var ca12 = document.createElement('td');
  ca12.appendChild(mybox);
  if(a == "pr")
  {
    ca12.align="left";
  }
