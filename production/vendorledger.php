...
if ( $_POST['code'] == "All" )  { header('Location:vendetlsled1.php?date2='.$_POST['date2'].'&date3='.$_POST['date3']); }
...
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Supplier Ledger</font></strong></td>
</tr>
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Supplier Name&nbsp;<?php echo $ven; ?></font></strong></td>
</tr>
<?php if($_SESSION[db]=='albustan' || $_SESSION[db]=='albustanlayer'){ ?>
<tr>
<td colspan="2" align="center"><strong><font color="#3e3276">Supplier Code&nbsp;<?php echo $vendorcode; ?></font></strong></td>
</tr>
<?php } ?>
<tr>
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="vendorledger.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="vendorledger.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="vendorledger.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
$obflag = "Dr";
include "config.php";
    $q = "select * from contactdetails  where name = '$ven'  ";
	...
	$q = "select * from ac_vgrmap  where vgroup = '$vengroup'  ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	...
	$q = "select sum(amount) as dramt from ac_financialpostings where coacode = '$vencontrollac'  and crdr = 'Dr' and date <'$fdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	...
	$q = "select sum(amount) as cramt from ac_financialpostings where coacode = '$vencontrollac'  and crdr = 'Cr' and date <'$fdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
...
$price='';
	$quantity='';
	  ...
	  if ( $qr['type'] == "SOBI" )
	  {
	    ...
	    $q1 = "select invoice,so,receivedquantity,rateperunit,description,remarks,credittermvalue from pp_sobi where so = '$qr[trnum]'  ";
	    ...
	    while($qr1 = mysql_fetch_assoc($qrs1))
	    {
		  if($cur == $cnt)
		  {
		    $narration = $narration.$qr1['description'];
			$quantity=$quantity.$qr1['receivedquantity'];
			$price=$price.$qr1['rateperunit'];
			$invoice = $qr1['invoice'];
			$remarks = $qr1['remarks'];
			$so=$qr1['so'];
		  }
	  ...
	  {
	      $mode = "Payment";
	   $q1 = "select remarks,paymentmode,cheque from pp_payment where tid = '$qr[trnum]'";
	      $qrs1 = mysql_query($q1,$conn1) ;
	      while($qr1 = mysql_fetch_assoc($qrs1))
	      {
	  ...
	  {
	    $mode = "VCN-Supplier Credit Note";
	  }
	  else if ( $qr['type'] == "VDN" )
	  {
	    $mode = "VDN-Supplier Debit Note";
	  }
...
?>
	<tr>
        ...
        <td class="ewRptGrpField2">
		<?php  echo ewrpt_ViewValue($qr[warehouse])?>
		</td>
		<td class="ewRptGrpField2">
		<?php  echo ewrpt_ViewValue($bkinvoice)?>
		</td>
		...
		<td class="ewRptGrpField3">
		<?php echo ewrpt_ViewValue($invoice) ?>
		</td>
		<td class="ewRptGrpField3">
...
<td>
<?php echo ewrpt_ViewValue($remarks) ?>
</td>
<?php if($_SESSION['db']=="vista") { ?>
<td>
...
<td align="right"><b>
<?php  echo ewrpt_ViewValue(changeprice($drtotal));   ?></b>
</td>
<td align="right"><b>
<?php  echo ewrpt_ViewValue(changeprice($crtotal));  ?></b>
</td>
<td>
<?php echo ewrpt_ViewValue() ?>
</td>
...
include "config.php";
	...
	$q = "select sum(amount) as dramt from ac_financialpostings where coacode = '$venprepayac'  and crdr = 'Dr' and date <'$fdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  $obdramt = $obdramt + $qr['dramt']; 
	...
	$q = "select sum(amount) as cramt from ac_financialpostings where coacode = '$venprepayac'  and crdr = 'Cr' and date <'$fdate' and venname = '$ven' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	  $obcramt = $obcramt + $qr['cramt']; 
...
<?php } ?>
	...
	<?php
	 ...
	 $q = "select * from ac_financialpostings where date >= '$fdate' and date <= '$tdate' and venname = '$ven' and coacode = '$venprepayac' ";
	$qrs = mysql_query($q,$conn1) ;
	while($qr = mysql_fetch_assoc($qrs))
	{
	$mode = "";
	...
	$quantity='';
	  ...
	  {
	    $mode = "SOBI";
	    $q1 = "select remarks,rateperuint,receicedquantity,invoice,description from pp_sobi where so = '$qr[trnum]'  ";
	    $qrs1 = mysql_query($q1,$conn1) ;
	    while($qr1 = mysql_fetch_assoc($qrs1))
	    {
		  $narration = $qr1['remarks'];
		  $quantity=$qr1['receivedquantity'];
		  $price=$qr1['rateperunit'];
		}
	  }
	  else if ( $qr['type'] == "PMT" )
	  {
...
<?php echo ewrpt_ViewValue() ?>
 ...
 <?php } ?>
<td align="right"><b>
<?php  echo ewrpt_ViewValue(changeprice($pdrtotal));   ?></b>
</td>
<td align="right"><b>
<?php  echo ewrpt_ViewValue(changeprice($pcrtotal));  ?></b>
</td>
	</tr>
...
<td align="right"><b>
<?php if ( $outstflag == "Cr") { echo ewrpt_ViewValue(changeprice($outstbal)); } else { echo ewrpt_ViewValue("&nbsp;");  }  ?></b>
</td>
<?php if($_SESSION['db']=="vista") { ?>
<td>
<?php echo ewrpt_ViewValue() ?>
