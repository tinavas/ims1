...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td style="width:100px;" align="center">Female Birds</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td style="width:100px;" align="center">Male Birds</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td style="width:100px;" align="center">F.Feed/Bird</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td style="width:100px;" align="center">M.Feed/Bird</td>
			</tr></table>
		</td>
<?php } ?>-->
<?php if (@$sExport <> "") { ?>
...
$fbirds = $fopening + $fpurchases[$flock] + $ftransferin[$flock] - ($fmort + $fculls + $ftransferout[$flock] + $fsales[$flock]);
 ...
 { 
  ...
  if($flkbreed != "")
  {
  $query13 = "SELECT mfeed,ffeed FROM breeder_standards WHERE age = '$week' AND breed='$flkbreed' AND client = '$client'";
  }
  else
  {
  ...
  if($r14 > 0)
  {
    ...
    //echo "<br/>$flock Age: $i Week $week, $fbirds * $ffeed Feed ".$qty['FD102'];
...
/*  if($week >= 0 && $week <= 23)
  {
   if($week >= 0 && $week <= 6)
    $qty[$af['0-6']] += $daymfeed + $dayffeed;
   elseif($week >=7 && $week <= 17)
    $qty[$af['7-17']] += $daymfeed + $dayffeed;
   elseif($week >= 18 && $week <= 23)
    $qty[$af['18-23']] += $daymfeed + $dayffeed;
  }
  else
  {
    $qty[$af['>23(M)']] += $daymfeed;
   if($week >= 24 && $week <= 40)
    $qty[$af['24-40(F)']] += $dayffeed;
   elseif($week >= 41 && $week <= 60)
    $qty[$af['41-60(F)']] += $dayffeed;
   elseif($week >= 61)
    $qty[$af['>60(F)']] += $dayffeed;	
  } */
...
<!--		<td class="ewRptGrpField3">
<?php echo ewrpt_ViewValue($fbirds); ?></td>
		<td class="ewRptGrpField3">
<?php echo ewrpt_ViewValue($mbirds); ?></td>
		<td class="ewRptGrpField3">
