...
<center>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="layerbreeder_flockwiseperformance.php?export=html&date=<?php echo $_GET['date']; ?>&flocktype=<?php echo $_GET['flocktype']; ?>&unit=<?php echo $_GET['unit']; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="layerbreeder_flockwiseperformance.php?export=excel&date=<?php echo $_GET['date']; ?>&flocktype=<?php echo $_GET['flocktype']; ?>&unit=<?php echo $_GET['unit']; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="layerbreeder_flockwiseperformance.php?export=word&date=<?php echo $_GET['date']; ?>&flocktype=<?php echo $_GET['flocktype']; ?>&unit=<?php echo $_GET['unit']; ?>">Export to Word</a>
<?php } else { ?>
<center>
<?php } ?>
<?php if (@$sExport == "") { ?>
...
<thead>
	<th class="ewTableHeader">Flock</th>
         <?php 
			...
			$q = "select distinct(flockcode) from layerbreeder_flock where cullflag = '$flocktype' AND flockcode <> 'NULL' and flockcode <> ''  and flockcode in (select distinct(flock) from layerbreeder_consumption where client='$client') and client='$client'  order by flockcode";
			$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
              $flo[$f] = $qr['flockcode'];
			  $qbr = "select breed from layerbreeder_flock where flockcode = '$qr[flockcode]' and client = '$client'"; 
...
<tr>
	<td<?php echo $sItemRowClass1; ?>>Age</td>
         <?php 
			...
			{
			$flockfinalid = $flo[$k];
			$q = "select max(age) as age from layerbreeder_consumption where flock like '$flockfinalid' and client='$client'"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
...
{
$flockfinalid = $flo[$k];
$query = "select quantity as tot,fweight as femaleweight from layerbreeder_consumption where flock like '$flockfinalid' and itemcode in (select distinct(code) from ims_itemcodes where cat ='Female Feed' and client='$client') and client='$client'  group by date2";
$totffeedd = 0;
...
$query1 = "select sum(ffeedqty) as tot,sum(fweight) as femaleweight from layerbreeder_initial where ( flock like '$flockfinalid' ) and client='$client'";
...
$query1 = "select min(date2) as mindate from layerbreeder_consumption where flock like '$flockfinalid' and client='$client'";
$birdsresult = mysql_query($query1,$conn1) or die(mysql_error());
if($mindt = mysql_fetch_assoc($birdsresult))
$mindate = $mindt['mindate'];

$query1 = "select female as openingbirds from layerbreeder_initial where flock like '$fflloo' and client='$client'";
$birdsresult = mysql_query($query1,$conn1) or die(mysql_error());
...
if($fopeningbirds == 0)
 {
   ...
   $query2 = "select sum(femaleopening) as fopeningbirds from layerbreeder_flock where flockcode = '$fflloo' and client='$client'";
...
$fcull = 0;
$flockfinalid = $flo[$k];
$query = "select distinct(date2),fmort,fcull from layerbreeder_consumption where  flock like '$flockfinalid' and client='$client'";
...
<tr>
 ...
 {      
    $flockfinalid = $flo[$k];
$query = "select max(fweight) as femaleweight from layerbreeder_consumption where  flock like '$flockfinalid' and client='$client'";
$result = mysql_query($query,$conn1);
...
<tr>
	<td style="background-color:#00CCCC" title="Standard Female Body Weight">Std. Body Weight(gms.)</td>
	  ...
	  if($flkbreed[$k] != "")
	  {
	  $q = "select * from layerbreeder_standards where age = '$age1[$k]' and breed='$flkbreed[$k]' and client='$client'";
	  }
	  else
	  {
		$q = "select * from layerbreeder_standards where age = '$age1[$k]' and client='$client'";
	  }
			$qrs = mysql_query($q,$conn1) or die(mysql_error());
			if($qr = mysql_fetch_assoc($qrs))
			{
			...
			if($flkbreed[$k] != "")
	  {
			$q = "select * from layerbreeder_standards where age in (select max(age) from layerbreeder_standards where breed='$flkbreed[$k]' and  client='$client') and breed = '$flkbreed[$k]' and client='$client'";
			}
			else
			{
			$q = "select * from layerbreeder_standards where age in (select max(age) from layerbreeder_standards where client='$client') and client='$client'";
			}
			$qrs = mysql_query($q,$conn1) or die(mysql_error());
			if($qr = mysql_fetch_assoc($qrs))
...
{
$flockfinalid = $flo[$k];
 $query = "select sum(quantity) as tot from layerbreeder_production where flock like '$flockfinalid' and itemcode in (select distinct(code) from ims_itemcodes where cat = 'Layer Breeder Eggs' and client='$client') and client='$client'";
$result = mysql_query($query,$conn1);
$result = mysql_fetch_assoc($result);
if($result['tot'])
$heggs = $result['tot'];
...
$index = 0;
$eggsquery = "select eggs from layerbreeder_initial where ( flock like '$flockfinalid' and eggs <> '' and eggs <> 'NULL' ) and client='$client' ";
$eggsresult = mysql_query($eggsquery,$conn1) or die(mysql_error());
while($eggsmain = mysql_fetch_assoc($eggsresult))
{
$maineggs[$index] = $eggsmain['eggs'];
...
while($eggsmain = mysql_fetch_assoc($eggsresult))
{
 $mxage = $eggsmain['maxage'];
		 ...
		 if($_SESSION['db'] == "chowdarypf")
		 {
		  ...
		  $stdquery = "select min(age) as minage,max(age) as maxage from layerbreeder_consumption where flock like '$flo[$k]' and client='$client'";
		 ...
		 $stdres = mysql_fetch_assoc($stdresult);
		  ...
		  $maxage = round($stdres['maxage']/7);
		   ...
		   if($flkbreed[$k] != "")
		  $query = "select hecum from layerbreeder_standards where breed = '$flkbreed[$k]' and ( age = '$minage' or age = '$maxage') and client='$client' order by age";
		  else		 		
		 $query = "select hecum from layerbreeder_standards where ( age = '$minage' or age = '$maxage') and client='$client' order by age";
		 ...
		 if($flkbreed[$k] != "")
		 {
		  if($age1[$k] <= $mxage)
		  {
		 $query = "select hecum from layerbreeder_standards where breed = '$flkbreed[$k]' and age = '$age1[$k]' and client='$client'";
		  }
		  else
		  {
		  $query = "select hecum from layerbreeder_standards where breed = '$flkbreed[$k]' and age = '$mxage' and client='$client'";
		  }
		 }
		 else
		 {
		  if($age1[$k] <= $mxage)
		  {
		  $query = "select hecum from layerbreeder_standards where age = '$age1[$k]' and client='$client'";
		  }
		  else
		  {
		  $query = "select hecum from layerbreeder_standards where age = '$mxage' and client='$client'";
		  }
		  }
...
document.location="layerbreeder_flockwiseperformance.php?flocktype="+a;
}
function selectunit(a)
{

var date1=document.getElementById("date1").value;
document.location="layerbreeder_flockwiseperformance.php?unit=" + a + "&date=" + date1; 
}
</script>
