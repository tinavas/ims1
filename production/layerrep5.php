...
//echo $minus;
             ...
             $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Birds' and client = '$client'  and fromwarehouse = '$flock' and date < '$mindatep'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
...
//echo $ftransfer ;
		   ...
		   while($qr = mysql_fetch_assoc($qrs))
            ...
            $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Birds' and client = '$client'  and towarehouse = '$flock' and date < '$mindatep'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
			...
			//echo $ttransfer;
			 ...
			 $q = "SELECT sum(shortagefemale) - sum(excessfemale) AS female FROM layer_birdsadjustment WHERE flock = '$flock' AND date < '$mindatep'";
			 $r = mysql_query($q,$conn1) or die(mysql_error());
			 $rr = mysql_fetch_assoc($r);
			 $fadjust = $rr['female'];
			
            $remaining = ($fopening - $minus - $ftransfer + $ttransfer) - $tsale + $fadjust;
			 $nxtbirds = $remaining;
?>
<?php
$minwkdate = "";
	 ...
	 $query1 = "SELECT  sum(quantity) as quant from layer_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' and itemcode in (select code from ims_itemcodes where cat = 'Layer Feed' and client = '$client') ";
	   ...
	   while($row2 = mysql_fetch_assoc($result1))
      {
	   ...
	   if($_SESSION['db'] == "sathireddy")
		...
		$q = "select avg(rateperunit) AS cost FROM pp_sobi WHERE date <= 'row2[date]' AND code = '$row2[itemcode]'"; 
	    ...
	    $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Birds' and client = '$client'  and fromwarehouse = '$flock' and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
                $wtrout = $qr['quantity'];
               }
          $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'Birds' and client = '$client'  and towarehouse = '$flock' and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
                $wtrin = $qr['quantity'];
			 ...
			 $q = "SELECT sum(shortagefemale) - sum(excessfemale) AS female FROM layer_birdsadjustment WHERE flock = '$flock' AND date < '$maxwkdate' AND date >= '$minwkdate'";
			 $r = mysql_query($q,$conn1) or die(mysql_error());
			 $rr = mysql_fetch_assoc($r);
			 $fadjust = $rr['female'];
             ...
             $nxtbirds = $nxtbirds - $wmort - $wcull - $wtrout + $wtrin - $wsale + $fadjust;
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($cost > 0) {  echo ewrpt_ViewValue(changeprice($cost)); $costincindex++; } else { echo ewrpt_ViewValue("0"); } ?>
</td>
			   </tr>
			   <?php
        $agefrom = $ageto;
...
<td<?php echo $sItemRowClass; ?> align="right"><b>
<?php if($totcost > 0){ echo ewrpt_ViewValue(changeprice($totcost)); } else { echo ewrpt_ViewValue("0"); }  ?></b>
</td>
</tr>
