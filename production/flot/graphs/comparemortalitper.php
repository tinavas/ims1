...
<html>
 <head>
    ...
    <title>Mortality % Comparision Graph</title>
    ...
    <table border="0">
     <tr>
      <td colspan="3">Mortality % of <?php echo $culltype; ?> flocks of Supervisor:<?php echo $supervisor; ?> </td>
     </tr> 
     <tr>
      <td>
...
$(function () {
 var d2 = [
        <?php        $i = 0;
	                 ...
	                 while ( $rowfrm = mysql_fetch_assoc($resultfrm))
	                {
	                  if ( $cullflag == 0)
					  {
					  $queryflk = "select distinct flock from broiler_daily_entry where supervisior = '$supervisor' and place = '$rowplc[place]'  and farm = '$rowfrm[farm]' and flock not in (select distinct(flock) from broiler_transferrate) order by flock ";
					  }
					  else
					  {
					  $queryflk = "select distinct flock from broiler_daily_entry where supervisior = '$supervisor' and place = '$rowplc[place]'  and farm = '$rowfrm[farm]' and flock in (select distinct(flock) from broiler_transferrate) order by flock ";
					  }
 ...
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$rowflk[flock]' and warehouse = '$rowfrm[farm]'  and description = 'Broiler Chicks'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $b = $b + $row111['chicks'];  } }
