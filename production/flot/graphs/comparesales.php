		...
		$y1 = substr($year, 2);
          $i = 0;
		   ...
		   $final = 0;$quant12 =0;$quant = 0;
		       ...
		       // $queryplc = "select distinct(invoice),finaltotal,quantity from oc_cobi where code in(select distinct(code) from ims_itemcodes where cat $cg '$category') and m = '$month' and y = '$y1' and party != ''";
				$queryplc = "select finaltotal,totalquantity from oc_cobi where code in(select distinct(code) from ims_itemcodes where cat $cg '$category') and m = '$month' and y = '$y1' and party != '' group by invoice";
  					 $resultplc = mysql_query($queryplc,$conn1);
                     while($rowplc = mysql_fetch_assoc($resultplc))
                     {
					$quant += $rowplc['totalquantity'];
