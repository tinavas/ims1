...
var currentweight = [
 ...
 $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$flock' and warehouse = '$farmer'  and description = 'Broiler Chicks' and date <= '$fromdate'  ";  $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
  if ( $rows > 0) {  while($row111 = mysql_fetch_assoc($result111))  {   $obirds = $obirds + $row111['chicks'];  } }
                $b = $obirds;
		  ...
		  var currentbodyweight = [
					...
					{*/
					 $queryf = "SELECT  MAX(average_weight) as avw FROM broiler_daily_entry WHERE place = '$place' AND farm = '$farmer' AND supervisior = '$supervisor' AND flock = '$flock' AND  age <= '$i'  ";
					/*}
