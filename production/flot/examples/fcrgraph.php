...
<html>
 <head>
    ...
    <title>FCR & B.Weight Graph</title>
    ...
    <table border="0">
     <tr>
	 <td colspan="4" align="center">FCR And Avg.Body Weight Graph</td>
	 </tr>
	 <tr>
...
var currentweight = [
 ...
 //$query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$flock' and warehouse = '$farmer'  and code in (select distinct(code) from ims_itemcodes where cat = 'Broiler Chicks') and date <= '$fromdate'  ";  
  $query111 = "SELECT sum(receivedquantity) as chicks FROM pp_sobi where flock = '$flock' and warehouse = '$farmer'  and code in (select distinct(code) from ims_itemcodes where cat = 'Broiler Chicks')  ";
 $result111 = mysql_query($query111,$conn1);   $rows = mysql_num_rows($result111);
	                ...
	                while($num3 >= 0)
	                 {
					  ...
					  while($rowf = mysql_fetch_assoc($resultf))
	                  {
						...
						while($rowcu = mysql_fetch_assoc($resultc))
						{
						 ...
						 $querycull = "SELECT SUM(birds) as sendbirds from broiler_chickentransfer where farmer = '$farmer' AND flock = '$flock' and date <= '$maxdate'";
						$resultc = mysql_query($querycull,$conn1);
						while($rowcu = mysql_fetch_assoc($resultc))
						{
						   $birdssent = $rowcu['sendbirds'];
						}
						...
						$cull = $rowf['totcull'];
						$remainb = $obirds - $mort - $birdsold - $cull - $birdssent;
						if ( $rowf['cull'] == 1)
						{
							...
							$querysent = "select SUM(weight) as totweight from broiler_chickentransfer where farmer = '$farmer' AND flock = '$flock' and date <= '$maxdate' ";
							$resultsent =  mysql_query($querysent,$conn1);
							while($rowsent = mysql_fetch_assoc($resultsent))
							{
							  $sentwt = $rowsent['totweight'];
							}
							 $salewt = $salewt + $sentwt;
							if($salewt > 0)
							{
  ...
  var plot =   $.plot($("#placeholder"),
           [ { data: stdweight, label: "Standard FCR",color:"#006600" },
		    { data: stdbodyweight, label: "Standard Body Wt(Kgs)",color:"#cc0000"  },
			{ data: currentweight, label: "Actual FCR ",color:"#00ff00"  },
			 { data: currentbodyweight, label: "Actual Body Wt(Kgs)",color:"#ff9999"  }			
             ],
           { 
		     series: {
                       lines: { show: true },
