 ...
 <head>
   ...
   <?php include "reportheader.php"; ?><br /><br />
    <table border="0">
     <tr>
       <td colspan="3">
         Production Summary For Flock <?php echo $c; ?>
       </td>
     </tr>
     <tr>
      <td colspan="3">
...
&nbsp;&nbsp;&nbsp;&nbsp;
        <input type="checkbox" id="reject" value="reject" onclick="change(this.value);" /> Rejections
&nbsp;&nbsp;&nbsp;&nbsp;
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
     ...
     <?php 
           ...
           while($row12 = mysql_fetch_assoc($result12))
           {
		      ...
		      while($qr = mysql_fetch_assoc($qrs))
		      {
				  ...
				  $heggtype = "'dummy'";
                  $q = "select * from ims_itemcodes where cat like 'Hatch Eggs'"; 
      		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		      while($qr = mysql_fetch_assoc($qrs))
		      {
                     $heggtype = $heggtype . ",'" . $qr['code'] . "'";
                  } 
				  ...
				  $inc = mysql_num_rows($resulti);
				   ...
				   $query13a = "SELECT sum(quantity) as 'totalheggs' FROM breeder_production where flock = '$c' and date1 >= '$odate' and date1 <= '$row12[date2]' and itemcode in ($heggtype) ";
                  $result13a = mysql_query($query13a,$conn1); 
                  while($row13a = mysql_fetch_assoc($result13a))
				  {
				 $totalheggs = $row13a['totalheggs']; 
				  }
				  
                    
                     $w = (($totaleggs - $totalheggs) / $totaleggs) * 100;
                     if($w == 0) { $w = $oldw; }
      ?>
                     [<?php echo $nrWeeksPassed; ?>,<?php echo $w; ?>],                             
      <?php 
...
if(document.getElementById("all").checked)
{
  ...
  var plot =   $.plot($("#placeholder"),
           [
             ...
             { data: rejection, label: "Rejection %", color:'#9999FF', yaxis: 2 }],
...
else
{
 ...
 {
            var plot =   $.plot($("#placeholder"),
           [ { data: rejection, label: "Rejection %", color:'#9900FF', yaxis: 2 }],
           { 
             grid: { hoverable: true, clickable: true },
             xaxis: { min: 0,max: 100,
                      tickSize: 2,
