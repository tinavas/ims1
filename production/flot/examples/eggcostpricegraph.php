...
$query1 = "SELECT sum(quantity) as pqty,date1 FROM breeder_production WHERE date1 BETWEEN '$fdump' AND '$tdump' and itemcode in(select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs' and client = '$client') GROUP BY date1 ORDER BY date1";
$result1 = mysql_query($query1,$conn1) or die(mysql_error());
while($rows1 = mysql_fetch_assoc($result1))
{
 $pqty = $rows1['pqty'];
 $date1 = $rows1['date1'];
 $query2 = "SELECT sum(materialcost) as mc,sum(production) as fproduction FROM feed_productionunit WHERE date <= '$date1' AND mash IN (SELECT distinct(code) FROM ims_itemcodes WHERE cat In ('Female Feed','Male Feed'))";
 $result2 = mysql_query($query2,$conn1) or die(mysql_error());
 $rows2 = mysql_fetch_assoc($result2);
 $feedcost = round(($rows2['mc'] / $rows2['fproduction']),2);

 $query3 = "SELECT sum(quantity) as cqty FROM breeder_consumption WHERE date2 = '$date1' and itemcode in(select distinct(code) from ims_itemcodes where cat In ('Female Feed','Male Feed') )";
