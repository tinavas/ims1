...
<a name="top"></a>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="balancesheet_horizontal.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="balancesheet_horizontal.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="balancesheet_horizontal.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
</div></td></tr>
<!-- Top Container (End) -->
...
while($rows = mysql_fetch_assoc($result))
 $drarray[$rows['coacode']] = $rows['dramount'];	
	...
	$quer11 = "select distinct(schedule),type from ac_schedule where  schedule in ( select distinct(schedule) from ac_coa where type = 'Capital' or type = 'Liability' ) order by schedule  ";
	   ...
	   {
	     $coacount = 0;
	      $quer = "select id from ac_financialpostings where coacode in (select distinct(code) from ac_coa where schedule = '$row111[schedule]')  and date <= '$tdate' LIMIT 1 ";
...
<td align="right"><b>
<?php echo ewrpt_ViewValue(changeprice($lbal)); ?></b>
</td>
</tr>   
<tr>
<?php if (@$sExport <> "") { ?>
...
while ( $dumm <= $leftcount )
   {
	...
	<tr>
		...
		<?php  echo ewrpt_ViewValue(changeprice($abal));  ?></b>
		</td>
