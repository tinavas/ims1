...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="eggcostprice.php?<?php echo $url; ?>&export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="eggcostprice.php?<?php echo $url; ?>&export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="eggcostprice.php?<?php echo $url; ?>&export=word">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
$medvacc = 0;
$medvacctotal = 0;
$q4="select sum(quantity) as quantity,itemcode from breeder_consumption where date2='$x_date1' and itemcode in(select distinct(code) from ims_itemcodes where cat In ('Medicines','Vaccines') ) group by itemcode";
...
$medvacctotal += ($rrows['quantity'] * $medvaccrate);
}
}
...
while($rrows=mysql_fetch_assoc($r4))
{
$cgmedvacc+= $rrows['quantity'];

$cgmedvaccitemcode = $rrows['itemcode'];
...
while($res = mysql_fetch_assoc($res))
{
$cgmedvaccmode = $res['mode']; 

$cgmedvaccrate = calculate($cgmedvaccmode,$cgmedvaccitemcode,$x_date1,$cgmedvacc);

$cgmedvacctotal += ($rrows['quantity'] * $cgmedvaccrate);
}
}
...
$totalcgprice += ($days1 + $cgmedvacctotal);
...
?>
<td<?php echo $sItemRowClass; ?> align="right"><?php  $totqty += $days1; echo changeprice(round(($days1),2));  ?></td>
...
<td<?php echo $sItemRowClass; ?> align="right"><?php if($cgmedvacc <> '') { ?><a href="details1.php?date=<?php echo $date; ?>&production=<?php echo $productionflocks; ?>" target="_blank"><?php echo changeprice(round($cgmedvacc,2)); $totalcgfeedmedvacc += $cgmedvacc;  ?> </a> <?php } else { echo '0.00'; } ?></td>
...
<td<?php echo $sItemRowClass; ?> align="right"><b><?php echo changeprice(round($totqty,2)); ?></b></td>
