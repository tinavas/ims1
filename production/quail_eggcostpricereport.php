...
define("EW_REPORT_TABLE_VAR", "quail_eggcostpricereport", TRUE);
define("EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE", "quail_eggcostpricereport_grpperpage", TRUE);
define("EW_REPORT_TABLE_SESSION_START_GROUP", "quail_eggcostpricereport_start", TRUE);
define("EW_REPORT_TABLE_SESSION_SEARCH", "quail_eggcostpricereport_search", TRUE);
define("EW_REPORT_TABLE_SESSION_CHILD_USER_ID", "quail_eggcostpricereport_childuserid", TRUE);
define("EW_REPORT_TABLE_SESSION_ORDER_BY", "quail_eggcostpricereport_orderby", TRUE);

// Table level SQL
$EW_REPORT_TABLE_SQL_FROM = "`quail_production`";
$EW_REPORT_TABLE_SQL_SELECT = "SELECT quail_production.date1,sum(quail_production.quantity) as quantity, sum(quail_production.price) as price FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_WHERE = "date1 between '$fdate' and '$tdate' and client = '$client' and itemcode in(select distinct(code) from ims_itemcodes where cat = 'Quail Hatch Eggs' and client = '$client')";
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="quail_eggcostpricereport.php?<?php echo $url; ?>&export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="quail_eggcostpricereport.php?<?php echo $url; ?>&export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="quail_eggcostpricereport.php?<?php echo $url; ?>&export=word">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
$qu="SELECT sum(receivedquantity * rateperunit)/sum(receivedquantity) as rate FROM pp_sobi WHERE code IN (SELECT distinct(code) FROM ims_itemcodes WHERE cat In ('Quail Female Feed','Quail Male Feed')) and date <='$x_date1'";
$re1=mysql_query($qu,$conn1);
$re1=mysql_fetch_assoc($re1);
$feedcost=$re1['rate'];

$q="select sum(quantity) as quantity from quail_consumption where date2='$x_date1' and itemcode in(select distinct(code) from ims_itemcodes where cat In ('Quail Female Feed','Quail Male Feed') ) ";
...
$dd = $x_date1;
$days1 = 0;
$qury1 = "select sum(dayspan) as dayamount from quail_unitwisemaintaincecost where fromdate <= '$dd' and todate >= '$dd'";
...
?>
		...
		<td<?php echo $sItemRowClass; ?> align="right">
<?php echo changeprice(round(($tp/$x_quantity),2)); ?>
</td>
	</tr>
<?php
...
<td <?php echo $sItemRowClass; ?> align="right"><b> <?php echo changeprice(round((($totalfeed * $feedcost)/$totaleggs),2)); ?></b></td></tr>
	</tbody>
...
function getSort()
{
	...
	if (strlen(@$_GET["cmd"]) > 0) {
		...
		if ($sCmd == "resetsort") {
			...
			$_SESSION["sort_quail_eggcostpricereport_date1"] = "";
			$_SESSION["sort_quail_eggcostpricereport_quantity"] = "";
			$_SESSION["sort_quail_eggcostpricereport_price"] = "";
		}
