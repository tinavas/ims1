...
if($warehouse!='' && $warehouse!='All') $cond="and warehouse='$warehouse'";
}
$url = "&fromdate=" . $_GET['fromdate'] . "&todate=" . $_GET['todate']. "&code=" . $_GET['code']. "&desc=" . $_GET['desc']."&warehouse=".$_GET['warehouse']. "&schedule=" . $_GET['schedule'];
?>
<?php
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Expense Ledger</font></strong></td>
</tr>
<?php if($_GET['code'] <> "All") { ?>
<tr>
<td colspan="2" align="center"><strong>COA Code:</strong>&nbsp;<?php if($coa=="") {$coa="All";} echo $coa; ?></td>
</tr>
<tr>
<td colspan="2" align="center"><strong>Description:</strong>&nbsp;<?php if($coa=="" || $coa="All") {$desc="All";} echo $desc;?></td>
</tr>
<?php } ?>
<tr>
<td colspan="2" align="center"><strong><font size="2">From Date&nbsp;<?php echo date($datephp,strtotime($fdatedump)); ?>
                To Date &nbsp;<?php echo date($datephp,strtotime($tdatedump));?></font></strong></td>
</tr>
<tr>
<td align="center"><font size="2"><strong>Warehouse&nbsp;:</strong><?php echo $warehouse; ?></font></td>
</tr>
</table>
<?php 
if($currencyunits == "")
...
<?php if ( $oflag == "Cr" ) { echo ewrpt_ViewValue(changeprice1($obal)); $crtotal = $crtotal + $obal; } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
	...
	$nxtdate = "";
	 ...
	 include "config.php";
       ...
       {
         $coalist = "'dummy'";
         $aq = "select * from ac_coa where schedule = '$shed' and client = '$client' order by code ";
   	   $aqrs = mysql_query($aq,$conn1) or die(mysql_error());
	   while($aqr = mysql_fetch_assoc($aqrs))
	   {
           $coalist = $coalist . ",'" . $aqr['code'] . "'";
         }
       $query1 = "SELECT * from ac_financialpostings where coacode in ($coalist) and client = '$client'  and date >= '$fdate' and date <= '$tdate' $cond order by date,trnum ";
       }
       else
       {
         $query1 = "SELECT * from ac_financialpostings where coacode = '$coa' and client = '$client'  and date >= '$fdate' and date <= '$tdate' $cond  order by date,trnum ";
       }
...
?>
	<tr>
		...
		<?php echo ewrpt_ViewValue(date($datephp,strtotime($row2['date']))) ?>
            ...
            <?php if($coa == "All") { ?>
	      <td class="ewRptGrpField1">
		<?php echo $row2['coacode']; ?>
		</td>		
            <td class="ewRptGrpField1">
		<?php 
              $aaq = "select * from ac_coa where code = '$row2[coacode]' and client = '$client' order by code ";
         	  $aaqrs = mysql_query($aaq,$conn1) or die(mysql_error());
	        while($aaqr = mysql_fetch_assoc($aaqrs))
	        {             
                  $coadesc = $aaqr['description'];
              }
              echo $coadesc; 
            ?>
		</td>
		...
		if ( $row2['type'] == "GR" )
		{
		   $type = "GR-Goods Receipt";
		   $query11 = "select * from pp_goodsreceipt where gr = '$row2[trnum]' and client = '$client'  ";
		...
		{
		  $type = "SOBI-Supplier Order Based Invoice";
		   $query11 = "select * from pp_sobi where so = '$row2[trnum]' and client = '$client'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['remarks'];
		...
		{
		   $query11 = "select * from breeder_consumption where flock = '$row2[trnum]' and client = '$client' and date2 = '$row2[date]' ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = "";
		...
		{
		  $type = "COBI-Customer Order Based Invoice";
		   $query11 = "select * from oc_cobi where invoice = '$row2[trnum]' and client = '$client'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = "";
		...
		{
		  $type = "PS-Pack Slip";
		  $query11 = "select * from oc_packslip where ps = '$row2[trnum]' and client = '$client'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = "";
		...
		{
		  $type = "RV-Receipt Voucher";
		  $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'R' and client = '$client'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['rremarks'];
		...
		else if ( $row2['type'] == "PV")
		{
		  ...
		  $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and code = '$row2[coacode]' and voucher = 'P' and client = '$client'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['rremarks'];
		...
		else if ( $row2['type'] == "Journal")
		{
		  ...
		  $query11 = "select * from ac_gl where transactioncode = '$row2[trnum]' and voucher = 'J' and client = '$client'  ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		     $narr = $row21['rremarks'];
		...
		{
		  $type = "PMT-Payment";
		  $query11 = "select * from pp_payment where tid = '$row2[trnum]' and client = '$client'   ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		...
		{
		  $type = "RCT-Receipt";
		  $query11 = "select * from oc_receipt where tid = '$row2[trnum]' and client = '$client'   ";
           $result11 = mysql_query($query11,$conn1);
	       while($row21 = mysql_fetch_assoc($result11))
           {
		...
		{
		  $type = "VCN-Vendor Credit Note";
		}
		else if ( $row2['type'] == "VDN")
		{
		  $type = "VDN-Vendor Debit Note";
		}
		else if ( $row2['type'] == "CCN")
		{
		  $type = "CCN-Customer Credit Note";
		}
		else if ( $row2['type'] == "CDN")
		{
		  $type = "CDN-Customer Debit Note";
		}
