...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Chicks Supply Report</font></strong></td>
</tr>
<tr>
 <td><strong><font color="#3e3276">From : </font></strong><?php echo date($datephp,strtotime($fdate)); ?>&nbsp;&nbsp;<strong><font color="#3e3276">To : </font></strong><?php echo date($datephp,strtotime($tdate)); ?></td>
</tr>
</table>
<?php
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="chicksupply.php?fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="chicksupply.php?fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="chicksupply.php?fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="chicksupply.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<br /><br />
<?php } ?>
...
<table border="0" cellspacing="0" cellpadding="0">
	<tr>
<td> From Date <input type="text" class="datepicker" id="fdate" size="11" value="<?php echo date($datephp,strtotime($fdate)) ?>" onchange="reloadpage()" name="fdate"  />
</td>

<td> To Date <input type="text" class="datepicker" id="tdate" size="11" value="<?php echo date($datephp,strtotime($tdate)) ?>" onchange="reloadpage()" name="tdate"  />
</td>
...
//	echo "sql: " . $sSql . "<br>";
	...
	if (1) { // Loop detail records
		...
		echo $qc = "Select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%' or cat like '%Layer%Chicks%'";
		else
		$qc = "Select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%'";		
		...
		if($_SESSION['db'] == "central")
		{
$query = "SELECT * FROM ims_itemcodes WHERE cat like '%Broiler Chicks%' or cat like '%Layer Chicks%'";
}
else
{
$query = "SELECT * FROM ims_itemcodes WHERE cat like '%Broiler Chicks%'";
}
$result = mysql_query($query,$conn1); 
while($row1 = mysql_fetch_assoc($result))
{ 
  $codelist = $codelist . ",'" . $row1['code'] . "'";
}
		
		$d1 = 0;$d2=0;
		if($_SESSION['db'] == "central")
		  $qd1 = "SELECT distinct(date) FROM `ims_stocktransfer` where code IN (select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%' or cat like '%Layer Chicks%') and date >= '$fdate' and date <= '$tdate' order by date asc ";
		else
		 $qd1 = "SELECT distinct(date) FROM `ims_stocktransfer` where code IN (select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%') and date >= '$fdate' and date <= '$tdate' order by date asc ";
		...
		if($_SESSION['db'] == "central")
		$qd2 = "SELECT distinct(date) FROM `oc_cobi` where code IN (select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%' or cat like '%Layer Chicks%') and date >= '$fdate' and date <= '$tdate' order by date asc ";
		else
		$qd2 = "SELECT distinct(date) FROM `oc_cobi` where code IN (select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%') and date >= '$fdate' and date <= '$tdate' order by date asc ";
		...
		if($d1 == 0) 	//Reason: If first array is empty, then array_merge is not working.
		 $datearr1[0] = $datearr2[d2-1]; 
		if($d2 == 0)	//Reason: If second array is empty, then array_merge is not working.
		 $datearr2[0] = $datearr1[d1-1];
		...
		{
		$datecmp = $datearru[$i];
		$datec = date($datephp,strtotime($datecmp));
		if($_SESSION['db'] == "central")
		$qr = "Select * FROM `ims_stocktransfer` where code IN (Select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%' or cat like '%Layer Chicks%') and date ='$datecmp'";
		else
		$qr = "Select * FROM `ims_stocktransfer` where code IN (Select distinct(code) from ims_itemcodes where cat like '%Broiler Chicks%') and date ='$datecmp'";
		$qrrs = mysql_query($qr,$conn1) or die(mysql_error());
		while($qrr = mysql_fetch_assoc($qrrs))
		{
		?>
		<tr>
		<td class="ewRptGrpField1"><?php //echo ewrpt_ViewValue($date("d-m-Y",strtotime($datecmp))); 
		echo ewrpt_ViewValue($datec);?></td>
		<td class="ewRptGrpField3"><?php echo ewrpt_ViewValue($qrr['tmno']); ?></td>
		<td<?php echo $sItemRowClass; ?>><?php echo ewrpt_ViewValue($typet); ?></td>
<?php if($_SESSION['db'] == "central") { ?>	<td<?php echo $sItemRowClass; ?>><?php echo ewrpt_ViewValue($qrr['cat']); ?></td><?php } ?>
		<td<?php echo $sItemRowClass; ?>><?php echo ewrpt_ViewValue($qrr['aflock']); ?></td>
        <td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue(changequantity($qrr['quantity'])); ?></td>
		<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($qrr['tmort']); ?></td>
		<td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($qrr['shortage']); ?></td>
        <td<?php echo $sItemRowClass; ?> align="right"><?php echo ewrpt_ViewValue($na); ?></td>
		</tr>
		<?php
		}   
		$qr1 = "Select * FROM `oc_cobi` where code in ($codelist) and date ='$datecmp'";
		...
		<td class="ewRptGrpField3"><?php echo ewrpt_ViewValue($qrr1['bookinvoice']); ?></td>
		<td<?php echo $sItemRowClass; ?>><?php echo $types; ?></td>
		<?php if($_SESSION['db'] == "central") { ?>	<td<?php echo $sItemRowClass; ?>>&nbsp;</td><?php } ?>
		<td<?php echo $sItemRowClass; ?>><?php echo $qrr1['party']; ?></td>
        <td<?php echo $sItemRowClass; ?> align="right"><?php echo changequantity($qrr1['quantity']); ?></td>
		<td<?php echo $sItemRowClass; ?> align="right"><?php echo $z; ?></td>
		<td<?php echo $sItemRowClass; ?> align="right"><?php echo $z; ?></td>
        <td<?php echo $sItemRowClass; ?> align="right"><?php echo $qrr1['price']; ?></td>
		</tr>
		<?php
		} 
		/*$qr1 = "Select * FROM `oc_cobi` where code='$code' and date ='$datecmp'";
		$qrrs1 = mysql_query($qr1,$conn1) or die(mysql_error());
		while($qrr1 = mysql_fetch_assoc($qrrs1))
		{
		?>
		<tr>
		<td ><?php echo $date("d-m-Y",strtotime($datecmp)); ?></td>
		<td ><?php echo $qrr1['bookinvoice']; ?></td>
		<td><?php echo $types; ?></td>
		<td><?php echo $qrr1['party']; ?></td>
        <td align="right"><?php echo $qrr1['quantity']; ?></td>
		<td align="right"><?php echo $z; ?></td>
		<td align="right"><?php echo $z; ?></td>
        <td align="right"><?php echo $qrr1['price']; ?></td>
		</tr>
		<?php
		} */
		}
...
<form action="chicksupply.php" name="ewpagerform" id="ewpagerform" class="ewForm">
<table border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td nowrap>
...
<!--first page button-->
	...
	<td><a href="chicksupply.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="chicksupply.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="chicksupply.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="chicksupply.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function reloadpage()
{
	...
	document.location = 'chicksupply.php?fdate='+fdate +'&tdate='+tdate;
