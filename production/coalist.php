...
// Table level SQL
$EW_REPORT_TABLE_SQL_FROM = "`ac_coa`";
$EW_REPORT_TABLE_SQL_SELECT = "SELECT * FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_WHERE = "";
$EW_REPORT_TABLE_SQL_GROUPBY = "";
$EW_REPORT_TABLE_SQL_HAVING = "";
$EW_REPORT_TABLE_SQL_ORDERBY = "`type` ASC, `schedule` ASC, `controltype` ASC";
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">COA List</font></strong></td>
</tr>
</table>
<br/>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="coalist.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="coalist.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="coalist.php?export=word">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php
	...
	// Build detail SQL
	//$sWhere = EW_REPORT_TABLE_FIRST_GROUP_FIELD . " = " . ewrpt_QuotedValue($x_type, EW_REPORT_DATATYPE_STRING);

	$sWhere = ewrpt_DetailFilterSQL(EW_REPORT_TABLE_FIRST_GROUP_FIELD, $x_type, EW_REPORT_DATATYPE_STRING, $gb_type, $gi_type, $gq_type);
	if ($sFilter != "")
...
//	echo "sql: " . $sSql . "<br>";
	...
	while ($rs && !$rs->EOF) { // Loop detail records
		...
		if ((is_null($x_type) && is_null($o_type)) ||
			(($x_type <> "" && $o_type == $dg_type) && !ChkLvlBreak(1))) {
			$dg_type = "&nbsp;";
		...
		$dg_schedule = $x_schedule;
		if ((is_null($x_schedule) && is_null($o_schedule)) ||
			(($x_schedule <> "" && $o_schedule == $dg_schedule) && !ChkLvlBreak(2))) {
			$dg_schedule = "&nbsp;";
		} elseif (is_null($x_schedule)) {
			$dg_schedule = EW_REPORT_NULL_LABEL;
		} elseif ($x_schedule == "") {
			$dg_schedule = EW_REPORT_EMPTY_LABEL;
		}
		$dg_controltype = $x_controltype;
		if ((is_null($x_controltype) && is_null($o_controltype)) ||
			(($x_controltype <> "" && $o_controltype == $dg_controltype) && !ChkLvlBreak(3))) {
			$dg_controltype = "&nbsp;";
		} elseif (is_null($x_controltype)) {
			$dg_controltype = EW_REPORT_NULL_LABEL;
		} elseif ($x_controltype == "") {
			$dg_controltype = EW_REPORT_EMPTY_LABEL;
		}
?>
	<tr>
...
<?php echo ewrpt_ViewValue($x_type) ?>
		...
		<?php $t_schedule = $x_schedule; $x_schedule = $dg_schedule; ?>
<?php echo ewrpt_ViewValue($x_schedule) ?>
		<?php $x_schedule = $t_schedule; ?></td>
		<td class="ewRptGrpField3">
		<?php $t_controltype = $x_controltype; $x_controltype = $dg_controltype; ?>
<?php echo ewrpt_ViewValue($x_controltype) ?>
		<?php $x_controltype = $t_controltype; ?></td>
		<td<?php echo $sItemRowClass; ?>>
<b><?php echo ewrpt_ViewValue($x_code) ?></b>
</td>
		<td<?php echo $sItemRowClass; ?>>
...
<?php
		...
		$o_type = $x_type;
		$o_schedule = $x_schedule;
		$o_controltype = $x_controltype;
