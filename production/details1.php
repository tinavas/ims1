...
$q8 = "select sum(quantity) as quantity,itemcode from breeder_consumption  where date2 = '$date' and itemcode in(select distinct(code) from ims_itemcodes where description <> 'chick' and description <> 'grower' and cat in('Medicines','Vaccines')) and flock in ($production) group by itemcode";
