...
define("EW_REPORT_TABLE_VAR", "chargealert", TRUE);
define("EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE", "chargealert_grpperpage", TRUE);
define("EW_REPORT_TABLE_SESSION_START_GROUP", "chargealert_start", TRUE);
define("EW_REPORT_TABLE_SESSION_SEARCH", "chargealert_search", TRUE);
define("EW_REPORT_TABLE_SESSION_CHILD_USER_ID", "chargealert_childuserid", TRUE);
define("EW_REPORT_TABLE_SESSION_ORDER_BY", "chargealert_orderby", TRUE);

// Table level SQL
$EW_REPORT_TABLE_SQL_FROM = "vehicle_master";
$EW_REPORT_TABLE_SQL_SELECT = "SELECT vehicle_master.unit, vehicle_master.vtype, vehicle_master.vnumber FROM " . $EW_REPORT_TABLE_SQL_FROM;
...
ewrpt_CreatePopup("chargealert_unit", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_vtype, $sel_vtype, $ft_vtype) ?>
ewrpt_CreatePopup("chargealert_vtype", [<?php echo $jsdata ?>]);
<?php $jsdata = ewrpt_GetJsData($val_vnumber, $sel_vnumber, $ft_vnumber) ?>
ewrpt_CreatePopup("chargealert_vnumber", [<?php echo $jsdata ?>]);
</script>
<div id="chargealert_unit_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="chargealert_vtype_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<div id="chargealert_vnumber_Popup" class="ewPopup">
<span class="phpreportmaker"></span>
</div>
<?php } ?>
<?php if (@$sExport == "") { ?>
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Charge Alerts<br /> </font></strong></td>
</tr>
</table>
<br/>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="chargealertsmry.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="chargealertsmry.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="chargealertsmry.php?export=word">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="chargealertsmry.php?cmd=reset">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<form action="chargealertsmry.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="chargealertsmry.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="chargealertsmry.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="chargealertsmry.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="chargealertsmry.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			...
			<td>Unit</td>
			<td style="width: 20px;" align="right"><?php /*?><a href="#" onClick="ewrpt_ShowPopup(this.name, 'chargealert_unit', false, '<?php echo $rf_unit; ?>', '<?php echo $rt_unit; ?>');return false;" name="x_unit<?php echo $cnt[0][0]; ?>" id="x_unit<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a><?php */?></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Vehicle Type</td>
			<td style="width: 20px;" align="right"><?php /*?><a href="#" onClick="ewrpt_ShowPopup(this.name, 'chargealert_vtype', false, '<?php echo $rf_vtype; ?>', '<?php echo $rt_vtype; ?>');return false;" name="x_vtype<?php echo $cnt[0][0]; ?>" id="x_vtype<?php echo $cnt[0][0]; ?>"><img src="phprptimages/popup.gif" width="15" height="14" align="texttop" border="0" alt=""></a><?php */?></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
//	echo "sql: " . $sSql . "<br>";
	...
	while ($rs && !$rs->EOF) { // Loop detail records
		...
		$q1 = "select * from vehicle_renewal where vehicletype = '$type' and vehiclenumber = '$number' ";
		$r1 = mysql_query($q1,$conn1);
		while($qr1 = mysql_fetch_assoc($r1))
		{
		$date = strtotime($qr1['date']);
		
		$vtime = $qr1['validtime'] *24*60*60;
		$edate = $date + $vtime;
		$edate1 = date("d.m.Y",($edate));
		$ccode=$qr1['chargecode'];
		
		?>
			<tr>
		<td<?php echo $sItemRowClass; ?>>
<?php if(($unit <> $oldcode) or ($dumm == 0)){ echo ewrpt_ViewValue($unit);
     $oldcode = $unit; $dumm = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
		...
		<td<?php echo $sItemRowClass; ?>>
<?php if(($type <> $oldcode1) or ($dumm1 == 0)){ echo ewrpt_ViewValue($type);
     $oldcode1 = $type; $dumm1 = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
	 } ?>
		...
		<td<?php echo $sItemRowClass; ?>>
<?php if(($number <> $oldcode2) or ($dumm2 == 0)){ echo ewrpt_ViewValue($number);
     $oldcode2 = $number; $dumm2 = 1; }
	 else{
	 echo ewrpt_ViewValue("&nbsp;");
	 } ?>
		...
		<td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue($ccode) ?>
</td>
<td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue($edate1) ?>
</td>
	</tr>
...
function SetupPopup() {
	...
	if (is_array(@$_SESSION["sel_chargealert_unit"])) {
		LoadSelectionFromSession('unit');
	} elseif (@$_SESSION["sel_chargealert_unit"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_unit"] = "";
	}

	// Get Vtype selected values
	if (is_array(@$_SESSION["sel_chargealert_vtype"])) {
		LoadSelectionFromSession('vtype');
	} elseif (@$_SESSION["sel_chargealert_vtype"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_vtype"] = "";
	}

	// Get Vnumber selected values
	if (is_array(@$_SESSION["sel_chargealert_vnumber"])) {
		LoadSelectionFromSession('vnumber');
	} elseif (@$_SESSION["sel_chargealert_vnumber"] == EW_REPORT_INIT_VALUE) { // Select all
		$GLOBALS["sel_vnumber"] = "";
	}
}
...
function ClearSessionSelection($parm) {
	$_SESSION["sel_chargealert_$parm"] = "";
	$_SESSION["rf_chargealert_$parm"] = "";
	$_SESSION["rt_chargealert_$parm"] = "";
}

// Load selection from session
function LoadSelectionFromSession($parm) {
	$GLOBALS["sel_$parm"] = @$_SESSION["sel_chargealert_$parm"];
	$GLOBALS["rf_$parm"] = @$_SESSION["rf_chargealert_$parm"];
	$GLOBALS["rt_$parm"] = @$_SESSION["rt_chargealert_$parm"];
}
...
function getSort()
{
	...
	if (strlen(@$_GET["cmd"]) > 0) {
		...
		if ($sCmd == "resetsort") {
			...
			$_SESSION["sort_chargealert_unit"] = "";
			$_SESSION["sort_chargealert_vtype"] = "";
			$_SESSION["sort_chargealert_vnumber"] = "";
		}
