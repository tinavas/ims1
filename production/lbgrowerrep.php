...
include "config.php";
 ...
 echo $q = "select * from layerbreeder_flock where flockcode = '$flock' and client = '$client'  "; 
...
if($flkbreed != "")
{
$agequery = "select max(age) as age,min(age) as minage from layerbreeder_standards where breed='$flkbreed' and client='$client'";
}
else
{
$agequery = "select max(age) as age,min(age) as minage from layerbreeder_standards where client='$client'";
}
...
$maxdatec = "";
   $query1 = "SELECT min(date2) as mindate,max(date2) as maxdate,min(age) as minage,max(age) as maxage from layerbreeder_consumption where flock = '$flock' and client = '$client'  ";
	  ...
	  if($mindatep <> "")
	  {
	  echo $query1 = "SELECT age from layerbreeder_consumption where date2 = '$mindatep' and flock = '$flock' and client = '$client'  ";
      $result1 = mysql_query($query1,$conn1);
	  while($row2 = mysql_fetch_assoc($result1))
      {
	    echo $maxage = $row2['age'];
	   }
	  }
     $maxage = $maxage - 1;
	  $startweek = $minage/7;
...
<?php
  ...
  $mopening=0;
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
		 {
             ...
             $minus = 0; 
             $q = "select distinct(date2),fmort,fcull,mmort,mcull from layerbreeder_consumption where flock = '$flock' and client = '$client'  and date2 < '$mindatec' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
             ...
             $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'LB Female Birds' and client = '$client'  and fromwarehouse = '$flock' and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		   ...
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             else
             {
			   ...
			   $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'LB Male Birds' and client = '$client'  and fromwarehouse = '$flock' and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
		   ...
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'LB Female Birds' and client = '$client'  and towarehouse = '$flock' and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
             ...
             else
             {
			  ...
			  $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'LB Male Birds' and client = '$client'  and towarehouse = '$flock' and date < '$mindatec'"; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs))
             {
		   while($qr = mysql_fetch_assoc($qrs))
			 ...
			 if($_SESSION['db']=='shine' or $_SESSION['db']=='mohanraj')
			 {
			 	 $q11 = "select sum(receivedquantity) as qt from pp_sobi where  code in (select distinct(code) from ims_itemcodes where cat = 'Birds' and client = '$client' ) and warehouse='$flock'"; 
    		 $qrs11 = mysql_query($q11,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrs11))
             {
		   while($qr11 = mysql_fetch_assoc($qrs11))
    		   {
                 $ttransferpur = $qr11['qt'];
               }
             }
             else
             {
             ...
             $remaining = ($fopening - $minus - $ftransfer + $ttransfer+ $ttransferpur) - $tsale;
			 $remaining1= ($mopening - $minus1 - $mtransfer + $mttransfer+$ttransferpur) - $tsale;
			 $nxtbirds = $remaining;
			 $nxtbirds1=$remaining1;
?>
<?php
$minwkdate = "";
$maxwkdate = "";
 $query1 = "SELECT  min(date2) as minwkdate,max(date2) as maxwkdate from layerbreeder_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' ";
     ...
     /* $query1 = "SELECT min(age) as minage,female from breeder_initial where flock = '$flock' and client = '$client' and eggs='' ";
      $result1 = mysql_query($query1,$conn1);
	  while($row2 = mysql_fetch_assoc($result1))
      {
	    $initage = $row2['minage'];
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="lbgrowerrep.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="lbgrowerrep.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="lbgrowerrep.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
//echo $agefrom."/".$maxage."/".$initage;
if(1) {$bShowFirstHeader = true;} else {$bShowFirstHeader = false;}
...
<?php } else { ?>
		<td class="ewTableHeader" colspan="2">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Transfer-In"><tr>
			<td >In</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php } else { ?>
		<td class="ewTableHeader" colspan="2">
			<table cellspacing="0" class="ewTableHeaderBtn" title="Transfer-Out" align="center"><tr>
			<td >Out</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader" colspan="8" >
		  
		</td>
<?php } else { ?>
		<td class="ewTableHeader" colspan="8">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td> </td>
			</tr></table>
		</td>
...
<?php
	 $count = 0;
	  ...
	  $nowbirds2=$initmale;$nxtbirds2 = $initmale;
	   $query1 = "SELECT * from layerbreeder_initial where flock = '$flock' and client = '$client'  and eggs ='' order by age asc  ";
      $result1 = mysql_query($query1,$conn1);
	  while($row2 = mysql_fetch_assoc($result1))
      {
	  $agewk = $row2['age'];
	  ...
	  if($flkbreed != "")
	  {
	  $q = "select * from layerbreeder_standards  where breed = '$flkbreed' and client = '$client' and age = '$agewkstd' "; 
	  }
	  else
	  {
	   $q = "select * from layerbreeder_standards  where client = '$client' and age = '$agewkstd' "; 
	  }
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
              while($qr = mysql_fetch_assoc($qrs))
    		   {
		...
		$nxtlive2= $nxtbirds2-$mmort -$mcull;
		$actliv = round((($nxtlive/$nowbirds1) * 100),2);
		$actliv2=round((($nxtlive2/$nowbirds2) *100),2);
        $nxtbirds1 = $nxtbirds1 - $wmort - $wcull - $wtrout + $wtrin ;
		$nxtbirds2=  $nxtbirds2 -$mmort -$mcull -$mtrout +$mtrin;
		$totmort = $totmort + $wmort;
	    $totcull = $totcull + $wcull;
		$mtotmort=$mtotmort+$mmort;
		$mtotcull=$mtotcull+$mcull;
	    $totfeedkgstd = $totfeedkgstd + round((($feedstd * $nowbirds1 * 7)/1000),2);
		$totfeedkgstd1= $totfeedkgstd1 + round((($feedstd * $nowbirds2 * 7)/1000),2);
		...
		$feedstd = round((($feedstd * 1000)/7),2);*/
			  ...
			  // $wfeedkg = round((($wfeedkg * 1000)/7),2);
			   ...
			   $mtottrin=$mtottrin+$mtrin;
			   $mtottrout=$mtottrout+$mtrout;
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($nowbirds2); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">

<?php if($mmort > 0){ echo ewrpt_ViewValue($mmort); } else { echo ewrpt_ViewValue("0"); } ?>
</td>
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($mcull > 0){ echo ewrpt_ViewValue($mcull); } else { echo ewrpt_ViewValue("0");  } ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
...
while($agefrom <= $maxage )
{
  ...
  $query1 = "SELECT  distinct(date2),fmort,fcull,mmort,mcull from layerbreeder_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' ";
	...
	$query1 = "SELECT  sum(quantity) as quant,max(avgwt) as bdwt from layerbreeder_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' and itemcode in (select code from ims_itemcodes where cat = 'Feed' and client = '$client') ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	    $wfeed = $row2['quant'];
	  ...
	  $query1 = "SELECT  itemcode,sum(quantity) as quant,max(date2) as date from layerbreeder_consumption where client = '$client' and flock = '$flock' and age >= '$agefrom' and age < '$ageto' and itemcode in (select code from ims_itemcodes where cat = 'Feed' and client = '$client') group by itemcode ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	  
	    if($_SESSION['db'] == "sathireddy")
		$q = "select cost from feed_dailyfeedcost where feedtype = '$row2[itemcode]' and date <= '$row2[date]' and client = '$client' order by date desc limit 1";
		else
	   ...
	   $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat ='LB Female Birds' and client = '$client'  and fromwarehouse = '$flock' and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
                 $wtrout = $qr['quantity'];
               }
        $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'LB Female Birds' and client = '$client'  and towarehouse = '$flock' and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
               $wtrin = $qr['quantity'];
			   ...
			   $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat ='LB Male Birds' and client = '$client'  and fromwarehouse = '$flock' and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
                 $mtrout = $qr['quantity'];
               }
        $q = "select sum(quantity) as 'quantity' from ims_stocktransfer where cat = 'LB Male Birds' and client = '$client'  and towarehouse = '$flock' and date < '$maxwkdate' and date >= '$minwkdate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
             while($qr = mysql_fetch_assoc($qrs))
    		   {
               $mtrin = $qr['quantity'];
			   ...
			   $agewk = ceil($agefrom/7);
			    $nowbirds1= $nxtbirds1;
				$nowbirds2=$nxtbirds2;
				$nbirds=$nxtbirds1+$nxtbirds2;
			   $nxtlive =$nbirds-$wmort - $wcull-$mmort -$mcull;
			   $actliv = round((($nxtlive/$nbirds) * 100),2);
               $nxtbirds1 = $nxtbirds1 - $wmort - $wcull - $wtrout + $wtrin - $wsale;
			   $nxtbirds2= $nxtbirds2 -$mmort -$mcull -$mtrout + $mtrin - $wsale;
			if($flkbreed != "")
			{
			 if($agewk > $maxagee)
			 $q = "select * from layerbreeder_standards  where breed = '$flkbreed' and client = '$client' and age = '$maxagee' "; 
			 else
			 $q = "select * from layerbreeder_standards  where breed = '$flkbreed' and client = '$client' and age = '$agewk' "; 
			}
			else
			{
			 if($agewk > $maxagee)
			 $q = "select * from layerbreeder_standards  where client = '$client' and age = '$maxagee' "; 
			 else
			 $q = "select * from layerbreeder_standards  where client = '$client' and age = '$agewk' "; 
			 }
    		 ...
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
			  ...
			  $wkfeedstd = $feedstd;
			   ...
			   $mfeedkg =$mfeed;
			   $wfeed = $wfeed/$nowbirds1;
			   $mfeed= $mfeed/$nowbirds2;
			   $wfeed = round((($wfeed * 1000)/7),2);
			   $mfeed = round((($mfeed * 1000)/7),2);
			   $tottrin = $tottrin + $wtrin;
			   $tottrout = $tottrout + $wtrout;
			   $mtottrin=$mtottrin + $mwtrin;
			   $mtottrout=$mtottrout+$mwtrout;
			   $totsale = $totsale + $wsale;
			   $totfeed = $totfeed +  $wfeed;
			   $totfeedkg = $totfeedkg + $wfeedkg;
			   $totfeedkgstd = $totfeedkgstd + round((($feedstd * $nowbirds1 * 7)/1000),2);
...
<td<?php echo $sItemRowClass; ?> align="right" colspan="2">
...
$remain= $remaining + $remaining1;
$totfeedstd = round(($totfeedstd/$count),2);
$totfeed = round(($totfeed/$count),2);
$finprodper = round(((($toteggs/$remain) * 100)/$span),2);
$finstdprodper = round((($hecumstd * 100)/$span),2);
$fineggpb = round((($totfeedkg/$toteggs)*1000),2);
$finallive = $remain - ($totmort + $totcull+$mtotmort+$mtotcull);
$finallive = round((($finallive/$remain) * 100),2);
$totfeedkg = $totfeedkg;
