...
<tr>
<td style="text-align:center" colspan="2"><font color="#3e3276"><strong>Sales Cost Analysis <br /> From <?php echo date($datephp,strtotime($fromdate));?> To <?php echo date($datephp,strtotime($todate));?><br />Category :: <?php echo $category;?></strong></font></td>
</tr>
</table>
<?php 
if($currencyunits == "")
...
<center>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="salescostanalysis.php?export=html&category=<?php echo $category;?>&fromdate=<?php echo $fromdate; ?>&todate=<?php echo $todate; ?>&customer=<?php echo $customer;?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="salescostanalysis.php?export=excel&category=<?php echo $category;?>&fromdate=<?php echo $fromdate;?>&todate=<?php echo $todate; ?>&customer=<?php echo $customer;?>">Export to Excel</a>
&nbsp;&nbsp;<a href="salescostanalysis.php?export=word&category=<?php echo $category;?>&fromdate=<?php echo $fromdate;?>&todate=<?php echo $todate; ?>&customer=<?php echo $customer;?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="salescostanalysis.php?cmd=reset&category=<?php echo $category;?>&fromdate=<?php echo $fromdate;?>&todate=<?php echo $todate; ?>&customer=<?php echo $customer;?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<?php if (@$sExport == "") { ?>
</center>
...
<form action="salescostanalysis.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
&nbsp;&nbsp;&nbsp;
Customer&nbsp;&nbsp;
	<select id="customer" onchange=" <?php if($_SESSION[db]=='albustan' || $_SESSION[db]=='albustanlayer'){ ?> document.getElementById('customer').value=this.value;  <?php } ?>reloadpage();">
	<option value="All" <?php if($customer == "All") { ?> selected="selected"<?php } ?>>All</option>
	<?php 
	$q = "SELECT distinct(party) FROM oc_cobi ORDER BY party ASC";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	?>
	<option value="<?php echo $qr['party'];?>" <?php if($qr['party'] == $customer) { ?> selected="selected"<?php } ?>><?php echo $qr['party'];?></option>
	<?php } ?>
</select>
...
&nbsp;&nbsp;&nbsp;
Customer Code&nbsp;&nbsp; </td><td>
	<select id="customercode" onchange="document.getElementById('customer').value=this.value; reloadpage();">
	<option value="All" <?php if($customer == "All") { ?> selected="selected"<?php } ?>>All</option>
	<?php 
	$q = "SELECT distinct(party),partycode FROM oc_cobi ORDER BY party ASC";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	?>
	<option value="<?php echo $qr['party'];?>" <?php if($qr['party'] == $customer) { ?> selected="selected"<?php } ?>><?php echo $qr['partycode'];?></option>
	<?php } ?>
</select>
<?php } ?>
</span>
...
<?php } } ?>
<?php if($_SESSION[db]=='albustan' || $_SESSION[db]=='albustanlayer') if ($customer == "All") { ?>
	<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader">
		Customer Code
		</td>
...
if($customer <> "All")
{
$query = "SELECT code,sum(quantity) as sentqty, max(price) as max, min(price) as min, sum(price) as sumrate,partycode FROM oc_cobi WHERE code IN (SELECT code FROM ims_itemcodes WHERE cat $cg '$category') AND date between  '$fromdate' AND '$todate' AND party = '$customer' GROUP BY code ORDER BY code ASC";
...
<?php
if($customer <> "All")
$query2 = "SELECT count(*) as count FROM oc_cobi WHERE code = '$rows[code]' AND date between  '$fromdate' AND '$todate' AND party = '$customer'";
elseif($customer == "All")
$query2 = "SELECT count(*) as count FROM oc_cobi WHERE code = '$rows[code]' AND date between  '$fromdate' AND '$todate' AND party = '$rowsa[party]' GROUP BY code ORDER BY code ASC";
$result2 = mysql_query($query2,$conn1) or die(mysql_error());
$rows2 = mysql_fetch_assoc($result2);

$avg = round($rows['sumrate'] / $rows2['count'],2);
?>
		<td<?php echo $sItemRowClass; ?> align="right">
...
{
$query = "SELECT distinct(code) FROM oc_cobi WHERE code IN (SELECT code FROM ims_itemcodes WHERE cat $cg '$category') AND date between  '$fromdate' AND '$todate' GROUP BY code ORDER BY code ASC";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
{
$querya = "SELECT distinct(party) FROM oc_cobi WHERE code = '$rows[code]' AND client = '$client'";
...
$queryaa = "SELECT code,sum(quantity) as sentqty, max(price) as max, min(price) as min, sum(price) as sumrate,partycode FROM oc_cobi WHERE code = '$rows[code]' AND party = '$rowsa[party]' AND date between  '$fromdate' AND '$todate' GROUP BY code ORDER BY code ASC";
...
?>
	<tr>
		...
		<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue($rowsaa['code']) ?>
</td>
<?php
...
<?php if($customer == "All") { ?>
		...
		<td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue($rowsa['party']) ?>
</td>

<?php if($_SESSION[db]=='albustan' || $_SESSION[db]=='albustanlayer') { ?>
<td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue($rowsaa['partycode']) ?>
</td>
<?php } ?>
...
function reloadpage()
{
	...
	//document.location = "salescostanalysis.php?category="+category+"&fromdate=" + fromdate + "&todate=" + todate + "&customer=" + customer;
