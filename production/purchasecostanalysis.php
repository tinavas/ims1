...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Purchase Cost Analysis <br /> From <?php echo date($datephp,strtotime($fromdate));?> To <?php echo date($datephp,strtotime($todate));?><br/> Category :: <?php echo $category;?></font></strong></td>
</tr>
</table>
<?php 
if($currencyunits == "")
...
&nbsp;&nbsp;&nbsp;
Supplier Code&nbsp;&nbsp;</td><td>
	<select id="suppliercode" onchange="document.getElementById('supplier').value=this.value; reloadpage();">
	<option value="All" <?php if($supplier == "All") { ?> selected="selected"<?php } ?>>All</option>
	<?php 
	$q = "SELECT distinct(vendor),vendorcode FROM pp_sobi ORDER BY vendor ASC";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	?>
	<option value="<?php echo $qr['vendor'];?>" <?php if($qr['vendor'] == $supplier) { ?> selected="selected"<?php } ?>><?php echo $qr['vendorcode'];?></option>
	<?php } ?>
</select>
<?php } ?>
</td>
...
<?php echo ewrpt_ViewValue($dcode) ?>
</td>
<?php
...
<?php echo ewrpt_ViewValue(changeprice($rows['sentqty'])) ?>
</td>
		<td<?php echo $sItemRowClass; ?> align="right">
...
$queryaa = "SELECT code,sum(sentquantity) as sentqty, max(rateperunit) as max, min(rateperunit) as min, sum(rateperunit) as sumrate , vendorcode FROM pp_sobi WHERE code = '$rows[code]' AND vendor= '$rowsa[vendor]' AND date between  '$fromdate' AND '$todate' GROUP BY code ORDER BY code ASC";
...
<?php if($_SESSION[db]=='albustan' || $_SESSION[db]=='albustanlayer') { ?>
<td<?php echo $sItemRowClass; ?>>
<?php echo ewrpt_ViewValue($rowsaa['vendorcode']) ?>
</td>
<?php } } ?>
	 
		<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changeprice($rowsaa['sentqty'])) ?>
</td>
		<td<?php echo $sItemRowClass; ?> align="right">
