...
if($status=='received')
{
 $cond1="and pp_sobi.receiveflag='1'";
$cond="and receiveflag='1'";
}
else if($status=='pending')
{
$cond1="and pp_sobi.receiveflag='0'";
$cond="and receiveflag='0'";
}
else
{
}
...
if($_SESSION['db'] == 'fortress' or $_SESSION['db'] == 'trupti' or $_SESSION['db'] == 'golden' or $_SESSION['db'] == 'mlcf' or $_SESSION['db'] == 'maharashtra' or $_SESSION['db'] == 'mbcf')
{
...
<?php if(($_SESSION['db'] == 'golden' or $_SESSION['db'] == 'mlcf' or $_SESSION['db'] == 'maharashtra' or $_SESSION['db'] == 'mbcf') && $warehouse <> 'All') { ?>
<tr>
 <td colspan="2" align="center"><strong><font color="#3e3276">Warehouse : </font></strong><?php echo $warehouse; ?></td>
</tr>
<?php } ?>
<?php if($_SESSION['db'] == 'trupti')  { ?>
<tr>
 <td colspan="2" align="center"><strong><font color="#3e3276">Warehouse : </font></strong><?php echo $warehouse; ?></td>
</tr>
<?php } ?>
...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT pp_sobi.vendor,pp_sobi.broker, pp_sobi.date, pp_sobi.so, pp_sobi.totalquantity, pp_sobi.grandtotal , pp_sobi.code , pp_sobi.description , pp_sobi.vendorcode FROM " . $EW_REPORT_TABLE_SQL_FROM;
if(($_SESSION['db'] == 'fortress' or $_SESSION['db'] == 'golden' or $_SESSION['db'] == 'mlcf' or $_SESSION['db'] == 'maharashtra' or $_SESSION['db'] == 'mbcf') && $warehouse <> 'All')
{
...
{
 $EW_REPORT_TABLE_SQL_WHERE = " pp_sobi.vendor $vc '$vendor' $cond1 and  pp_sobi.code $cod '$code' and pp_sobi.description $desc '$description'and pp_sobi.date >= '$fdate' and pp_sobi.date <= '$tdate' ";
}
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="purchasereportsmry.php?export=html&fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&vendor=<?php echo $vendor; ?>&warehouse=<?php echo $warehouse; ?>&code=<?php echo $code; ?>&description=<?php echo $description; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="purchasereportsmry.php?export=excel&fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&vendor=<?php echo $vendor; ?>&warehouse=<?php echo $warehouse; ?>&code=<?php echo $code; ?>&description=<?php echo $description; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="purchasereportsmry.php?export=word&fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&vendor=<?php echo $vendor; ?>&warehouse=<?php echo $warehouse; ?>&code=<?php echo $code; ?>&description=<?php echo $description; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="purchasereportsmry.php?cmd=reset&fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&vendor=<?php echo $vendor; ?>&warehouse=<?php echo $warehouse; ?>&code=<?php echo $code; ?>&description=<?php echo $description; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
...
$result1 = mysql_query($q1,$conn1) or die(mysql_error());
while($rows1 = mysql_fetch_assoc($result1))
{
?>
<option value="<?php echo $rows1['sector']; ?>" title="<?php echo $rows1['sector']; ?>" <?php if($warehouse == $rows1['sector']) { ?> selected="selected" <?php } ?>><?php echo $rows1['sector']; ?></option>
...
?>
<select id="status" name="status" onchange="reloadpage()">
<option value="">All</option>
<option value="received" <?php if($status=='received'){?> selected="selected" <?php }?>>Received </option>
...
<?php } ?>
<?php  if($_SESSION['db'] == 'suriya' ){if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader">
		Bill Date		</td>
<?php } else { ?>
		<td class="ewTableHeader">
...
<?php } else { ?>
		<td class="ewTableHeader" align="center">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center" title="Goods Receipt Date">Vehicle No</td>
			</tr></table>
		</td>
<?php } ?>
<?php } ?>
...
<?php 
//echo $status;
$code = $description = $quantity = $price = $binvoice = ""; $totqty = 0;
 $q = "select * from pp_sobi where date = '$x_date' and vendor = '$x_vendor' $cond and so = '$x_so' order by code";
...
{  
  ...
  { 
	...
	$quantity.= changeprice1($qr['sentquantity']) . "/<br>";
	$price.= $qr['rateperunit'] . "/<br>";
	$itemunits.=$qr['itemunits']. "/<br>";
  $quan=$qr['sentquantity'];
  $rate=$qr['rateperunit'];
	}
	else
	{
	...
	$quantity.= changeprice1($qr['sentquantity']) . "/";
	$price.= $qr['rateperunit'] . "/";
	$itemunits.=$qr['itemunits']. "/";
    $quan=$qr['sentquantity'];
	 $rate=$qr['rateperunit'];
	}
	 $binvoice =$qr['invoice'];
  $totqty +=$quan;
    $x_grandtotal += $quan*$rate; 
...
$itemunits = substr($itemunits,0,-1);
$allow = 1;
$holdso = $x_so;
if($holdso == $oldso)
$allow = 0;
...
<?php echo ewrpt_ViewValue($x_vendor) ?>
		...
		<td class="ewRptGrpField1" align="left">
		<?php $t_vendor = $x_vendor; $x_vendor = $dg_vendor; ?>
<?php echo ewrpt_ViewValue($vendorcode) ?>
		<?php $x_vendor = $t_vendor; ?></td>
		<?php } ?>
...
<?php echo ewrpt_ViewValue(changeprice1($totqty)); $totalquantity+= $totqty; $finalqty += $totqty;?>
</td>
		<td<?php echo $sItemRowClass; ?> align="right">
...
<td<?php echo $sItemRowClass; ?> align="right">
<?php if($status=='received') {echo ewrpt_ViewValue("Received"); } elseif($status=='pending') { echo "Pending"; } else
{ }
?>
</td>
<?php
...
$query = "select ge,vno from pp_sobi where so='$x_so' and  vendor = '$x_vendor'";
...
if($ge != "")
{
	$query = "select vehicleno from pp_gateentry where vendor = '$x_vendor' and ge = '$ge'";
	$result = mysql_query($query,$conn1) or die(mysql_error());
	$result = mysql_fetch_assoc($result);
	echo $vehicle = $result['vehicleno'];
}
echo $vechicle = $result['vno'];
	?>
	</td>
	<?php } ?>
...
<?php echo changeprice($totalamount); $totalamount = 0; #echo ewrpt_ViewValue($x_grandtotal) ?>
	...
	<?php if(($_SESSION['db'] == 'fortress' or $_SESSION['db'] == 'golden' or $_SESSION['db'] == 'mlcf' or $_SESSION['db'] == 'maharashtra' or $_SESSION['db'] == 'mbcf') && $warehouse == "All") { ?>
...
<?php } ?>
	<tr>
	 <td colspan="8" align="right"><b>Total (or) Average</b></td>
	 <td align="right"><b><?php echo changeprice($finalamt/$finalqty); ?></b></td>
	 <td align="right"><b><?php echo changequantity($finalqty); ?></b></td>
	  <td align="right"><b><?php echo changeprice($finalamt); ?></b></td>
...
<form action="purchasereportsmry.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="purchasereportsmry.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="purchasereportsmry.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="purchasereportsmry.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="purchasereportsmry.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function GetRow($opt) {
	...
	if (!$rs->EOF) {
		...
		$GLOBALS['x_vendorcode'] = $rs->fields('x_vendorcode');
		...
		$val[4] = $GLOBALS['x_vendorcode'];
	} else {
		$GLOBALS['x_vendorcode'] = "";
		$GLOBALS['x_date'] = "";
		$GLOBALS['x_so'] = "";
...
var date = document.getElementById('date').value;
	...
	document.location = "purchasereportsmry.php?fdate=" + date + "&tdate=" + date1 + "&vendor=" + vendor + "&code=" + code + "&description=" + "All";
...
var date = document.getElementById('date').value;
	...
	document.location = "purchasereportsmry.php?fdate=" + date + "&tdate=" + date1 + "&vendor=" + vendor + "&code=" +"All" + "&description=" + "All";
...
else
{
	...
	document.location = "purchasereportsmry.php?fdate=" + date + "&tdate=" + date1 + "&vendor=" + vendor + "&code=" + code + "&description=" + description;
}
}
...
<?php	if($_SESSION['db'] == 'golden' or $_SESSION['db'] == 'mlcf' or $_SESSION['db'] == 'maharashtra' or $_SESSION['db'] == 'mbcf' or $_SESSION['db'] == 'trupti') { ?>
    var warehouse = document.getElementById('warehouse').value;
	document.location = "purchasereportsmry.php?fdate=" + date + "&tdate=" + date1 + "&vendor=" + vendor + "&warehouse=" + warehouse + "&code=" + code + "&description=" + description;
<?php } else { ?>
	document.location = "purchasereportsmry.php?fdate=" + date + "&tdate=" + date1 + "&vendor=" + vendor + "&code=" + code + "&description=" + description + "&status=" + status;
<?php } ?>
}
