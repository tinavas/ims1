...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="orevenue.php?flock=<?php echo $flock;?>&export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="orevenue.php?flock=<?php echo $flock;?>&export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="orevenue.php?flock=<?php echo $flock;?>&export=word<?php echo $url; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="orevenue.php?cmd=reset&flock=<?php echo $flock; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
...
$query = "SELECT date,sum(quantity) as qty,price,sum(quantity*price) as amt, code,description,units FROM oc_cobi WHERE flock like  '%$flock' and code NOT IN (SELECT code FROM ims_itemcodes WHERE cat LIKE '%Hatch Eggs' OR cat LIKE '%Eggs') AND client = '$client' group by date order by date";
