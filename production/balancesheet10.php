...
while($rows = mysql_fetch_assoc($result))
 ...
 $drarray[$rows['coacode']] = $rows['dramount'];	
	...
	//Arrays starts here
	   ...
	   $leftcount = 0;
	   $quer11 = "select distinct(schedule),type from ac_schedule where  type in('Capital','Liability') and ptype ='Direct' order by schedule  ";
	   ...
	   while($row111 = mysql_fetch_assoc($quers11))
	   {
		 ...
		 $query1 = "select distinct(schedule) from ac_schedule where type in('Capital','Liability') and pschedule = '$row111[schedule]' and ptype = 'InDirect'";
		 $result1 = mysql_query($query1,$conn1) or die(mysql_error());
		 while($rows1 = mysql_fetch_assoc($result1))
		 {
		 $schedule = $rows1['schedule'];
		 
		 $q1 = "SELECT code FROM ac_coa WHERE schedule = '$schedule'";
		  ...
		  $coacount = mysql_num_rows($quers);
		   if($coacount > 0)
		    ...
		    $leftcount = $leftcount + 1;
		      ...
		      $query = "select distinct(schedule) from ac_schedule where type in('Capital','Liability') and pschedule = '$row111[schedule]' and ptype = 'InDirect'";
		      $result = mysql_query($query,$conn1) or die(mysql_error());
		      while($rows = mysql_fetch_assoc($result))
		      {
		         $leftarray2[$leftcount] = $rows['schedule'];
		         $mbal = 0;

		            $q1 = "SELECT code FROM ac_coa WHERE type in('Capital','Liability') and schedule = '$rows[schedule]' ";
       		        $r1 = mysql_query($q1,$conn1) or die(mysql_error());
		            while($rr1 = mysql_fetch_assoc($r1))
                       {
		               $code = $rr1['code']; 
	...
	$rightcount = 0;
	   ...
	   $quer11 = "select distinct(schedule),type from ac_schedule where  type = 'Asset' and ptype ='Direct' order by schedule  ";
	   $quers11 = mysql_query($quer11,$conn1) or die(mysql_error());
	   while($row111 = mysql_fetch_assoc($quers11))
	   {
	     $coacount = 0;
		 $coacodes = "'dummy',";
		 $query = "select distinct(schedule) from ac_schedule where pschedule = '$row111[schedule]' and ptype = 'InDirect'";
		 $result = mysql_query($query,$conn1) or die(mysql_error());
		 while($rows = mysql_fetch_assoc($result))
		 {
		  
		  $q1 = "SELECT code FROM ac_coa WHERE schedule = '$rows[schedule]'";
		 $r1 = mysql_query($q1,$conn1) or die(mysql_error());
		 while($rr1 = mysql_fetch_assoc($r1))
		  $coacodes .= "'".$rr1[code]."',";
		  }
		       ...
		       while($rows = mysql_fetch_assoc($result))
		        {
				    ...
				    $rightarray2[$rightcount] = $rows['schedule'];
					$mbal = 0;
  ...
  $revsum = 0;
   ...
   $quer = "select * from ac_coa where type = 'Expense' or type = 'Revenue' order by description   ";
