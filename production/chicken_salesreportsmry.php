...
<?php if($category <> "All") { ?>
<tr>
 <td colspan="2" align="center"><strong><font color="#3e3276">Category : </font></strong><?php echo $category; ?></td>
</tr>
<?php } ?>
...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT oc_cobi.date, oc_cobi.party, oc_cobi.invoice, oc_cobi.totalquantity, oc_cobi.finaltotal, oc_cobi.code,oc_cobi.description FROM " . $EW_REPORT_TABLE_SQL_FROM;
...
$EW_REPORT_TABLE_SQL_GROUPBY = "oc_cobi.warehouse, oc_cobi.date, oc_cobi.party,oc_cobi.invoice, oc_cobi.totalquantity, oc_cobi.finaltotal , oc_cobi.code,oc_cobi.description";
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="chicken_salesreportsmry.php?export=html&fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&party=<?php echo $party; ?>&warehouse=<?php echo $warehouse; ?>&code=<?php echo $code; ?>&description=<?php echo $description; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="chicken_salesreportsmry.php?export=excel&fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&party=<?php echo $party; ?>&warehouse=<?php echo $warehouse; ?>&code=<?php echo $code; ?>&description=<?php echo $description; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="chicken_salesreportsmry.php?export=word&fdate=<?php echo $fdate; ?>&tdate=<?php echo $tdate; ?>&party=<?php echo $party; ?>&warehouse=<?php echo $warehouse; ?>&code=<?php echo $code; ?>&description=<?php echo $description; ?>">Export to Word</a>
<?php } ?>
...
{
?>
<option title = "<?php echo $qr['cat'];?>" value="<?php echo $qr['cat']; ?>" <?php if($category == $qr['cat']) { ?> selected="selected" <?php } ?> ><?php echo $qr['cat']; ?></option>
<?php } ?>
</select>
...
<?php
$query1 = "SELECT sector FROM tbl_sector WHERE type1 IN ('Administration Office', 'Warehouse','Chicken Center','Head Office','Processing Unit') AND client = '$client'";
...
<?php if (@$sExport <> "") { ?>
		<td valign="bottom" class="ewTableHeader">
		Total <?php if($_SESSION['db']=="vista") { ?>Weight <?php } else { ?>Quantity<?php } ?>
		</td>
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td>Total <?php if($_SESSION['db']=="vista") { ?>Weight <?php } else { ?>Quantity<?php } ?></td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
<?php
$x_party="";
$query = "SELECT warehouse,date,party,invoice,ps,credittermvalue FROM oc_cobi WHERE date BETWEEN '$fdate' AND '$tdate' AND client = '$client' AND party $pc '$party' AND code $cod '$code' AND warehouse $ware '$warehouse' AND code IN (SELECT code FROM ims_itemcodes WHERE cat $cate '$category') GROUP BY invoice,party,date,warehouse ORDER BY warehouse,date,invoice";
$result = mysql_query($query,$conn1) or die(mysql_error());
while($rows = mysql_fetch_assoc($result))
{
 $x_date = $rows['date'];
 $x_warehouse = $rows['warehouse'];
 $x_invoice = $rows['invoice'];
 $x_party = $rows['party'];
 $customer=$rows['party'];
 $creditterm1=$rows['ps'];
 $creditterm=$rows['credittermvalue'];
...
$code = $description = $quantity = $price  = $flock ="" ;$count=0;
//$q = "select * from oc_cobi where date = '$x_date' and party = '$x_party' and invoice = '$x_invoice' order by code";
if($_SESSION['db']=="vista")
$q="select sum(weight) as quantity,code,description,flock,avg(price) as price from oc_cobi where date = '$x_date' and party = '$x_party' and invoice = '$x_invoice' AND code IN (SELECT code FROM ims_itemcodes WHERE cat $cate '$category') group by code order by code";
else
$q = "select sum(quantity) as quantity,code,description,flock,avg(price) as price from oc_cobi where date = '$x_date' and party = '$x_party' and invoice = '$x_invoice' AND code IN (SELECT code FROM ims_itemcodes WHERE cat $cate '$category') group by code order by code";
...
{$count++;
	...
	$price.= $qr['price'] . " / ";
	$totalqty += $qr['quantity'];
	$totalamt += $qr['quantity'] * $qr['price'];
...
<?php if($pwarehouse == $x_warehouse) echo ewrpt_ViewValue(); else echo ewrpt_ViewValue($x_warehouse); ?></td>
<?php
}
?>	
...
<?php echo ewrpt_ViewValue($customer) ?>
...
<?php echo ewrpt_ViewValue(changeprice($totalqty)); $totalquantity+= $totalqty; ?>
</td>
		<td<?php echo $sItemRowClass; ?> align="right">
<?php echo ewrpt_ViewValue(changeprice($totalamt)); $totalamount+= $totalamt; ?>
</td>
...
{
	$creditterm=0;
} echo ewrpt_ViewValue($creditterm); ?>
</td>
<td<?php echo $sItemRowClass; ?> align="right">
<?php 
...
<?php } ?>	
		...
		<td class="ewRptGrpSummary1">&nbsp;</td>
		<td class="ewRptGrpSummary1">&nbsp;</td>
		<td class="ewRptGrpSummary1" align="right"><b><?php echo changeprice($grandamount/$totalquantity); ?></b></td>
...
<form action="chicken_salesreportsmry.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="chicken_salesreportsmry.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="chicken_salesreportsmry.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="chicken_salesreportsmry.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="chicken_salesreportsmry.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function reloadpage()
{
	...
	document.location = "chicken_salesreportsmry.php?fdate=" + date + "&tdate=" + date1 + "&party=" + party + "&code=" + code + "&description=" + description + "&warehouse=" + warehouse + "&category=" + cat;
...
var date = document.getElementById('fdate').value;
	...
	var warehouse = document.getElementById('warehouse').value;
	document.location = "chicken_salesreportsmry.php?fdate=" + date + "&tdate=" + date1 + "&party=" + party + "&code=" + code + "&description=" + "All" + "&warehouse=" + warehouse + "&category=" + cat;
}
...
var date = document.getElementById('fdate').value;
	...
	var warehouse = document.getElementById('warehouse').value;
	document.location = "chicken_salesreportsmry.php?fdate=" + date + "&tdate=" + date1 + "&party=" + party + "&code=" + "All" + "&description=" + description + "&warehouse=" + warehouse + "&category=" + cat;
}
else
{
	var date = document.getElementById('fdate').value;
