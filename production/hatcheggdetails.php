...
$url = "&fromdate=" . $fromdate . "&todate=" . $todate . "&warehouse=" . $warehouse ."&cat=" . $cat."&flock=" . $flock;
$url1 = "?fromdate=" . $fromdate . "&todate=" . $todate . "&warehouse=" . $warehouse ."&cat=" . $cat."&flock=" . $flock;
?>
<?php
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Hatch Egg Detail Stock Report <br /> From <?php echo date($datephp,strtotime($fromdate));?> To <?php echo date($datephp,strtotime($todate));?><br/> Flock :: <?php echo $flock;?></font></strong></td>
</tr>
</table>
<?php 
if($currencyunits == "")
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="hatcheggdetails.php?export=html<?php echo $url; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="hatcheggdetails.php?export=excel<?php echo $url; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="hatcheggdetails.php?export=word<?php echo $url; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
$q = "select distinct(type) from ims_itemtypes WHERE (type = 'Hatch Eggs') order by type";
...
&nbsp;&nbsp;&nbsp;
Flock&nbsp;
<select id="flock" name="flock" onChange="reloadpage();">
<option value="">-Select-</option>
<?php
$q = "select distinct(flkmain) as flock from breeder_flock WHERE client = '$client' order by flock";
$qrs = mysql_query($q,$conn1);
$n1 = mysql_num_rows($qrs);
while($qr = mysql_fetch_assoc($qrs))
{
...
<form action="hatcheggdetails.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<?php
		...
		$d1 = 0; $d2 = 0; $d3 = 0; $d4 = 0;
       $qd1 = "SELECT distinct(settingdate) as date FROM `hatchery_traysetting` where settingdate between '$fromdate' and '$todate' and (hatchery in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || hatchery =  '$warehouse') and flock LIKE '%$flock%' ORDER BY hatchdate ASC";
		$qd1rs = mysql_query($qd1,$conn1) or die(mysql_error());
		while($qd1r = mysql_fetch_assoc($qd1rs))
		{
		$datearr1[$d1] = $qd1r['date'];
		$d1 = $d1 + 1;
		}
		
		$qd2 = "SELECT distinct(date) as date FROM `ims_eggreceiving` where tocode IN (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and date between '$fromdate' and '$todate' and (towarehouse in (select distinct(warehouse) from tbl_sector where sector = '$warehouse') || (towarehouse =  '$warehouse')) and flock = '$flock' order by date asc ";
		$qd2rs = mysql_query($qd2,$conn1) or die(mysql_error());
		while($qd2r = mysql_fetch_assoc($qd2rs))
		{
		$datearr2[$d2] = $qd2r['date'];
		$d2 = $d2 + 1;
		}
			
	/*//$qd3 = "SELECT distinct(eggreceiveddate) as date FROM `oc_cobi` WHERE date between '$fromdate' and '$todate' and (warehouse in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || warehouse =  '$warehouse') and code in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and dflag = 0 and flock LIKE '%$flock%'";
		$qd3rs = mysql_query($qd3,$conn1) or die(mysql_error());
		while($qd3r = mysql_fetch_assoc($qd3rs))
		{
		$datearr3[$d3] = $qd3r['date'];
		$d3 = $d3 + 1;
		}*/
		
	
		
		/*$qd4 = "SELECT distinct(receiveddate) as date FROM oc_packslip WHERE date between '$fromdate' and '$todate' and (warehouse in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || warehouse =  '$warehouse') and itemcode in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs')";
		$qd4rs = mysql_query($qd4,$conn1) or die(mysql_error());
		while($qd4r = mysql_fetch_assoc($qd4rs))
		{
		$datearr4[$d4] = $qd4r['date'];
		$d4 = $d4 + 1;
		}*/
		 $qd4= "SELECT distinct(date) as date FROM oc_packslip WHERE date between '$fromdate' and '$todate'  and flock = '$flock' and itemcode in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs')";
		$qd4rs = mysql_query($qd4,$conn1) or die(mysql_error());
		while($qd4r = mysql_fetch_assoc($qd4rs))
		{
		 $datearr4[$d4] = $qd4r['date'];
		...
		if(count($datearr2)>0) $datearru =$datearr2;
	
		
		if(count($datearru)>0)
		 { if(count($datearr1)>0) $datearru =array_merge($datearru,$datearr1); }
		else
		  $datearru =$datearr1;
		  	  
		if(count($datearru)>0)
		 {  if(count($datearr3)>0) $datearru =array_merge($datearru,$datearr3); }
		else
		  $datearru =$datearr3;
		  
        if(count($datearru)>0)
		 { if(count($datearr4)>0) $datearru =array_merge($datearru,$datearr4); }
		else
		  $datearru =$datearr4; 		
		  
		$datearru = array_unique($datearru);
		sort($datearru);
	
for($i=0;$i<count($datearru);$i++)
		{
		$settingdate = $datearru[$i];
		
		$datec = date($datephp,strtotime($settingdate));
if($closing=='')
{
///////////////Intermediate Receipt
$irecop = 0;
$getop = "SELECT SUM(quantity) as quantity FROM ims_intermediatereceipt WHERE date < '$settingdate' AND (warehouse in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || warehouse =  '$warehouse') and code in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and riflag = 'R' and flock = '$flock'";
...
///////////////Intermediate Issue
$iiscop = 0;
$getop = "SELECT SUM(quantity) as quantity FROM ims_intermediatereceipt WHERE date < '$settingdate' AND (warehouse in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || warehouse =  '$warehouse') and riflag = 'I' and code in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and flock = '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
...
$trayop = 0;
$getop = "SELECT SUM(eggsset) as quantity FROM hatchery_traysetting WHERE settingdate < '$settingdate' and (hatchery in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || hatchery =  '$warehouse') and flock = '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
$trayop =$rowop['quantity'];
////////////////////Stock Adjustment (Add)
$staadd = 0;
$getop = "SELECT SUM(quantity) as quantity FROM ims_stockadjustment WHERE date < '$settingdate' AND (unit in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || unit =  '$warehouse') and type = 'Add' and code in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and flock = '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
...
$staded = 0;
$getop = "SELECT SUM(quantity) as quantity FROM ims_stockadjustment WHERE date < '$settingdate' AND (unit in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || unit =  '$warehouse') and type = 'Deduct' and code in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and flock = '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
...
////////////Eggs Received
$eggreceive = 0;
 $getop = "select sum(toeggs) as toeggs from ims_eggreceiving where date < '$settingdate' and tocode in (select distinct(code) from ims_itemcodes where cat = '$cat') and (towarehouse in (select distinct(warehouse) from tbl_sector where sector = '$warehouse') || (towarehouse =  '$warehouse')) and flock = '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
...
////////////////////////////Sales
$sales = 0;
$getop = "SELECT SUM(quantity) as quantity FROM oc_cobi WHERE date < '$settingdate' and (warehouse in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || warehouse =  '$warehouse') and code in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and dflag = 0 and flock LIKE '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
...
$packslip = 0;
$getop = "SELECT SUM(quantity) as quantity FROM oc_packslip WHERE date < '$settingdate' and flock LIKE '$flock' and  itemcode in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs')";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
$packslip = $rowop['quantity'];

/////////end of getting the OPENING/////////
//echo $purchasedop."/".$stadd."/".$grop."/".$trayop."/".$stockto."/".$irecop."/".$staded."/".$stockfrom."/".$iiscop; echo "<br/>";
...
$rowop = mysql_fetch_assoc($resultop);
$staadd = $rowop['quantity'];
if($staadd == "")  $staadd = 0;  // else echo 'a'.$staadd;
//$purchase = $eggreceive + $staadd;

$getop = "SELECT SUM(quantity) as quantity FROM ims_stockadjustment WHERE date = '$settingdate' AND (unit in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || unit =  '$warehouse') and type = 'Deduct' and code in (select distinct(code) from ims_itemcodes where cat = 'Hatch Eggs') and flock = '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
$stdsub = $rowop['quantity'];
if($stdsub == "")  $stdsub = 0; // else echo 'd'.$staadd;
$purchase = $eggreceive - $stdsub + $staadd;
///////////////////////////Eggs Set

$eggset = 0;
$getop = "SELECT SUM(eggsset) as quantity,sum(reject) as reject,sum(waste) as waste FROM hatchery_traysetting WHERE settingdate = '$settingdate' and (hatchery in(select distinct(warehouse) from tbl_sector where sector = '$warehouse') || hatchery =  '$warehouse') and flock = '$flock'";
$resultop = mysql_query($getop,$conn1);
$rowop = mysql_fetch_assoc($resultop);
...
$packslip = $rowop['quantity'];
$eggset = $eggset1;
$eggsale= $sales + $packslip;
//////////////////Closing

$closing = $opening + $purchase - ($eggset+$eggsale);
...
<tr>
<td<?php echo $sItemRowClass; ?> style="text-align:right;"><?php echo $datec; ?></td>
...
<td<?php echo $sItemRowClass; ?> style="text-align:right;">
<?php if($eggsale <> "") { echo changequantity($eggsale); } else {echo "0";}$total_eggssale+=$eggsale; ?>
</td>
		<td<?php echo $sItemRowClass; ?> style="text-align:right;">
...
<td <?php echo $sItemRowClass; ?> align="right"><b><?php echo changequantity($total_eggssale);?></b></td>
<td <?php echo $sItemRowClass; ?> align="right">&nbsp;</td>
	</tbody>
	<tfoot>
<?php
...
if(Date.parse(date1) <= Date.parse(date2))
	{
	document.location = "hatcheggdetails.php?fromdate=" + fromdate + "&todate=" + todate + "&warehouse=" + warehouse + "&cat=" + cat + "&flock=" + flock;
	}
	else
	{
