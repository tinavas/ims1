...
$q = "select amount,quantity,type,trnum from ac_financialpostings where crdr = 'Cr' and itemcode  = '$code' and coacode = '$itemcode' and date < '$fdate' order by date ";
  ...
  $q = "select amount,quantity,type,trnum from ac_financialpostings where itemcode  = '$code' and crdr = 'Dr' and coacode = '$itemcode' and date < '$fdate' order by date ";
	...
	$obval = $prercvval - $preissueval;	
	 ...
	 include "config.php";
 $query1 = "SELECT sum(quantity) as itotqty,sum(amount) as itotamt from ac_financialpostings where itemcode = '$code' and crdr = 'Dr' and coacode = '$itemcode' and date >= '$fdate' and date <= '$tdate'     ";
	 ...
	 include "config.php";
  $query1 = "SELECT sum(quantity) as itotqty,sum(amount) as itotamt from ac_financialpostings where itemcode = '$code' and crdr = 'Cr' and coacode = '$itemcode' and date >= '$fdate' and date <= '$tdate'         ";
      $result1 = mysql_query($query1,$conn1);
	   while($row2 = mysql_fetch_assoc($result1))
      {
	...
	<tr>
		...
		<td class="ewRptGrpField2">
		<?php if($_SESSION['db'] == 'albustanlayer' and strlen(strstr($qr11['code'],'LFD'))) echo "Kgs"; else echo $qr11['sunits']; ?>
		</td>
		<td align="right">
