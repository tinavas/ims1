...
<tr>
	<td<?php echo $sItemRowClass1; ?>>Age</td>
		...
		while($qr = mysql_fetch_assoc($qrs))
		{
              ...
              $age = round($age / 7);
         ?>
	     <td<?php echo $sItemRowClass1; ?>><?php echo $age.".".$age1; ?></td>
         <?php } ?>   
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Total Eggs</td>
         <?php 
           $l = 0;
            include "config.php";
            $q = "select sum(quantity) as 'quantity' from breeder_production where itemcode in ($eggtype) and date1 = '$comparedate' group by flock order by flock"; 
  		$qrs = mysql_query($q,$conn1) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
         ?>
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Opening Birds</td>
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
		 {
               $q1 = "select sum(fmort + fcull) as 'mort' from breeder_consumption where flock = '$flo[$f]' and date2 < '$comparedate' "; 
    		   $qrs1 = mysql_query($q1,$conn1) or die(mysql_error());
		   while($qr1 = mysql_fetch_assoc($qrs1))
  		   {
                $minus1 = $qr1['mort'];
...
<tr>
	<td<?php echo $sItemRowClass2; ?>>Closing Birds</td>
		 ...
		 while($qr = mysql_fetch_assoc($qrs))
		 {
             ...
             $q = "select sum(fmort + fcull) as 'mort' from breeder_consumption where flock = '$flo[$f]' and date2 <= '$comparedate' "; 
    		 $qrs = mysql_query($q,$conn1) or die(mysql_error());
		 while($qr = mysql_fetch_assoc($qrs))
		 {
                $minus = $qr['mort'];
