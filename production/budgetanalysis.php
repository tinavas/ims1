...
$y = $_GET['year'];
if($m == '01') { $mon = "January";}
else if($m == '02') {$mon = "February";}
else if($m == '03') {$mon = "March";}
else if($m == '04') {$mon = "April";}
else if($m == '05') {$mon = "May";}
else if($m == '06') {$mon = "June";}
else if($m == '07') {$mon = "July";}
else if($m == '08') {$mon = "August";}
else if($m == '09') {$mon = "September";}
else if($m == '10') {$mon = "October";}
else if($m == '11') {$mon = "November";}
else { $mon = "December"; }

$fromdate = $y."-".$m."-01";
$monthenddate = date("t",strtotime($fromdate));	//It gives no. of days in a month
$todate = $y."-".$m."-".$monthenddate;
$wh = $_GET['costcentre'];
if($wh == 'All')
{
$cond = "in (select distinct(sector) from tbl_sector where type1 = 'Processing Unit' || type1 = 'Chicken Center')";
}
else
{
$cond = "="." "."'".$wh."'";
...
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Budget &nbsp; Analysis</font></strong></td>
</tr>
<tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276">Cost Centre : <?php echo $_GET['costcentre'];?></font></strong></td>
</tr><tr>
<td style="text-align:center" colspan="2"><strong><font color="#3e3276"><?php echo $mon;?> - <?php echo $_GET['year'];?></font></strong></td>
</tr>
</table>
<?php 
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="budgetanalysis.php?export=html&costcentre=<?php echo $_GET['costcentre']; ?>&month=<?php echo $_GET['month']; ?>&year=<?php echo $_GET['year']; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="budgetanalysis.php?export=excel&costcentre=<?php echo $_GET['costcentre']; ?>&&month=<?php echo $_GET['month']; ?>&year=<?php echo $_GET['year']; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="budgetanalysis.php?export=word&costcentre=<?php echo $_GET['costcentre']; ?>&month=<?php echo $_GET['month']; ?>&year=<?php echo $_GET['year']; ?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<?php } else { ?>
		<td class="ewTableHeader" colspan="4">
			...
			<td colspan="4" align="center">Out Flow</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php } else { ?>
		<td class="ewTableHeader" colspan="4">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td colspan="4" align="center">In Flow</td>
			</tr></table>
		</td>
<?php } ?>
...
<?php } else { ?>
		<td class="ewTableHeader">
			<table cellspacing="0" class="ewTableHeaderBtn"><tr>
			<td align="center">Budget Estimation</td>
			</tr></table>
		</td>
<?php } ?>
<?php if (@$sExport <> "") { ?>
...
$ebud = 0;
	...
	<?php 
	 ...
	 $q = "select distinct(coacode) from ac_financialpostings where warehouse $cond and client = '$client' and (date between '$fromdate' and '$todate') and crdr = 'Dr' order by coacode";
	$r = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($r))
	{
	$code = $qr['coacode'];
	$q2 = "select sum(amount) as amt,coacode from ac_financialpostings where coacode = '$qr[coacode]' and (date between '$fromdate' and '$todate') and  warehouse $cond and client = '$client' and crdr = 'Dr'";
	$r2 = mysql_query($q2,$conn1) or die(mysql_error());
	$qr2 = mysql_fetch_assoc($r2);
	$amount = $qr2['amt'];
	$q1 = "select description from ac_coa where code = '$code'";
	$r1 = mysql_query($q1,$conn1) or die(mysql_error());
	$qr1 = mysql_fetch_assoc($r1);
	$desc = $qr1['description'];
	$q3 = "select sum(amount) as budget from ims_budget where coacode = '$code' and month = '$m' and year = '$y' and crdr = 'Dr'  and client = '$client'";
	$r3 = mysql_query($q3,$conn1) or die(mysql_error());
	$qr3 = mysql_fetch_assoc($r3);
	$ebud = $qr3['budget'];
	
	  
	  $leftarray1[$leftcount] = $code;
	  $leftarray2[$leftcount] = $desc;
	   $leftarray3[$leftcount] = $amount;
	   $leftarray4[$leftcount] = $ebud;
	  $leftcount = $leftcount + 1;
	  $grleftcount = $grleftcount + round($amount,2);
	  $grleftcountbud = $grleftcountbud + round($ebud,2);
	 } 
...
?>
	<?php 
	 ...
	 $q = "select distinct(coacode) from ac_financialpostings where warehouse $cond and client = '$client' and (date between '$fromdate' and '$todate') and crdr = 'Cr' order by coacode";
	$r = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($r))
	{
	$codenew = $qr['coacode'];
	$q2a = "select sum(amount) as amt,date from ac_financialpostings where coacode = '$codenew' and (date between '$fromdate' and '$todate') and warehouse $cond and client = '$client' and crdr = 'Cr'";
	$r2a = mysql_query($q2a,$conn1) or die(mysql_error());
	$qr2a = mysql_fetch_assoc($r2a);
	$amountnew = $qr2a['amt'];
	$q1a = "select description from ac_coa where code = '$codenew'";
	$r1a = mysql_query($q1a,$conn1) or die(mysql_error());
	$qr1a = mysql_fetch_assoc($r1a);
	$descnew = $qr1a['description'];
	
	$q3 = "select sum(amount) as budget from ims_budget where coacode = '$code' and month = '$m' and year = '$y' and crdr = 'Cr'  and client = '$client'";
	$r3 = mysql_query($q3,$conn1) or die(mysql_error());
	$qr3 = mysql_fetch_assoc($r3);
	$ebudnew = $qr3['budget'];
	 
	   $rightarray1[$rightcount] = $codenew;
	   $rightarray2[$rightcount] = $descnew;
	   $rightarray3[$rightcount] = $amountnew;
	   $rightarray4[$rightcount] = $ebudnew;
	   $rightcount = $rightcount + 1;
	  $grrightcount = $grrightcount + round($amountnew,2);
	   $grrightcountbud = $grrightcountbud + round($ebudnew,2);
...
?>
  ...
  $expsum = $grleftcount;
  $revsum = $grrightcount;
  $expsumbud = $grleftcountbud;
  $revsumbud = $grrightcountbud;
 ...
 $dumm = 0;
   ...
   { ?>
    <tr>
		...
		<td class="ewRptGrpField4" align="right">
		<?php if( $leftarray3[$dumm] == "" ) { echo ewrpt_ViewValue("&nbsp;");  } else { echo ewrpt_ViewValue(changeprice($leftarray3[$dumm]));  } ?>
		</td>
		
		<td align="right">
		<?php if( $leftarray4[$dumm] == "" ) { echo ewrpt_ViewValue("&nbsp;");  } else { echo ewrpt_ViewValue(changeprice($leftarray4[$dumm]));  } ?>
		</td>
...
<td align="right"> 
		 <?php if ( $rightarray4[$dumm] == "" ) { echo ewrpt_ViewValue("&nbsp;");  } else { echo ewrpt_ViewValue(changeprice($rightarray4[$dumm])); $lbal = $lbal + $rightarray4[$dumm]; } ?>
         </td>
	</tr> 
  <?php $dumm = $dumm + 1;  }
   ?>
	...
	while ($dumm < $leftcount) { ?>
	 <tr>
		...
		<?php if( $leftarray4[$dumm] == "" ) { echo ewrpt_ViewValue("&nbsp;");  } else { echo ewrpt_ViewValue(changeprice($leftarray4[$dumm])); $abal = $abal + $leftarray3[$dumm]; } ?>
		</td>
		<td>
	...
	<?php if ( $highflag == "right" ) { 
	 ...
	 <tr>
		...
		<td>
		 ...
		 <td align="right"> 
		 <?php if ( $rightarray4[$dumm] == "" ) { echo ewrpt_ViewValue("&nbsp;");  } else { echo ewrpt_ViewValue(changeprice($rightarray4[$dumm]));  $lbal = $lbal + $rightarray3[$dumm]; } ?>
         </td>
	...
	<tr>
		...
		<td class="ewRptGrpField3" align="right">
		<?php  echo ewrpt_ViewValue(changeprice($expsum));  ?>
		</td>
		<td class="ewRptGrpField3" align="right">
		<?php  echo ewrpt_ViewValue(changeprice($expsumbud));  ?>
		</td>
		<td class="ewRptGrpField3" align="right">&nbsp;</td>	
		<td>
<?php echo ewrpt_ViewValue("Total") ?>
</td>
	
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($revsum)); ?>
</td>
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($revsumbud)); ?>
</td>
	...
	<tr>
		<td class="ewRptGrpField3" align="right">&nbsp;</td>	
		<td class="ewRptGrpField2">
		<?php echo ewrpt_ViewValue("Deficit/Excess") ?>
			
		<td class="ewRptGrpField3" align="right">
		<?php  echo ewrpt_ViewValue(changeprice($expsumbud-$expsum));  ?>
		</td>
		<td class="ewRptGrpField3" align="right">&nbsp;</td>		
		<td class="ewRptGrpField3" align="right">&nbsp;</td>	
		<td>
...
<td align="right">
<?php echo ewrpt_ViewValue(changeprice($revsumbud-$revsum)); ?>
</td>
<td class="ewRptGrpField3" align="right">&nbsp;</td>		
