...
<?php } ?>
<br />
<center><strong>Sales Cost Analysis <br /> From <?php echo date("d.m.Y",strtotime($fromdate));?> To <?php echo date("d.m.Y",strtotime($todate));?><br />Category :: <?php echo $category;?></strong></center>
<br />
<center>
<?php if (@$sExport == "") { ?>
...
&nbsp;&nbsp;&nbsp;
Customer&nbsp;&nbsp;
	<select id="customer" onchange="reloadpage();">
...
if($customer <> "All")
{
$query = "SELECT code,sum(quantity) as sentqty, max(price) as max, min(price) as min, sum(price) as sumrate FROM oc_cobi WHERE code IN (SELECT code FROM ims_itemcodes WHERE cat $cg '$category') AND date between  '$fromdate' AND '$todate' AND party = '$customer' GROUP BY code ORDER BY code ASC";
...
$queryaa = "SELECT code,sum(quantity) as sentqty, max(price) as max, min(price) as min, sum(price) as sumrate FROM oc_cobi WHERE code = '$rows[code]' AND party = '$rowsa[party]' AND date between  '$fromdate' AND '$todate' GROUP BY code ORDER BY code ASC";
