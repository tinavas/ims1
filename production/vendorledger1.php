...
include "config.php";
	...
	$q = "select sum(orgamount) as cramt from pp_sobi where vendor = '$ven' and date <'$fdate'";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	$qr = mysql_fetch_assoc($qrs);
	  $obcramt += $qr['cramt']; 

	$q = "select sum(orgamount) as dramt from pp_payment where vendor = '$ven' and date <'$fdate'";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	$qr = mysql_fetch_assoc($qrs);
	  $obdramt += $qr['dramt']; 
...
<?php  if ( $obflag == "Cr" ) { echo ewrpt_ViewValue(changeprice($obbal)); $crtotal = $crtotal + $obbal;  } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
	...
	<?php
	 ...
	 $q = "select distinct(trnum),type,date from ac_financialpostings where date >= '$fdate' and date <= '$tdate'  and venname = '$ven' and coacode = '$vencontrollac' order by date ";
...
$bkinvoice = $qr['trnum'];
	  ...
	  if ( $qr['type'] == "SOBI" )
	  {
	    ...
	    $q1 = "select description,orgamount from pp_sobi where so = '$qr[trnum]'  ";
...
<td align="right">
<?php if ( ($qr['type'] == "PMT")) { echo ewrpt_ViewValue(changeprice($amount)); $drtotal = $drtotal + $amount; }
      else { echo ewrpt_ViewValue("&nbsp;"); } ?>
</td>
<td align="right">
<?php if ( $qr['type'] == "SOBI" ) { echo ewrpt_ViewValue(changeprice($amount)); $crtotal = $crtotal + $amount; } 
      else { echo ewrpt_ViewValue("&nbsp;"); }?>
</td>
	  ...
	  {
	      $mode = "PMT-Payment";
	     $q1 = "select paymentmode,cheque,orgamount from pp_payment where tid = '$qr[trnum]'  ";
	      $qrs1 = mysql_query($q1,$conn1) ;
	      while($qr1 = mysql_fetch_assoc($qrs1))
	      {
