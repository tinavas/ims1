...
$EW_REPORT_TABLE_SQL_WHERE = "crdr = 'Cr' and voucher = 'R' and (date >= '$datef' and date <='$datet')";
$EW_REPORT_TABLE_SQL_GROUPBY = "";
$EW_REPORT_TABLE_SQL_HAVING = "";
$EW_REPORT_TABLE_SQL_ORDERBY = "`transactioncode` ASC, `date` DESC, `code` ASC";
...
<?php if (@$sExport != "") { ?>
<center>
<h5>Receipt Voucher Report &nbsp;&nbsp;From:&nbsp;&nbsp;<?php echo date("d-m-Y",strtotime($datef));?>&nbsp;&nbsp;&nbsp;To:&nbsp;&nbsp;<?php echo date("d-m-Y",strtotime($datet));?></h5>
<?php }?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="receiptvouchersummary.php?export=html&from=<?php echo $datef;?>&to=<?php echo $datet;?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="receiptvouchersummary.php?export=excel&from=<?php echo $datef;?>&to=<?php echo $datet;?>">Export to Excel</a>
&nbsp;&nbsp;<a href="receiptvouchersummary.php?export=word&from=<?php echo $datef;?>&to=<?php echo $datet;?>">Export to Word</a><br /><br />
<?php } ?>
...
document.location = 'receiptvouchersummary.php?from=' + fdate + '&to=' + tdate;
}
</script>
...
<form action="receiptvouchersummary.php" name="ewpagerform" id="ewpagerform" class="ewForm">
<center>
...
<?php if ($Pager->RecordCount > 0) { ?>
	<table border="0" cellspacing="0" cellpadding="0"><tr><td><span class="phpreportmaker">From Date&nbsp;<input type="text" id="datef" style="width:70px" class="datepicker" onchange="reloadpage()" <?php if($_GET['from']){?> value="<?php echo $_GET['from']; ?>"<?php }else{?> value="<?php echo date("d.m.Y",strtotime($datef));?>" <?php } ?> />&nbsp;&nbsp; To Date&nbsp;<input type="text" id="datet" style="width:70px" class="datepicker" onchange="reloadpage()"  <?php if($_GET['to']){?> value="<?php echo $_GET['to']; ?>"<?php }else{?> value="<?php echo date("d.m.Y",strtotime($datet));?>" <?php } ?> /></span></td>
...
<?php echo ewrpt_ViewValue($x_description) ?>
		...
		<?php 
$query = "select code,description from ac_gl where transactioncode = '$x_transactioncode' and date = '$x_date' and bccodeno = '$x_bccodeno' and crdr = 'Dr' and voucher = 'R'";
$result = mysql_query($query,$conn1) or die(mysql_error());
$res = mysql_fetch_assoc($result);
?>
<td<?php echo $sItemRowClass; ?>>
...
<?php echo changeprice($x_crtotal); $totreceived += $x_crtotal; ?>
</td>
<td <?php echo $sItemRowClass; ?>>
...
?>
<?php if ($nTotalGrps > 0) { ?>
	<tr class="ewRptGrandSummary"><td><b>TOTAL</b></td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td><?php echo changeprice($totreceived); ?></td><td>&nbsp;</td></tr>
