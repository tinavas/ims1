...
{	
$q = "insert into pp_sobi (remarks,date,sobiincr,m,y,so,invoice,vendor,broker,code,description,receivedquantity,rateperunit,itemunits,bags,bagtype,bagunits,taxvalue,taxamount,freightamount,totalamount,pocost,grandtotal,balance,empid,empname,sector,flag,vno,driver,freighttype,freightie,viaf,datedf,cashbankcode,dflag,discountamount,coa,cno,flock,sentquantity,warehouse,credittermcode,credittermdescription,credittermvalue) 
	values('$remarks','$date','$sobiincr','$m','$y','$so','$bookinvoice','$vendor','$broker','$code','$description','$qtyr',($price * $conversion),'$units','$bags','$bagtype','$bagunits',($vat * $conversion),($taxamount * $conversion),($freight * $conversion),($basic * $conversion),($grandtotal * $conversion),($grandtotal * $conversion),($grandtotal * $conversion),'$empid','$empname','$sector','0','$vno','$driver','$freighttype','$freighttype','$viaf','$datedf','$cashbankcode','0',($discountamount * $conversion),'$coa','$cheque','$autoflock','$sentquantity','$warehouse','$ctermcode','$ctermdesc','$ctermvalue')";
}
...
$type = "PMT";
   ...
   $amount = $amount;
    ...
    $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client,cash,bank,cashcode,bankcode,schedule) VALUES('".$date."','','Dr','".$code1."','$quantity','".$amount."','".$tid."','".$type."','".$venname."','".$venname."','$client','$cash','$bank','$cashcode','$bankcode','$schedule')";
    $result4 = mysql_query($query4,$conn) or die(mysql_error());
