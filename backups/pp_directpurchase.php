...
<tbody>
	  <?php
         include "config.php"; 
		   ...
		   {
		   $query1 = "SELECT distinct(so) FROM pp_sobi WHERE dflag = '0' and date >= '$fromdate' and date <= '$todate'  order by date DESC";
		   }
		   else
		   {
		  $sectorlist = $_SESSION['sectorlist'];
		
		    $query1 = "SELECT distinct(so) FROM pp_sobi WHERE dflag = '0' and date >= '$fromdate' and date <= '$todate' and warehouse in ($sectorlist)  order by date DESC";
		   }
