...
<form id="form1" name="form1" method="post" action="pp_savesobi2.php">
<br />
...
<td><input type="text" id="date" name="date" value="<?php echo date("d.m.o"); ?>" onchange="getsobi();" size="11" class="datepicker"/>&nbsp;</td>
...
<td><select id="gr" name="gr" onchange="fun();" style="width:120px"><option value="">-Select-</option>
<?php	
if($type == "vendor")
{
...
<?php $q = "select vendorid,brokerid,broker from pp_goodsreceipt where gr = '$gr'"; $qrs = mysql_query($q,$conn) or die(mysql_error());
...
<?php 	$i = 1;
		...
		if($type == "vendor")
		{
		$q = "select * from pp_goodsreceipt where gr = '$gr' and vendor ='$vendor'  and aflag = '1' and sobiflag = '0' order by code";
		}
		else if($type == "broker")
		{
		$q = "select * from pp_goodsreceipt where gr = '$gr' and broker = '$broker'  and aflag = '1' and sobiflag = '0' order by code";
		}
		else
		{
		$q = "select * from pp_goodsreceipt where gr = '$gr'  and aflag = '1' and sobiflag = '0' order by code";
		}
		$qrs = mysql_query($q,$conn) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
		
$query1 = "select camount from pp_purchaseorder where po = '$qr[po]' and code = '$qr[code]'";
...
<td width="10px"></td>
<td><input style="border:none;background:none;text-align:right" type="text" id="receivedquantity<?php echo $i; ?>" size="8" name="receivedquantity[]" value="<?php echo $qr['receivedquantity']; ?>" readonly/></td>
<td width="10px"></td>
<td><input style="border:none;background:none;text-align:right" type="text" id="rateperunit<?php echo $i; ?>" size="8" name="rateperunit[]" value="<?php echo $qr['rateperunit'] / $conversion; ?>" readonly/></td>
<td width="10px"></td>
<td><input type="text" style="background:none;border:none;text-align:right"id="tot<?php echo $i; ?>" size="10" name="tot[]" value="<?php echo $qr['receivedquantity'] * $qr['rateperunit'] / $conversion; ?>" readonly /></td>
<td width="10px"></td>
<td><input type="text" style="background:none;border:none;text-align:right"id="taxamount<?php echo $i; ?>" size="6" name="taxamount[]" value="<?php echo $qr['taxamount'] / $conversion; ?>" readonly/></td>
<td width="10px"></td>
...
<td width="10px"></td>
<td><input type="text" style="background:none;border:none;text-align:right"id="discountamount<?php echo $i; ?>" size="6" name="discountamount[]" value="<?php echo $qr['discountamount'] / $conversion; ?>" readonly/></td>
<td width="10px"></td>
<td><input style="border:none;background:none;text-align:right" type="text" id="totalamount<?php echo $i; ?>" size="10" name="totalamount[]" value="<?php echo $qr['totalamount'] / $conversion; $sum+=$qr['totalamount']; $sum2 += ($qr['totalamount'] / $conversion);  ?>" readonly/></td>
<td width="10px"></td>
</tr>
...
<input type="hidden" id="conversion<?php echo $i; ?>" name="conversion[]" value="<?php echo $conversion; ?>" />
<?php $i++; } ?>
<tr style="height:20px"></tr>
<tr>
<td colspan="18" align="right"><strong>Grand Total</strong></td>
<td><input style="border:none;background:none;text-align:right" type="text" id="grandtotal2" size="10" name="grandtotal2" value="<?php echo $sum2; ?>"readonly/> <!-- This total is according to Vendor Currency -->
<input type="hidden" id="grandtotal" size="10" name="grandtotal" value="<?php echo $sum; ?>" />
<!-- This total is according to Base Currency --></td>
</tr>
</table>
<br />
<table align="center">
...
function fun()
{
	...
	document.location = "dashboardsub.php?page=pp_addsobi2&gr=" + gr + "&vendor=" + vendor + "&type=" + type + "&brok=" + brok + "&inv=" + inv;
}
...
<?php $q = "select distinct(vendor) from pp_goodsreceipt order by vendor"; 
$qrs = mysql_query($q,$conn) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs)) { ?>
	  	   theOption1=document.createElement("OPTION");
