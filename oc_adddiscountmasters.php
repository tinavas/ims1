...
$qdb=mysql_query("select fromdate,todate,customer,cat,code,`desc` from oc_discounts");
while($rdb=mysql_fetch_array($qdb))
{
$f=$rdb[fromdate];
$t=$rdb[todate];
$dbs[]=array("fromdate"=>$f,"todate"=>$t,"customer"=>$rdb[customer],"category"=>$rdb[cat],"code"=>$rdb[code],"desc"=>$rdb[desc]);
}
$db=json_encode($dbs);
...
<section class="grid_8">
  ...
  <center>
	 ...
	 <form class="block-content form" id="complex_form" method="post" action="oc_savediscountmasters.php"> 
	<input type="hidden" name="edit" value="0">
	  <h1>Discount Masters</h1>
	  
	<br />  
	  <b>Discount Masters</b>
<br />
...
<th><strong>Discount</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></th>
...
<?php } ?>
       ...
       <td style="text-align:left;">
			<select style="Width:75px" name="code[]" id="code@-1" onchange="selectdesc(this.id),checkdb(this.id);">
     		<option value="">-Select-</option>
...
<input type="text" size="6" id="price@-1" style="text-align:right;" name="price[]" value="" onkeyup="decimalrestrict(this.id,this.value)" onfocus="makeForm(this.id);"  onkeypress="return num(this.id,event.keyCode)"    />
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <input type="button" value="Cancel" onclick="document.location='dashboardsub.php?page=oc_discountmaster';">
</center>
...
//alert(dbfdate.getTime()+"<="+fdate1.getTime() +" &&  "+dbs[k].customer+"=="+aaa+" && "+  dbs[k].code+"=="+code);
if((dbfdate.getTime()<=fdate1.getTime()  &&  dbs[k].customer==aa[0] &&   dbs[k].code==code )|| ( dbtdate.getTime()>=tdate1.getTime() &&  dbs[k].customer==aa[0] &&   dbs[k].code==code )){
alert("Price of this item is already entered");
document.getElementById('code@-'+ind[1]).focus();
document.getElementById('code@-'+ind[1]).options[0].selected="selected";
document.getElementById('description@-'+ind[1]).options[0].selected="selected";
document.getElementById('units@-'+ind[1]).value="";
} 
else
{
}
...
myselect1.style.width = "75px";
 var op1=new Option("-Select-","");
myselect1.options.add(op1);
myselect1.onchange = function () { selectdesc(this.id),checkdb(this.id) };
