...
<section class="grid_8">
  <div class="block-border">
     ...
     <form class="block-content form" id="complex_form" name="form1" method="post" action="pp_savedirectpurchase_a.php" onsubmit="return checkcoa();">
...
<?php if($_SESSION['db'] == "suriya") { ?>
			<tr>
				...
				<input type="text" id="billdate" name="billdate" class="datepicker" value="<?php echo date("d.m.Y"); ?>" size="10" /></td>
			</tr>
			<tr height="10px"></tr>
			<?php } ?>   	
			<?php if($_SESSION['db'] == "albustan" || $_SESSION['db'] == "albustanlayer") { ?>
			<tr>
				<td colspan="16" align="center"><strong>Vendor</strong>&nbsp;&nbsp; <select  id="vendor" onchange="getcterm(this.value); document.getElementById('vendorcode').value=this.value;"  name="vendor" style="width:175px" <?php if($_SESSION['db'] == 'central') { ?> onchange="fvalidatecurrency();" <?php } ?>>
					...
					<?php
							...
							{
					?>
					<option title="<?php echo $qr['name']; ?>" value="<?php echo $qr['name'].'@'.$qr['cterm'].'@'.$qr['code'];?>"><?php echo $qr['name']; ?></option>
					<?php   }   ?>
					</select>   &nbsp;&nbsp; <strong>Vendor Code </strong> &nbsp;&nbsp;  <select id="vendorcode" onchange="getcterm(this.value); document.getElementById('vendor').value=this.value;"  name="vendorcode" style="width:175px"  <?php if($_SESSION['db'] == 'central') { ?> onchange="fvalidatecurrency();" <?php } ?>>
					<option value="" >-Select-</option>
					<?php
							$q = "select distinct(name),cterm,code from contactdetails where type = 'vendor' OR type = 'vendor and party' order by name";
							$qrs = mysql_query($q,$conn) or die(mysql_error());
							...
							{
					?>
					<option title="<?php echo $qr['name']; ?>" value="<?php echo $qr['name'].'@'.$qr['cterm'].'@'.$qr['code'];?>"><?php echo $qr['code']; ?></option>
					<?php   }   ?>
					</select></td>
			</tr>
			<tr height="10px"></tr>
...
<?php if($_SESSION['db'] == "albustan" || $_SESSION['db'] == "albustanlayer") {} else  { ?>
                ...
                <td>&nbsp;
					<select id="vendor"  onchange="getcterm(this.value)" name="vendor" style="width:175px" <?php if($_SESSION['db'] == 'central') { ?> onchange="fvalidatecurrency();" <?php } ?>>
					...
					<?php
							...
							{
					?>
					<option title="<?php echo $qr['name']; ?>" value="<?php echo $qr['name'].'@'.$qr['cterm'];?>"><?php echo $qr['name']; ?></option>
					<?php   }   ?>
					</select>
				</td>
                <td width="5px"></td>
...
<select style="Width:100px" name="cat[]" id="cat@-1" onchange="getcode(this.id);">
     ...
     <?php 
	     $query = "SELECT distinct(type) FROM ims_itemtypes ORDER BY type ASC";
           ...
           {
     ?>
             <option value="<?php echo $row['type'];?>" title="<?php echo $row['type']; ?>"><?php echo $row['type']; ?></option>
     <?php } ?>
</select>
       </td>
       <td width="10px"></td>
...
<?php if($_SESSION['db'] == 'albustanlayer'){ ?>
<td>
<input type="text" size="5" id="qtyf@-1" style="text-align:right;" name="fqty[]" value="" onkeyup="calnet('');" onblur="calnet('');"/>
</td>
<td width="10px">&nbsp;</td>
<?php } ?>
<td>
	...
	<option value="">-Select-</option>
     ...
     <?php 
	     $query1 = "SELECT * FROM ims_itemcodes WHERE type = 'Packing Material' ORDER BY code ASC";
           ...
           {
		   $a = "";
		   $query010 = "SELECT * FROM ims_bagdetails where code = '$row1[code]'";
		   $result010 = mysql_query($query010,$conn) or die(mysql_error());
		   while($rows010 = mysql_fetch_assoc($result010))
		   {
		    $a .= "@".$rows010['capacity'];
		    $a .= " ".$rows010['units'];
		   }
     ?>
             <option title="<?php echo $row1['description'] . $a; ?>"  value="<?php echo $row1['code'];?>"><?php echo $row1['code']; ?></option>
     <?php } ?>
</select>
</td>
<td width="10px">&nbsp;</td><td>
...
?>
<td width="10px">&nbsp;</td><td>
<input type="text" size="6" id="mrp@-1" style="text-align:right;" name="mrp[]" value="0" onkeyup="calnet('');" onblur="calnet('');"/>
</td>
<?php
}
?>
...
<select style="width:60px" name="vat[]" id="vat@-1" onchange="calnet('');">
     ...
     <?php 
	     $query = "SELECT distinct(code),codevalue FROM ims_taxcodes where type = 'Tax' ORDER BY codevalue ASC";
           $result = mysql_query($query,$conn);
           while($row = mysql_fetch_assoc($result))
           {
     ?>
...
<table id="batchexp" style="display:none" align="center">
 ...
 <tr id="batchexprow@-1">
  ...
  <td><input class="datepicker" type="text" size="15" id="expdate@-1" name="expdate[]" value="<?php echo date("d.m.Y"); ?>"></td><td width="10px">&nbsp;</td>
  <?php if($_SESSION['db'] == 'feedatives') { ?>
...
<table border="1">
  ...
  <tr style="height:20px"></tr>
   <tr>
       ...
       <td align="left"><select name="freighttype" id="freighttype" onchange="calnet('dcreate')"><option value="Included">Included</option>
	   <option value="Excluded">Excluded</option>
	   <option value="Freight Amount in Bill" title = "Freight Amount in Bill">Freight Amount in Bill</option>
	   </select></td>
...
function selectdesc(codeid)
{
	...
	?>
<?php if($_SESSION['db'] == 'albustanlayer') { ?>
var temp = document.getElementById("code@" + tempindex).value.split('@');
document.getElementById("code2@" + tempindex).value = temp[0];
document.getElementById("desc2@" + tempindex).value = description;
<?php } ?>
<?php
if($_SESSION['client'] == 'FEEDATIVES')
{
...
removeAllOptions(document.getElementById('existflock@'+tempindex));
myselect1 = document.getElementById('existflock@' + tempindex);
theOption1=document.createElement("OPTION");
theText1=document.createTextNode("-Select-");
theOption1.value = "";
theOption1.appendChild(theText1);
...
return false;
	...
	if(document.getElementById('cfamount').value != "" && document.getElementById('cfamount').value != "0" && document.getElementById('freighttype').value != "Freight Amount in Bill" )
	{
