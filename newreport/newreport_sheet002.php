...
$query = "select * from breeder_entry_procedure where client='$client'";
...
while($row1 = mysql_fetch_assoc($result))
{
 ...
 while($rowff = mysql_fetch_assoc($resultff))
  ...
  $query1t2 = "SELECT cat,sum(quantity) as quantity FROM `ims_stocktransfer` WHERE cat = 'Male Birds' AND client = '$client' AND towarehouse not like '$flockget' and fromwarehouse like '$flockget' AND date = '$row1[date2]'";
     ...
     $qfpur = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$flockget' AND date = '$row1[date2]' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Female Birds')";
  	$rfpur = mysql_query($qfpur,$conn); 
  	while($rowfpur= mysql_fetch_assoc($rfpur))  
  	{
     
	  $allfpur = $allfpur + $rowfpur['quant'];
	  }
	  
	     $qmpur = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$flockget' AND date = '$row1[date2]' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Male Birds')";
  	$rmpur = mysql_query($qmpur,$conn); 
  	while($rowmpur= mysql_fetch_assoc($rmpur))  
  	{    
	  $allmpur = $allmpur + $rowmpur['quant'];
	  }
	  
	  
	     $qfsale = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$flockget' AND date = '$row1[date2]' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Female Birds')";
	  ...
	  $allfsale = $allfsale + $rowfsale['quant'];
	    ...
	    $qmsale = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$flockget' AND date = '$row1[date2]' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Male Birds')";
...
<?php 
 ...
 //$male = $startmale - ($tmmort + $tmcull) + $tmtrin - $tmtrout;
  ...
  //$female = $startfemale - ($tfmort + $tfcull) + $tftrin - $tftrout + $tfpur - $tfsale;
 //$male = $startmale - ($tmmort + $tmcull) + $tmtrin - $tmtrout  + $tmpur - $tmsale;
 
  $female = $startfemale - ($tfmort + $tfcull) + $tftrin - $tftrout + $tfpur  - $tfexcess + $tfshrtage;
 $male = $startmale - ($tmmort + $tmcull) + $tmtrin - $tmtrout  + $tmpur  - $tmexcess + $tmshrtage;
...
?>
 <tr height=16 style='height:12.0pt'>
  ...
  <td class=xl269><?php echo $tempcomp1 = round((($weekeggs/$inc1)/$weekfemale)*100,2); $_SESSION['hd'][$tempage] = $tempcomp1; ?></td>
  <td class=xl268>&nbsp;</td>
  ...
  <td class=xl268><?php echo $tempcomp5 = round($weekeggwt/$inc1,2); $_SESSION['ieggwt'][$tempage] = $tempcomp5; ?></td>
...
<!--<center><div id="loadingimg" ><img src="../images/logout.png"/></div></center>
-->
