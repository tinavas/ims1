...
$cramount = 0;
 ...
 $query3 = "select distinct(mode) from ac_mode where '$date' between fromdate and todate order by mode";
 ...
 if($mode == "Monthly")
 {
 $dep_amount = round(($dep_amount / 12),2);
 }
 if($mode == "Quarterly")
 {
 $dep_amount = round(($dep_amount / 4),2);
 }
 if($mode == "Yearly")
 {
 ...
 { $dramount2 = $dep_amount; $cramount2 = 0; }
  ...
  $query1 = "INSERT INTO ac_gl (code,description,type,controltype,schedule,crdr,cramount,dramount,crtotal,drtotal,voucher,vstatus,transactioncode,mode,bccodeno,date,client,rremarks,warehouse) VALUES ('$code','$desc1','$type1','$ctype1','$schedule1','$crdr1','$cramount1','$dramount1','$dep_amount','$dep_amount','J','A','$tid','Journal','0','$date','$client','Depreciation','Administrative Office'),('$ecode','$desc2','$type2','$ctype2','$schedule2','$crdr2','$cramount2','$dramount2','$dep_amount','$dep_amount','J','A','$tid','Journal','0','$date','$client','Depreciation','Administrative Office')";
  mysql_query($query1,$conn) or die(mysql_error());

  $query2 = "INSERT INTO ac_financialpostings (date,crdr,coacode,quantity,amount,trnum,type,venname,warehouse) VALUES ('$date','$crdr1','$code','0','$dep_amount','$tid','Journal','Depreciation','Administrative Office'), ('$date','$crdr2','$ecode','0','$dep_amount','$tid','Journal','Depreciation','Administrative Office')";
  mysql_query($query2,$conn) or die(mysql_error());
}
}
$query1 = "INSERT INTO ac_autodepreciation (date,client) VALUES ('$date','$client')";
$result1 = mysql_query($query1,$conn) or die(mysql_error());
}
else
 $alertmsg = "The Depreciation has been done for this Date";
