...
<?php 
$q  = "select distinct(vcode) from ac_crdrnote where mode = 'VDN' order by vcode";
...
<option value = "All">All</option>
<?php 
$q  = "select distinct(vcode),vpcode from ac_crdrnote where mode = 'VDN' order by vcode";
$r = mysql_query($q,$conn) or die(mysql_error());
while($qr = mysql_fetch_assoc($r))
{
?>
...
window.open('production/supplierdebitnote.php?fromdate=' + fromdate + '&todate=' + todate + '&name=' + name1);
<?php } elseif($_SESSION['db'] == "central") { ?>
window.open('production/supplierdebitnote2.php?fromdate=' + fromdate + '&todate=' + todate + '&name=' + name1);
<?php } ?>
}
}
</script>
