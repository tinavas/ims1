...
<tr>
<td style="text-align:center" colspan="4"><strong><font color="#3e3276">Shed History</font></strong></td>
</tr>
<tr>
...
<?php } ?>
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="shedhistory.php?export=html&shed=<?php echo $shed; ?>&unit=<?php echo $unit; ?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="shedhistory.php?export=excel&shed=<?php echo $shed; ?>&unit=<?php echo $unit; ?>">Export to Excel</a>
&nbsp;&nbsp;<a href="shedhistory.php?export=word&shed=<?php echo $shed; ?>&unit=<?php echo $unit; ?>">Export to Word</a>
<?php if ($bFilterApplied) { ?>
&nbsp;&nbsp;<a href="shedhistory.php?cmd=reset&shed=<?php echo $shed; ?>&unit=<?php echo $unit; ?>">Reset All Filters</a>
<?php } ?>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
...
<table>
 <tr>
  ...
  <td><select id="unitcode" name="unitcode" onchange="getSheds(this.value);">
      ...
      <?php
	  include "config.php";
	  $query = "SELECT distinct(unitcode),unitdescription FROM breeder_unit WHERE client = '$client' ORDER BY unitcode ASC";
 ...
 <td width="20px"></td>
  ...
  <td><select id="shed" name="shed" onchange="reloadpage();">
      ...
      <?php
	  include "config.php";
	  $query = "SELECT distinct(shedcode),sheddescription FROM breeder_shed WHERE client = '$client' AND unitcode = '$unit' ORDER BY shedcode ASC";
	  $result = mysql_query($query,$conn1) or die(mysql_error());
	  while($rows = mysql_fetch_assoc($result))
	  {
	   ?>
...
$femaleopening = $maleopening = 0;		
 $query2 = "SELECT distinct(flock) FROM `breeder_consumption` WHERE flock IN ( SELECT distinct(flockcode) FROM breeder_flock WHERE shedcode = '$shed' AND client = '$client' and unitcode = '$unit') AND client = '$client'";
...
<?php
 $query3 = "SELECT max(age) as age,min(date2) as fromdate,max(date2) as todate FROM breeder_consumption WHERE flock = '$rows2[flock]' AND client = '$client'";
$result3 = mysql_query($query3,$conn1) or die(mysql_error());
$rows3 = mysql_fetch_assoc($result3);

$fromdate = date("d.m.Y",strtotime($rows3['fromdate']));
$todate = date("d.m.Y",strtotime($rows3['todate']));
?>
...
<?php echo ewrpt_ViewValue($todate) ?>
	...
	echo $qj = "select * from breeder_flock where flockcode = '$rows2[flock]'"; 
		 ...
		 while($qrj = mysql_fetch_assoc($qrsj))
		 {
             ...
             $minus = 0; 
			 $minus1 = 0;
             $qj = "select distinct(date2),fmort,fcull,mmort,mcull from breeder_consumption where flock = '$rows2[flock]'  "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		 while($qrj = mysql_fetch_assoc($qrsj))
                $minus = $minus + $qrj['fmort'] + $qrj['fcull'];

            $ftransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Female Birds' and fromwarehouse = '$rows2[flock]' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		   ...
		   while($qrj = mysql_fetch_assoc($qrsj))
			...
			$ttransfer = 0;
            $qj = "select * from ims_stocktransfer where cat = 'Female Birds' and towarehouse = '$rows2[flock]' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
			 ...
			 $fquantfc = 0;
			 $q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$rows2[flock]'  AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Female Birds')";  
		   ...
		   while($qr = mysql_fetch_assoc($r))
			 ...
			 $fquantso = 0;
			 $q = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$rows2[flock]' AND date < '$comparedate' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Female Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
	...
	$qj = "select * from breeder_flock where flockcode = '$rows2[flock]'"; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		 while($qrj = mysql_fetch_assoc($qrsj))
             ...
             $minus = 0; 
			 $minus1 = 0;
             $qj = "select distinct(date2),mmort,mcull from breeder_consumption where flock = '$rows2[flock]'  "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
		 while($qrj = mysql_fetch_assoc($qrsj))
                $mminus = $mminus + $qrj['mmort'] + $qrj['mcull'];

            $mtransfer = 0;
             $qj = "select * from ims_stocktransfer where cat = 'Male Birds' and fromwarehouse = '$rows2[flock]' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
			...
			$mttransfer = 0;
            $qj = "select * from ims_stocktransfer where cat = 'Male Birds' and towarehouse = '$rows2[flock]' "; 
    		 $qrsj = mysql_query($qj,$conn1) or die(mysql_error());
             if(mysql_num_rows($qrsj))
             {
		   while($qrj = mysql_fetch_assoc($qrsj))
			 ...
			 $mfquantfc = 0;
			 $q = "SELECT sum(receivedquantity) as quant FROM pp_sobi WHERE flock = '$rows2[flock]'  AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Male Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
			 ...
			 $mfquantso = 0;
			 $q = "SELECT sum(quantity) as quant FROM oc_cobi WHERE flock = '$rows2[flock]' AND code IN (SELECT code FROM ims_itemcodes WHERE cat = 'Male Birds')";  
             $r = mysql_query($q,$conn1) or die(mysql_error());
			 if(mysql_num_rows($r))
             {
		   while($qr = mysql_fetch_assoc($r))
             ...
             $mremaining = 0;
			 //echo $flockkk."/".$fopening."/".$minus."/".$ftransfer."/".$ttransfer."/".$fquantfc; echo"/B";
             $mremaining = $mopening - $mminus - $mtransfer + $mttransfer + $mfquantfc - $mfquantso;
			 
	?>

		<td class="ewRptGrpField4"  align="right" style="padding-right:15px;">
<?php if($cullflag == 1){ echo ewrpt_ViewValue("Shifted"); } else { echo ewrpt_ViewValue($remaining); } ?>
	</tr>

		<td class="ewRptGrpField4"  align="right" style="padding-right:15px;">
<?php if($cullflag == 1){ echo ewrpt_ViewValue("Shifted"); } else { echo ewrpt_ViewValue($mremaining); }  ?>
	</tr>

<?php
$femaleopening += $rows4['femaleopening'];
$maleopening += $rows4['maleopening'];
}
...
<tr>
 ...
 <?php
  $query = "SELECT shedcapacity FROM breeder_shed WHERE shedcode = '$shed' AND client = '$client'";
  $result = mysql_query($query,$conn1) or die(mysql_error());
...
<?php
$query1 = "SELECT distinct(unitcode) FROM breeder_unit WHERE client = '$client' ORDER BY unitcode ASC";
$result1 = mysql_query($query1,$conn1) or die(mysql_error());
while($rows1 = mysql_fetch_assoc($result1))
{
 echo "if(a == '$rows1[unitcode]') { ";
 $query2 = "SELECT distinct(shedcode),sheddescription FROM breeder_shed WHERE unitcode = '$rows1[unitcode]' AND client = '$client' ORDER BY shedcode ASC";
 $result2 = mysql_query($query2,$conn1) or die(mysql_error());
 while($rows2 = mysql_fetch_assoc($result2))
 {
 ?>
...
function reloadpage()
{
	...
	document.location = "shedhistory.php?shed=" + shed + "&unit=" + unitcode;
}
</script>
