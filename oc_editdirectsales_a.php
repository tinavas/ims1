...
<section class="grid_8">
  <div class="block-border">
     <form class="block-content form" id="complex_form" name="form1" method="post" onsubmit="return checkcoa();" action=<?php if( $flag == 0){ ?>  "oc_savedirectsales_a.php" <?php } else { ?> "oc_updatedirectsales_a.php" <?php } ?> >
		...
		<input type="hidden" name="discountamount" id="discountamount" value="<?php echo $discountamount; ?>"/>
		<input type="hidden" name="finaldiscount" id="finaldiscount" value="<?php echo $finaldiscountamount; ?>"/>
			 ...
			 <tr align="center">
                <td colspan="16" align="center"><strong>Customer</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font>&nbsp;
					...
					<?php
							...
							{
					?>
					<option <?php if ( $qr['name'] == $party) echo 'selected="selected"'; ?>  value="<?php echo $qr['name'].'@'.$qr['cterm'].'@'.$qr['code'];?>" title="<?php echo $qr['name']; ?>"><?php echo $qr['name']; ?></option>
					<?php   }   ?>
					</select>
				<strong>Customer Code</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font>
					...
					<?php
							...
							{
					?>
					<option <?php if ( $qr['name'] == $party) echo 'selected="selected"'; ?> value="<?php echo $qr['name'].'@'.$qr['cterm'].'@'.$qr['code'];?>" title="<?php echo $qr['name']; ?>"><?php echo $qr['code']; ?></option>
					<?php   }   ?>
					</select>
				</td>
				 </tr>
...
<?php } ?>
              ...
              <tr>
                ...
                <td>&nbsp;<input class="datepicker" type="text" size="15" id="date" name="date" value="<?php echo $datemain; ?>"></td>
                <td width="5px"></td>
		 <?php if($_SESSION['db'] == "albustan" || $_SESSION['db'] == "albustanlayer") { } else {?>
                <td><strong>Customer</strong></td>
                <td>&nbsp;
					<select onchange="getcterm(this.value)" id="party" name="party" style="width:175px">
					<option>-Select-</option>
					<?php
...
<?php } ?>
				...
				<td width="5px"></td>
                <td>
					<input type="text" size="5" id="bookinvoice" name="bookinvoice" value="<?php echo $bkinvoice; ?>"></td>
...
<?php if(($_SESSION['client']=="GOLDEN") or ($_SESSION['db'] == "johnson")){?>
<th><strong>Unit</strong></th><td width="10px">&nbsp;</td>
<?php } else {?>
<th><strong>&nbsp;</strong></th><td width="10px">&nbsp;</td>
<?php } ?>
...
<input type="text" size="7" style="text-align:right;" id="qtys@<?php echo $i; ?>" name="qtys[]" value="<?php 
 if(($_SESSION['db'] == "alkhumasiyabrd")&&($cat1=='Female Feed'||$cat1=='Male Feed'))
	  {
	  $q='select capacity from ims_bagdetails where code='."'".$qr['bagtype']."'";
	  if($a=mysql_fetch_assoc(mysql_query($q)))
	   echo  $qr['quantity']/$a[capacity];
	  }
  else { echo $qr['quantity']; } ?>
" onkeyup="calnet('');" onblur="calnet('');" />
<?php if($_SESSION['db'] == "alkhumasiyabrd") { ?> <input type="hidden"  name="oldqtys[]" value="<?php echo $qr['quantity'];  ?>"  /><?php } ?>
</td>
<?php if($_SESSION['db']=="feedatives" || $_SESSION['db']=="albustan" || $_SESSION['db']=="albustanlayer" ){?>
<td width="10px">&nbsp;</td><td>
...
<?php if($_SESSION['db']=='alkhumasiyabrd') { ?>
<td width="10px">&nbsp;</td><td>
<select type="text" id="bagtype@<?php echo $i; ?>" style="text-align:right; width:80px; <?php if($cat1=='Male Feed'||$cat1=='Female Feed') echo ''; else echo 'display:none' ; ?>" name="bagtype[]" >
<option value="">-Select-</option>
<?php 
           ...
           {
     ?>
	 <option <?php if ($row1['code'] == $qr['bagtype']) echo 'selected="selected"'; ?> title="<?php echo $row1[description];  ?>" value="<?php echo $row1[code];  ?>" ><?php echo $row1[code]; ?></option>
	 <?php } ?>
</select>
</td>
<?php } ?>
<td width="10px">&nbsp;</td>
<td><input  type="text" size="6" id="weight@<?php  echo $i; ?>"  
 <?php if(($qr1['cat'] == "Broiler Birds") or  ($qr1['cat'] == "Male Birds") or ($qr1['cat'] == "Female Birds")) { ?> style="text-align:right; " name="weight[]" value="<?php echo $qr['weight']; ?>" <?php } 
else { ?> style="text-align:right; display:none" name="weight[]" value="" <?php } ?>  onkeyup="calnet('');" onblur="calnet('');" /> </td>


<td width="10px">&nbsp;</td>
<td> <select  id="unit@<?php echo $i;   ?>" onchange="getflock(this.id);"  name="unit[]" style=" <?php if((($client == "GOLDEN") or ($client== "JOHNSON")) and(($cat1 == "Eggs") or  ($cat1 == "Hatch Eggs"))) {?>  width:100px; <?php } else { ?> width:100px;display:none;<?php } ?>"
>
...
<td> 
<?php  if($_SESSION['db'] =='alkhumasiyabrd') { ?> 
<input type="hidden" name="oldflock[]" value="<?php echo $qr['flock']; ?>" id="oldflock@<?php echo $i; ?>" />
<input type="hidden" name="oldrdate[]" value="<?php echo $qr['eggreceiveddate']; ?>" id="oldrdate@<?php echo $i; ?>" /> <?php } ?>
 <select <?php  if($_SESSION['db'] =='alkhumasiyabrd') { ?> onchange="getrdate(this.id,this.value)" <?php } ?> id="flock@<?php  echo $i; ?>" name="flock[]" style="<?php if(($cat1 == "Broiler Birds") or ($cat1 == "Male Birds") or ($cat1 == "Female Birds") or ($cat1 == "Eggs") or ($cat1 == "Hatch Eggs") or ($cat1 == "Broiler Chicks") or ($_SESSION['db'] == "mlcf" and $cat1 == "Layer Birds") or (($cat1 == "Fattener Birds") or ($cat1 == "Quail Male Birds") or ($cat1 == "Quail Female Birds") or ($cat1 == "Quail Eggs") or ($cat1 == "Quail Hatch Eggs") or ($cat1 == "Quail Chicks")   and $_SESSION['db'] == "albustan")) { ?> width:100px <?php } else { ?> display:none;width:100px <?php  } ?>">
 <option value="All">All</option>
<option value="">-Select Flock-</option>
	<?php
if($_SESSION['db'] == "albustan")
{
	if($cat1 == "Quail Male Birds" or $cat1 == "Quail Female Birds" or $cat1 == "Quail Eggs" or $cat1 == "Quail Hatch Eggs")
	{
	  $queryf = "SELECT flockcode FROM quail_flock ORDER BY flockcode";
	  $resultf = mysql_query($queryf,$conn) or die(mysql_error());
	  while($rowsf = mysql_fetch_assoc($resultf))
	  {
	   ?>
	   <option value="<?php echo $rowsf['flockcode']; ?>" <?php if($rowsf['flockcode'] == $qr['flock']) { ?> selected="selected" <?php } ?>><?php echo $rowsf['flockcode']; ?></option>
	   <?php
	  }
	}
	elseif($cat1 == "Quail Chicks" or $cat1 == "Fattener Birds")
	{
	  $queryf = "SELECT farm FROM quail_broiler_farm ORDER BY farm";
	  $resultf = mysql_query($queryf,$conn) or die(mysql_error());
	  while($rowsf = mysql_fetch_assoc($resultf))
	  {
	   ?>
	   <option value="<?php echo $rowsf['farm']; ?>" <?php if($rowsf['farm'] == $qr['flock']) { ?> selected="selected" <?php } ?>><?php echo $rowsf['farm']; ?></option>
	   <?php
	  }	
	}
}	
...
<?php 
     ...
     ?>
	   <option value="<?php echo $row1['sector'];?>" <?php if($row1['sector'] == $qr['flock']) { ?> selected="selected" <?php } ?>><?php echo $row1['sector']; ?></option>
	   <?php }
...
<?php } }
		...
		{
		if($_SESSION['db'] == "golden" || $_SESSION['db'] == "alkhumasiyabrd")
		{
		$rdateflock[$i]=date('d.m.Y',strtotime($qr[eggreceiveddate])).','.$qr[flock];
		$query = "SELECT distinct(flkmain) as flockcode  FROM breeder_flock ORDER BY flkmain ASC";
		}
		else
		{
		$query = "SELECT distinct(flockcode)  FROM breeder_flock ORDER BY flockcode ASC";
...
{
  $mlresult = mysql_query("select distinct(flockcode) from layer_flock order by flockcode",$conn) or die(mysql_error());
  while($mlres = mysql_fetch_assoc($mlresult))
  {
?>
<option value="<?php echo $mlres['flockcode'];?>" title="<?php echo $mlres['flockcode'];?>" <?php if($mlres['flockcode'] == $qr['flock']){ ?> selected="selected" <?php } ?> > <?php echo $mlres['flockcode'];?> </option>
...
<?php if($_SESSION['db'] =='alkhumasiyabrd' ) { ?>
<td>
<select id="rdate@<?php echo $i; ?>" name="rdate[]" <?php if($cat1!='Hatch Eggs') { ?>  style="display:none" <?php } ?>>
<option value="<?php echo date('d.m.Y',strtotime($qr[eggreceiveddate])).'@0@0' ?>"> <?php echo date('d.m.Y',strtotime($qr['eggreceiveddate'])); ?></option>
</select>
</td>
<td width="10px">&nbsp;</td>
<?php } ?>
...
<?php
  ...
  <tr style="height:20px"></tr>
   <tr>
	   ...
	   <option value="Excluded" <?php if($freighttype =="Excluded"){?> selected="selected"<?php } ?>>Excluded</option></select></td>
...
//alert(index);
var ind= index-1;
if(document.getElementById('qtys@'+ind).value != "")
{
makeForm1();
}
}
...
function getcode(cat)
{
	...
	<?php if(($_SESSION['client'] == "GOLDEN") or ($_SESSION['db'] == "johnson")){?>
	document.getElementById('unit@' + index1).style.display = "none";
	<?php } ?>
...
echo "if(flock=='$array[flock]') {   "; 
 $query="select date,age,sum(availableeggs) as availableeggs from ims_eggreceiving where flock='$array[flock]' and availableeggs >0 and tocode in (select code from ims_itemcodes where cat='hatch eggs') group by date order by date";
...
<?php if($_SESSION[db]=='alkhumasiyabrd') { for($i=0;$i<count($rdateflock);$i++) { ?>
rdateArray[++rdateArrayIndex]="<?php echo $rdateflock[$i]; ?>";
<?php } } ?>
