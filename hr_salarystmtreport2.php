...
?>
<center>
<h4 style="color:red;font-weight:bold;padding-top:10px"><u>Salary Statement</u></h2>
</center>
<br />
<table align="center">
<tr height="20px"></tr>
...
<td width="10px">&nbsp;</td>
<td>
<select id="sector" name="sector"   style="width:150" onChange="fun1()" tabindex="2">
<option> Select  </option>
<option value="All">All</option>
<?php
           include "config.php"; 
           $query = "SELECT distinct(sector) FROM hr_employee where client ='$client'  ORDER BY sector ASC ";
...
<td width="10px">&nbsp;</td>
<td>
<select name="desig" id="desig" tabindex="2" onChange="" style="width:150">
...
var f = 0;
   ...
   if( (sector =='Select') || (desig =='Select') || (month =='Select') || (year =='Select') )
   {
   alert('You have not selected SECTOR or DESIGNATION or MONTH or YEAR, Please select all these');
   }
  
   else 
   {
  
       window.open('production/Salary_Statementsmry2.php' + '?m=' + fields + '&n=' + fields1 + '&desig=' + desig + '&month=' + month+ '&year=' + year+ '&sector=' + sector); 
	}
...
function fun1()
{
			  removeAllOptions(document.getElementById("desig"));
			  
			  myselect1 = document.getElementById('desig');
			  theOption1=document.createElement("OPTION");
			  theText1=document.createTextNode("Select");
			  theOption1.value = "Select";
			  theOption1.appendChild(theText1);
...
<?php
     ...
     $q2=mysql_query("select distinct(sector) from hr_employee where client ='$client' order by sector ASC");
	 
     while($nt2=mysql_fetch_array($q2)){
     echo "if(document.getElementById('sector').value == '$nt2[sector]'){";
     $q3=mysql_query("SELECT distinct(designation) FROM hr_employee where sector  = '$nt2[sector]' and client ='$client' ORDER BY designation ASC");
     while($nt3=mysql_fetch_array($q3))
	 { ?>
			 

              theOption1=document.createElement("OPTION");
			  theText1=document.createTextNode("<?php echo $nt3['designation']; ?>");
			  theOption1.value = "<?php echo $nt3['designation']; ?>";
			  theOption1.appendChild(theText1);
			  myselect1.appendChild(theOption1);
 ...
 <?php   } 
  ...
  else
  {
     ...
     $q3=mysql_query("select distinct(designation) from hr_employee where client ='$client' order by name ASC");
  ...
  <?php echo "if(document.getElementById('sector').selectedIndex == 0){ " ; ?>
			  removeAllOptions(document.getElementById('desig'));
			  myselect1 = document.getElementById('desig');
			  theOption1=document.createElement('OPTION');
			  theText1=document.createTextNode('Select');
