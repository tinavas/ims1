...
<form id="form1" name="form1" method="post" action="pp_updategateentry.php" onsubmit="return checkform();">
...
<input type="text" size="15" id="date" name="date" readonly="readonly" class="datepicker" value="<?php echo date("d.m.Y",strtotime($gres['date'])); ?>" onchange="getge()" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<table id="test" border="0" align="center">
  <tr>
    ...
    <th width="10px"></th>
    <th><strong title="Balance Birds">Bal Birds.</strong></th>
    <?php } ?>
    <th width="10px"></th>
    <th><strong title="Balance Quantity">Balance
      <?php if($_SESSION['db']=="vista"){ ?>
      Weight.
      <?php } else {?>
      Qty
    ...
    <?php if($_SESSION['db']=="vista"){ ?>
    <th width="10px"></th>
    <th><strong title="Balance Quantity">Birds.</strong></th>
    <?php } ?>
    <th width="10px"></th>
    <th><strong>
      <?php if($_SESSION['db']=="vista"){ ?>
...
?>
  <tr>
    <td style="vertical-align:middle;"><select style="width:90px;" name="itemcode[]" id="itemcode/<?php echo $i; ?>" onchange="getdescription(this.id)">
      <option value="<?php echo $gres['code']; ?>"><?php echo $gres['code']; ?></option>
    </select></td>
    <td width="10px"></td>
    <td style="vertical-align:middle;"><select id="description/<?php echo $i; ?>" name="description[]" style="width:100px" onchange="getcode(this.id);">
...
if($_SESSION['db']=="vista")
  	      $query2 = "select (weight - receivedquantity) as balqty,(quantity - recbird) as balbird from pp_purchaseorder where description = '$gres[desc1]' AND po = '$gres[combinedpo]' AND flag = '1' AND geflag = '0' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0)";
else
    ...
    <td ><select id="po/<?php echo $i; ?>" name="po[]" onclick="getvalue(this.id);" style="width:150px">
      <option disabled="disabled" value="">-Select-</option>
      <option value="<?php echo $gres['combinedpo']."@".$row2['balqty']; ?>" selected="selected"><?php echo $gres['combinedpo'];?></option>
    </select></td>
    <td width="10px"><input type="hidden" id="hpo/<?php echo $i; ?>" name="hpo[]" value="<?php echo $gres['combinedpo'];?>" size="10"  /></td>
    <td style="vertical-align:left;"><input type="text" id="bird/<?php echo $i;?>" name="bird[]"  size="10" value="<?php echo $row2['balbird']+$gres['balbird'];?>" readonly style="background:none; border:none; text-align:left"  /></td>
    <td style="vertical-align:middle;"><input type="text" id="balqty/<?php echo $i;?>" name="bquantity[]" value="<?php echo $row2['balqty'] + $gres['acceptedquantity']; ?>" size="10" readonly style="background:none; border:none; text-align:right"  /></td>
    <?php  if($_SESSION['db']=="vista") {?>
    <td width="10px"></td>
    <?php } ?>
    <?php  if($_SESSION['db']=="vista") {?>
    <td width="10px"></td>
    <td style="vertical-align:middle;"><input type="text" id="balbird/<?php echo $i;?>" name="balbird[]" value="<?php echo $gres['balbird'];?>" size="10" style="text-align:right" onkeyup="validate1(this.id,this.value)" onkeypress="return num(this.id,event.keyCode);"   /></td>
    <?php } ?>
    <td width="10px"></td>
    <td style="vertical-align:middle;"><input type="text" id="quantity/<?php echo $i; ?>" name="quantity[]" value="<?php echo $gres['acceptedquantity'];?>" size="10" onfocus="makeForm(this.id);" style="text-align:right" onkeypress="return num1(this.id,event.keyCode);"  onkeyup="validate(this.id,this.value)"  /></td>
    <td width="10px"></td>
    <?php
if($_SESSION['db'] == "feedatives") { ?>
    <td style="vertical-align:middle;"><input type="text" id="price/<?php echo $i;?>" name="price[]" value="<?php echo $gres['rateperunit'];?>" size="10" style="text-align:left" /></td>
    <td width="10px"></td>
    <?php } ?>
...
<center>
<input type="submit" id="Save" value="Update"  />&nbsp;&nbsp;&nbsp;<input type="button" id="Cancel" value="Cancel" onClick="document.location='dashboardsub.php?page=pp_gateentry';"/>
</center>
</td>
</tr>
</table>
...
var id=id.substr(9,id.length);

for(j=0;j<=index;j++)
	{
	
	
	var a=document.getElementById("itemcode/"+j).value;
var b=document.getElementById("description/"+j).value;
var c=document.getElementById("balbird/"+j).value;
...
var e=document.getElementById("quantity/"+j).value;
...
theOption1.value = "No";
myselect1.onchange = function () { makeForm(this.id); };
theOption1.appendChild(theText1);
myselect1.appendChild(theOption1);
...
<?php $query1 = "select distinct(code),vendor from pp_purchaseorder WHERE flag = '1' AND geflag = '0' order by code ASC"; $result1 = mysql_query($query1,$conn);
      while($row1 = mysql_fetch_assoc($result1)) {
		...
		if($_SESSION['db']=="vista")
  	      $query2 = "select distinct(description),po,(weight - receivedquantity) as balqty,rateperunit,(quantity-recbird) as balbird from pp_purchaseorder where code = '$row1[code]' and vendor ='$row1[vendor]' AND flag = '1' AND geflag = '0' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0)";
		  else
...
<?php $query1 = "select distinct(description),vendor from pp_purchaseorder WHERE flag = '1' AND geflag = '0' order by code ASC"; $result1 = mysql_query($query1,$conn);
      while($row1 = mysql_fetch_assoc($result1)) {
		...
		if($_SESSION['db']=="vista")
  	      ...
  	      $query2 = "select distinct(code),po,(weight - receivedquantity) as balqty,rateperunit,(quantity-recbird) as balbird from pp_purchaseorder where description = '$row1[description]' and vendor ='$row1[vendor]' AND flag = '1' AND geflag = '0' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0)";
...
<?php $query1 = "select distinct(code),broker from pp_purchaseorder WHERE flag = '1' AND geflag = '0' order by code ASC"; 
      ...
      while($row1 = mysql_fetch_assoc($result1)) {
	    echo "if((document.getElementById('itemcode/' + index1).value == '$row1[code]') && (document.getElementById('vendor').value == '$row1[broker]')) { ";
		  ...
		  if($_SESSION['db']=="vista") 
		  $query2 = "select distinct(description),po,(weight - receivedquantity) as balqty,rateperunit,(quantity-recbird) as balbird from pp_purchaseorder where code = '$row1[code]' and broker ='$row1[broker]' AND flag = '1' AND geflag = '0' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0)";
		  else
...
<?php $query1 = "select distinct(description),broker from pp_purchaseorder WHERE flag = '1' AND geflag = '0' order by code ASC"; $result1 = mysql_query($query1,$conn);
	...
	if($_SESSION['db']=="vista")	  
		  $query2 = "select distinct(code),po,(weight - receivedquantity) as balqty,rateperunit,(quantity-recbird) as balbird from pp_purchaseorder where description = '$row1[description]' and broker ='$row1[broker]' AND flag = '1' AND geflag = '0' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0)";
		  else
