...
for($i=0;$i<count($_POST['chicks']);$i++) 
{
 ...
 { $total = 0; $total1 = 0; $cost = 0; $cost1 = 0;
  ...
  // $newflock = $_POST['flkcount'][$i];
   	$q3 = mysql_query("select max(flock) as flock from ims_stocktransfer where towarehouse = '$farmer' AND cat = 'Broiler Feed' AND client = '$client' ") or die(mysql_error());
	if($q3r = mysql_fetch_assoc($q3))
	{
   $newflock1 = $q3r['flock'];
   }
   $q21 = "select count(*)  as c1 from ims_stocktransfer WHERE aflock ='$aflock' and  towarehouse = '$farmer' and client = '$client'";
$qrs = mysql_query($q21,$conn) or die(mysql_error());
if($qr21 = mysql_fetch_assoc($qrs))
{
$c1 = $qr21['c1'];
}
if($c1 > 0)
{
	$q3 = mysql_query("select flock from ims_stocktransfer where towarehouse = '$farmer' AND aflock='$aflock' and cat = 'Broiler Chicks' AND client = '$client' ") or die(mysql_error());
	if($q3r = mysql_fetch_assoc($q3))
	{
   $newflock = $q3r['flock'];
   }
...
{
$newflock = $newflock1+1;
}
 $q22 = "select count(*)  as c2 from broiler_daily_entry where flock ='$aflock' and client = '$client'";
$qrs = mysql_query($q22,$conn) or die(mysql_error());
if($qr22 = mysql_fetch_assoc($qrs))
{
$c2 = $qr22['c2'];
}
