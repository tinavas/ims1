...
$q = "SELECT tid FROM pp_payment WHERE posobi = '$id' AND client = '$client'";
$r = mysql_query($q,$conn) or die(mysql_error());
while($rows = mysql_fetch_assoc($r))
{
 $tid = $rows['tid'];
 $get_entriess = "DELETE FROM ac_financialpostings WHERE trnum = '$tid' and type = 'PMT' and client = '$client'";
 $get_entriess_res1 = mysql_query($get_entriess,$conn) or die(mysql_error());
}

 $get_entriess = "DELETE FROM pp_payment WHERE posobi = '$id' and client = '$client'";
 $get_entriess_res1 = mysql_query($get_entriess,$conn) or die(mysql_error());
