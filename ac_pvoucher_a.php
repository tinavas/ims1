...
<script type="text/javascript">
	...
	<div class="float-left">
		...
		<div class="float-right"> 
			<button type="button" onClick="document.location='dashboardsub.php?page=ac_addpvoucher_a'"><img src="images/icons/fugue/tick-circle.png" width="16" height="16"> Add</button>
	   ...
	   if($_SESSION['db'] == "feedatives")
		{
		   ...
		   {
		   	$query = "SELECT * FROM ac_gl where voucher = 'P' and vstatus <> 'R' and date >= '$fromdate' and date <= '$todate' AND crdr = 'Cr' group by transactioncode ORDER BY transactioncode  desc ";
		   }
		   else
		   {
		   $sectorlist = $_SESSION['sectorlist'];
		 $query = "SELECT * FROM ac_gl where voucher = 'P' and vstatus <> 'R' and date >= '$fromdate' and date <= '$todate' AND code IN (SELECT coacode FROM ac_bankmasters WHERE code IN (SELECT code FROM ac_bankcashcodes WHERE sector in ($sectorlist) ORDER BY code ASC)) AND crdr = 'Cr' group by transactioncode ORDER BY transactioncode  desc ";
		   }
		 }
	   else
	   {
	    if($_SESSION['sectorall'] == "all" || ($_SESSION['sectorall'] == "" && $_SESSION['sectorlist'] == ""))
		{
	   $query = "SELECT * FROM ac_gl where voucher = 'P' and vstatus <> 'R' and date >= '$fromdate' and date <= '$todate' group by transactioncode ORDER BY transactioncode  desc ";
	   }
	   else
	   {
	   $sectorlist = $_SESSION['sectorlist'];
	     $query = "SELECT * FROM ac_gl where voucher = 'P' and vstatus <> 'R' and date >= '$fromdate' and date <= '$todate'  AND code IN (SELECT coacode FROM ac_bankmasters WHERE code IN (SELECT code FROM ac_bankcashcodes WHERE sector in ($sectorlist) ORDER BY code ASC)) group by transactioncode ORDER BY transactioncode  desc ";
	   }
	   }
...
<a href="dashboardsub.php?page=ac_editpvouchern_a&id=<?php echo $row1['transactioncode']; ?>"><img src="images/icons/fugue/pencil.png" style="border:0px" title="Edit"/></a>&nbsp;
<a onclick="if(confirm('Are you sure,want to delete')) document.location ='ac_deletepvouchern_a.php?id=<?php echo $row1['transactioncode']; ?>'"><img src="images/icons/fugue/cross-circle.png"  style="border:0px" title="Delete" /></a>&nbsp;&nbsp;

<?php 
} 
else 
{ ?>					<!-- After Authorization -->
<?php if ( strlen(strstr(','.$_SESSION['authorizesectors'],',PV,' ))>0 ) { ?>	<!-- If he has authority power -->
<a href="dashboardsub.php?page=ac_editpvouchern_a&id=<?php echo $row1['transactioncode']; ?>"><img src="images/icons/fugue/pencil.png" style="border:0px" title="Edit"/></a>&nbsp;
<a onclick="if(confirm('Are you sure,want to delete')) document.location ='ac_deletepvouchern_a.php?id=<?php echo $row1['transactioncode']; ?>'"><img src="images/icons/fugue/cross-circle.png" style="border:0px" title="Delete" /></a>&nbsp;&nbsp;
...
<?php } ?>
<?php } ?>	 
<a href="ac_pvoucherprint.php?id=<?php echo $row1['transactioncode']; ?>" target="_new"><img src="images/icons/fugue/report.png" style="border:0px" title="print" /></a>&nbsp;&nbsp;		  </td>
		   </tr>
           <?php 
           }
           ?>   
...
document.location = "dashboardsub.php?page=ac_pvoucher_a&month=" + month + "&year=" + year;
}
</script>
