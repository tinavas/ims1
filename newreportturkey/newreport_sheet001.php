...
$query = "SELECT min(date2) as 'date2' FROM turkey_consumption WHERE flock like '$flockget'";
$result = mysql_query($query,$conn); 
while($row1 = mysql_fetch_assoc($result))
{
...
$mincmpdate = $row1min['date2'];
 ...
 $femalet = 0;$malet=0;
 $query1t = "SELECT * FROM `pp_sobi` WHERE code in (select distinct(code) from ims_itemcodes where cat IN ('Turkey Female Birds', 'Turkey Male Birds')) AND client = '$client' AND flock like '$flockget' AND date<='$mincmpdate'";
  $result1t = mysql_query($query1t,$conn); 
  while($row11t = mysql_fetch_assoc($result1t))  
  {
   
      if($row11t['code'] == 'TRFB101')
	  {
...
<?php 
	  ...
	  if($breedi != "")
	{
	 $query34 = "SELECT * FROM turkey_standards where client = '$client' and breed = '$breedi'  and age = '$tempax' ORDER BY age ASC ";
	}
	else
	{
	 $query34 = "SELECT * FROM turkey_standards where client = '$client' and age = '$tempax' ORDER BY age ASC ";
	}
