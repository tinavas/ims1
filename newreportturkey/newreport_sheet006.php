<?php session_start(); $flock = $_SESSION['flock']; ?>
...
$query1fd = "SELECT * FROM turkey_flock where flockcode = '$flock'";
...
while($row2fd = mysql_fetch_assoc($result1fd))
{
 ...
 $query1 = "SELECT distinct(age) as 'age' FROM turkey_vacschedule where flock is null or flock = '' or flock = '$flock' ORDER BY age ASC ";
 $result1 = mysql_query($query1,$conn);
 while($row2 = mysql_fetch_assoc($result1))
 {
       $vaccode = "";
       $query1w = "SELECT * FROM turkey_vacschedule where age = '$row2[age]' and (flock is null or flock = '' or flock = '$flock') ORDER BY age ASC ";
