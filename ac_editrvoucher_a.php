...
$mode = "";
		...
		{
		$q = "select warehouse from ac_financialpostings where trnum = '$tnum' AND type = 'RV' LIMIT 1";
		$qrs = mysql_query($q,$conn) or die(mysql_error());
		while($qr = mysql_fetch_assoc($qrs))
		{
		$unitc  = $qr['warehouse'];
...
<br/>
<br/><br />
<form id="form1" name="form1" method="post" action=<?php if($vstatus=="A"){?>"ac_updatervoucher_a.php"<?php } else {?>"ac_savervoucher_a.php"<?php }?> <?php if(($sdb == "mallikarjunkld") || ($sdb == "skdnew") or $_SESSION['db'] == "alwadi") {?> onSubmit="return checkform(this);" <?php } ?> >
...
{
     	$q1 = "select code,controltype from ac_coa  where code not like 'CG%' and code not like  'PV%' and code not like  'PR%' and code not like 'WP%' order by code   ";
...
<br />
<center>
<input type="submit" value="Update" id="Update" name="Update"/>&nbsp;&nbsp;&nbsp;<input name="button" type="button" onClick="top.location='dashboard.php?page=ac_rvoucher_a';" value="Cancel">
</center>
<br />
<!-- </center> -->
</form>
...
function descriptionf()
{
	...
	?>
		...
		document.getElementById('cramount' + 1).setAttribute('readonly',true);
		<?php 
		}
		echo " } "; 
		}
...
<?php
           include "config.php"; 
           $query = "SELECT * FROM ac_coa where code not like 'CG%' and code not like  'PV%' and code not like  'PR%' and code not like 'WP%' order by code    ";
