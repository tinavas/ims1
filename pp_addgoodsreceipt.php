...
$query2 = "SELECT * FROM pp_goodsreceipt ORDER BY date ASC"; $result2 = mysql_query($query2,$conn);
...
$date1 = date("d.m.o"); $strdot1 = explode('.',$date1); $ignore = $strdot1[0]; $m = $strdot1[1]; $y = substr($strdot1[2],2,4);
  ...
  $query1 = "SELECT MAX(grincr) as grincr FROM pp_goodsreceipt WHERE m = '$m' AND y = '$y'";

$result1 = mysql_query($query1,$conn); while($row1 = mysql_fetch_assoc($result1)) {$grincr = $row1['grincr']; }
...
else if($grincr < 100 && $geincr >= 10) { $gr = 'GR-'.$m.$y.'-00'.$grincr; }
...
(Fields marked <font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font> are mandatory)<br /><br />
     ...
     <input type="hidden" name="grincr" id="grincr" value="<?php echo $grincr; ?>"/>
				...
				<input type="hidden" name="saed" value="0" />
...
<td>
<input type="text" size="15" id="date" name="date" class="datepicker" onchange="getgr()" value="<?php echo date("d.m.Y"); ?>" readonly="true" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
<td>
<strong>Warehouse</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font>&nbsp;&nbsp;</td>
<td>

<select id="warehouse" name="warehouse" style="width:150px" onchange="getvendor(this.id);">
...
$query = "select distinct(deliverylocation) from pp_purchaseorder where grflag = '0' and deliverylocation <> '' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0) and deliverylocation in ($sectorlist) order by deliverylocation";
...
<tr>
 ...
 <td align="left"><input type="text" style="background:none; border:none;" id="gr" name="gr" value="<?php echo $gr; ?>" size="14" readonly/>&nbsp;&nbsp;&nbsp;</td>
 ...
 <td align="right"><strong>Vendor</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font>&nbsp;&nbsp;&nbsp;</td>
  ...
  <td align="left"><select name="vendor" id="vendor" style="width:130px" onchange="getitemsven(this.id);">
   <option value="">-Select-</option>
...
<th><strong title="Balance Quantity">Balance Qty.</strong>
</th>
...
<select style="width:90px;" name="itemcode[]" id="itemcode/0" onchange="getdescriptionven(this.id)">
<option value="">-Select-</option>
</select></td>
...
<select id="description/0" name="description[]" style="width:100px" onchange="getcodeven(this.id);">
...
<td ><select id="po/0" name="po[]" onchange="compare(this.id,this.value);" style="width:120px"><option disabled="disabled" value="">-Select-</option></select></td>
...
<td style="vertical-align:middle;"><input type="text" id="quantity/0" name="quantity[]" value="" size="10" style="text-align:right" onkeyup="validate(this.id,this.value)" onfocus="makeForm()" onkeypress="return num(event.keyCode)" /></td>

<td width="10px"></td>
<td><input type="text" name="freightie[]" id="freightie/0" value="" size="20" style="background:none; border:0px;" /> </td>

<td width="10px"></td>
<td><input type="text" name="freightamt[]" id="freightamt/0" value="" size="8" readonly="true" /> </td>

<td width="10px"></td>
<td>

<select name="fricode[]" id="fricode/0"  style="visibility:hidden; size:90px;">
<option value="">--Select--</option>
...
<td>
<select name="cash[]" id="cash/0"  style="visibility:hidden; size:90px;">
<option value="">--Select--</option>
...
<option value="<?php echo $row1['coacode'];?>"><?php echo $row1['coacode'];?></option>
<?php
...
<input type="submit" id="Save" value="Save" />
 ...
 <td width="10px"></td>
<td><input type="button" id="Cancel" value="Cancel" onClick="document.location='dashboardsub.php?page=pp_goodsreceipt';"/></td>
