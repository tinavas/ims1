    ...
    // print "Column: $col, row: $row, type: \"$type\", value: \"$data\"<br>\n";
     if ( $row > 0 )
       {  
        ...
        if ($col == "8") { $data8 = htmlentities($data, ENT_QUOTES); }	//Freight		8
...
$qr = mysql_query($q,$conn) or die(mysql_error());
 ...
 $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client) 
	          VALUES('".$date."','','Cr','".$vca."','$totqty','".$totamount."','".$sobi."','SOBI','".$data2."','".$data9."','".$client."')";
 ...
 ///Vendor Account Credit
   ...
   $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client) 
	          VALUES('".$date."','','Cr','".$vca."','$totqty','".$amount."','".$sobi."','SOBI','".$data2."','".$data9."','".$client."')";
    $result4 = mysql_query($query4,$conn) or die(mysql_error());
	}
	
	
	////Item Account Credit
 $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse) VALUES('".$date."','".$data3."','Dr','".$iac."','$data4','".$amount."','".$sobi."','SOBI','".$data2."','".$data9."')";
	$result4 = mysql_query($query4,$conn) or die(mysql_error());			
				}
				else
				{
