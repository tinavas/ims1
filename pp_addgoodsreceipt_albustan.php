...
<form id="form1" name="form1" method="post" action="pp_savegoodsreceipt_albustan.php"> 
...
if($type == "vendor")
{
$query = "SELECT distinct(vendor) FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0'"; 
...
{
$query = "SELECT distinct(broker) FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0'"; 
$result = mysql_query($query,$conn) or die(mysql_error());
      while($row1 = mysql_fetch_assoc($result))
	   {
	    ?>
...
<td align="left"><select  id="vendorcode" name="vendorcode" onChange=" document.getElementById('vendor').value=this.value; getges(this.value);">
<option value="">-Select-</option>
<?php 
if($type == "vendor")
{
$query = "SELECT distinct(vendor),vendorcode FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0'"; 
$result = mysql_query($query,$conn) or die(mysql_error());
      while($row1 = mysql_fetch_assoc($result))
	   {
	    ?>
...
if($type == "vendor")
{
$query = "SELECT distinct(po),vendor,vendorid FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0'"; 
$result = mysql_query($query,$conn) or die(mysql_error()); 
     while($row1 = mysql_fetch_assoc($result))
	  { 
	 if($vendor == $row1['vendor']) { ?>
<option value="<?php echo $row1['po']; ?>" <?php if($po == $row1['po']) { ?> selected="selected" <?php } ?>><?php echo $row1['po']; ?></option>
<?php }}}

if($type == "broker")
{
$query = "SELECT distinct(po),broker FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0'"; 
$result = mysql_query($query,$conn) or die(mysql_error()); 
     while($row1 = mysql_fetch_assoc($result)) 
	 { 
...
<tr>
<th style="visibility:hidden"><strong>PO #</strong></th>
...
if($type == "vendor")
{
$query1 = "select * from pp_purchaseorder where vendor = '$vendor' AND po = '$po' and flag = '1' AND geflag = '0' order by vendor";
}
else if($type == "broker")
{
$query1 = "select * from pp_purchaseorder where broker = '$vendor' AND po = '$po' and flag = '1' AND geflag = '0' order by broker";
}
...
<td><input type="hidden" readonly size="15" style="background:none; border:none;text-align:center;" type="text" name="po[]" id="po<?php echo $i; ?>" value="<?php echo $row1['po'];?>" /></td>
<td width="10px"></td>
...
<td width="10px"></td>
<td><input readonly size="15" style="background:none; border:none;text-align:left;" type="text" name="description[]" id="description<?php echo $i; ?>" value="<?php echo $row1['description'];?>" /></td>
<td width="10px"></td>
<td align="center"><input readonly size="7" type="text" style="background:none; border:none;text-align:right;" name="aquantity[]" id="aquantity<?php echo $i; ?>" value="<?php echo $row1['quantity'];?>" /></td>
<td width="10px"></td>

<td align="center"><?php if($po != "" && $vendor != "") { ?><input size="7" type="text" style="text-align:right" name="rquantity[]" id="rquantity<?php echo $i; ?>" value="<?php echo 0; ?>" onKeyUp="calc(<?php echo $i; ?>);" /><?php } ?></td>
<td width="10px"></td>

<td align="right"><?php if($po != "" && $vendor != "") { ?><input  size="7" type="text"  value="<?php echo $row1['rateperunit']; ?>"  id="rateperunit<?php echo $i; ?>" name="rateperunit[]"  style="text-align:right" /><?php } ?></td>
<td width="10px"></td>
...
<td width="10px"></td>
<td><input readonly size="6" style="background:none; border:none;text-align:center;" type="text" name="units[]" id="units<?php echo $i; ?>" value="<?php echo $row1['unit'];?>" /></td>
<td width="10px"></td>
<td><?php if($po != "" && $vendor != "") { ?><select id="bagtype<?php echo $i; ?>" name="bagtype[]"><option value="">-Select-</option>
...
<td width="10px"></td>
<td><?php if($po != "" && $vendor != "") { ?><input size="5" type="text" name="bags[]" style="text-align:right" id="bags<?php echo $i; ?>" value="0"  /><?php } ?></td>
<td width="10px"></td>
<?php
if($_SESSION['client'] == 'FEEDATIVES')
{
...
<input type="hidden" id="tot<?php echo $i;?>" name="tot[]" value="<?php echo $tot = ($row1['receivedquantity'] * $row1['rateperunit']) + $row1['taxamount'] + $row1['freightamount'] - $row1['discountamount'];?>" />
...
<input type="submit" id="save" value="Save" />&nbsp;&nbsp;<input type="button" id="cancel" value="Cancel" onClick="document.location='dashboardsub.php?page=pp_goodsreceipt_albustan'"/>
...
function fun()
{
	...
	document.location = "dashboardsub.php?page=pp_addgoodsreceipt_albustan&vendor=" + vendor;
}
...
function fun1()
{
	...
	document.location = "dashboardsub.php?page=pp_addgoodsreceipt_albustan&po=" + ge + "&vendor=" + vendor + "&type=" +type;
}
...
?>
    ...
    {
?>
var recvd=parseFloat(document.getElementById("rquantity" + i).value);
document.getElementById("rquantity" + i).value = recvd;
<?php } 
	 else {
...
<?php $query = "SELECT distinct(vendor),vendorcode FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0' order by vendor"; 
$result = mysql_query($query,$conn) or die(mysql_error());
      while($row1 = mysql_fetch_assoc($result)) { ?>
	  	   theOption1=document.createElement("OPTION");
...
<?php $query = "SELECT distinct(broker) FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0' order by broker"; 
$result = mysql_query($query,$conn) or die(mysql_error());
      while($row1 = mysql_fetch_assoc($result)) { ?>
...
<?php $query = "SELECT distinct(po),vendor,vendorid FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0'";
      $result = mysql_query($query,$conn) or die(mysql_error()); 
     while($row1 = mysql_fetch_assoc($result)) { 
	 echo "if(ve == '$row1[vendor]') { ";
	 ?>
	  	   theOption1=document.createElement("OPTION");
                     theText1=document.createTextNode("<?php echo $row1['po'];?>");
			   theOption1.appendChild(theText1);
			   theOption1.value = "<?php echo $row1['po'];?>";
                     myselect1.appendChild(theOption1);
<?php echo "}"; } ?>
}
...
<?php $query = "SELECT distinct(po),broker,brokerid FROM pp_purchaseorder WHERE flag = '1' AND geflag = '0'"; $result = mysql_query($query,$conn) or die(mysql_error()); 
     while($row1 = mysql_fetch_assoc($result)) { 
