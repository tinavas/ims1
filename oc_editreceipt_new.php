...
$query = "select * from oc_receipt where tid = '$_GET[id]' limit 0,1";
...
var index = -1;
var invoices = [<?php
           $query = "SELECT distinct(invoice),balance,bookinvoice FROM oc_cobi where party = '$party' AND flag = 1 AND balance > 0 group by bookinvoice order by date";
           $result = mysql_query($query,$conn) or die(mysql_error()); 
           while($row1 = mysql_fetch_assoc($result))
		    $invoices .= "\"".$row1['invoice']."@".$row1['balance']."@".$row1['bookinvoice']."\",";
...
<form method="post" id="form1" action = "oc_savereceipt_new.php" onsubmit="return checkform(this)" >
...
<option value="COBIs" <?php if($gres['choice'] == "COBIs"){?> selected="selected" <?php } ?>>COBIs</option>
</select></td>
<td width="10px">&nbsp;</td>
<td>
<strong>Reception Mode</strong></td>
<td width="10px">&nbsp;</td>
<td>
...
<td width="10px">&nbsp;</td><td id="bankname" <?php if($gres['paymentmode'] == "Cash"){?> style="display:none" <?php } ?>>
<strong>Bank</strong>
</td>
<td width="10px">&nbsp;</td><td id="branchname" <?php if($gres['paymentmode'] == "Cash"){?> style="display:none" <?php } ?>>
...
<tr>
<td><input type="text" id="code1" size="14" name="code1" value="<?php echo $gres['code1']; ?>" readonly/></td>
<td width="10px">&nbsp;</td>
<td>
<input type="text" id="description" size="20" name="description" value="<?php echo $gres['description']; ?>" readonly/></td>
<td width="10px">&nbsp;</td>
<td>
<input type="text" id="dr" name="dr" size="6" value="Dr" value="<?php echo $gres['totalamount']; ?>" readonly/></td>
<td width="10px">&nbsp;</td>
<td>
<input type="text" id="amount" name="amount" size="10" value="<?php echo $gres['totalamount']; ?>" style="text-align:right"onkeyup="checkamount('amount@-1');"/></td>
<td width="10px">&nbsp;</td>
<td <?php if($gres['paymentmode'] == "Cash"){?> style="display:none" <?php } ?>>
<input type="text" id="bank" name="bank" size="12" value="<?php echo $gres['bank']; ?>" /></td>
<td width="10px">&nbsp;</td>
<td <?php if($gres['paymentmode'] == "Cash"){?> style="display:none" <?php } ?>>
<input type="text" id="branch" name="branch" size="12" value="<?php echo $gres['branch']; ?>" /></td>
<td width="10px">&nbsp;</td>
...
<span style="display:none" id="loading" ><img  title="Verifying the Cheque Number" src="images\mask-loader.gif" ></span></td>

<td width="10px">&nbsp;</td>
<td id="cheq_date_text" <?php if($gres['paymentmode'] == "Cash"){?> style="display:none" <?php } ?>>
<input type="text" id="cdate" name="cdate" size="15" class="datepicker" value="<?php echo date("d.m.Y",strtotime($gres['cdate']));?>"/></td>
</tr>
</table>
<br id="cobibr1" <?php if($gres['choice']<>"COBIs"){?> style="display:none" <?php } ?> />
<br id="cobibr2" <?php if($gres['choice']<>"COBIs"){?> style="display:none" <?php } ?> />
<table id="cobitable" style="width:325px;<?php if($gres['choice']<>"COBIs"){?> display:none <?php } ?>">
<tr>
<td>
...
<?php 
if($gres['choice']=="COBIs")
{
$tamount = 0;
$query = "select invoice,bookinvoice from oc_cobi where invoice in (select socobi from oc_receipt where tid = '$_GET[id]') group by invoice";
$result = mysql_query($query,$conn) or die(mysql_error());
while($res = mysql_fetch_assoc($result))
$book[$res['invoice']] = $res['bookinvoice'];
$i = 0;
$query = "select * from oc_receipt where tid = '$_GET[id]'";
$result = mysql_query($query,$conn) or die(mysql_error());
while($res = mysql_fetch_assoc($result))
{
$tamount += $res['amountreceived'];
?>
<tr>
<td>
<select name="book[]" id="book@<?php echo $i; ?>" onchange="loadamounts(this.id);">
<option value="">-Select-</option>
<option value="<?php echo $res['socobi']."@".$res['actualamount']."@".$book[$res['socobi']]; ?>" selected="selected"><?php echo $book[$res['socobi']]; ?></option>
</select>
</td>
<td>
<select name="cobi[]" id="cobi@<?php echo $i; ?>" onchange="loadamounts(this.id);" style=" width:200px;">
<option value="">-Select-</option>
<option value="<?php echo $res['socobi']."@".$res['actualamount']."@".$book[$res['socobi']]; ?>" selected="selected"><?php echo $res['socobi']; ?></option>
</select>
</td>
<td align="right">
...
<td align="center">
<input type="text" id="amountreceived@<?php echo $i; ?>" size="10" value="<?php echo $res['amountreceived']; ?>" name="amountreceived[]" onfocus="makeRow();" onkeyup="checkamount(this.id);"/>
</td>
</tr>
<tr height="10px"><td></td></tr>
<?php $i++;}} ?>
...
<br />
<input type="submit" id="save" <?php if($gres['choice']=="COBIs" && $gres['totalamount'] <> $tamount){?> disabled="disabled" <?php } ?> name="save" value="Pay" /> &nbsp;&nbsp;&nbsp;
...
{
<?php 
	     $query1 = "SELECT name,code FROM contactdetails where  type = 'party' OR type = 'vendor and party' ORDER BY name ";
...
function reloadpage()
{
	...
	document.location = "dashboardsub.php?page=oc_addreceipt_new&party=" + party + "&partycode=" + partycode + "&date=" + date + "&docno=" + docno  + "&id=" + id ;
}
...
document.getElementById('save').disabled = true;
if(document.getElementById('amount').value == '0')
  if(document.getElementById('choice').value == "COBIs")
{
document.getElementById('amount').focus();
document.getElementById('amountreceived@' + index1).value = "0";
alert("Please enter amount");
