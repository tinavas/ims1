...
define("EW_REPORT_TABLE_VAR", "stocksmry", TRUE);
define("EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE", "stocksmry_grpperpage", TRUE);
define("EW_REPORT_TABLE_SESSION_START_GROUP", "stocksmry_start", TRUE);
define("EW_REPORT_TABLE_SESSION_SEARCH", "stocksmry_search", TRUE);
define("EW_REPORT_TABLE_SESSION_CHILD_USER_ID", "stocksmry_childuserid", TRUE);
define("EW_REPORT_TABLE_SESSION_ORDER_BY", "stocksmry_orderby", TRUE);

// Table level SQL
$EW_REPORT_TABLE_SQL_FROM = "ims_itemcodes Inner Join chicken_category On ims_itemcodes.cat = chicken_category.cat";
$EW_REPORT_TABLE_SQL_SELECT = "SELECT chicken_category.cat, ims_itemcodes.code, ims_itemcodes.description FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_WHERE = "";
$EW_REPORT_TABLE_SQL_GROUPBY = "";
$EW_REPORT_TABLE_SQL_HAVING = "";
$EW_REPORT_TABLE_SQL_ORDERBY = "chicken_category.cat ASC";
$EW_REPORT_TABLE_SQL_USERID_FILTER = "";
$EW_REPORT_TABLE_SQL_CHART_BASE = "";

// Table Level Group SQL
define("EW_REPORT_TABLE_FIRST_GROUP_FIELD", "chicken_category.cat", TRUE);
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="stocksmrysmry.php?export=html&fdate=<?php echo $_GET['fdate'];?>&tdate=<?php echo $_GET['tdate']; ?>&pu=<?php echo $_GET['pu'];?>">Printer Friendly</a>
&nbsp;&nbsp;<a href="stocksmrysmry.php?export=excel&fdate=<?php echo $_GET['fdate'];?>&tdate=<?php echo $_GET['tdate']; ?>&pu=<?php echo $_GET['pu'];?>">Export to Excel</a>
&nbsp;&nbsp;<a href="stocksmrysmry.php?export=word&fdate=<?php echo $_GET['fdate'];?>&tdate=<?php echo $_GET['tdate']; ?>&pu=<?php echo $_GET['pu'];?>">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<form action="stocksmrysmry.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="stocksmrysmry.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="stocksmrysmry.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="stocksmrysmry.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="stocksmrysmry.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
$quant = 0;
 ...
 $query = "select sum(quantity) as quantity from chicken_chickentransfer where category = '$cat' and tocode = '$code'";
...
if($pu == 'All')
{
$query = "SELECT sum(quantity) as transferout from ims_stocktransfer where date < 'f$date' and fromwarehouse in (select distinct(sector) from tbl_sector where type1 = 'Processing Unit' and client = '$client') and cat LIKE '%Birds%' and client = '$client'";
}
else
{
$query = "SELECT sum(quantity) as transferout from ims_stocktransfer where date < 'f$date' and fromwarehouse = '$pu' and cat LIKE '%Birds%' and client = '$client'";
}
$result = mysql_query($query,$conn1) or die(mysql_error());
$rows = mysql_fetch_assoc($result);
...
if($pu == 'All')
{
$query = "SELECT sum(birds) as cprocessing from chicken_chickentransfer where date < 'f$date' and unit in (select distinct(sector) from tbl_sector where type1 = 'Processing Unit' and client = '$client') and category LIKE '%Birds%' ";
}
else
{
$query = "SELECT sum(birds) as cprocessing from chicken_chickentransfer where date < 'f$date' and unit = '$pu' and category LIKE '%Birds%' ";
}
...
if($pu == 'All')
{
$query = "SELECT sum(quantity) as transferin from ims_stocktransfer where date < 'f$date' and towarehouse in (select distinct(sector) from tbl_sector where type1 = 'Processing Unit' and client = '$client') and cat LIKE '%Birds%' and client = '$client'";
}
else
{
$query = "SELECT sum(quantity) as transferin from ims_stocktransfer where date < 'f$date' and towarehouse = '$pu' and cat LIKE '%Birds%' and client = '$client'";
}
...
function getSort()
{
	...
	if (strlen(@$_GET["cmd"]) > 0) {
		...
		if ($sCmd == "resetsort") {
			...
			$_SESSION["sort_stocksmry_cat"] = "";
			$_SESSION["sort_stocksmry_code"] = "";
			$_SESSION["sort_stocksmry_description"] = "";
		}
