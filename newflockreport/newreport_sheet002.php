 ...
 $query1tpur = "SELECT * FROM `pp_sobi` WHERE code in (select distinct(code) from ims_itemcodes where cat ='Female Birds') AND client = '$client' AND flock like '%$flockget' AND date < '$prodate'";
  ...
  while($row11tpur = mysql_fetch_assoc($result1tpur))  
  {
	  //echo "female pur    ";echo 
	   $startmale = $startmale + $row11tpur['receivedquantity'];
  }
...
<center>
  <table><tr><td style="font-size:13px"><b>Laying Report For Flock <?php session_start(); echo $_SESSION['displayflock'] ; ?></b></td></tr></table>
</center>
<br /><br />
<table border=0 cellpadding=0 cellspacing=0 width=2526 style='border-collapse:
 ...
 1572;width:32pt'>
 <col class=xl198 width=31 span=4 style='mso-width-source:userset;mso-width-alt:
 694;width:26pt'>
 <col class=xl280 width=43 span=2 style='mso-width-source:userset;mso-width-alt:
 1572;width:32pt'>
 ...
 width:37pt'>
 <col class=xl198 width=53 style='mso-width-source:userset;mso-width-alt:1572;
 width:45pt'>
 ...
 width:35pt'>
 <col class=xl279 width=53 style='mso-width-source:userset;mso-width-alt:1572;
 width:45pt'>
...
<?php } ?>
 ...
 <tr height=17 style='mso-height-source:userset;height:12.75pt'>
  ...
  <td colspan=2 class=xl249 style='border-left:none'><?php echo $_SESSION['displayflock']; ?></td>
  <td class=xl245>&nbsp;</td>
...
$weekfexcess = 0;$weekmexcess = 0;
$weekfshrtage = 0;$weekmshrtage = 0;
...
$cumfexcess = 0;$cummexcess = 0;
$cumfshrtage = 0;$cummshrtage = 0;
...
while($row1 = mysql_fetch_assoc($result))
{
 ...
 $queryff = "SELECT sum(shortagefemale) as fshrtage, sum(shortagemale) as mshrtage, sum(excessfemale) as fexcess, sum(excessmale) as mexcess FROM breeder_birdsadjustment WHERE flock like '%$flockget' and date = '$row1[date2]' GROUP BY date,flock ORDER BY date ASC";
 $resultff = mysql_query($queryff,$conn); 
 while($rowff = mysql_fetch_assoc($resultff))
   {
   $allfshrtage = $allfshrtage + $rowff['fshrtage'];
   $allmshrtage = $allmshrtage + $rowff['mshrtage'];
   $allfexcess = $allfexcess + $rowff['fexcess'];
   $allmexcess = $allmexcess + $rowff['mexcess'];
   }
  ...
  $query1t = "SELECT cat,sum(quantity) as quantity  FROM `ims_stocktransfer` WHERE cat ='Female Birds' AND  towarehouse like '%$flockget' and fromwarehouse not like '%$flockget' AND date = '$row1[date2]' ";
    ...
    $result1t = mysql_query($query1t,$conn); 
	  ...
	  /*if($row1['date2'] == "2012-02-26")
  {
  echo $allmtrin;
  }*/
/*   $query1t = "SELECT cat,quantity as quantity  FROM `ims_stocktransfer` WHERE cat IN ('Female Birds', 'Male Birds') AND  towarehouse like '%$flockget' and fromwarehouse not like '%$flockget' AND date = '$row1[date2]' ";
    $result1t = mysql_query($query1t,$conn); 
  	while($row11t = mysql_fetch_assoc($result1t))  
  	{
	  ...
	  else if($row11t['cat'] == 'Male Birds')
	  {
	  $allmtrin = $allmtrin + $row11t['quantity'];
	  }
  }*/
      /*$query1t2 = "SELECT cat,quantity FROM `ims_stocktransfer` WHERE cat IN ('Female Birds', 'Male Birds') AND client = '$client' AND towarehouse not like '%$flockget' and fromwarehouse like '%$flockget' AND date = '$row1[date2]'";
  	$result1t2 = mysql_query($query1t2,$conn); 
  	while($row11t2= mysql_fetch_assoc($result1t2))  
  	{
      if($row11t2['cat'] == 'Female Birds')
	  {
	  $allftrout = $allftrout + $row11t2['quantity'];
	  }
	  else if($row11t2['cat'] == 'Male Birds')
	  {
  ...
  $query1t2 = "SELECT cat,sum(quantity) as quantity FROM `ims_stocktransfer` WHERE cat = 'Male Birds' AND client = '$client' AND towarehouse not like '%$flockget' and fromwarehouse like '%$flockget' AND date = '$row1[date2]'";
   ...
   $query1tpur = "SELECT * FROM `pp_sobi` WHERE code in (select distinct(code) from ims_itemcodes where cat ='Female Birds') AND client = '$client' AND flock like '%$flockget' AND date = '$row1[date2]'";
  $result1tpur = mysql_query($query1tpur,$conn); 
  while($row11tpur = mysql_fetch_assoc($result1tpur))  
  {
  ...
  $query1tpur = "SELECT * FROM `pp_sobi` WHERE code in (select distinct(code) from ims_itemcodes where cat ='Male Birds') AND client = '$client' AND flock like '%$flockget' AND date = '$row1[date2]'";
  $result1tpur = mysql_query($query1tpur,$conn); 
  while($row11tpur = mysql_fetch_assoc($result1tpur))  
  {
...
/*if($row1['date2'] == "2012-02-26" || $row1['date2'] == "2012-02-27" )
  {
  echo $row1['date2'];echo "<br>";
  echo "ftrout".$allftrout."fsale".$allfsale;echo "<br>";
   echo "mtrout".$allmtrout."msale".$allmsale;echo "<br>";
   echo "fpur".$allfpur."mpur".$allmpur;echo "<br>";
   echo "ftrin".$allftrin."mtrin".$allmtrin;echo "<br>";
  }*/
   ...
   $allfweight = 0;
   $allmweight = 0;
   $queryff = "SELECT max(eggwt) as 'eggwt',max(fweight) as 'fweight',max(mweight) as 'mweight' FROM breeder_consumption WHERE flock like '%$flockget'  and date2 = '$row1[date2]' GROUP BY date2 ORDER BY date2 ASC";
   $resultff = mysql_query($queryff,$conn); 
   while($rowff = mysql_fetch_assoc($resultff))
   {
...
//if($row1['date2'] >= $prodate)
 ...
 $weekflag = 1;
  ...
  $tfexcess = $tfexcess + $allfexcess;
  $tmexcess = $tmexcess + $allmexcess;
  $tfshrtage = $tfshrtage + $allfshrtage;
  $tmshrtage = $tmshrtage + $allmshrtage;
...
?>
 <tr height=15 style='height:11.25pt'>
  ...
  $weekfexcess = $weekfexcess + $allfexcess;
  $weekmexcess = $weekmexcess + $allmexcess;
  $weekfshrtage = $weekfshrtage + $allfshrtage;
  $weekmshrtage = $weekmshrtage + $allmshrtage;
  ?>
  ...
  <td class=xl344 style='border-top:none;border-left:none'><?php echo number_format(round(($allfmort/$female)*100,2),2);// number_format(round(($tfmort/$female)*100,2),2); ?></td>
  <td class=xl345 style='border-left:none'><?php echo number_format(round(($allmmort/$male)*100,2),2);//number_format(round(($tmmort/$male)*100,2),2); ?></td>
  ...
  <td class=xl344 style='border-top:none;border-left:none'><?php echo number_format(round(($allfcull/$female)*100,2),2);//number_format(round(($tfcull/$female)*100,2),2); ?></td>
  <td class=xl345 style='border-left:none'><?php echo number_format(round(($allmcull/$female)*100,2),2);//number_format(round(($tmcull/$female)*100,2),2); ?></td>
  ...
  <td class=xl344 style='border-top:none;border-left:none'><?php echo $tdayeggsper = number_format(round(($tdayeggs/$female)*100,2),2); $tdayeggsperw = $tdayeggsperw + $tdayeggsper; $z1p = $z1p +  $tdayeggsper;$z1=$z1+1;?></td>
  ...
  <td class=xl347 style='border-top:none'><?php echo $tdayhatcheggs;
   if($tdayhatcheggs>0 && $j==0)
	  {
		  $f1=$female;
		  $j=$j+1;
		  }
  ...
  <td class=xl351 style='border-top:none'><?php echo number_format(round(($thatcheggs/$f1),2),2); ?></td>
...
/* if($row1['date2'] == '2011-06-24')
  {
  echo $allftrin ;echo "<br>";echo $allftrout;echo "<br>";
  echo $allmtrin ;echo "<br>";echo $allmtrout;echo "<br>";
...
// $female = $startfemale + $tfpur - ($tfmort + $tfcull) + $tftrin - $tftrout ;
 ...
 //$female = $startfemale + $tfpur - ($tfmort + $tfcull) + $tftrin - $tftrout - $tfsale ;
 //$male = $startmale - ($tmmort + $tmcull) + $tmtrin - $tmtrout + $tmpur - $tmsale;
 

 $female = $startfemale + $tfpur - ($tfmort + $tfcull) + $tftrin - $tftrout - $tfexcess + $tfshrtage ;
 $male = $startmale + $tmpur - ($tmmort + $tmcull) + $tmtrin - $tmtrout - $tmexcess + $tmshrtage ;
 ...
 if($nrDaysPassed == 7)
 {
   $weekflag = 1; 
	...
	$cumfexcess =$cumfexcess + $weekfexcess;$cummexcess =$cummexcess + $weekmexcess;
	$cumfshrtage =$cumfshrtage + $weekfshrtage;$cummshrtage =$cummshrtage + $weekmshrtage;
...
?>
 <tr height=16 style='height:12.0pt'>
  ...
  <td class=xl268><?php echo $weekfmort;$_SESSION['ifmort'][$tempage] = $weekfmort; ?></td>
  
  <td class=xl268><?php echo $weekmmort; $_SESSION['immort'][$tempage] = $weekmmort; ?></td>
  <td colspan=2 class=xl268 style='mso-ignore:colspan'>&nbsp;</td>
  ...
  <td class=xl269><?php echo $tempz1 = round(($z1p/$z1),2); $tempcomp1 = round((($weekeggs/7)/$weekfemale)*100,2); $_SESSION['hd'][$tempage] = $tempz1; ?></td>
  <td class=xl268>&nbsp;</td>
  ...
  <?php /*?><td class=xl269><?php echo $temp3 = round($weekffeedg/7,2); $_SESSION['iffeedpb'][$tempage] = $temp3; $cummffeedg = $cummffeedg + $temp3; ?></td>
  <td class=xl269><?php echo $temp4 = round($weekmfeedg/7,2); $_SESSION['imfeedpb'][$tempage] = $temp4; $cummmfeedg = $cummmfeedg + $temp4; ?></td><?php */?>
  <td class=xl269><?php echo $temp3 = round(((($weekffeed/$weekfemale)*1000)/$inc1),2); $_SESSION['iffeedpb'][$tempage] = $temp3; $cummffeedg = $cummffeedg + $temp3; ?></td>
  <td class=xl269><?php echo $temp4 = round(((($weekmfeed/$weekmale)*1000)/$inc1),2); $_SESSION['imfeedpb'][$tempage] = $temp4; $cummmfeedg = $cummmfeedg + $temp4; ?></td>
...
<?php
  $qb = "SELECT breed FROM breeder_flock WHERE flockcode like '%$flockget' order by breed desc limit 1";
