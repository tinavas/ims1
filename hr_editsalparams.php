...
<?php
		...
		$q = "select * from hr_params where id ='$id'";
...
<form id="form1" name="form1" method="post" action="hr_updatesalparams.php"  onsubmit="return checkform(this);">
...
<td align="left"><input type="text" name="code" size="17" id="code" value="<?php echo $code;?>"/></td>

<td align="right"><strong>Description</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font>&nbsp;&nbsp;&nbsp;</td>

<td align="left"><input type="text" name="description" size="20" id="description" value="<?php echo $description;?>" /></td>
...
<td align="left" ><select style="Width:120px" name="unittd" id="unittd" onchange="return checkper()" >

  <option value="">-Select-</option>

  <option value="Per" <?php if($unit == "Per") {?> selected="selected" <?php }?>>%</option>

  <option value="Flat" <?php if($unit == "Flat") {?> selected="selected" <?php }?>>Flat</option>
...
<option value="Salary" <?php if($basis == "Salary") {?> selected="selected" <?php }?>>Salary</option>
</select></td>
...
<td align="right" id="def" name ="def" ><strong>Default</strong>&nbsp;&nbsp;&nbsp;</td>

<td align="left"><input  name="default" id="default" size="17"  value="<?php echo $defaultval;?>" onkeyup="return checkper()" onblur="return checkper()"/></td>
...
<td align="left" id="based" ><select style="Width:120px" name="incex" id="incex" >
  ...
  <option value="include" <?php if($incex == "include") {?> selected="selected" <?php } ?>>Include</option>

  <option value="exclude" <?php if($incex == "exclude") {?> selected="selected" <?php } ?>>Exclude</option>
...
<input type="checkbox" id="ded" name="ded" value="ded" <?php if($coa!="") { ?> checked="checked" <?php } ?> onclick="checkfun1()"  />
...
<select style="Width:120px" name="coa" id="coa" onchange="checkded()"  >
  ...
  {
  ?>
  <option value="<?php echo $rows['code'];?>" title="<?php echo $rows['description'];?>" <?php if($coa==$rows['code'] && $rows['code']!="") { ?> selected="selected" <?php } ?> ><?php echo $rows['code'];?></option>
  <?php
  } 
  ?>
</select></td>
...
<td colspan="4" align="center"  style="padding-left:80px;<?php if($basis == "Salary") { ?> visibility:hidden <?php }?>" >
...
<select name="paramcode[]" id="paramcode"  style="width:150px; " multiple onclick="getdescr(this.value)">
  ...
  <?php
				  ...
				  $dsc = $row1['description'];

            ?>

  <option value="<?php echo $row1['description']; ?>" <?php if(in_array($dsc,$paramsarr)){ ?> selected="selected"<?php }?>  ><?php echo $row1['description']; ?></option>
...
<td colspan="4" align="center" id="for" name ="for" <?php if($basis == "Salary") { ?>style="visibility:hidden" <?php }?>><strong>Formula</strong>

&nbsp;&nbsp;&nbsp;&nbsp;

<textarea id="formula" name="formula" type="html"  rows="5"  ><?php echo $formula;?></textarea></td>
...
FROM `hr_params`";
			...
			if(form.code.value=="<?php echo $row['code'];?>"&& form.code.value!="<?php echo $code;?>")
			{
			...
			if(form.description.value=="<?php echo $row['description'];?>" && form.description.value!="<?php echo $description;?>") 
			{
