...
$gquery = "select * from oc_payment where tid = '$_GET[id]'";
...
<section class="grid_8">
  <div class="block-border">
     ...
     <form class="block-content form" id="complex_form" name="form1" method="post" action="oc_savepayment.php" onsubmit="return checkform1();">
...
<input type="hidden" name="cuser" value="<?php echo $addemp;?>" />
...
<tr>
<td>
<strong>Code</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></td>
<td width="10px">&nbsp;</td><td>
...
<td width="10px">&nbsp;</td><td>
<strong>Amount<font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></strong>
</td>
...
<input type="text" id="code1" size="14" name="code1" value="<?php echo $globalrow['code1'];?>" readonly/>
...
$query = "select * from ac_financialpostings where trnum = '$globalrow[tid]' and type = 'OCPMT' and coacode <> '$globalrow[code1]' and crdr = 'Cr'";
...
<select id="tdscode@<?php echo $i; ?>" name="tdscode[]" style="width:100px" onChange="loadtdsdesc(this.id);">
...
<?php for($j=0;$j<count($codedesc);$j++)
	...
	<option value="<?php echo $codedesc1[1];?>"  <?php if($res['coacode'] ==  $codedesc1[0]) { $tdsdesc = $codedesc1[1];?> selected="selected" <?php } ?> title="<?php echo $codedesc1[1];?>"><?php echo $codedesc1[0];?></option>
...
<input type="text" id="tdsdescription@<?php echo $i; ?>" size="20" name="tdsdescription[]" value="<?php echo $tdsdesc; ?>" readonly/>
...
<input type="button" id="cancel" name="cancel" value="Cancel" onClick="document.location='dashboardsub.php?page=oc_payment';"/>
</center>
</form>
</div>
</section>
...
<script type="text/javascript">
function script1() {
window.open('P2PHelp/help_p_editpayment.php','IMS','width=500,height=600,toolbar=no,menubar=yes,scrollbars=yes,resizable=no');
}
</script>
