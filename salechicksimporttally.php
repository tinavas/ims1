...
<?php
if($_POST['cat'] == "Chick Sales"){
if( !$ee->IsEmptyWorksheet($sheet) ) {

 print "<br>\nData:<br>\n";
 
 $query1 = "SELECT* from ims_itemcodes where cat = 'Broiler Chicks'";
        ...
        if ($col == "6") { $data6 = htmlentities($data, ENT_QUOTES); }
          ...
          $totalcol = "6";
                ...
                {  
                   ...
                   if($data3 == "Broiler Chicks")
				   {
				   $num = 0;
				   $cgroup = "";
				   $vca = "";
                    $query2 = "SELECT * FROM contactdetails where name =  '$data2' and type = 'party'  ";
...
$stdamount = $stdcost * $data4;
  $q = "insert into oc_cobi (date,cobiincr,m,y,invoice,bookinvoice,party,broker,code,description,quantity,price,freightamount,total,finaltotal,balance,empid,empname,sector,flag,vno,driver,freighttype,viaf,datedf,cashbankcode,dflag,discountamount,coacode,cno,units,warehouse,client,flock) 
	values('$date','$cobiincr','$m','$y','$cobi','$data1','$data2','','$code','$description','$data4','$data5','0','$amount','$amount','$amount','0','','','0','0','','','','$date','','0','0','','','','','".$client."','')";
	
	$qrs = mysql_query($q,$conn) or die(mysql_error());
	
	if($hold == 1)
	{
	 $q = "update oc_cobi set totalquantity = '$totamount',finaltotal = '$totamount' where invoice = '$cobi'  ";
...
$qr = mysql_query($q,$conn) or die(mysql_error());
 ...
 $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client) 
	          VALUES('".$date."','','Dr','".$vca."','$totalquantity','".$totamount."','".$cobi."','COBI','".$data2."','".$data2."','".$client."')";
 ...
 ///Customer Account Debit
   ...
   $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client) 
	          VALUES('".$date."','','Dr','".$vca."','$totalquantity','".$amount."','".$cobi."','COBI','".$data2."','".$data2."','".$client."')";
    $result4 = mysql_query($query4,$conn) or die(mysql_error());
	}
...
$stdamount = round($stdamount,3);
   ...
   /// COGS A/C Debit
    ...
    $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client) 
	          VALUES('".$date."','".$code."','Dr','".$cogsac."','".$data4."','".$stdamount."','".$cobi."','COBI','".$data2."','".$warehouse."','".$client."')";
    $result4 = mysql_query($query4,$conn) or die(mysql_error());
	
	///Sales A/C Credit
	$mainitemcost = round($mainitemcost,3);
   $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client) 
	          VALUES('".$date."','','Cr','".$salesac."','$data4','$amount','".$cobi."','COBI','".$vendor."','".$warehouse."','".$client."')";
 $result4 = mysql_query($query4,$conn) or die(mysql_error());
...
//Feed Sales Import
else if($_POST['cat'] == "Feed Sales")
{
if( !$ee->IsEmptyWorksheet($sheet) ) {

 print "<br>\nData:<br>\n";
 
 $query1 = "SELECT* from ims_itemcodes where cat = 'Broiler Feed'";
$result1 = mysql_query($query1,$conn);
while($row1 = mysql_fetch_assoc($result1)) 
{
$itemac = $row1['iac'];
        ...
        if ($col == "6") { $data6 = htmlentities($data, ENT_QUOTES); }
          ...
          $totalcol = "6";
                ...
                {  
                   ...
                   if(($data3 == "Pre - Starter Feed") or ($data3 == "Starter Feed") or ($data3 == "Finisher Feed"))
				   {
				   $num = 0;
				   $cgroup = "";
				   $vca = "";
...
$ignore = "0".$ignore;
 ...
 $vcaorig = $vca;
	...
	if($data3 == "Pre - Starter")
	{
	$code = "FD107";
	$description = "PRESTARTER"
	}
	else if($data3 == "Starter")
	{
	$code = "FD108";
	$description = "STARTER";
	}
	else if($data3 == "Finisher Feed")
	{
	$code = "FD109";
	$dscription = "FINISHER I";
	}				
