...
<script type="text/javascript">
	...
	<div id="control-bar" class="grey-bg clearfix"><div class="container_12">
		...
		<div class="float-right"> 
			<button type="button" onClick="document.location='dashboardsub.php?page=oc_addsalesorder'"><img src="images/icons/fugue/tick-circle.png" width="16" height="16"> Add</button>
...
<th style="text-align:left"><span class="column-sort">
								...
								<th style="text-align:left; width:200px;"><span class="column-sort">
									<a href="javascript:void(0)" title="Sort up" class="sort-up"></a>
									<a href="javascript:void(0)" title="Sort down" class="sort-down"></a>
								</span>Vendor</th>
<th style="text-align:left; width:200px;"><span class="column-sort">
...
<tbody>
		       ...
		       if($_SESSION['sectorall'] == "all" || ($_SESSION['sectorall'] == "" && $_SESSION['sectorlist'] == ""))
	{
	 ...
	 $sectorlist = $_SESSION['sectorlist'];
		...
		if($sectorlist=="")   
		  $query = "SELECT distinct(po),vendor,date,flag,empname FROM oc_salesorder where date >= '$fromdate' and date <= '$todate'  order by date DESC ";
		  else
	$query = "SELECT distinct(po),vendor,date,flag,empname FROM oc_salesorder where date >= '$fromdate' and date <= '$todate' and warehouse in ($sectorlist)  order by date DESC ";	  
           ...
           {	
               ...
               $name = ""; $name1 = "";
               $quantity = "";
               $query2 = "SELECT * FROM oc_salesorder WHERE po = '$row1[po]' ORDER BY date DESC ";
               $result2 = mysql_query($query2,$conn); 
               $num2 = mysql_num_rows($result2);
               while($row2 = mysql_fetch_assoc($result2))
               {
			     $name1 = $name1."/".$row2['description'];
                 $name = $name."/".$row2['code'];
                 $quantity = $quantity."/".$row2['quantity'];
               
               } 
			   $name1 = substr($name1,1,(strlen($name1)-1));
           ...
           ?>
            <tr>
			 ...
			 <td title="<?php echo $name1; ?>"><?php echo $name; ?></td>
			 <td><?php echo $quantity; ?></td>
			 <td>
			 
<?php
$flag = $row1['flag'];			 
$tquery	= "SELECT id FROM oc_packslip WHERE so = '$row1[po]'";
...
?>
		...
		<a href="dashboardsub.php?page=oc_editsalesorder&po=<?php echo $row1['po']; ?>"><img src="images/icons/fugue/pencil.png" style="border:0px" title="Edit" /></a>&nbsp;
		<a href="dashboardsub.php?page=oc_deletesalesorder&po=<?php echo $row1['po']; ?>"><img src="images/icons/fugue/cross-circle.png" title="Delete"/></a>
...
{
 ?><img src="images/icons/fugue/lock.png" width="16px" style="border:0px" title="Pack Slip Completed" /><?php
}
?>	
&nbsp;&nbsp;<a href="salesorderprint.php?id=<?php echo $row1['po']; ?>" target="_blank"><img src="images/icons/fugue/report.png" width="16px" style="border:0px" title="Invoice" /></a>		 
			 </td>
           </tr>
           <?php 
           }
...
document.location = "dashboardsub.php?page=oc_salesorder&month=" + month + "&year=" + year;
}
