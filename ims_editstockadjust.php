...
$id=$_GET['id'];
$globalquery="select * from ims_stockadjustment where trnum='$id'";
...
$rows1 = mysql_fetch_assoc($res1);
 ...
 $sec1=explode(",",$rows1["sector"]);	
	...
	$q = "select group_concat(Code,'@',description) as codedesc from ac_coa where (controltype = '' OR controltype IS NULL) AND (type != 'Capital') and code not like 'CG%' and code not like  'PV%' and code not like  'PR%' and code not like 'WP%' and code not like 'AS%' and code not like 'SR%' and code not like 'FU%' and code not like 'CS%' and code not like 'SA%' and code not like 'LI%' order by code";
							$qrs = mysql_query($q,$conn) or die(mysql_error());
							$qr = mysql_fetch_assoc($qrs);
							{
				
						 
					 $codedesc=explode(",",$qr["codedesc"]);	
...
<section class="grid_8">
  <div class="block-border">
     <form class="block-content form" id="complex_form" method="post" action="ims_saveeditstockadjust.php" onsubmit="return validate()" >	
...
<br /><br />
            ...
            <table align="center">
              <tr>
                ...
                <td>&nbsp;
					<select id="unit" name="unit" style="width:175px">
 ...
 ?><option value="<?php echo $sec1[$j]; ?>" <?php  if ($globalrow['unit'] == $sec1[$j]) {?> selected="selected" <?php } ?>  title="<?php echo $sec1[$j]; ?>"><?php echo $sec1[$j]; ?></option>
<?php }	   ?></select>
				</td>
                <td><strong>Coa Code</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></td>
                <td>&nbsp;
					<select id="coa" name="coa" style="width:125px">
					<option value="">-Select-</option>
		   ...
		   for($j=0;$j<count($codedesc);$j++)
		   {
			$codedesc2=explode('@',$codedesc[$j]);
           ?>
<option value="<?php echo $codedesc2[0];?>" <?php  if ($globalrow['coacode'] == $codedesc2[0]) {?> selected="selected" <?php } ?> title="<?php echo $codedesc2[1]; ?>"><?php echo $codedesc2[0]; ?></option>
<?php } ?>
...
<th><strong>Quantity</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></th><td width="10px">&nbsp;</td>
	 ...
	 <?php
	 $i=-1;
	 $gquery="select * from ims_stockadjustment where trnum = '$id' order by id";
...
<td width="10px">&nbsp;</td><td>
<input type="text" size="10" style="text-align:right;" id="qtys@<?php echo $i; ?>" name="qtys[]" value="<?php echo $globalrow['quantity'];?>"  onblur="makeform(this.id)" />
...
<?php 
$gquery="select narration from ims_stockadjustment where trnum = '$id' order by id";
	 $gresult=mysql_query($gquery,$conn) or die(mysql_error());
...
<td><textarea id="remarks" id="remarks" name="remarks" rows="3" cols="50"><?php echo $row['narration'];?></textarea></td>
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <input type="button" value="Cancel" onClick="document.location='dashboardsub.php?page=ims_stockadjust';">
</center>
