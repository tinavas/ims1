...
if($_POST['code'][$i] != "" && $_POST['towarehouse'][$i] != "" && $_POST['squantity'][$i] != "")
{
	...
	$quantity = $_POST['squantity'][$i];
	 if($_SESSION['client'] == 'RAYBON')
    {
    $q_cat="select distinct(type) from ims_itemtypes WHERE type like '%Feed%'"; 
    $r_cat=mysql_query($q_cat,$conn) or die(mysql_error()); 
    while($a_cat=mysql_fetch_array($r_cat))
      if($a_cat['type']==$cat){ $quantity*=50; break;}
    }
    $price = $_POST['price'][$i];	
	$tmno = $_POST['tmno'][$i];
	$tcost = $_POST['tcost'][$i];
...
if(($cat == 'Broiler Feed') OR ($cat == 'Medicines') OR ($cat == 'Vaccines'))
{
 ...
 elseif($_SESSION['client'] == 'SOUZANEW')
 {
...
{
    if($batch=="") $batch=0; 
	if($expiry_date=="") $expiry_date="0000-00-00" ; else $expiry_date=date("Y-m-d",strtotime($expiry_date));
	$insert_fedative=',expiry_date,batch_no';  
    $values_fedative=",'".$expiry_date."','".$batch."'";
}

	$q = "insert into ims_stocktransfer (tid,date,cat,fromwarehouse,fromunits,towarehouse,tounits,code,quantity,empid,empname,sector,adate,aempid,aempname,asector,flock,aflock,tflock,price,vehicleno,driver,tmno,tcost,remarks,flag,client $insert_fedative) values ('$tid','$date','$cat','$fromwarehouse','$units','$towarehouse','$units','$code','$quantity','$empid','$empname','$sector','$date','$empid','$empname','$sector','$flock','$aflock','$tflock','$price','$vno','$driver','$tmno','$tcost','$remarks','1','$client' $values_fedative) ";
	$qrs = mysql_query($q,$conn) or die(mysql_error());
