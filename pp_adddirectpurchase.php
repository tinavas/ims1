...
$acno1=json_encode($acno);
	   		...
	   		/*$q = "select group_concat(distinct(code),'@',description) as code  from ac_coa where (controltype = '' or controltype is NULL) and type = 'Expense' and schedule = 'INDIRECT EXPENSE' and description like '%Auto%' and  code not like 'CG%' and code not like  'PV%' and code not like  'PR%' and code not like 'WP%' order by code";*/
			$q = "select group_concat(distinct(code),'@',description) as code  from ac_coa where description like '%Auto%' order by code";
...
$c_q="select fromdate,todate,supplier,warehouse,code,price from pp_pricemaster";
				
				$c_qr = mysql_query($c_q,$conn) or die(mysql_error());
				$k=0;
					while($qrc = mysql_fetch_assoc($c_qr))
					{
						$price_list[$k]=array("fromdate"=>"$qrc[fromdate]","todate"=>"$qrc[todate]","supplier"=>"$qrc[supplier]","warehouse"=>"$qrc[warehouse]","code"=>"$qrc[code]","price"=>"$qrc[price]");
						$k++;
					}
...
<br />
            ...
            <table align="center">
              <tr>
                ...
                <td>&nbsp;<input class="datepickerSOBIT" type="text" size="15" id="date" name="date" value="<?php echo date("d.m.Y"); ?>" readonly="readonly" onchange="getsobi();"></td>
                <td width="5px"></td>
...
<th  <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?>><strong>Price<br />/Unit</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></th><td width="10px">&nbsp;</td>
...
<?php } ?>
       ...
       <td style="text-align:left;">
			<select style="Width:75px" name="code[]" id="code@-1" onchange="selectdesc(this.id); get_price(this.id,this.value);">
     		<option value="">-Select-</option>
			</select>       </td>
<td width="10px">&nbsp;</td><td>
...
<td width="10px">&nbsp;</td>
<td>
<input type="text" size="6" <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?> id="price@-1" name="price[]" value="0" readonly="readonly"  onkeypress="return num1(event.keyCode);" style="text-align:right; background:none; border:0px;" />
</td>
...
<select style="width:60px" name="vat[]" id="vat@-1" onchange="calnet('');">
 ...
 <td  width="10px">&nbsp;</td><td> 
<select id="flock@-1" name="flock[]" style="width:120px" onchange="makeForm(this.id); get_price(this.id,this.value);"   onkeyup="decimalrestrict(this.id,this.value),calnet('');" onblur="calnet('');">
<option value="">-Select-</option>
...
<table border="1">
   ...
   <tr>
      <td align="right" <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?>><strong>Basic&nbsp;Amount</strong>&nbsp;&nbsp;</td>
      <td align="left"><input type="text" size="12" id="basic" <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?> name="basic" value="0.00" style="text-align:right; background:none; border:0px;" readonly  /></td>
  ...
  <tr style="height:20px"></tr>
  <tr>
   <td align="right" <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?>><strong>Total&nbsp;Amount</strong>&nbsp;&nbsp;</td>
   <td align="left"><input type="text" size="12" <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?> name="totalprice" id="totalprice" style="text-align:right;background:none; border:0px;" value="0.00" readonly/></td>
   <td></td><td></td><td></td><td></td>
  ...
  <tr style="height:20px"></tr>
   <tr>
	   ...
	   <select id="coa" name="coa" style="width:110px" >
	   <option value="">-Select-</option>
	   <?php 
	   ...
	   ?>
	   <option title="<?php echo $coacode1[0]; ?>" value="<?php echo $coacode1[0]; ?>"><?php echo $coacode1[1]; ?></option>
	   <?php } ?>
	   </select>	   </td>
...
<td></td><td></td><td></td>
  ...
  <tr style="height:20px"></tr>
<tr>
	  <td align="right" <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?>><strong>&nbsp;Grand&nbsp;Total</strong>&nbsp;&nbsp;</td>
        <td align="left"><input type="text" size="12" <?php if($_SESSION[db]=='singhsatrang' && $_SESSION[admin]!='1') {  ?> style="visibility:hidden" <?php }?> name="tpayment" id="tpayment" value="0.00" readonly style="text-align:right; background:none; border:0px;"/></td>
</tr>
</table>
<br>	
<center>	
...
theOption1.value = "";
myselect1.appendChild(theOption1);
myselect1.onchange = function () { selectdesc(this.id); get_price(this.id,this.value); };
...
theOption1.value = "";
myselect1.appendChild(theOption1);
myselect1.onchange = function () { selectcode(this.id); get_price(this.id,this.value); };
...
myselect1.onchange = function () { makeForm(this.id); get_price(this.id,this.value); calnet(''); };
...
{
var i=-1;
if(document.getElementById('cat@'+i).selectedIndex==0)
{
	alert("Please Select Category");
	 document.getElementById('cat@'+i).focus();
	return false;
}
else if(document.getElementById('code@'+i).selectedIndex==0)
{
	alert("Please Select Itemcode");
	 document.getElementById('code@'+i).focus();
	return false;
}
else if(document.getElementById('qtys@'+i).value=="" || document.getElementById('qtys@'+i).value==0)
{
	alert("Please Enter Quantity Sent");
	 document.getElementById('qtys@'+i).focus();
	return false;
}
else if(document.getElementById('qtyr@'+i).value=="" || document.getElementById('qtyr@'+i).value==0)
{
	alert("Please Enter Quantity Received");
	 document.getElementById('qtyr@'+i).focus();
	return false;
}
else if(document.getElementById('price@'+i).value=="" || document.getElementById('price@'+i).value==0)
{
	alert("Please Enter Price");
	 document.getElementById('price@'+i).focus();
	return false;
}
else if(document.getElementById('flock@'+i).selectedIndex==0)
{
...
function get_price(i,v)
{

	var id_arr=i.split("@");
	
	document.getElementById("price@"+id_arr[1]).readOnly=true;

	var code=document.getElementById('code@'+id_arr[1]).value;
	var code_arr=code.split("@");
	var vendor=document.getElementById('vendor').value;
	var date=document.getElementById('date').value;
	var	date_arr=date.split('.');
	var date_r=new Date(date_arr[2]+"-"+date_arr[1]+"-"+date_arr[0]);
	var	show='no';
	var flock=document.getElementById('flock@'+id_arr[1]).value;
	
	if(flock=='Sojat City Singh Mehandi')
	{
	document.getElementById("price@"+id_arr[1]).readOnly=false;
	}
	
	
	if(document.getElementById('flock@'+id_arr[1]).value!='')
	{
		for(var i=0;i<price_data.length;i++)
		{
			if(price_data[i].code==code_arr[0] && price_data[i].supplier==vendor && flock==price_data[i].warehouse)
			{
				
				var from_date_j=new Date(price_data[i].fromdate);
				var to_date_j=new Date(price_data[i].todate);
				if(from_date_j<=date_r && date_r<=to_date_j)
				{
					document.getElementById("price@"+id_arr[1]).value=price_data[i].price;
					var show='yes';
				}
				else
				{
					if(show!='yes')
					{					
						document.getElementById("price@"+id_arr[1]).value=0;
					}
				}
			}
			else
