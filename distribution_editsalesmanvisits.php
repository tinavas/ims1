...
$q1="select * from distribution_salesmanvisits where trnum='$trnum' limit 1";

$q1=mysql_query($q1) or die(mysql_error());

$result=$details=mysql_fetch_assoc($q1);
...
{
$fromdate=date("d.m.Y",strtotime($details['date']));
}
...
<strong>Area<font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></strong>
<select name="area" id="area"  onchange="document.getElementById('areaname').options[this.selectedIndex].selected='selected';reloadpage('area')" >
<option value="">-Select-</option>
<?php
$q1="SELECT areacode FROM `distribution_area` where superstockist='$superstockist'";
...
{
?>
<option value="<?php echo $allnames[$i];?>" <?php if($allnames[$i]==$distributor){?> selected="selected" <?php }?>><?php echo $allnames[$i];?></option>
<?php }?>
...
?>
<option value="<?php echo $r1['name'];?>" title="<?php echo $r1['name'];?>" <?php if($r1['name']==$details['shop']){?> selected="selected"<?php }?>><?php echo $r1['name'];?></option>
<?php }?>
...
<option value="<?php echo $r1['salesman'];?>"<?php if($r1['salesman']==$details['salesman']){?> selected="selected" <?php }?>><?php echo $r1['salesman'];?></option>
...
<input type="radio" name="order" value="YES" id="order1" onclick="displaytable('1')" <?php if($details['order']=="YES"){?> checked="checked" <?php }?>  />&nbsp;&nbsp;<strong>Order</strong>

<input type="radio" name="order" value="NO" id="order2" onclick="displaytable('0')" <?php if($details['order']=="NO"){?> checked="checked" <?php }?>/>&nbsp;&nbsp;<strong>No Order</strong>


&nbsp;&nbsp;&nbsp;

<strong>Time<font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></strong>
<input type="text" name="time" id="time" style="width:100px;" class="timepicker" value="<?php echo $details['time'];?>"/>


</br></br></br>
<table id="tab" <?php $k=0;if($details['order']=="NO"){?> style="display:none" <?php }?> >
<tr>
...
$k=-1;
 ...
 $q1="select *  from  distribution_salesmanvisits where trnum='$trnum'";
...
<input type="text" name="units[]" id="units@<?php echo $k;?>" style="width:100px;border:none;background:none"  readonly="readonly" />
<?php }?>
...
<strong>Narration:&nbsp;&nbsp;</strong><textarea name="narration" id="narration"><?php echo $details['narration'];?></textarea>
...
document.location="dashboardsub.php?page=distribution_editsalesmanvisits&date="+date+"&superstockist="+superstockist+"&area="+area+"&trnum=<?php echo $_GET['trnum'];?>"+"&areaname="+encodeURIComponent(areaname)+"&distributor="+distributor;
