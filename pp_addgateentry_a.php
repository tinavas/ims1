...
<form id="form1" name="form1" method="post" action="pp_savegateentry_a.php" onSubmit="return checkform();">
...
<input type="text" size="15" id="date" name="date" class="datepicker" onChange="getge()" value="<?php echo date("d.m.Y"); ?>" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<strong>Warehouse</strong>&nbsp;&nbsp;
<select id="warehouse" name="warehouse" style="width:150px" onChange="getitems(this.id);">
<option value="">-Select-</option>
<?php 
$query = "select distinct(deliverylocation) from pp_purchaseorder where geflag = '0' and deliverylocation <> '' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0) order by deliverylocation";
...
<td width="10px"></td>
<td ><select id="po/0" name="po[]" onchange="compare(this.id,this.value);" style="width:150px">
...
<td width="10px"></td>
<td style="vertical-align:middle;"><input type="text" id="quantity/0" name="quantity[]" value="" size="10" style="text-align:right" onKeyUp="validate(this.id,this.value)" /></td>
<td width="10px"></td>
<?php
...
<center>
<input type="submit" id="Save" value="Save"  />&nbsp;&nbsp;&nbsp;<input type="button" id="Cancel" value="Cancel" onClick="document.location='dashboardsub.php?page=pp_gateentry_a';"/>
</center>
</td>
</tr>
</table>
...
<?php $query1 = "SELECT distinct(vendor) FROM pp_purchaseorder ORDER BY vendor ASC"; 
	  ...
	  { 
        echo "if(document.getElementById('vendor').value == '$row1[vendor]') { ";
        $query2 = "select distinct(code),description,deliverylocation from pp_purchaseorder where vendor = '$row1[vendor]' AND flag = '1' AND geflag <> '1' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0) ORDER BY code";
		$result2 = mysql_query($query2,$conn) or die(mysql_error());
	    while($row2 = mysql_fetch_assoc($result2)) { ?> 
...
<?php $query1 = "SELECT distinct(broker) FROM pp_purchaseorder ORDER BY vendor ASC"; 
	  ...
	  { 
        echo "if(document.getElementById('vendor').value == '$row1[broker]') { ";
        $query2 = "select distinct(code),description,deliverylocation from pp_purchaseorder where broker = '$row1[broker]' AND flag = '1' AND geflag <> '1' AND (quantity <> receivedquantity or receivedquantity = '' or receivedquantity = 0) ORDER BY code";
		$result2 = mysql_query($query2,$conn) or die(mysql_error());
	    while($row2 = mysql_fetch_assoc($result2)) { ?> 
		if(document.getElementById("warehouse").value == "<?php echo $row2['deliverylocation']; ?>")
					{ 
