...
$query2="SELECT * FROM pp_sobi WHERE vendor = '$name'";
$result2=mysql_query($query2,$conn) or die(mysql_error());
$rows2=mysql_num_rows($result2);

$query3="SELECT * FROM pp_receipt WHERE vendor = '$name'";
$result3=mysql_query($query3,$conn) or die(mysql_error());
$rows3=mysql_num_rows($result3);

$query4="SELECT * FROM pp_payment WHERE vendor = '$name'";
...
if($count==0)
{
 $delete_query="DELETE FROM contactdetails WHERE id = '$id'";
 $delete_result=mysql_query($delete_query,$conn) or die(mysql_error());
}
else
{
 echo "<script type='text/javascript'>";
 echo "alert('Cannot be deleted,already made transactions with the Supplier');";
 echo "</script>";
}

echo "<script type='text/javascript'>";
echo "document.location = 'dashboardsub.php?page=pp_supplier';";
echo "</script>";
?>
