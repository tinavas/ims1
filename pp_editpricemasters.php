...
$q="select * from pp_pricemaster where id=$_GET[id]";
$r=mysql_query($q);
...
$qdb=mysql_query("select fromdate,todate,supplier,warehouse,cat,code,`desc` from pp_pricemaster");
while($rdb=mysql_fetch_array($qdb))
{
$f=date("d.m.Y",strtotime($rdb[fromdate]));
$t=date("d.m.Y",strtotime($rdb[todate]));
$dbs[]=array("fromdate"=>$f,"todate"=>$t,"supplier"=>$rdb[supplier],"warehouse"=>$rdb[warehouse],"category"=>$rdb[cat],"code"=>$rdb[code],"desc"=>$rdb[desc]);
}
$db=json_encode($dbs);
 ...
 for($j=0;$j<count($names);$j++)
		   {
			$na=explode("@",$names[$j]);
           ?>
<option value="<?php echo $names[$j];?>" title="<?php echo $na[0]; ?>" <?php if($na[0]==$supplier) {?> selected="selected" <?php  } ?>><?php echo $na[0]; ?></option>
<?php } ?>
</select>
...
$sec_arr=explode(',',$sec_arr[1]);
 for($j=0;$j<count($sec_arr);$j++)
		   {		
           ?>
<option value="<?php echo $sec_arr[$j];?>" title="<?php echo $sec_arr[$j]; ?>" <?php if($sec_arr[$j]=$warehouse) {?> selected="selected" <?php  } ?>><?php echo $sec_arr[$j]; ?></option>
<?php } ?>
</select>
...
<?php } ?>
       ...
       <td style="text-align:left;">
			<select style="Width:75px" name="code" id="code@-1" onChange="selectdesc(this.id);checkitem(this.id,this.value);">
     		<option value="">-Select-</option>
