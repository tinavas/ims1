...
");
    ...
    $pdf->setLineStyle(0.5);
	 ...
	 $pdf->addText(493,712,10,"DATE :");
	  $pdf->addText(235,712,12,"<b> RECEIPT VOUCHER</b>");
	...
	if($_SESSION['db']=='vista')
	 ...
	 if($_SESSION['db']=='vista')
	 $pdf->addText(30,380,12,"<b>Narration:</b>");
...
$cramount=0;
$dramount1=0;
$query = "SELECT * FROM ac_gl WHERE  transactioncode = '$trno' AND client = '$client' and voucher = 'R' ORDER BY id ASC LIMIT 1,100";
...
while($row = mysql_fetch_assoc($result))
{
 ...
 if($crdr == "Cr")
 {
  $cramount = $cramount + $row['cramount'];
  $amount = $row['cramount'];
 } 
  if($crdr == "Dr")
 {
 ...
 $dramount1 = $dramount1 + $row['dramount'];
 } 
...
$finaltotal1 = changeprice($finaltotal);
	 ...
	 //$pdf->addTextWrap($x,684,($y-20),12,"A/c No. $acno",'right');
	 if($acno <> "")
