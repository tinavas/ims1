...
<section class="grid_8">
  <div class="block-border">
     <form class="block-content form" id="complex_form"style="min-height:500px" enctype="multipart/form-data" method="post" action="dashboardsub.php?page=import_purchases" >
	  <h1 id="title1">View Puchases Data</h1>
...
<tr>
 ...
 <th><strong>VAT/TAX</strong></th>
 <th><strong>Discount</strong></th>
 <th><strong>Freight<br/>Type</strong></th>
 <th><strong>Freight<br/>Amount</strong></th>
 <th><strong>Freight<br/>Account</strong></th>
 ...
 //print "<br>\nData:<br>\n";
     ...
     if ( $row > 0 )
       {  
        ...
        if ($col == "2") { $data2 = strtoupper($data); $data2 = str_replace("&AMP;","&",$data2); } //htmlentities($data, ENT_QUOTES); }	//Vendor		2
        ...
        if ($col == "8") { $data8 = htmlentities($data, ENT_QUOTES); }	//Freight Type	8
        if ($col == "9") { $data9 = htmlentities($data, ENT_QUOTES); }	//Freight Amount9
...
if ($col == $totalcol && $data3 <> "" && $data4 <> "" && $data5 <> "")	//	IF-1
{
 ...
 $query = "SELECT id FROM contactdetails WHERE name = \"$vendor\" AND type LIKE '%vendor%'";
 ...
 elseif($data8 == "" && ($data9 <> "0" && $data9 <> ""))
  $alertmsg = "Please select Freight Type";
 elseif($data9 <> "0" && $data9 <> "" && $data10 == "")
  $alertmsg = "Please enter Freight Account Code";
 ...
 if($fcode <> "")
 {
 $query = "SELECT id FROM ac_coa WHERE code = '$fcode' AND controltype IN ('Cash','Bank')";
 $result = mysql_query($query,$conn) or die(mysql_error());
 if( ! $rows = mysql_fetch_assoc($result))
  $alertmsg = "Freight Account COA Code: $fcode should be Cash or Bank Account";

  $query = "SELECT id FROM ac_coa WHERE code = '$fcode'";
 $result = mysql_query($query,$conn) or die(mysql_error());
 if( ! $rows = mysql_fetch_assoc($result))
  $alertmsg = "Freight Account COA Code: $fcode is not added in the software";
 }
 $totqty = $data4;
 $countrows = 1;
 $invoiceamount = ($data4 * $data5) + $data6;
 if($data8 == "Included")
  $tempfreight = $data9;
 else
  $tempfreight = 0; 
 ...
 { 
  ...
  { 
   ...
   if($tempdate == "" && $tempbi == "" && $tempvendor == "")
   {
	...
	$invoiceamount += ( ($ee->GetCellData($sheet,4,$temp) * $ee->GetCellData($sheet,5,$temp)) + $ee->GetCellData($sheet,6,$temp));
   }
   else
    break;
  }
...
?>
 <tr>
   ...
   <td><input type="text" name="vendor[]" value="<?php echo $vendor; ?>" size="10" readonly  style="background:none; border:none"/></td>
   ...
   <td><input type="text" name="vat[]" value="<?php echo $data6; ?>" size="5" readonly  style="background:none; border:none; text-align:right"/></td>
   <td><input type="text" name="discount[]" value="<?php echo $data7; ?>" size="5" readonly  style="background:none; border:none; text-align:right"/></td>
   <td><input type="text" name="freightie[]" value="<?php echo $data8; ?>" size="7" readonly  style="background:none; border:none"/></td>
   <td><input type="text" name="famount[]" value="<?php echo $data9; ?>" size="5" readonly  style="background:none; border:none; text-align:right"/></td>
   <td><input type="text" name="faccount[]" value="<?php echo $data10; ?>" size="5" readonly  style="background:none; border:none"/></td>
   <td><input type="text" name="warehouse[]" value="<?php echo $data11; ?>" size="8" readonly  style="background:none; border:none"/>
   <input type="hidden" name="narration[]" value="<?php echo $data12; ?>" />
   <input type="hidden" name="totqty[]" value="<?php echo $totqty; ?>" size="5" />
   <input type="hidden" name="grandttotal[]" value="<?php echo $invoiceamount - $tempfreight - $data7; ?>" size="5" />
...
<?php } ?>
<input type="button" value="Cancel" onclick="document.location='dashboardsub.php?page=tally_purchases'">
</form>
</body>
</html>