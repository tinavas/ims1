...
<head>
	<title>Tulasi Technologies - IMS</title>
	<meta charset="utf-8">
...
<?php  $q = "select * from ac_bankcashcodes where mode = '$_GET[mode]' and flag = '0'order by code ASC";
  $qrs = mysql_query($q,$conn) or die(mysql_error()); while($qr = mysql_fetch_assoc($qrs)) { ?>
<option value="<?php echo $qr['code']; ?>" title="<?php echo $qr['name']; ?>"><?php echo $qr['code']; ?></option>
<?php } ?></td>
</tr>
...
<br />
<center>
<input type="submit" value="Save" />&nbsp;&nbsp;&nbsp;<input type="button" value="Cancel" onClick="parent.location='dashboardsub.php?page=ac_bankcashcoamapping';">
</center>
</td>
</tr>
