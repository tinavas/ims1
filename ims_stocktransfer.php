...
<script type="text/javascript">
	...
	<div id="control-bar" class="grey-bg clearfix"><div class="container_12">
		...
		<div class="float-right"> 
			<button type="button" onClick="document.location='dashboardsub.php?page=ims_addstocktransfer'"><img src="images/icons/fugue/tick-circle.png" width="16" height="16"> Add</button>
...
<tbody>
 ...
 session_start();
		       ...
		       if($_SESSION['sectorall'] == "all" || ($_SESSION['sectorall'] == "" && $_SESSION['sectorlist'] == ""))
	{
     ...
     $query = "SELECT * FROM ims_stocktransfer where  date >= '$fromdate' and date <= '$todate' order by date desc"; 
	 
	 else
	  $query = "SELECT * FROM ims_stocktransfer where  date <= '$todate' and fromwarehouse in ($sectorlist) order by date desc"; 
           ...
           {  
              ...
              $q = "select * from ims_stocktransfer where tid = '$row1[tid]' order by code";
			  ...
			  $q2=mysql_query("select description from ims_itemcodes where code='$row1[code]'");
...
<a href="dashboardsub.php?page=ims_editstocktransfer&id=<?php echo $row1['id']; ?>&col=<?php echo "id"; ?>&tid=<?php echo $row1['tid']; ?>&date=<?php echo $row1['date']; ?>&flock=<?php echo $row1['aflock']; ?>&type=<?php echo "STR"; ?>&name=<?php echo "ims_stockreceive"; ?>"><img src="images/icons/fugue/pencil.png" style="border:0px" title="Edit" /></a>
              &nbsp; <a onclick="if(confirm('Are you sure,want to delete')) document.location ='ims_deletestocktransfer.php?id=<?php echo $row1['id']; ?>&tid=<?php echo $row1['tid']; ?>&date=<?php echo $row1['date']; ?>&flock=<?php echo $row1['aflock']; ?>'"><img src="images/icons/fugue/cross-circle.png" style="border:0px" title="Delete" /></a>&nbsp;
...
document.location = "dashboardsub.php?page=ims_stocktransfer&month=" + month + "&year=" + year;
}
</script>
