...
$q = "SELECT GROUP_CONCAT(code,'@',description ORDER BY code ) AS codedesc  FROM ac_coa  where controltype in('','Bank','Cash') and type <> 'Expense' and schedule not in('Inventories','Trade Payable','Trade Receivable','Inventories Work In Progress','Revenue from operations')
";
		$qrs = mysql_query($q,$conn) or die(mysql_error());
...
<form id="form1" name="form1" method="post" action="ac_savervoucher.php" class="block-content form"  onsubmit="return checkform1(this);" >
...
<td width="10px"></td>
 ...
 <td width="10px"></td>
 <td><input type="text" size="15" id="date" class="datepicker" onchange="check_totamt(this);" name="date" value="<?php echo date("d.m.Y"); ?>" /></td>
<td width="10px"></td>
...
<td width="40px"></td>
<td>
<strong >Cash/Bank Codes</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font>
</td>
<td>
...
<td width="10px"></td>
<td>
<select id="code0" name="code[]" onchange="getdesc(this.id);"   style="width:100px">
<option value="" >-Select-</option>

</select>
</td>
<td width="10px"></td>
<td><select id="desc0" name="desc[]"  onchange="getcode(this.id)" style="width:170px"  >
        <option value="">-Select-</option>
...
<td width="10px"></td>
<td>
<input type="text" id="dramount0" name="dramount[]" value="0" style="text-align:right" size="8" onkeyup="total();check_totamt(this);" onblur="total();check_totamt(this);"  />
</td>
<td width="10px"></td>
<td>
<input type="text" id="cramount0" name="cramount[]" value="0" style="text-align:right" size="8" onkeyup="total();check_totamt(this);"  onblur="total();check_totamt(this);" onfocus="makeForm()" readonly />
</td>
<td width="10px"></td>
...
<input type="submit" value="Save" id="Save" disabled="disabled" name="Save"/>&nbsp;&nbsp;&nbsp;<input name="button" type="button" onclick="document.location='dashboardsub.php?page=ac_rvoucher';" value="Cancel" />
</form>
</div>
</section>
</center>
...
for(i=0;i<bankcodes.length;i++)
 {
	...
	
	{
	  ...
	  var op1=new Option(cashbank1[1],cashbank1[1]);
		...
		var op1=new Option("Dr","Dr");
		op1.selected = "true";
		
			myselect2.options.add(op1);
	
	
	document.getElementById('dramount0').readOnly=false;
document.getElementById('cramount0').readOnly=true;
...
if(document.getElementById('code'+k).value=="" || document.getElementById('drcr'+k).value=="" || document.getElementById('desc'+k).value=="")
{
	...
	alert("Select code for the row"+k)
	return false;
