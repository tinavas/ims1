...
var bankaccno=<?php if(empty($bankaccno1)){echo 0;} else{ echo $bankaccno1; }?>;
</script>
...
<br/>
<br/><br />
<form id="form1" name="form1" method="post" action="ac_updatervoucher.php"  onSubmit="return checkform1(this)"  >
...
<td><input type="text" id="tno" name="tno" size="6" value="<?php echo $tnum; ?>" readonly style="border:0px;background:none"  /></td>
 ...
 <td width="10px"></td>
 <td><input type="text" size="15" id="date" class="datepicker" readonly="true" onChange="check_totamt(this);" name="date" value="<?php echo date('d.m.Y',strtotime($stdate)); ?>" /></td>
 <td width="10px"></td>
...
<td><input type="text" id="vno" name="vno" size="15" value="<?php echo $vno; ?>" /></td>

<td width="40px"></td>
<td style="width:150px;" >
<strong >Cash/Bank Codes</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></td>
...
<td width="10px"></td>
<td>
<select id="code<?php echo $i; ?>" name="code[]" onChange="getdesc(<?php echo $i; ?>);"  style="width:100px">
<option value="">Select</option>
<?php if($i==1)
{?>
...
<td  id="chtd"  <?php if($pmode == "Cheque") { ?> style="visibility:visible"  <?php  } else { ?> style="visibility:hidden"<?php }?> >
...
<br />
<center>
<input type="submit" value="Update" id="Update" name="Update"/>&nbsp;&nbsp;&nbsp;<input name="button" type="button" onClick="document.location='dashboardsub.php?page=ac_rvoucher';" value="Cancel"> 
</center>
<br />
<!-- </center> -->
</form>
...
for(i=0;i<bankcodes.length;i++)
 {
	...
	document.getElementById('dramount1').readOnly=false;
document.getElementById('cramount1').readOnly=true;
...
function chequeenable()
{
	...
	document.getElementById('chtd').style.visibility = "visible";
	 ...
	 for(i=0;i<bankaccno.length;i++)
 {

	if(document.getElementById('cno').value==bankaccno[i])
