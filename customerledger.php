...
if ( $_POST['code'] == "All" )  { header('Location:customerledgerall.php?date2='.$_POST['date2'].'&date3='.$_POST['date3']); }
...
<tr>
<td>From Period </td>
<td>From <?php echo $fdatedump; ?></td>
<td>To <?php echo $tdatedump; ?></td>
</tr>
</table>
...
<?php  if ( $obflag == "Cr" ) { echo ewrpt_ViewValue(changeprice($obbal)); $crtotal = $crtotal + $obbal;  } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
	...
	echo $q = "select * from ac_financialpostings where date >= '$fdate' and date <= '$tdate' and venname = '$ven' and coacode = '$vencontrollac' ";
	...
	$narration = "";
	  ...
	  {
	    $mode = "COBI";
	    $q1 = "select * from oc_cobi where invoice = '$qr[trnum]' ";
	  ...
	  {
	      $mode = "Receipt";
	      $q1 = "select * from oc_receipt where tid = '$qr[trnum]' ";
	      $qrs1 = mysql_query($q1,$conn1) ;
	      while($qr1 = mysql_fetch_assoc($qrs1))
	      {
...
<?php  if ( $obflag == "Cr" ) { echo ewrpt_ViewValue(changeprice($obbal)); $pcrtotal = $pcrtotal + $obbal;  } else { echo ewrpt_ViewValue("&nbsp;"); } ?>
	...
	$mincndndate = "";
	$maxcndndate = "";
	 $q = "select min(date) as minadate,max(date) as maxadate from ac_financialpostings where coacode = '$venprepayac' and date >= '$fdate' and date <='$tdate' ";
	$qrs = mysql_query($q,$conn1) or die(mysql_error());
	while($qr = mysql_fetch_assoc($qrs))
	{
	...
	while ( $lastdate >= $predate ) 
{
	
	 $q = "select * from ac_financialpostings where date = '$predate' and venname = '$ven' and coacode = '$venprepayac' ";
	$qrs = mysql_query($q,$conn1) ;
	while($qr = mysql_fetch_assoc($qrs))
	{
	$mode = "";
	...
	$narration = "";
	  ...
	  {
	      $mode = "Receipt";
	      $q1 = "select * from oc_receipt where socobi = '$qr[trnum]' ";
	      $qrs1 = mysql_query($q1,$conn1) ;
	      while($qr1 = mysql_fetch_assoc($qrs1))
	      {
		    $pmode = $qr1['paymentmode'];
...
<?php if ( $qr['crdr'] == "Cr" ) { echo ewrpt_ViewValue(changeprice($qr['amount'])); $pcrtotal = $pcrtotal + $qr['amount']; } 
	...
	$minnxtmaindate = "";
	 ...
	 $q1 = "select min(date) as minadate from ac_financialpostings where coacode = '$venprepayac'  and date > '$predate' and date <='$tdate' ";
