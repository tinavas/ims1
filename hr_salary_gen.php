...
<script type="text/javascript">
	...
	<div id="control-bar" class="grey-bg clearfix"><div class="container_12">
		...
		<div class="float-right"> 
					...
					<button type="button" onClick="document.location='dashboardsub.php?page=hr_salary_addgen'"><img src="images/icons/fugue/tick-circle.png" width="16" height="16"> Add</button>
...
<th style="text-align:left">Generation Date</th>
...
<tbody>
	  ...
	  <?php
           include "config.php"; 
          $query = "SELECT * FROM hr_salary_generation where month1='$monthcnt' and year1='$year'  ORDER BY date";
           $result = mysql_query($query,$conn); 
           while($row1 = mysql_fetch_assoc($result))
           {
		   $k =0;
		   		$eid = $name = $totalsal = $paid = $deduction =  "";
		        $q = "select * from hr_salary_generation where id = '$row1[id]' order by id";
		...
		<td><?php echo $date=date("d.m.Y",strtotime($row1['date'])); ?></td>
			 ...
			 <td>
		<?php if($qr['pflag']=="1"){ ?>
        <img src="images/icons/fugue/lock.png" width="16px" style="border:0px" title="Salary payment is done" />
        <?php } else { ?>
		
		<?php if($_SESSION['valid_user']==$user || ($_SESSION['superadmin']=="1") ||($_SESSION['admin']=="1") ){?> 
			<a href="hr_salary_deletegen.php?id=<?php echo $row1['id']; ?>&name=<?php echo $name;?>&date=<?php echo $row1['date'];?>&salid=<?php echo $row1['salparamid']; ?>&eid=<?php echo $row1['eid']; ?>&tid=<?php echo $row1['tid']; ?>">
...
document.location = "dashboardsub.php?page=hr_salary_gen&month=" + month + "&year=" + year;
}
</script>
