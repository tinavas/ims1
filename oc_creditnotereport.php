...
<?php 
$q  = "select distinct(vcode) from ac_crdrnote where mode = 'CCN' order by vcode";
...
<option value = "All">All</option>
<?php 
$q  = "select distinct(vcode),vpcode from ac_crdrnote where mode = 'CCN' order by vpcode";
$r = mysql_query($q,$conn) or die(mysql_error());
while($qr = mysql_fetch_assoc($r))
{
?>
...
{
window.open('production/customercreditnote.php?fromdate=' + fromdate + '&todate=' + todate + '&name=' + name1);
