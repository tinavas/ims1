...
<?php
           include "config.php"; 
           $query1 = "SELECT * FROM ac_coa where controltype='Customer A/c' and code not in(select distinct(vca) from ac_vgrmap) ORDER BY code ASC ";
           $result1 = mysql_query($query1,$conn); 
           while($row11 = mysql_fetch_assoc($result1))
           {
		      ?>
<option value="<?php echo $row11['code']; ?>" selected="selected"><?php echo $row11['code']; ?></option>
<?php }  ?>
</select>
</td>
<td>
...
<?php
           include "config.php"; 
           $query1 = "SELECT * FROM ac_coa where controltype='Customer Advance A/c' and code not in(select distinct(vppac) from ac_vgrmap) ORDER BY code ASC ";
           $result1 = mysql_query($query1,$conn); 
           while($row11 = mysql_fetch_assoc($result1))
           { ?>
...
<br />
<center>
<input type="submit" value="Update" id="Save" name="Save"/>&nbsp;&nbsp;&nbsp;<input type="button" value="Cancel" onclick="document.location='dashboardsub.php?page=oc_customergroup';">
</center>
<br />
<!-- </center> -->
</form>
