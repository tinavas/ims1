...
if($type == 'Credit') {  $mode1 = 'VCN'; }
 else if($type == 'Debit') { $mode1 = 'VDN'; }
...
$q = "select * from ac_crdrnote where crnum = '$tno' and mode = '$mode' and client = '$client' and tid='$tid'  LIMIT 1"; 
$qrs = mysql_query($q,$conn) or die(mysql_error());
...
$q = "select * from ac_crdrnote where crnum = '$tno' and mode = '$mode' and client = '$client'and tid='$tid'  "; 
$qrs = mysql_query($q,$conn) or die(mysql_error());
while($qr = mysql_fetch_assoc($qrs)) 
{ 
  if($qr['dramount'] > 0)
