...
$q="select * from oc_discounts where id=$_GET[id]";
$r=mysql_query($q);
 ...
 for($j=0;$j<count($names);$j++)
		   {
			$na=explode("@",$names[$j]);
           ?>
<option value="<?php echo $names[$j];?>" title="<?php echo $na[0]; ?>" <?php if($na[0]==$wh) {?> selected="selected" <?php  } ?>><?php echo $na[0]; ?></option>
<?php } ?>
</select>
...
<th><strong>Discount<br />in %</strong><font style="color:red;font-weight:bold;padding-top:10px"><sup>*</sup></font></th>
...
if((dbfdate.getTime()<=fdate1.getTime()  &&  dbs[k].warehouse==aa[0] &&   dbs[k].code==code )|| ( dbtdate.getTime()>=tdate1.getTime() &&  dbs[k].warehouse==aa[0] &&   dbs[k].code==code )){
