...
<td>From</td>
 ...
 <td>Customer</td>
 <td><select name="customer" id="customer" onchange="reloadpage()">
 <?php $qq=mysql_query("select distinct(warehouse) as party from oc_cobi ");
 while($rr=mysql_fetch_array($qq))
 {?>
 <option value="<?php echo $rr[party];?>" <?php if($rr[party]==$_GET[cus]){?> selected="selected" <?php }?>><?php echo $rr[party];?></option> 
 <?php }?>
 </select></td>
 </tr>
 </table>
...
$qdif=mysql_query("select datediff('$r1[date]','$fromdate') as days");
...
$a=mysql_query("select * from product_productionunit where date='$dt' and warehouse='$cus'");
...
echo "<br>";
echo $c="update ac_financialpostings set amount='$amt' where date='$dt' and itemcode='$a1[producttype]' and trnum='$a1[formula]'  and warehouse='$a1[warehouse]'";
mysql_query($c);
...
$qs1="select * from ims_stocktransfer where date='$dt' and fromwarehouse='$cus'";
$qs=mysql_query($qs1);
while($rs=mysql_fetch_array($qs))
{
$pr=$at=0;
echo $mode="Weighted Average";
 echo $pr=calculatenew($rs[fromwarehouse],$mode,$rs[code],$dt);
$at=$pr*$rs[quantity];
$qu="update ims_stocktransfer set price='$pr' where tid='$rs[tid]' and code='$rs[code]'";
mysql_query($qu);
echo $qp="update ac_financialpostings set amount='$at' where trnum='$rs[id]' and type='STR' and itemcode='$rs[code]'";
mysql_query($qp);
if($rs[flag]=='1')
{
$qu="update ims_stockreceive set price='$pr' where transferid='$rs[tmno]' and code='$rs[code]'";
mysql_query($qu);
$qt=mysql_query("select tid from ims_stockreceive where transferid='$rs[tmno]' and code='$rs[code]'");
$rt=mysql_fetch_array($qt);
 $qp="update ac_financialpostings set amount=quantity*$pr where trnum='$rt[tid]' and type='SRC' and itemcode='$rs[code]'";
mysql_query($qp);
}
echo "<br>";
}
...
$rd=mysql_fetch_array($qd);
   ...
   $discvalue = $rd['discount'];
	...
	$discountamount =0;
$discountamount = (($r2[cquantity]*$r2[price])* $discvalue)/100;
...
$salesamount = round($salesamount,5);
 ...
 $query4 = "INSERT INTO ac_financialpostings(date,itemcode,crdr,coacode,quantity,amount,trnum,type,venname,warehouse,client,empname,adate) 
	          VALUES('".$adate."','".$code."','Cr','".$itemac."','".$qtyr*$capacity."','".$stdcost."','".$so."','".$type."','".$vendor."','".$globalwarehouse."','".$client."','$empname','$adate'),
			  ('".$adate."','".$code."','Dr','".$cogsac."','".$qtyr*$capacity."','".$stdcost."','".$so."','".$type."','".$vendor."','".$globalwarehouse."','".$client."','$empname','$adate'),
...
function reloadpage()
{
	...
	var flk = document.getElementById('customer').value;
	document.location = "dashboardsub.php?page=cobirerun&fromdate=" + fdate + "&todate=" + tdate+ "&cus=" + flk;
 //+"&flk=" +flk;
}
</script>
</html>
