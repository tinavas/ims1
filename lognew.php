...
$ppd=explode("-",$pdate);
 ...
 if($q5=="")
 {
 createxsl($db,"69.162.85.106","TuL@Ss!12","poultry",$ddform);
...
include "mainconfig.php";
 ...
 $dt=date("Y-m-d");
$q1=mysql_query("select attempts from  blockedusers where  uname='$user'",$conn) or die(mysql_error());
$r1=mysql_fetch_array($q1,$conn);
if($r1[attempts]>=3) 
{
...
{
$dt=date("Y-m-d");
$q1=mysql_query("select attempts from  blockedusers where date='$dt' and uname='$user'");
$r1=mysql_fetch_array($q1);
if(mysql_num_rows($q1))
$q=mysql_query("update blockedusers set attempts=attempts+1 where date='$dt' and uname='$user'");
else
$q=mysql_query("insert into blockedusers(date,uname,pwd,attempts) values('$dt','$user','$pass',1)") or die(mysql_error());
$q2=mysql_query("select attempts from  blockedusers where date='$dt' and uname='$user'");
